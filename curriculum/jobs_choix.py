...
NOTE = ("CAMPAGNE {nom} : le choix {levier} de la phase {ph} selon la situation. Criteres ecrits avant : depot curriculum/CRITERES_CHOIX_17_09.md.\n"
 "Case {bras} ( menace_p{ph} = {men} ), {levier} = {val}, monde {g}, {R} repetitions. 32 jobs = 2 bras x 2 options x 8 mondes ; les 4 cases d un monde partent ensemble.\n"
    ...
    for g in MONDES:
        for bras, men in (("TEMOIN", 0), ("MENACE", 3)):
            for val in c["options"]:
                ...
                j.update({c["levier"]: val, f"menace_p{c['ph']}": men, "graines": [g], "repetitions": c["R"], "situation": 1, "vignette": 1,
                          "campagne": c["nom"], "version": f"P{c['ph']}-{bras}-{c['levier']}{val}-g{g}",
                          "note": NOTE.format(nom=c["nom"], levier=c["levier"], ph=c["ph"], bras=bras, men=men, val=val, g=g, R=c["R"])})
...
for c in CAMPAGNES:
    ...
    print(f"   {c['nom']} : {len(js)} jobs, {sum(j['repetitions'] for j in js)} episodes, cases {dict(Counter((j['version'].split('-')[1], j[c['levier']]) for j in js))}")
