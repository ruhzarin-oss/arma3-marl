...
MC6_gFS = ([west, 6] call MULTI_fnc_groupe);
MC6_FS = [];
...
private _rolesTous = [];
for "_p" from 1 to (round (MC6_EFFECTIF / 10)) do { _rolesTous append _rolesBase };

{
    _x params ["_cls", "_role"];
    private _u = MC6_gFS createUnit [_cls, MC6_LZ getPos [12 call MC6_fnc_al, 360 call MC6_fnc_al], [], 0, "NONE"];
    [_u, _role] call MC6_fnc_kitOuest;
    MC6_FS pushBack _u;
} forEach _rolesTous;if (count MC6_FS < MC6_EFFECTIF) exitWith {
    MC6_ISSUE = "VOID"; MC6_CAUSE = "DETACHEMENT_INCOMPLET";
    (format ["CHACAL|VOID|blufor|%1", count MC6_FS]) call MC6_LOG;
};

MC6_gFS selectLeader (MC6_FS select 0);
MC6_gFS setBehaviour "STEALTH";
MC6_gFS setCombatMode "GREEN";      // ne tire que si on lui tire dessus
MC6_gFS setSpeedMode "LIMITED";
MC6_gFS setFormation "FILE";        // la formation de nuit hors piste
MC6_gFS allowFleeing 0;
...
{ for "_i" from 1 to 3 do { _x addMagazine "DemoCharge_Remote_Mag" }; }
    forEach (MC6_FS select { (_x getVariable ["chacal_role",""]) in ["DEMO_1","DEMO_2"] });
{ _x addMagazine "DemoCharge_Remote_Mag"; }
    forEach (MC6_FS select { (_x getVariable ["chacal_role",""]) in ["CHEF","ADJOINT"] });

MC6_fnc_role = {
    private _r = _this;
    MC6_FS select { alive _x && { (_x getVariable ["chacal_role",""]) in _r } }
};
...
MC6_RECO    = ["TIREUR_1","TIREUR_2"];
MC6_APPUI   = ["TIREUR_1","TIREUR_2"];
MC6_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN"];
MC6_BOUCHON = ["AT","FUSILIER_1","FUSILIER_2"];
...
if (MC6_PARTAGE == 1) then {
    MC6_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN","FUSILIER_1","FUSILIER_2"];
    MC6_BOUCHON = ["AT"];
};

MC6_gAppui = grpNull; MC6_gAssaut = grpNull;
MC6_gBouchon = grpNull; MC6_gReco = grpNull; MC6_gDemo = grpNull;
...
MC6_fnc_detacher = {
    params ["_roles", "_nom"];
    private _u = _roles call MC6_fnc_role;
    if (count _u == 0) exitWith { grpNull };
    private _g = ([west, 6] call MULTI_fnc_groupe);
    _u joinSilent _g;
    _g selectLeader (_u select 0);
    _g setVariable ["chacal_element", _nom, true];
    _g allowFleeing 0;
    (format ["CHACAL|E|scission|%1|%2|%3", round (time * 100) / 100, _nom, count _u]) call MC6_LOG;
    _g
};

if (MC6_IMMORTEL == 1) then {
    { _x allowDamage false } forEach MC6_FS;
    "CHACAL|AVERT|hors_corpus|immortel|1|couture_sous_le_feu" call MC6_LOG;
};

(format ["CHACAL|OK|blufor|%1|roles|%2", count MC6_FS,
    (MC6_FS apply { _x getVariable ["chacal_role", "?"] })]) call MC6_LOG;
