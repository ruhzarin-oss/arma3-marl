...
// ne vaut plus rien.
// =====================================================================
if (MC6_ORACLE_CMD <= 0) exitWith {};

MC6_O_BUDGET = MC6_ORACLE_B;
MC6_O_CIBLE = -1;
...
private _lz = MC6_LZ; private _rt = MC6_ROUTE; private _op = MC6_OP; private _st = MC6_SITE;
private _ra = if (count MC6_RALLY > 1) then { MC6_RALLY } else { _lz };
MC6_O_CASES = [
    ["POSER",   _lz],
...
MC6_O_B = _poids apply { _x / _sp };
MC6_O_PRIOR = +MC6_O_B;   // la doctrine, gardee a part : c est vers elle que revient le doute
// les cases que la patrouille de route peut atteindre, et le troncon de route le plus proche de chacune
MC6_O_RTE = MC6_O_CASES apply {
...
private _carte = [];
{
    ...
    _carte pushBack [(_x select 0), (if (count (MC6_O_RTE select _forEachIndex) > 0) then {1} else {0}), _d2];
} forEach MC6_O_CASES;
(format ["CHACAL|O|carte|%1|cases_routieres|%2|detail|%3", round (time * 100) / 100,
    count (MC6_O_RTE select { count _x > 0 }), str _carte]) call MC6_LOG;
...
MC6_O_fnc_yeux = {
    ...
    { if (!isNull _x) then { { if (alive _x) then { _y pushBack _x } } forEach (units _x) } } forEach
        ((MC6_GROUPES_EST + [MC6_gRoute]) select { !isNull _x });
    { if (alive _x && { !(_x in _y) }) then { _y pushBack _x } } forEach MC6_EST_SITE;
    _y
};

MC6_O_fnc_caseLaPlusProche = {
    params ["_p"];
    private _d = MC6_O_CASES apply { (_x select 1) distance2D _p };
...
MC6_O_fnc_croire = {
    ...
    for "_i" from 0 to (_n - 1) do {
        private _v = 0.70 * (MC6_O_B select _i);
        if (_i > 0) then { _v = _v + (0.30 * (MC6_O_B select (_i - 1))) } else { _v = _v + (0.30 * (MC6_O_B select _i)) };
        _bp pushBack _v;
    };
        ...
        if (!isNull _chef && { alive _chef }) then {
            {
                ...
                if (_kn select 0) then {
                    ...
                    if (_age < _fraicheur) then { _fraicheur = _age; _vu = [(_kn select 6)] call MC6_O_fnc_caseLaPlusProche };
                };
            } forEach MC6_FS;                                  // on parcourt la liste, on ne lit QUE sa connaissance
        };
    } forEach ((MC6_GROUPES_EST + [MC6_gRoute]) select { !isNull _x });
    // 3. ce qu il n a pas vu : une case regardee et vide devient moins probable
    private _yeux = call MC6_O_fnc_yeux;
    private _l = [];
    for "_i" from 0 to (_n - 1) do {
        private _p = (MC6_O_CASES select _i) select 1;
        private _q = 1;
        { _q = _q * (1 - ([_x distance2D _p] call MC6_O_fnc_pd)) } forEach _yeux;
        _l pushBack _q;
    };
    ...
    for "_i" from 0 to (_n - 1) do { _bp set [_i, ((1 - _nu) * ((_bp select _i) / _s)) + (_nu * (MC6_O_PRIOR select _i))] };
    MC6_O_B = _bp;
    _vu
};
...
MC6_O_fnc_agir = {
    params ["_cible"];
    private _p = (MC6_O_CASES select _cible) select 1;
    ...
    private _bR = MC6_O_B apply { 0 };
    { if (count (MC6_O_RTE select _forEachIndex) > 0) then { _bR set [_forEachIndex, _x] } } forEach MC6_O_B;
    private _mxR = selectMax _bR;
    private _candR = []; { if ((_x > 0) && { _x >= (_mxR - 0.01) }) then { _candR pushBack _forEachIndex } } forEach _bR;
    if (MC6_O_BUDGET > 0 && { count _candR > 0 } && { !isNull MC6_gRoute }
        && { ({ alive _x } count (units MC6_gRoute)) > 0 }) then {
        private _cibleR = if (MC6_O_CIBLE in _candR) then { MC6_O_CIBLE } else { selectRandom _candR };
        if (_cibleR != MC6_O_CIBLE) then {
            private _a = MC6_O_RTE select _cibleR;
            private _r2 = (_a nearRoads 350) select { (_x distance2D _a) > 250 };
            private _b2 = if (count _r2 > 0) then { getPosATL (_r2 select 0) } else { _a };
            while { (count (waypoints MC6_gRoute)) > 0 } do { deleteWaypoint ((waypoints MC6_gRoute) select 0) };
            private _w = MC6_gRoute addWaypoint [_a, 15];  _w setWaypointType "MOVE";  _w setWaypointSpeed "NORMAL";
            private _w2 = MC6_gRoute addWaypoint [_b2, 15]; _w2 setWaypointType "MOVE"; _w2 setWaypointSpeed "LIMITED";
            (MC6_gRoute addWaypoint [_a, 15]) setWaypointType "CYCLE";
            ...
            MC6_gRoute setCurrentWaypoint _w;
            MC6_O_BUDGET = MC6_O_BUDGET - 1;
            MC6_O_CIBLE = _cibleR;
            _action = format ["PATROUILLE_%1", (MC6_O_CASES select _cibleR) select 0];
        };
    };
    _action
};
...
[] spawn {
    waitUntil { sleep 2; MC6_FIN || { !isNil "MC6_TPHASE" } };
    while { !MC6_FIN } do {
        private _vu = call MC6_O_fnc_croire;
        private _b = +MC6_O_B;
        ...
        if ((random 1) < (MC6_ORACLE_EPS / 100)) then {
            private _c = +_b; _c set [_cible, -1];
            _cible = _c find (selectMax _c);
        };
        private _action = [_cible] call MC6_O_fnc_agir;
        private _dPat = if ((MC6_O_CIBLE >= 0) && { !isNull MC6_gRoute } && { !isNull (leader MC6_gRoute) }) then {
            round ((leader MC6_gRoute) distance2D (MC6_O_RTE select MC6_O_CIBLE)) } else { -1 };
        (format ["CHACAL|O|decision|%1|action|%2|case|%3|p|%4|vu|%5|budget|%6|croyance|%7|cible_patrouille|%8|patrouille_a|%9|vitesse_patrouille|%10",
            round (time * 100) / 100, _action, (MC6_O_CASES select _cible) select 0,
            round ((_b select _cible) * 100) / 100, (if (_vu >= 0) then { (MC6_O_CASES select _vu) select 0 } else { "RIEN" }),
            MC6_O_BUDGET, str (_b apply { round (_x * 100) }),
            (if (MC6_O_CIBLE >= 0) then { (MC6_O_CASES select MC6_O_CIBLE) select 0 } else { "AUCUNE" }), _dPat,
            (if (!isNull MC6_VEH_ROUTE) then { round (speed MC6_VEH_ROUTE) } else { -1 })]) call MC6_LOG;
        sleep (MC6_ORACLE_DELTA * MC6_ECHELLE);
    };
};
