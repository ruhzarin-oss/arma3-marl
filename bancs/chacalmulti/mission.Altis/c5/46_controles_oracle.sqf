...
// apres 45_oracle.sqf pour disposer de MC5_O_CASES et de MC5_O_B.
// =====================================================================
if (MC5_ORACLE_CTRL <= 0) exitWith {};

// --- 3 : NON-TRICHE. On deplace NOS hommes, sans rien dire a personne. ---
if (MC5_ORACLE_CTRL == 3) then {
    if (MC5_ORACLE_CMD <= 0) exitWith {
        (format ["CHACAL|O|ctrl|erreur|%1|cause|NON_TRICHE_SANS_ORACLE", round (time * 100) / 100]) call MC5_LOG;
    };
    [] spawn {
        waitUntil { sleep 2; MC5_FIN || { !isNil "MC5_TPHASE" } };
        sleep (150 * MC5_ECHELLE);
        if (MC5_FIN) exitWith {};
        private _vivants = MC5_FS select { alive _x };
        ...
        private _dep = [_pA] call MC5_O_fnc_caseLaPlusProche;
        ...
        private _d = MC5_O_CASES apply { (_x select 1) distance2D _pA };
        { if (_forEachIndex in [5, 6]) then { _d set [_forEachIndex, -1] } } forEach _d;
        private _arr = _d find (selectMax _d);
        private _pB = (MC5_O_CASES select _arr) select 1;
        ...
        (format ["CHACAL|O|ctrl|teleport_avant|%1|de|%2|vers|%3|i_arr|%4|saut|%5|p_arr|%6|croyance|%7",
            round (time * 100) / 100, (MC5_O_CASES select _dep) select 0,
            (MC5_O_CASES select _arr) select 0, _arr, _saut,
            round ((MC5_O_B select _arr) * 100),
            str (MC5_O_B apply { round (_x * 100) })]) call MC5_LOG;
        ...
        (format ["CHACAL|O|ctrl|teleport_apres|%1|case_reelle|%2|croyance|%3",
            round (time * 100) / 100,
            (MC5_O_CASES select ([getPosATL _chef] call MC5_O_fnc_caseLaPlusProche)) select 0,
            str (MC5_O_B apply { round (_x * 100) })]) call MC5_LOG;
    };
};

// --- 1 : POSITIF. On amene la patrouille sur eux, de jour, a decouvert. Elle doit les prendre. ---
if (MC5_ORACLE_CTRL == 1) then {
    [] spawn {
        waitUntil { sleep 2; MC5_FIN || { !isNil "MC5_TPHASE" } };
        sleep (30 * MC5_ECHELLE);
        if (MC5_FIN) exitWith {};
        private _vivants = MC5_FS select { alive _x };
        if (count _vivants == 0 || { isNull MC5_VEH_ROUTE }) exitWith {
            (format ["CHACAL|O|ctrl|erreur|%1|cause|POSITIF_SANS_PATROUILLE", round (time * 100) / 100]) call MC5_LOG;
        };
        private _chef = _vivants select 0;
        ...
        private _gp = group MC5_VEH_ROUTE;
        ...
        (format ["CHACAL|O|ctrl|positif|%1|patrouille_a|%2|hommes|%3|route|%4",
            ...
            (if (count _rs > 0) then {1} else {0})]) call MC5_LOG;
        ...
        for "_k" from 1 to 12 do {
            if (MC5_FIN) exitWith {};
            private _vs = MC5_FS select { alive _x };
            if (count _vs == 0 || { !alive MC5_VEH_ROUTE }) exitWith {};
            _gp move (getPosATL (_vs select 0));
            (format ["CHACAL|O|ctrl|positif_suivi|%1|patrouille_a|%2|vivants|%3",
                round (time * 100) / 100,
                round ((getPosATL MC5_VEH_ROUTE) distance2D (getPosATL (_vs select 0))),
                count _vs]) call MC5_LOG;
            sleep (15 * MC5_ECHELLE);
        };
    };
};
