...
[] spawn {
    ...
    private _rngSauve = MC5_RNG;
    MC5_RNG = ((MC5_GRAINE * 2749) + 40507) % 65537;
    if (MC5_RNG == 0) then { MC5_RNG = 1 };
    for "_i" from 1 to 12 do { call MC5_fnc_rnd };
    private _p = []; private _pt = []; private _essaisT = 0; private _trouveT = false;
    for "_k" from 1 to 14 do {
        private _c0 = [MC5_SITE getPos [2400 + (1200 call MC5_fnc_al), 360 call MC5_fnc_al], 150] call MC5_fnc_plat;
        if (surfaceIsWater _c0 || { !([_c0, 5] call MULTI_fnc_loin) }) then { continue };
        private _ha = [(_c0 select 0), (_c0 select 1), (getTerrainHeightASL _c0) + 1.5];
        for "_j" from 1 to 18 do {
            _essaisT = _essaisT + 1;
            private _c1 = _c0 getPos [30 + (30 call MC5_fnc_al), _j * 20];
            _c1 set [2, 0];
    ...
    if (!_trouveT) then {
        (format ["CHACAL|AVERT|temoin_sans_vue|%1|aucun_couple_franc_en_%2_essais", round (time * 100) / 100, _essaisT]) call MC5_LOG;
        _p = [MC5_SITE getPos [3000, 360 call MC5_fnc_al], 150] call MC5_fnc_plat;
        { private _c9 = [MC5_SITE getPos [3000, _x], 150] call MC5_fnc_plat; if (!surfaceIsWater _c9 && { [_c9, 5] call MULTI_fnc_loin }) exitWith { _p = _c9 } } forEach [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330];
        _pt = _p getPos [40, 0];
    };
    (format ["CHACAL|CANARI|couple|%1|essais|%2|franc|%3|distance|%4", round (time * 100) / 100,
        _essaisT, (if (_trouveT) then {1} else {0}), round (_pt distance2D _p)]) call MC5_LOG;
    MC5_RNG = _rngSauve;   // le generateur de la mission est rendu intact

    private _g = ([east, 5] call MULTI_fnc_groupe);
    MC5_CANARI = _g createUnit ["O_Soldier_F", _p, [], 0, "NONE"];
    MC5_CANARI setVariable ["chacal_role", "CANARI", true];
    MC5_CANARI call MC5_fnc_identifier;   // l instrument ne doit pas subir son propre defaut
    _g setBehaviour "CARELESS"; _g allowFleeing 0;
    doStop MC5_CANARI;
    sleep 4;
    private _id = MC5_CANARI getVariable ["chacal_id", -1];
    (format ["CHACAL|CANARI|pose|%1|id|%2|pos|%3", round (time * 100) / 100, _id, _p]) call MC5_LOG;
    ...
    MC5_CANARI setBehaviour "AWARE";
    private _arme = primaryWeapon MC5_CANARI;
    MC5_CANARI selectWeapon _arme;
    sleep 2;
    (format ["CHACAL|CANARI|arme|%1|muzzle|%2|mode|%3", _arme,
        currentMuzzle MC5_CANARI, currentWeaponMode MC5_CANARI]) call MC5_LOG;
    (format ["CHACAL|CANARI|tire|%1|id|%2|coups|3", round (time * 100) / 100, _id]) call MC5_LOG;
    for "_i" from 1 to 3 do {
        if (alive MC5_CANARI) then {
            private _m = currentMuzzle MC5_CANARI;
            if (_m == "") then { _m = _arme };
            MC5_CANARI forceWeaponFire [_m, currentWeaponMode MC5_CANARI];
        };
        sleep 1;
    };
    ...
    private _gt = ([west, 5] call MULTI_fnc_groupe);
    MC5_TEMOIN = _gt createUnit ["B_Soldier_F", _pt, [], 0, "NONE"];
    MC5_TEMOIN setVariable ["chacal_role", "TEMOIN_VUE", true];
    MC5_TEMOIN call MC5_fnc_identifier;
    MC5_TEMOIN setDir (MC5_TEMOIN getDir MC5_CANARI);
    ...
    for "_j" from 1 to 24 do {
        _essaisV = _j;
        if ([MC5_TEMOIN, MC5_CANARI] call MC5_fnc_voit) exitWith { _okVue = true };
        private _cand = _p getPos [28 + (34 call MC5_fnc_al), _j * 15];
        _cand set [2, 0];
        if (!surfaceIsWater _cand) then {
            MC5_TEMOIN setPosATL _cand;
            MC5_TEMOIN setDir (MC5_TEMOIN getDir MC5_CANARI);
            sleep 0.5;
        };
    };
    (format ["CHACAL|CANARI|placement_temoin|%1|essais|%2|vue|%3|distance|%4", round (time * 100) / 100,
        _essaisV, (if (_okVue) then {1} else {0}), round (MC5_TEMOIN distance MC5_CANARI)]) call MC5_LOG;
    if (!_okVue) then {
        (format ["CHACAL|AVERT|temoin_sans_vue|%1|aucune_position_mesuree_franche_en_%2_essais", round (time * 100) / 100, _essaisV]) call MC5_LOG;
    };
    ...
    // AWARE ; on lui `reveal` un ennemi a 34 m, il l abat, et `MC5_fnc_voit`
    // sortait alors sur sa toute premiere garde - `!alive _u`.
    MC5_TEMOIN allowDamage false;
    MC5_CANARI setCombatMode "BLUE";
    MC5_CANARI disableAI "AUTOTARGET";
    MC5_CANARI disableAI "TARGET";
    // On ne coupe JAMAIS "PATH" : mesure du 11/08, ca retire les jambes.
    MC5_CANARI reveal [MC5_TEMOIN, 4];
    MC5_TEMOIN reveal [MC5_CANARI, 4];
    MC5_TEMOIN doWatch MC5_CANARI;
    sleep 5;
    private _rel = abs ((((MC5_TEMOIN getDir MC5_CANARI) - (getDir MC5_TEMOIN) + 540) % 360) - 180);
    private _a = (getPosASL MC5_TEMOIN) vectorAdd [0,0,1.5];
    private _b = (getPosASL MC5_CANARI) vectorAdd [0,0,1.2];
    private _los = (count (lineIntersectsSurfaces [_a, _b, MC5_TEMOIN, MC5_CANARI, true, 1, "VIEW", "VIEW"])) == 0;
    (format ["CHACAL|CANARI|temoin|%1|id|%2|distance|%3|vue|%4|ecart_regard|%5|ligne_de_vue|%6|posture|%7|vivant|%8",
        round (time * 100) / 100, (MC5_TEMOIN getVariable ["chacal_id", -1]),
        round (MC5_TEMOIN distance MC5_CANARI),
        (if ([MC5_TEMOIN, MC5_CANARI] call MC5_fnc_voit) then {1} else {0}),
        round _rel, (if (_los) then {1} else {0}), stance MC5_TEMOIN,
        (if (alive MC5_TEMOIN) then {1} else {0})]) call MC5_LOG;

    // --- CANAL MORT ---
    sleep 45;
    (format ["CHACAL|CANARI|tue|%1|id|%2", round (time * 100) / 100, _id]) call MC5_LOG;
    MC5_CANARI setDamage 1;
};
...
// =====================================================================
[] spawn {
    waitUntil { sleep 2; MC5_FIN };
    sleep 3;

    private _actes = count MC5_CHARGES;
    private _objTotal = count MC5_OBJETS;
    private _vivants = count (MC5_FS select { alive _x });
    private _exf = if (isNil "MC5_EXFILTRES") then {0} else {MC5_EXFILTRES};
    private _pertesEst = (count MC5_EST_SITE) - (count (MC5_EST_SITE select { alive _x }));
    private _lat = if (MC5_T_ALARME < 0 || MC5_T_COMPROMIS < 0) then {-999}
                   else { round ((MC5_T_COMPROMIS - MC5_T_ALARME) * 100) / 100 };

    if (MC5_ISSUE != "VOID" && !MC5_LAMBS) then { MC5_ISSUE = "VOID"; MC5_CAUSE = "LAMBS_ABSENT" };
    if (MC5_ISSUE != "VOID") then {
        if (MC5_ABANDON && _actes == 0) then {
            MC5_ISSUE = "ABANDON";
            ...
            MC5_CAUSE = if (!isNil "MC5_CAUSE_ABANDON" && { MC5_CAUSE_ABANDON != "" })
                           then {MC5_CAUSE_ABANDON} else {"ABANDON_SANS_CAUSE"};
        } else {
            if (_actes >= _objTotal && _exf >= (round (0.6 * MC5_EFFECTIF))) then { MC5_ISSUE = "SUCCES"; MC5_CAUSE = "CHARGES_ET_EXFIL" }
            else {
                MC5_ISSUE = "ECHEC";
                ...
                MC5_CAUSE = if (_vivants == 0) then {"DETACHEMENT_DETRUIT"}
                    else { if (_actes < _objTotal) then {"CHARGES_INCOMPLETES"} else {
                    if (_vivants < (round (0.6 * MC5_EFFECTIF))) then {"PERTES_EXCESSIVES"} else {"EXFIL_MANQUEE"} } };
            };
        };
    };
    ...
    (format ["CHACAL|FINI|%1|%2|graine|%3|echelle|%4|dt|%5|bras|%6|palier|%24|phase_max|%7|charges|%8|sur|%9|detruits_scriptes|%10|intel|%11|exfiltres|%12|vivants|%13|pertes_est|%14|renseignement|%15|alarme|%16|compromis|%17|latence_surprise|%18|abandon|%19|insertion_forcee|%20|lambs|%21|ticks|%22|duree|%23|tenir|%25|arret|%26|depart|%27|effectif|%28|appui_feu|%29|accessible|%30|feu_avant|%31|mg_assaut|%32|delai_porteur|%33|appui_fixe|%34|oracle|%35|socle|%36|tactique|%37|relances|%38|fumigenes|%39|zones|%40|ablation|%41|banc_appui|%42|tirs_banc|%43|placeur|%44|couverture|%45|exfil|%46|azimut|%47|azimut_joue|%48|sans_jambes|%49|obs|%50",
        MC5_ISSUE, MC5_CAUSE, MC5_GRAINE, MC5_ECHELLE, MC5_DT, MC5_BRAS, MC5_PHASE,
        _actes, _objTotal, count MC5_DETRUITS, (if (MC5_INTEL) then {1} else {0}),
        _exf, _vivants, _pertesEst, count MC5_VUES,
    ...
    if (MC5_ECHELLE != 1) then {
        (format ["CHACAL|AVERT|hors_corpus|echelle|%1", MC5_ECHELLE]) call MC5_LOG;
    };

    call MC5_fnc_arreterCapture;
    ...
    "CHACAL|OK|serveur_au_repos|le_harnais_peut_relancer" call MC5_LOG;
};
