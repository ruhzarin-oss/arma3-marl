...
MC5_gFS = ([west, 5] call MULTI_fnc_groupe);
MC5_FS = [];
...
private _rolesTous = [];
for "_p" from 1 to (round (MC5_EFFECTIF / 10)) do { _rolesTous append _rolesBase };

{
    _x params ["_cls", "_role"];
    private _u = MC5_gFS createUnit [_cls, MC5_LZ getPos [12 call MC5_fnc_al, 360 call MC5_fnc_al], [], 0, "NONE"];
    [_u, _role] call MC5_fnc_kitOuest;
    MC5_FS pushBack _u;
} forEach _rolesTous;if (count MC5_FS < MC5_EFFECTIF) exitWith {
    MC5_ISSUE = "VOID"; MC5_CAUSE = "DETACHEMENT_INCOMPLET";
    (format ["CHACAL|VOID|blufor|%1", count MC5_FS]) call MC5_LOG;
};

MC5_gFS selectLeader (MC5_FS select 0);
MC5_gFS setBehaviour "STEALTH";
MC5_gFS setCombatMode "GREEN";      // ne tire que si on lui tire dessus
MC5_gFS setSpeedMode "LIMITED";
MC5_gFS setFormation "FILE";        // la formation de nuit hors piste
MC5_gFS allowFleeing 0;
...
{ for "_i" from 1 to 3 do { _x addMagazine "DemoCharge_Remote_Mag" }; }
    forEach (MC5_FS select { (_x getVariable ["chacal_role",""]) in ["DEMO_1","DEMO_2"] });
{ _x addMagazine "DemoCharge_Remote_Mag"; }
    forEach (MC5_FS select { (_x getVariable ["chacal_role",""]) in ["CHEF","ADJOINT"] });

MC5_fnc_role = {
    private _r = _this;
    MC5_FS select { alive _x && { (_x getVariable ["chacal_role",""]) in _r } }
};
...
MC5_RECO    = ["TIREUR_1","TIREUR_2"];
MC5_APPUI   = ["TIREUR_1","TIREUR_2"];
MC5_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN"];
MC5_BOUCHON = ["AT","FUSILIER_1","FUSILIER_2"];
...
if (MC5_PARTAGE == 1) then {
    MC5_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN","FUSILIER_1","FUSILIER_2"];
    MC5_BOUCHON = ["AT"];
};

MC5_gAppui = grpNull; MC5_gAssaut = grpNull;
MC5_gBouchon = grpNull; MC5_gReco = grpNull; MC5_gDemo = grpNull;
...
MC5_fnc_detacher = {
    params ["_roles", "_nom"];
    private _u = _roles call MC5_fnc_role;
    if (count _u == 0) exitWith { grpNull };
    private _g = ([west, 5] call MULTI_fnc_groupe);
    _u joinSilent _g;
    _g selectLeader (_u select 0);
    _g setVariable ["chacal_element", _nom, true];
    _g allowFleeing 0;
    (format ["CHACAL|E|scission|%1|%2|%3", round (time * 100) / 100, _nom, count _u]) call MC5_LOG;
    _g
};

if (MC5_IMMORTEL == 1) then {
    { _x allowDamage false } forEach MC5_FS;
    "CHACAL|AVERT|hors_corpus|immortel|1|couture_sous_le_feu" call MC5_LOG;
};

(format ["CHACAL|OK|blufor|%1|roles|%2", count MC5_FS,
    (MC5_FS apply { _x getVariable ["chacal_role", "?"] })]) call MC5_LOG;
