...
MC5_CAP_VERSION = 1;
if (isNil "MC5_NEXT_ID") then { MC5_NEXT_ID = 1 };
MC5_TICK = 0; MC5_LAST = -1;
MC5_MAXCAR = 850;
MC5_CHRONO = [0, 0];
MC5_DTV = 2;                                   // la perception se rejoue moins vite que la position

MC5_TOUS = { (allUnits + allDeadMen) select { private _c = _x getVariable ["multi_c", -1]; if (_c < 0) then { _c = _x call MULTI_fnc_cellule }; _c == 5 } };
MC5_fnc_side = {
    private _s = _this;
...
MC5_fnc_identifier = {
    ...
    _u setVariable ["chacal_id", MC5_NEXT_ID];
    _u setVariable ["chacal_side", (side _u) call MC5_fnc_side];
    _u setVariable ["chacal_fire", 0];
    (format ["CHACAL|E|spawn|%1|%2|%3|%4|%5", round (time * 100) / 100, MC5_NEXT_ID,
        (side _u) call MC5_fnc_side, typeOf _u,
        (_u getVariable ["chacal_role", ""])]) call MC5_LOG;
    _u addEventHandler ["Fired", {
        ...
        (format ["CHACAL|E|tir|%1|%2|%3|%4", round (time * 100) / 100,
            ...
            MC5_PHASE]) call MC5_LOG;
    }];
    MC5_NEXT_ID = MC5_NEXT_ID + 1;
    MC5_NEXT_ID - 1
};
...
MC5_fnc_identifierVehicule = {
    ...
    _v addEventHandler ["Fired", {
        ...
        (format ["CHACAL|E|tir|%1|%2|%3|%4|bord|%5", round (time * 100) / 100,
            ...
            MC5_PHASE, typeOf _veh]) call MC5_LOG;
    }];
};

MC5_fnc_recenser = {
    { _x call MC5_fnc_identifier } forEach (call MC5_TOUS);
    { if ((count (crew _x) > 0) && { ((crew _x select 0) call MULTI_fnc_cellule) == 5 }) then { _x setVariable ["multi_c", 5]; _x call MC5_fnc_identifierVehicule } } forEach vehicles;
};
call MC5_fnc_recenser;
[] spawn { while { !MC5_FIN } do { call MC5_fnc_recenser; sleep 2 } };
(format ["CHACAL|OK|recenseur|1|au_premier_passage|%1", MC5_NEXT_ID - 1]) call MC5_LOG;
...
MC5_EH_FRAME = addMissionEventHandler ["EachFrame", {
    if (MC5_CAP_VERSION != 1) exitWith {};
    private _t = time;
    if (_t - MC5_LAST < MC5_DT) exitWith {};
    MC5_LAST = _t;
    private _d0 = diag_tickTime;
    MC5_TICK = MC5_TICK + 1;
    private _tr = round (_t * 100) / 100;
    private _tous = call MC5_TOUS;
    private _n = count _tous;
    private _bouts = []; private _cour = "";
    {
        private _u = _x;
        private _id = _u getVariable ["chacal_id", -1];
        if (_id < 0) then { _id = _u call MC5_fnc_identifier };
        ...
        private _e = str [
            ...
            (_u call MC5_fnc_posture),
            (_u call MC5_fnc_comport),
            round (speed _u),
            (_u getVariable ["chacal_fire", 0])
        ];
        if ((_u getVariable ["chacal_fire", 0]) == 1) then { _u setVariable ["chacal_fire", 0] };
        if ((count _cour) + (count _e) + 1 > MC5_MAXCAR) then { _bouts pushBack _cour; _cour = "" };
        _cour = if (_cour == "") then { _e } else { _cour + "," + _e };
    } forEach _tous;
    _bouts pushBack _cour;
    { (format ["CHACAL|S|%1|%2|%3|%4|%5|[%6]", MC5_TICK, _forEachIndex, _tr, _n, MC5_PHASE, _x]) call MC5_LOG; } forEach _bouts;

    MC5_CHRONO set [0, (MC5_CHRONO select 0) + (diag_tickTime - _d0)];
    MC5_CHRONO set [1, (MC5_CHRONO select 1) + 1];
    if ((MC5_CHRONO select 1) % 400 == 0) then {
        (format ["CHACAL|C|cout_ms|%1|ticks|%2|unites|%3|fps|%4",
            round (1000 * (MC5_CHRONO select 0) / (MC5_CHRONO select 1) * 100) / 100,
            (MC5_CHRONO select 1), _n, round diag_fps]) call MC5_LOG;
        MC5_CHRONO = [0, 0];
    };
}];
"CHACAL|OK|emetteur|1" call MC5_LOG;

// ======================= 3. LES TROIS VUES =======================
MC5_T_VUES = time - (5 * 0.37);
MC5_EH_VUES = addMissionEventHandler ["EachFrame", {
    if (MC5_FIN || { MC5_CAP_VERSION != 1 } || { (time - MC5_T_VUES) < MC5_DTV }) exitWith {};
    MC5_T_VUES = time;
    call {
        private _tr = round (time * 100) / 100;
        private _vivants = (call MC5_TOUS) select { alive _x };
        ...
        (format ["CHACAL|VC|%1|%2|est_sur_ouest|%3", _tr, MC5_PHASE, str _cb]) call MC5_LOG;
        (format ["CHACAL|VC|%1|%2|ouest_sur_est|%3", _tr, MC5_PHASE, str _co]) call MC5_LOG;
        ...
        private _fnc = {
            ...
            (format ["CHACAL|VH|%1|%2|%3|%4", _tr, MC5_PHASE, _etiq, str _lignes]) call MC5_LOG;
        };
        ...
        private _vg = [];
        {
            ...
            {
                if ([_u, _x] call MC5_fnc_voit) then {
        ...
        (format ["CHACAL|VG|%1|%2|ouest|%3", _tr, MC5_PHASE, str _vg]) call MC5_LOG;

    };
}];
"CHACAL|OK|vues|1" call MC5_LOG;
...
[] spawn { while { !MC5_FIN } do { sleep 2; (format ["CHACAL|C|pouls|%1", round (time * 100) / 100]) call MC5_LOG } };

// ======================= 4. LES MORTS =======================
MC5_EH_MORT = addMissionEventHandler ["EntityKilled", {
    params ["_vic", "_tueur", ["_instig", objNull]];
    if ((_vic call MULTI_fnc_cellule) != 5) exitWith {};   // MULTI : une mort d une autre cellule ne s ecrit pas ici
    ...
    private _t = _tueur call MC5_fnc_idTueur;
    (format ["CHACAL|E|mort|%1|%2|%3|%4|%5|par|%6", round (time * 100) / 100,
        (_vic getVariable ["chacal_id", -1]),
        (_t select 0), MC5_PHASE,
        (_vic getVariable ["chacal_role", ""]), (_t select 1)]) call MC5_LOG;
}];
"CHACAL|OK|morts|1" call MC5_LOG;
...
MC5_fnc_arreterCapture = {
    if (MC5_CAP_VERSION == 0) exitWith {};
    MC5_CAP_VERSION = 0;
    removeMissionEventHandler ["EachFrame", MC5_EH_FRAME];
    removeMissionEventHandler ["EachFrame", MC5_EH_VUES];
    removeMissionEventHandler ["EntityKilled", MC5_EH_MORT];
    { _x removeAllEventHandlers "Fired" } forEach ((call MC5_TOUS) + (vehicles select { (_x getVariable ["multi_c", -1]) == 5 }));
    (format ["CHACAL|OK|capture_arretee|ticks|%1|t|%2", MC5_TICK, round (time * 100) / 100]) call MC5_LOG;
};

(format ["CHACAL|OK|capture|%1|dt|%2|dtv|%3", MC5_CAP_VERSION, MC5_DT, MC5_DTV]) call MC5_LOG;
