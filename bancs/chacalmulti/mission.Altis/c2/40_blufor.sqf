...
MC2_gFS = ([west, 2] call MULTI_fnc_groupe);
MC2_FS = [];
...
private _rolesTous = [];
for "_p" from 1 to (round (MC2_EFFECTIF / 10)) do { _rolesTous append _rolesBase };

{
    _x params ["_cls", "_role"];
    private _u = MC2_gFS createUnit [_cls, MC2_LZ getPos [12 call MC2_fnc_al, 360 call MC2_fnc_al], [], 0, "NONE"];
    [_u, _role] call MC2_fnc_kitOuest;
    MC2_FS pushBack _u;
} forEach _rolesTous;if (count MC2_FS < MC2_EFFECTIF) exitWith {
    MC2_ISSUE = "VOID"; MC2_CAUSE = "DETACHEMENT_INCOMPLET";
    (format ["CHACAL|VOID|blufor|%1", count MC2_FS]) call MC2_LOG;
};

MC2_gFS selectLeader (MC2_FS select 0);
MC2_gFS setBehaviour "STEALTH";
MC2_gFS setCombatMode "GREEN";      // ne tire que si on lui tire dessus
MC2_gFS setSpeedMode "LIMITED";
MC2_gFS setFormation "FILE";        // la formation de nuit hors piste
MC2_gFS allowFleeing 0;
...
{ for "_i" from 1 to 3 do { _x addMagazine "DemoCharge_Remote_Mag" }; }
    forEach (MC2_FS select { (_x getVariable ["chacal_role",""]) in ["DEMO_1","DEMO_2"] });
{ _x addMagazine "DemoCharge_Remote_Mag"; }
    forEach (MC2_FS select { (_x getVariable ["chacal_role",""]) in ["CHEF","ADJOINT"] });

MC2_fnc_role = {
    private _r = _this;
    MC2_FS select { alive _x && { (_x getVariable ["chacal_role",""]) in _r } }
};
...
MC2_RECO    = ["TIREUR_1","TIREUR_2"];
MC2_APPUI   = ["TIREUR_1","TIREUR_2"];
MC2_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN"];
MC2_BOUCHON = ["AT","FUSILIER_1","FUSILIER_2"];
...
if (MC2_PARTAGE == 1) then {
    MC2_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN","FUSILIER_1","FUSILIER_2"];
    MC2_BOUCHON = ["AT"];
};

MC2_gAppui = grpNull; MC2_gAssaut = grpNull;
MC2_gBouchon = grpNull; MC2_gReco = grpNull; MC2_gDemo = grpNull;
...
MC2_fnc_detacher = {
    params ["_roles", "_nom"];
    private _u = _roles call MC2_fnc_role;
    if (count _u == 0) exitWith { grpNull };
    private _g = ([west, 2] call MULTI_fnc_groupe);
    _u joinSilent _g;
    _g selectLeader (_u select 0);
    _g setVariable ["chacal_element", _nom, true];
    _g allowFleeing 0;
    (format ["CHACAL|E|scission|%1|%2|%3", round (time * 100) / 100, _nom, count _u]) call MC2_LOG;
    _g
};

if (MC2_IMMORTEL == 1) then {
    { _x allowDamage false } forEach MC2_FS;
    "CHACAL|AVERT|hors_corpus|immortel|1|couture_sous_le_feu" call MC2_LOG;
};

(format ["CHACAL|OK|blufor|%1|roles|%2", count MC2_FS,
    (MC2_FS apply { _x getVariable ["chacal_role", "?"] })]) call MC2_LOG;
