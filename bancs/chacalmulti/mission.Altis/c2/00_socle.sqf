...
MC2_GRAINE  = (["MC2_GRAINE", 0] call MULTI_fnc_param)
               + 32 * (["MC2_GRAINE_HAUT", 0] call MULTI_fnc_param);
MC2_ECHELLE = (["MC2_ECHELLE", 100] call MULTI_fnc_param) / 100;
MC2_DT      = (["MC2_DTCS", 100] call MULTI_fnc_param) / 100;
if (MC2_ECHELLE <= 0) then { MC2_ECHELLE = 1 };
if (MC2_DT <= 0) then { MC2_DT = 1 };
// 0 = le PLAN en six phases, 1 = le TEMOIN qui marche droit
MC2_BRAS   = if ((["MC2_BRAS", 0] call MULTI_fnc_param) == 1) then {"NUL"} else {"PLAN"};
MC2_JOUR   = ["MC2_JOUR", 0] call MULTI_fnc_param;
MC2_DEPART = ["MC2_DEPART", 1] call MULTI_fnc_param;
MC2_ARRET  = ["MC2_ARRET", 6] call MULTI_fnc_param;   // vignette : derniere phase jouee
MC2_PALIER = ["MC2_PALIER", 3] call MULTI_fnc_param;
MC2_IMMORTEL = ["MC2_IMMORTEL", 0] call MULTI_fnc_param;
...
// ! MC2_TENIR : poursuivre le plan malgre la compromission (0 = regle prudente d origine).
// La compromission reste JOURNALISEE a l identique ; seule la REACTION change.
// ! MC2_ACCESSIBLE : exiger qu une position soit ATTEIGNABLE, pas seulement plate.
// 0 = comportement d origine, conserve pour que tout ce qui precede reste comparable.
// ! MC2_EFFECTIF : taille du detachement. 10 = d origine.
// A 20, la liste des roles est jouee deux fois : chaque element double, et le corpus
// continue de porter les memes noms de role.
// ! MC2_APPUI_FEU : donner a l appui une position choisie pour TIRER, distincte de
// l observatoire. 0 = comportement d origine, ou l appui reste a l observatoire.
MC2_APPUI_FEU = ["MC2_APPUI_FEU", 0] call MULTI_fnc_param;
// ! MC2_FEU_AVANT ( Fable, 10/09 ) : l appui connait et vise les defenseurs, l assaut attend son
// premier coup, puis marche en AWARE avec un ordre relance toutes les 10 s. 0 = comportement d origine.
MC2_FEU_AVANT = ["MC2_FEU_AVANT", 0] call MULTI_fnc_param;
// ! CAMPAGNE GRAINE 8 ( Fable, 11/09 ) - trois leviers, 0 / 45 = comportement d origine.
MC2_MG_ASSAUT = ["MC2_MG_ASSAUT", 0] call MULTI_fnc_param;
MC2_DELAI_PORTEUR = ["MC2_DELAI_PORTEUR", 45] call MULTI_fnc_param;
// ! BOUCLE EVOGP ( equation/PROTOCOLE_BOUCLE_EVOGP_P5.md, 17/09 ) : le delai du porteur peut etre decide par une formule
// apprise par EvoGP, passee en codes entiers ( MC2_F0..F31, ordre prefixe, equation/formule.py ). Mode 0 = impose par
// le job ( origine ). MC2_DELAI_PORTEUR reste la valeur du job ( ligne FINI ) ; MC2_DELAI_JOUE est le delai joue.
MC2_DELAI_MODE = ["MC2_DELAI_MODE", 0] call MULTI_fnc_param;
MC2_F_LEN = ["MC2_F_LEN", 0] call MULTI_fnc_param;
MC2_F = [];
for "_k" from 0 to 31 do { MC2_F pushBack (["MC2_F" + str _k, 0] call MULTI_fnc_param) };
MC2_F resize MC2_F_LEN;
MC2_DELAI_JOUE = MC2_DELAI_PORTEUR;
MC2_APPUI_FIXE = ["MC2_APPUI_FIXE", 0] call MULTI_fnc_param;
// ! MC2_ORACLE ( Fable, 11/09 ) : les defenseurs reveles a tous et inscrits comme vus. 0 = origine.
MC2_ORACLE = ["MC2_ORACLE", 0] call MULTI_fnc_param;
// ! LE SOCLE ET LES TACTIQUES ( 11/09 ). 0 = comportement d origine dans les deux cas.
MC2_SOCLE = ["MC2_SOCLE", 0] call MULTI_fnc_param;
...
MC2_AZIMUT = ["MC2_AZIMUT", 0] call MULTI_fnc_param;
...
// ou on la reparera.
MC2_OBS = ["MC2_OBS", 1] call MULTI_fnc_param;
// ! MC2_GEOMETRIE : rendre la geometrie du site SANS jouer la mission.
...
MC2_GEOMETRIE = ["MC2_GEOMETRIE", 0] call MULTI_fnc_param;
MC2_AZIMUT_VAL = ["MC2_AZIMUT_VAL", 0] call MULTI_fnc_param;
MC2_AZIMUT_CHOISI = -1;   // ce qui a REELLEMENT ete joue, ecrit dans la ligne FINI
// ! MC2_EXFIL : ce qui fait echouer l exfiltration. 12 episodes sur 12 finissent en PLAFOND
...
MC2_EXFIL = ["MC2_EXFIL", 0] call MULTI_fnc_param;
MC2_TACTIQUE = ["MC2_TACTIQUE", 0] call MULTI_fnc_param;
...
MC2_PLACEUR = ["MC2_PLACEUR", 0] call MULTI_fnc_param;
MC2_COUV = -1;   // part des points interieurs battus depuis la position retenue
MC2_BANC_APPUI = ["MC2_BANC_APPUI", 0] call MULTI_fnc_param;
MC2_TIRS_BANC = 0;
...
MC2_ABLATION = ["MC2_ABLATION", 0] call MULTI_fnc_param;
MC2_fnc_sans = { (floor (MC2_ABLATION / _this)) % 2 == 1 };   // _this = le bit teste
MC2_CONNUS = [];          // defenseurs qui se sont trahis en tirant
MC2_T_PREMIER_TIR_APPUI = -1;
MC2_RELANCES_SOCLE = 0; MC2_FUMIGENES = 0; MC2_ZONES = 0;
MC2_SANS_JAMBES = 0;   // hommes vivants incapables de marcher a l exfiltration - la garde du 13/09
MC2_CIBLE_ASSAUT = [];   // ! posee ici : le chien de garde demarrait avant son initialisation
MC2_EFFECTIF = ["MC2_EFFECTIF", 10] call MULTI_fnc_param;
MC2_ACCESSIBLE = ["MC2_ACCESSIBLE", 0] call MULTI_fnc_param;
MC2_TENIR = ["MC2_TENIR", 0] call MULTI_fnc_param;
MC2_HMG_FORCE = ["MC2_HMG", -1] call MULTI_fnc_param;
MC2_ASSAUT_X  = (["MC2_ASSAUT_X", 100] call MULTI_fnc_param) / 100;
...
MC2_PARTAGE = ["MC2_PARTAGE", 0] call MULTI_fnc_param;
...
MC2_QRF_N     = ["MC2_QRF_N", -1] call MULTI_fnc_param;
MC2_QRF_DELAI = ["MC2_QRF_DELAI", -1] call MULTI_fnc_param;
// Distance imposee de la base de reserve, appliquee dans 10_monde.sqf APRES le tirage.
MC2_QRF_DIST  = ["MC2_QRF_DIST", -1] call MULTI_fnc_param;
MC2_ACC       = ["MC2_ACC", 1] call MULTI_fnc_param;
// La situation : voir 35_menaces.sqf. Tout a 0 par defaut.
MC2_SITUATION = ["MC2_SITUATION", 0] call MULTI_fnc_param;
MC2_MENACE_P1 = ["MC2_MENACE_P1", 0] call MULTI_fnc_param;
MC2_MENACE_P2 = ["MC2_MENACE_P2", 0] call MULTI_fnc_param;
MC2_MENACE_P3 = ["MC2_MENACE_P3", 0] call MULTI_fnc_param;
MC2_MENACE_P4 = ["MC2_MENACE_P4", 0] call MULTI_fnc_param;
MC2_MENACE_P5 = ["MC2_MENACE_P5", 0] call MULTI_fnc_param;
MC2_MENACE_P6 = ["MC2_MENACE_P6", 0] call MULTI_fnc_param;
// ! LES CHOIX DES VIGNETTES ( plans/plan-choix-par-vignette.md, 17/09 ). 0 = la regle d origine, inchangee.
MC2_P1_ATTENTE = ["MC2_P1_ATTENTE", 0] call MULTI_fnc_param;   // 1 partir tout de suite, 2 se terrer 3 min
MC2_TRAVERSEE  = ["MC2_TRAVERSEE", 0] call MULTI_fnc_param;    // 1 traverser tout de suite, 2 attendre la patrouille
MC2_OBS_DUREE  = ["MC2_OBS_DUREE", 0] call MULTI_fnc_param;    // secondes d observation : 120 ou 480
MC2_ITINERAIRE = ["MC2_ITINERAIRE", 0] call MULTI_fnc_param;
...
MC2_OBSERVATION = ["MC2_OBSERVATION", 0] call MULTI_fnc_param;
MC2_CONTROLE_PERCEPTION = ["MC2_CONTROLE_PERCEPTION", 0] call MULTI_fnc_param;   // 3 = 150 m devant, DEBOUT
MC2_SONDE = ["MC2_SONDE", 0] call MULTI_fnc_param;
// ! L ORACLE COMMANDANT ( plan a139c63 ). 0 = temoin, le decor d aujourd hui. 1 = il cherche.
MC2_PORTEE_SON = ["MC2_PORTEE_SON", 600] call MULTI_fnc_param;   // portee du moteur entendu, en metres
// ! 1 = une ligne sonde_decision toutes les 10 s pendant la fenetre : n importe quelle duree de fenetre se relit apres coup
MC2_SONDE_PERCEPTION = ["MC2_SONDE_PERCEPTION", 0] call MULTI_fnc_param;   // portee du moteur entendu, en metres
MC2_ORACLE_CMD   = ["MC2_ORACLE_CMD", 0] call MULTI_fnc_param;
MC2_ORACLE_B     = ["MC2_ORACLE_B", 6] call MULTI_fnc_param;      // budget : nombre de deplacements de patrouille
MC2_ORACLE_CTRL  = ["MC2_ORACLE_CTRL", 0] call MULTI_fnc_param;   // controle de l Oracle : 0 aucun, 1 positif, 3 non-triche
MC2_ORACLE_NU    = ["MC2_ORACLE_NU", 15] call MULTI_fnc_param;    // doute, en pourcent : il se trompe
MC2_ORACLE_EPS   = ["MC2_ORACLE_EPS", 15] call MULTI_fnc_param;   // erreur volontaire, en pourcent
MC2_ORACLE_DELTA = ["MC2_ORACLE_DELTA", 60] call MULTI_fnc_param; // periode de decision, en secondes
// ! PRIX DU TEMPS ( saisine de Fable, 18/09 ) : attente imposee AVANT la phase 5, hors plafond de phase. 0 = origine.
MC2_ATTENTE_TEST = ["MC2_ATTENTE_TEST", 0] call MULTI_fnc_param;
MC2_AVANT = ["MC2_AVANT", 260] call MULTI_fnc_param;   // phase 2 : d ou l on observe la route ( origine 260 m )
MC2_BALAYAGE = ["MC2_BALAYAGE", 0] call MULTI_fnc_param;   // fenetre : 0 balayage d origine, 1 balayage repare
MC2_CONTROLE_DIST = ["MC2_CONTROLE_DIST", 150] call MULTI_fnc_param;   // banc de perception : distance de la cible   // 1 : une ligne sonde_perception toutes les 5 s pendant la fenetre   // 1 direct, 2 detour de 350 m
...
MC2_RNG = ((MC2_GRAINE * 7919) + 104729) % 65537;
if (MC2_RNG == 0) then { MC2_RNG = 1 };
// ! UN TEMOIN QUI NE TIRE QU UNE VALEUR N EST PAS UN TEMOIN ( 14/09 ).
// MC2_fnc_rnd est seme par la graine, et c est voulu : le MONDE doit etre reproductible.
...
MC2_RNG_T = ((MC2_GRAINE * 2654435761) + (round (serverTime * 1000)) + (round (diag_tickTime * 997))) % 65537;
if (MC2_RNG_T == 0) then { MC2_RNG_T = 1 };
MC2_fnc_rndTemoin = {
    MC2_RNG_T = ((MC2_RNG_T * 75) + 74) % 65537;
    MC2_RNG_T / 65537
};

MC2_fnc_rnd = {
    MC2_RNG = ((MC2_RNG * 75) + 74) % 65537;
    MC2_RNG / 65537
};
MC2_fnc_al = { (call MC2_fnc_rnd) * _this };      // uniforme sur [0 ; _this[
// On brule les premiers tirages : deux graines voisines commenceraient sinon
// au meme endroit.
for "_i" from 1 to 20 do { call MC2_fnc_rnd };

// --- etat de l episode ---
MC2_PHASE      = 0;
MC2_PHASE_NOM  = "AUCUNE";
MC2_ALARME     = false;      // le camp EST a compris - irreversible
MC2_COMPROMIS  = false;      // NOUS nous en sommes rendu compte
MC2_FIN        = false;
...
if (MC2_ACC != 1) then {
    setAccTime MC2_ACC;
    (format ["CHACAL|AVERT|hors_corpus|acc|%1", MC2_ACC]) call MC2_LOG;
    [] spawn {
        while { !MC2_FIN } do {
            (format ["CHACAL|E|horloge|%1|reel|%2|acc_demande|%3|acc_moteur|%4",
                round (time * 100) / 100, round (diag_tickTime * 100) / 100, MC2_ACC, accTime]) call MC2_LOG;
            sleep 30;
        };
    };
};
...
MC2_DUREES = [900, 1800, 1500, 900, 900, 600];
MC2_fnc_duree = { (MC2_DUREES select (_this - 1)) * MC2_ECHELLE };

// L HORLOGE DE PHASE. Toute attente s y refere : une sous-etape ne peut pas
// emprunter du temps a la phase suivante.
MC2_TPHASE = 0;
MC2_PLAFOND_COURANT = 1e9;
MC2_fnc_reste = { (MC2_TPHASE + MC2_PLAFOND_COURANT) - time };

MC2_fnc_has = { isClass (configFile >> "CfgVehicles" >> (_this select 0)) };
...
// La phase 6 demandait le budget du repli a MC2_gFS - un groupe VIDE depuis
...
MC2_fnc_budget = {
    ...
    if (count _u == 0) exitWith {
        (format ["CHACAL|AVERT|budget_defaut|%1|aucun_homme|vers|%2", round (time * 100) / 100, _p]) call MC2_LOG;
        120
    };
    private _c = _u call MC2_fnc_centre;
    if (count _c == 0) exitWith {
        (format ["CHACAL|AVERT|budget_defaut|%1|centre_vide|vers|%2", round (time * 100) / 100, _p]) call MC2_LOG;
        120
    };
...
MC2_fnc_pose = {
    ...
    if (!([_cls] call MC2_fnc_has)) exitWith { MC2_MANQUANTES pushBackUnique _cls; objNull };
...
MC2_fnc_poseP = {
    ...
    private _p = _c getPos [_dist, MC2_AZ + _gis];
    [_cls, _c, (_p select 0) - (_c select 0), (_p select 1) - (_c select 1), MC2_AZ + _dirRel] call MC2_fnc_pose
};
...
// MC2_ACCESSIBLE vaut 0, la fonction se comporte exactement comme avant.
MC2_fnc_penteTrajet = {
    params ["_a", "_b"];
...
MC2_fnc_plancher = {
    private _R = 45;
    if (count MC2_OBJETS > 0) then {
        { _R = _R max ((_x distance2D MC2_SITE) + 15) } forEach MC2_OBJETS;
    };
    _R = (_R max 30) min 80;
    private _pts = [];
    private _c = +MC2_SITE; _c set [2, (getTerrainHeightASL MC2_SITE) + 1.1];
    _pts pushBack _c;
    {
        _x params ["_r", "_n", "_dec"];
        for "_i" from 0 to (_n - 1) do {
            private _p = MC2_SITE getPos [_R * _r, _dec + (_i * 360 / _n)];
...
MC2_fnc_positionAppui = {
    ...
    if (MC2_PLACEUR == 1) exitWith {
        ...
        private _plancher = call MC2_fnc_plancher;
        ...
        while { _passe < 3 && { count _best == 0 } } do {
            ...
            for "_a" from 0 to 35 do {
                for "_k" from 3 to 10 do {
                    ...
                    private _c = [_p, _plancher] call MC2_fnc_couverture;
            ...
            // ! SONDES : quatre points fixes a 150 m, pour NOMMER ce qui arrete les rayons.
            {
                ...
                (format ["CHACAL|E|appui_sonde|%1|passe|%2|azimut|%3|libres|%4|sur|%5|terrain|%6|objets|%7|gain|%8",
                    ...
                    round ((getTerrainHeightASL _ps) - _solSite)]) call MC2_LOG;
            ...
            (format ["CHACAL|E|appui_balayage|%1|passe|%2|temoin|%3|balayes|%4|refus_eau|%5|refus_pente|%6|refus_axe|%7|meilleure_couverture|%8|retenue|%9",
                ...
                _refus select 2, round (_meilleurBrut * 1000) / 1000, count _best]) call MC2_LOG;
        };
        MC2_COUV = round (_couv * 1000) / 1000;
        if (count _best == 0 || { _couv < 0.25 }) exitWith {
            (format ["CHACAL|AVERT|appui_feu|aucune_position_battante|meilleure_couverture|%1|temoin|%2|passes|%3",
                round (_meilleurBrut * 1000) / 1000, round (_temoin * 1000) / 1000, _passe]) call MC2_LOG;
            (format ["CHACAL|E|appui_feu|%1|placeur|1|couverture|%2|temoin|%3|refuse|1",
                round (time * 100) / 100, MC2_COUV, round (_temoin * 1000) / 1000]) call MC2_LOG;
            _repli
        };
        _best set [2, 0];
        (format ["CHACAL|E|appui_feu|%1|placeur|1|position|%2|couverture|%3|temoin|%4|passes|%5|points|%6|dist_site|%7|gain_sol_site|%8|score|%9|azimut|%10",
            round (time * 100) / 100, _best, MC2_COUV, round (_temoin * 1000) / 1000, _passe, _np,
            round (_best distance2D _site), round ((getTerrainHeightASL _best) - _solSite),
            round (_sc * 100) / 100, round (_site getDir _best)]) call MC2_LOG;
        _best
    };
    ...
    for "_i" from 1 to 160 do {
        ...
        private _cote = if ((call MC2_fnc_rnd) < 0.5) then {1} else {-1};
        private _dec = _cote * (40 + (80 call MC2_fnc_al));
        private _dist = 160 + (190 call MC2_fnc_al);
    ...
    if (count _best == 0) exitWith {
        (format ["CHACAL|AVERT|appui_feu|aucune_position|repli_observatoire"]) call MC2_LOG;
        _repli
    };
    _best set [2, 0];
    (format ["CHACAL|OK|appui_feu|position|%1|dist_ouverture|%2|gain|%3|score|%4|decal_axe|%5",
        ...
        round (abs ((_site getDir _best) - (_site getDir _axeAssaut)))]) call MC2_LOG;
    _best
};
...
MC2_fnc_socleAssaut = {
    private _ass = (units MC2_gAssaut) select { alive _x };
    ...
    if (!isNull MC2_gAppui && { !(4 call MC2_fnc_sans) }) then {
        MC2_gAppui setBehaviour "COMBAT"; MC2_gAppui setCombatMode "RED";
        MC2_gAppui setVariable ["lambs_danger_disableGroupAI", true, true];
        {
            if (alive _x) then {
                ...
                _x addEventHandler ["Fired", {
                    if (MC2_T_PREMIER_TIR_APPUI < 0) then { MC2_T_PREMIER_TIR_APPUI = time };
                }];
            };
    ...
    {
        if (alive _x && { !(8 call MC2_fnc_sans) }) then {
            _x addEventHandler ["Fired", {
                params ["_t"];
                if (!(_t in MC2_CONNUS)) then {
                    MC2_CONNUS pushBack _t;
                    { _x reveal [_t, 4] } forEach ((units MC2_gAppui) + (units MC2_gAssaut));
                    (format ["CHACAL|E|trahi_par_son_tir|%1|%2|connus|%3", round (time * 100) / 100,
                        (_t getVariable ["chacal_id", -1]), count MC2_CONNUS]) call MC2_LOG;
                };
            }];
        };
    } forEach MC2_EST_SITE;
    (format ["CHACAL|E|socle|%1|porteurs|%2|appui_cloue|%3|ablation|%4|sans_chien|%5|sans_porteurs|%6|sans_appui|%7|sans_revelation|%8",
        ...
        (if (isNull MC2_gAppui || { 4 call MC2_fnc_sans }) then {0} else {count (units MC2_gAppui)}),
        MC2_ABLATION,
        (if (1 call MC2_fnc_sans) then {1} else {0}), (if (2 call MC2_fnc_sans) then {1} else {0}),
        (if (4 call MC2_fnc_sans) then {1} else {0}), (if (8 call MC2_fnc_sans) then {1} else {0})]) call MC2_LOG;
};

// La cible que l appui doit prendre : le fusilier-mitrailleur d abord, puis le chef, puis le reste.
MC2_fnc_ciblePrio = {
    private _viv = (MC2_EST_SITE select { alive _x });
...
MC2_fnc_tactiqueAppui = {
    private _t0 = time; private _derniere = objNull; private _tCible = 0; private _tSupp = -99;
    while { !MC2_FIN && { MC2_PHASE == 5 } && { !isNull MC2_gAppui } } do {
        private _app = (units MC2_gAppui) select { alive _x };
        ...
        if (MC2_TACTIQUE == 2) then {
            ...
            if (isNull _derniere || { !alive _derniere } || { time - _tCible > 60 }) then {
                _derniere = call MC2_fnc_ciblePrio; _tCible = time;
                if (!isNull _derniere) then {
                    if (!MC2_FIN) then {
                        (format ["CHACAL|E|cible_designee|%1|%2|restants|%3", round (time * 100) / 100,
                            (_derniere getVariable ["chacal_id", -1]), count _viv]) call MC2_LOG;
                    };
                };
            };
            if (!isNull _derniere) then {
            ...
            private _cn = MC2_CONNUS select { alive _x };
            if (count _cn == 0) then {
                ...
                if (time - _tSupp > 30) then {
                    _tSupp = time; MC2_ZONES = MC2_ZONES + 1;
                    private _z = if (!isNull MC2_PC) then { getPosATL MC2_PC } else { MC2_SITE };
                    { _x doWatch _z; _x doSuppressiveFire _z } forEach _app;
                    (format ["CHACAL|E|suppression_de_zone|%1|vers|%2|n|%3", round (time * 100) / 100, str _z, MC2_ZONES]) call MC2_LOG;
                };
            };
            if (count _cn > 0) then {
                private _ca = ((units MC2_gAssaut) select { alive _x }) call MC2_fnc_centre;
                ...
                if (!isNull _cible && { time - _tSupp > 30 }) then {
                    ...
                    if (!MC2_FIN) then {
                        (format ["CHACAL|E|suppression|%1|%2|distance_assaut|%3", round (time * 100) / 100,
                            (_cible getVariable ["chacal_id", -1]), round _dmax]) call MC2_LOG;
                    };
                };
            };
        };
...
MC2_fnc_chienDeGarde = {
    ...
    while { !MC2_FIN && { MC2_PHASE == 5 } } do {
        ...
        // apres le debut de la phase 6. C est le bug corrige le 12/09 pour MC2_FIN, dans cette meme
        ...
        // Quand MC2_FIN est pose, le verdict a DEJA ecrit sa ligne CHACAL|FINI| ; le chien, lui,
        ...
        if (MC2_FIN) exitWith {};
        if (MC2_PHASE != 5) exitWith {
            (format ["CHACAL|E|chien_de_garde|%1|phase_quittee|%2|relances|%3", round (time * 100) / 100,
                MC2_PHASE, MC2_RELANCES_SOCLE]) call MC2_LOG;
        };
        if (count MC2_CIBLE_ASSAUT > 0 && { !isNull MC2_gAssaut }) then {
            private _v = (units MC2_gAssaut) select { alive _x };
            if (count _v > 0) then {
                private _c = _v call MC2_fnc_centre;
                private _d = _c distance2D MC2_CIBLE_ASSAUT;
                if (_d < _dRef - 5) then { _dRef = _d; _tRef = time; _rates = 0 }
                else {
                    if (time - _tRef > 30) then {
                        _tRef = time; _rates = _rates + 1; MC2_RELANCES_SOCLE = MC2_RELANCES_SOCLE + 1;
                        MC2_gAssaut setBehaviour "AWARE";
                        { if (alive _x) then { _x doMove MC2_CIBLE_ASSAUT } } forEach _v;
                            if (MC2_FIN) exitWith {};   // ! rien apres la ligne FINI : la porte de lecture refuse l episode
                        (format ["CHACAL|E|chien_de_garde|%1|relance|%2|reste|%3", round (time * 100) / 100,
                            _rates, round _d]) call MC2_LOG;
                        if (_rates >= 2) then {
                            private _cn = MC2_CONNUS select { alive _x };
                            private _vers = if (count _cn > 0) then { getPosATL (_cn select 0) } else { MC2_CIBLE_ASSAUT };
                            private _p = _c getPos [25, _c getDir _vers];
                            createVehicle ["SmokeShell", _p, [], 0, "CAN_COLLIDE"];
                            MC2_FUMIGENES = MC2_FUMIGENES + 1; _rates = 0;
                            (format ["CHACAL|E|fumigene|%1|entre|%2|et|%3", round (time * 100) / 100, str _p, str _vers]) call MC2_LOG;
                        };
                    };
                };
            };
...
MC2_fnc_bancAppui = {
    ...
    if (count _app == 0 || { count _def == 0 }) exitWith {
        (format ["CHACAL|E|banc_appui_fin|%1|coups|0|tireurs|%2|defenseurs|%3|cause|RIEN_A_MESURER",
            round (time * 100) / 100, count _app, count _def]) call MC2_LOG;
    };
    "CHACAL|AVERT|hors_corpus|banc_appui|episode_de_certification" call MC2_LOG;
    ...
    private _cible = _tri select 0;
    {
        ...
        if (MC2_BANC_APPUI == 1) then {
            _u disableAI "PATH";
            _u setVariable ["lambs_danger_disableAI", true, true];
        };
        { _u reveal [_x, 4] } forEach _def;
        _u addEventHandler ["Fired", { MC2_TIRS_BANC = MC2_TIRS_BANC + 1; (_this select 0) setVariable ["banc_tirs", ((_this select 0) getVariable ["banc_tirs", 0]) + 1] }];
        ...
        (format ["CHACAL|E|banc_appui_poste|%1|%2|role|%3|distance_cible|%4|defenseurs_vus|%5|sur|%6|connait|%7|occulteur|%8|mode|%9|couverture|%10",
            ...
            (if (_occulteur == "") then {"aucun"} else {_occulteur}), (combatMode (group _u)), MC2_COUV]) call MC2_LOG;
    } forEach _app;
    MC2_gAppui setBehaviour "COMBAT"; MC2_gAppui setCombatMode "RED";
    if (MC2_BANC_APPUI == 1) then { MC2_gAppui setVariable ["lambs_danger_disableGroupAI", true, true] };
    { if (alive _x) then { _x doWatch _cible; _x doTarget _cible; _x doFire _cible } } forEach _app;
    private _t0 = time; private _morts0 = { !alive _x } count MC2_EST_SITE;
    while { (time - _t0) < (300 * MC2_ECHELLE) && { !MC2_FIN } } do {
        sleep 10;
        private _viv = MC2_EST_SITE select { alive _x };
        private _a = (units MC2_gAppui) select { alive _x };
        ...
        // s ouvrir ensuite. On ecrit toutes les 10 s ce que chaque tireur atteint reellement.
        {
            ...
            (format ["CHACAL|E|banc_appui_vue|%1|%2|vus|%3|sur|%4|coups|%5|mode|%6", round (time * 100) / 100,
                ...
                (combatMode (group _u))]) call MC2_LOG;
        } forEach _a;
    };
    private _detail = "";
    ...
    (format ["CHACAL|E|banc_appui_fin|%1|coups|%2|par_tireur|%3|defenseurs_tues|%4|duree|%5|appui_vivant|%6|cloue|%7",
        round (time * 100) / 100, MC2_TIRS_BANC, _detail,
        ({ !alive _x } count MC2_EST_SITE) - _morts0, round (time - _t0),
        count ((units MC2_gAppui) select { alive _x }), MC2_BANC_APPUI]) call MC2_LOG;
};

MC2_fnc_plat = {
    params ["_c", "_r", ["_essais", 220], ["_depuis", []]];
    private _best = +_c; private _sc = 1e9;
    private _garde = (MC2_ACCESSIBLE == 1) && { count _depuis > 0 };
    for "_i" from 1 to _essais do {
        private _p = _c getPos [sqrt(call MC2_fnc_rnd) * _r, 360 call MC2_fnc_al];
        if (!surfaceIsWater _p) then {
            ...
            if (_garde) then {
                private _pt = [_depuis, _p] call MC2_fnc_penteTrajet;
    ...
    if (_garde) then {
        (format ["CHACAL|OK|plat|accessible|1|depuis|%1|choisi|%2|pente_trajet|%3|score|%4",
            _depuis, _best, round ([_depuis, _best] call MC2_fnc_penteTrajet), round _sc]) call MC2_LOG;
    };
    _best set [2, 0]; _best
};
...
MC2_LAMBS = !(isNil "lambs_wp_fnc_taskPatrol") && { !(isNil "lambs_danger_fnc_tactics") };
...
(format ["CHACAL|EMPREINTE|version|%1|addons|%2|somme|%3|lune|%4|radioShout|%5|radioBackpack|%6|maxRange|%7|cqbRange|%8|manoeuvres|%9",
    productVersion select 2, count activatedAddons, call MC2_fnc_empreinte,
    round ((moonPhase date) * 1000) / 1000,
    ("lambs_main_radioShout" call MC2_fnc_reglage), ("lambs_main_radioBackpack" call MC2_fnc_reglage),
    ("lambs_danger_maxRange" call MC2_fnc_reglage), ("lambs_danger_cqbRange" call MC2_fnc_reglage),
    ("lambs_danger_disableAIAutonomousManoeuvres" call MC2_fnc_reglage)]) call MC2_LOG;

(format ["CHACAL|OK|socle|%1|graine|%2|echelle|%3|dt|%4|lambs|%5|bras|%6|depart|%7|jour|%8|palier|%9|immortel|%10|tenir|%11|arret|%12",
    MC2_VERSION, MC2_GRAINE, MC2_ECHELLE, MC2_DT,
    (if (MC2_LAMBS) then {1} else {0}), MC2_BRAS, MC2_DEPART, MC2_JOUR, MC2_PALIER, MC2_IMMORTEL, MC2_TENIR, MC2_ARRET]) call MC2_LOG;
