...
MC2_SITE = []; MC2_OP = []; MC2_LZ = []; MC2_PZ = [];
MC2_ROUTE = []; MC2_QRF_BASE = []; MC2_OP_GAIN = -1e9; MC2_OP_VUE = false;
MC2_ROUTE_A = []; MC2_ROUTE_B = []; MC2_RALLY = [];

private _essais = 0; private _trouve = false;
// Un refus qui ne nomme pas sa cause envoie chercher au hasard : on compte.
MC2_REFUS = ["ancre",0,"replat",0,"crete",0,"vue",0,"route",0,"lz",0,"qrf",0,"pz",0];
MC2_fnc_refus = {
    private _i = MC2_REFUS find _this;
    if (_i >= 0) then { MC2_REFUS set [_i + 1, (MC2_REFUS select (_i + 1)) + 1] };
};
...
while { _essais < 120 && !_trouve } do {
    ...
    for "_k" from 1 to 80 do {
        private _p = [4000 + (22000 call MC2_fnc_al), 4000 + (22000 call MC2_fnc_al), 0];
        if (!surfaceIsWater _p && { (getTerrainHeightASL _p) > 12 }
            && { count (nearestObjects [_p, ["House"], 90]) <= 2 }) exitWith { _a = _p };
    };
    if (count _a == 0) then { "ancre" call MC2_fnc_refus; continue };

    private _site = [_a, 90, 200] call MC2_fnc_plat;
    if (surfaceIsWater _site) then { "replat" call MC2_fnc_refus; continue };
    ...
    for "_i" from 1 to 500 do {
        private _p = _site getPos [480 + (340 call MC2_fnc_al), 360 call MC2_fnc_al];
        _p set [2, 0];
    ...
    if (count _op == 0 || { _gain < 18 }) then { "crete" call MC2_fnc_refus; continue };
    _vue = [_op, _site] call MC2_fnc_libre;
    if (!_vue) then {
        ...
        for "_i" from 1 to 220 do {
            private _q = _op getPos [90 call MC2_fnc_al, 360 call MC2_fnc_al]; _q set [2, 0];
            if (!surfaceIsWater _q && { [_q, _site] call MC2_fnc_libre }) exitWith { _op = _q; _vue = true };
        };
    };
    if (!_vue) then { "vue" call MC2_fnc_refus; continue };
    ...
    if (count _rs == 0) then { "route" call MC2_fnc_refus; continue };
    ...
    for "_j" from 1 to 30 do {
        private _c1 = [_site getPos [3500 + (1000 call MC2_fnc_al), _azLz + (60 call MC2_fnc_al) - 30], 130, 160] call MC2_fnc_plat;
    ...
    if (count _lz == 0) then { "lz" call MC2_fnc_refus; continue };
    ...
    if (count _qrs == 0) then { "qrf" call MC2_fnc_refus; continue };
    private _qcles = _qrs apply { [round ((getPosATL _x) select 0) * 100000 + round ((getPosATL _x) select 1), _x] };
    _qcles sort true;
    private _qb = getPosATL ((_qcles select (floor ((count _qcles) call MC2_fnc_al))) select 1);
    _qb set [2, 0];

    // --- 6. le point d extraction de secours ---
    private _pz = [];
    for "_j" from 1 to 15 do {
        private _c2 = [_site getPos [1200 + (400 call MC2_fnc_al), _azLz + 120 + (80 call MC2_fnc_al)], 110, 160] call MC2_fnc_plat;
        if (!surfaceIsWater _c2) exitWith { _pz = _c2 };
    };
    if (count _pz == 0) then { "pz" call MC2_fnc_refus; continue };

    MC2_SITE = _site; MC2_OP = _op; MC2_LZ = _lz; MC2_PZ = _pz;
    MC2_ROUTE = _route; MC2_QRF_BASE = _qb;
    MC2_OP_GAIN = _gain; MC2_OP_VUE = _vue;
    _trouve = true;
};

if (!_trouve) exitWith {
    MC2_ISSUE = "VOID"; MC2_CAUSE = "SITE_INTROUVABLE";
    (format ["CHACAL|VOID|site|essais|%1|refus|%2", _essais, MC2_REFUS]) call MC2_LOG;
};
(format ["CHACAL|OK|tirage|essais|%1|refus|%2", _essais, MC2_REFUS]) call MC2_LOG;
...
// MC2_fnc_rnd est un cycle unique, deux marches differentes rendent deux mondes ).
...
if (MC2_QRF_DIST > 0) then {
    private _d0 = round (MC2_SITE distance2D MC2_QRF_BASE);
    private _az = MC2_SITE getDir MC2_QRF_BASE;
    private _cible = MC2_SITE getPos [MC2_QRF_DIST, _az];
    ...
    (format ["CHACAL|E|qrf_rapprochee|%1|dist_tiree|%2|dist_imposee|%3|dist_reelle|%4|sur_route|%5|azimut|%6|base|%7",
        round (time * 100) / 100, _d0, MC2_QRF_DIST, round (MC2_SITE distance2D _base),
        _surRoute, round _az, _base]) call MC2_LOG;
    (format ["CHACAL|AVERT|hors_corpus|qrf_dist|%1", MC2_QRF_DIST]) call MC2_LOG;
};

// les deux bouts du circuit patrouille
private _ext = MC2_ROUTE nearRoads 900;
MC2_ROUTE_A = MC2_ROUTE; MC2_ROUTE_B = MC2_ROUTE;
if (count _ext > 3) then {
    private _far = 0;
    { private _d = _x distance2D MC2_ROUTE; if (_d > _far) then { _far = _d; MC2_ROUTE_B = getPosATL _x } } forEach _ext;
    MC2_ROUTE_B set [2, 0];
    private _far2 = 0;
    { private _d = _x distance2D MC2_ROUTE_B; if (_d > _far2) then { _far2 = _d; MC2_ROUTE_A = getPosATL _x } } forEach _ext;
    MC2_ROUTE_A set [2, 0];
};
...
MC2_RALLY = [MC2_SITE getPos [900, (MC2_SITE getDir MC2_OP) + 25], 90] call MC2_fnc_plat;
...
MC2_VIT_INFIL = 0.7;
MC2_VIT_REPLI = 1.8;
private _dApp = MC2_LZ distance2D MC2_RALLY;
private _dObs = MC2_RALLY distance2D MC2_OP;
private _dMep = (MC2_SITE distance2D MC2_QRF_BASE) min 900;
private _dExf = (MC2_SITE distance2D MC2_PZ) + 400;
MC2_DUREES = [
    480,                                    // 1 insertion : vol + mise en ordre
    (_dApp / MC2_VIT_INFIL) + 600,       // 2 approche + attente de la fenetre
    (_dObs / MC2_VIT_INFIL) + 1200,      // 3 montee + 20 min d observation
    (_dMep / MC2_VIT_INFIL) + 420,       // 4 articulation des trois elements
    900 * MC2_ASSAUT_X,                  // 5 action sur objectif (seul plafond CONSTANT,
                                            //   d ou le multiplicateur : les autres derivent
                                            //   de la geometrie, celui-ci non)
    (_dExf / MC2_VIT_REPLI) + 300        // 6 rupture jusqu au point de secours
];
(format ["CHACAL|OK|plafonds|%1|total_prevu_min|%2", (MC2_DUREES apply { round _x }),
    round (((MC2_DUREES select 0) + (MC2_DUREES select 1) + (MC2_DUREES select 2)
    + (MC2_DUREES select 3) + (MC2_DUREES select 4) + (MC2_DUREES select 5)) / 60)]) call MC2_LOG;
...
if (MC2_JOUR == 1) then {
    ...
    (format ["CHACAL|AVERT|hors_corpus|jour|1|controle_positif_observation|heure|%1", date select 3]) call MC2_LOG;
};

(format ["CHACAL|OK|monde|essais|%1|site|%2|alt|%3|op|%4|gain|%5|vue|%6|lz|%7|route|%8|qrf|%9|pz|%10|rally|%11",
    _essais, MC2_SITE, round (getTerrainHeightASL MC2_SITE), MC2_OP,
    round MC2_OP_GAIN, (if (MC2_OP_VUE) then {1} else {0}),
    MC2_LZ, MC2_ROUTE, MC2_QRF_BASE, MC2_PZ, MC2_RALLY]) call MC2_LOG;
(format ["CHACAL|OK|distances|lz_site|%1|op_site|%2|route_site|%3|qrf_site|%4|pz_site|%5|lz_route|%6",
    round (MC2_LZ distance2D MC2_SITE), round (MC2_OP distance2D MC2_SITE),
    round (MC2_ROUTE distance2D MC2_SITE), round (MC2_QRF_BASE distance2D MC2_SITE),
    round (MC2_PZ distance2D MC2_SITE), round (MC2_LZ distance2D MC2_ROUTE)]) call MC2_LOG;
