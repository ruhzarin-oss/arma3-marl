...
MC4_CAUSE_ABANDON = "";
MC4_INTEL = false;
MC4_CHARGES = [];              // ACTES : objectifs sur lesquels une charge a ete posee
MC4_DETRUITS = [];             // EFFET SCRIPTE : hors jugement tactique
MC4_VUES = [];                 // [unite, k] reellement observees par la reco
MC4_INSERTION_FORCEE = false;
MC4_QRF_PARTIE = false;
MC4_OUV_CHOISIE = [];
...
MC4_CHARGES_OBJ = [];          // [objectif, charge] : ce qui reste a faire sauter
MC4_RESERVE_FEU = 180;         // s gardees en fin de phase : un degagement, une mise a feu
MC4_SECURITE_FEU = 35;         // m : personne a moins de ca d une charge quand elle part
MC4_EXPLOITANT = objNull;
// ! MARQUEUR-SEUIL-RENS-DERIVE - SEUIL DERIVE DE LA MESURE, PAS CHOISI ( 15/09 ).
// Distribution de count MC4_VUES en fin de phase 3, sur les 148 episodes des journaux qui
...
// ! CE CORRECTIF NE FAIT PAS JOUER LA PHASE 3. Elle ne tourne que si MC4_DEPART < 4 et
// MC4_OBS = 1 ; c est un reglage de job, pas de code.
MC4_SEUIL_RENS = 1;            // sous ce compte, la crete n a rien vu
MC4_T_ALARME = -1; MC4_T_COMPROMIS = -1;
MC4_EXFIL_POINT = [];
MC4_GEL = false;

MC4_fnc_debutPhase = {
    params ["_n", "_nom", ["_plafond", 1e9]];
    // ! VIGNETTE : une phase posterieure a MC4_ARRET n ouvre rien et n ecrit
    // rien. Sinon le RPT porterait des phases jamais jouees et `phase_max` mentirait.
    if (MC4_FIN) exitWith {};
    MC4_PHASE = _n; MC4_PHASE_NOM = _nom;
    MC4_TPHASE = time; MC4_PLAFOND_COURANT = _plafond;
    (format ["CHACAL|PH|%1|%2|debut|%3|vivants|%4|compromis|%5|alarme|%6", _n, _nom, round (time * 100) / 100,
        count (MC4_FS select { alive _x }),
        (if (MC4_COMPROMIS) then {1} else {0}), (if (MC4_ALARME) then {1} else {0})]) call MC4_LOG;
};
...
"CHACAL|OK|decision|version|3" call MC4_LOG;   // 3 : perceptions de la menace en fin de ligne ( plans/plan-menace-visible.md )
// ! LA MENACE VISIBLE ( 17/09 ). Deux canaux : ce qu un homme du detachement VOIT maintenant ( MC4_fnc_voit, geometrie ) et
...
MC4_fnc_unitesMenace = {
    private _t = [];
    { private _g = _x select 2; if (!isNull _g) then { _t append ((units _g) select { alive _x }) } } forEach MC4_MENACES;
    _t
};
MC4_fnc_chefsDetachement = {
    private _g = [];
    { if (alive _x) then { _g pushBackUnique (group _x) } } forEach MC4_FS;
    (_g select { !isNull _x && { alive (leader _x) } }) apply { leader _x }
};
MC4_fnc_suiviMenaces = {
    while { !MC4_FIN } do {
        private _chefs = call MC4_fnc_chefsDetachement;
        private _hommesS = MC4_FS select { alive _x };
        {
            ...
            if (({ [_x, _t, 800, 70] call MC4_fnc_voit } count _hommesS) > 0) then {
        ...
        } forEach (call MC4_fnc_unitesMenace);
        sleep 5;
    };
};
[] spawn MC4_fnc_suiviMenaces;
MC4_fnc_perceptionMenace = {
    ...
    private _menaces = call MC4_fnc_unitesMenace;
    private _hommes = MC4_FS select { alive _x };
    private _chefs = call MC4_fnc_chefsDetachement;
    ...
    {
        private _t = _x;
        private _vu = ({ [_x, _t, 800, 70] call MC4_fnc_voit } count _hommes) > 0;
    ...
    private _veh = 0;
    if ((!isNil "MC4_VEH_ROUTE") && { !isNull MC4_VEH_ROUTE } && { ({ (_x select 1) == "PATROUILLE_ROUTE" } count MC4_MENACES) > 0 }) then {
        if (({ (_x targetKnowledge MC4_VEH_ROUTE) select 0 } count _chefs) > 0) then { _veh = 1 };
    };
    ...
    private _moteur = 0; private _dMoteur = -1; private _moteurAllume = 0; private _vueVeh = 0;
    {
        ...
        if ((_v != _x) && { isEngineOn _v }) then {
            ...
            if ((_dMoteur >= 0) && { _dMoteur < MC4_PORTEE_SON }) then { _moteur = 1; MC4_MOTEUR_FENETRE = 1 };
            if (({ [_x, _v, 1200, 70] call MC4_fnc_voit } count _hommes) > 0) then { _vueVeh = 1 };
        };
    } forEach _menaces;
    private _vDist = -1;
    ...
    format ["|menace_percue|%1|menaces_vues|%2|menaces_connues|%3|menaces_camp|%4|menaces_homme|%5|distance_menace|%6|erreur_position|%7|menace_mobile|%8|vue_depuis|%9|vehicule_connu|%10|menace_mobile_vue|%11|moteur_entendu|%12|distance_moteur|%13|moteur_allume|%14|vue_vehicule|%15|n_vues_menace|%16|moteur_depuis_fenetre|%17|verite_menaces|%18|verite_distance_menace|%19|azimut_chef|%20",
        ...
        (missionNamespace getVariable ["MC4_MOTEUR_FENETRE", 0]),
        count _menaces, round _vDist, (if (count _chefs > 0) then { round (getDir (_chefs select 0)) } else { -1 })]
};
MC4_fnc_secteurPhase = {
    params ["_phase"];
    switch (_phase) do {
        case 1: { if (isNil "MC4_LZ") then { [] } else { MC4_LZ } };
        case 2: { if (isNil "MC4_ROUTE") then { [] } else { MC4_ROUTE } };
        case 4: { if (isNil "MC4_SITE") then { [] } else { MC4_SITE } };
        default { [] };
    };
};
MC4_fnc_fenetreObservation = {
    params ["_phase"];
    if (MC4_OBSERVATION <= 0) exitWith {};
    private _t0 = time;
    private _hommes = MC4_FS select { alive _x };
    private _secteur = [_phase] call MC4_fnc_secteurPhase;
    ...
    if ((MC4_CONTROLE_PERCEPTION in [1, 5]) && { count _hommes > 0 }) then {
        ...
        {
            private _p = _chef getPos [MC4_CONTROLE_DIST, _x];
        ...
        if (!_trouve) exitWith {
            ...
            (format ["CHACAL|E|banc_refuse|%1|phase|%2|distance|%3|cause|AUCUNE_LIGNE_DE_VUE", round (time * 100) / 100,
                _phase, MC4_CONTROLE_DIST]) call MC4_LOG;
        };
        private _p = _chef getPos [MC4_CONTROLE_DIST, _az];
        ...
        MC4_MENACES pushBack [_phase, "BANC_PERCEPTION", _g];
        if (MC4_CONTROLE_PERCEPTION == 5) then {
        ...
        (format ["CHACAL|E|banc_perception|%1|phase|%2|distance|%3|azimut|%4|ligne_de_vue|%5|heure|%6|lune|%7|jumelles|%8|hommes|%9",
            round (time * 100) / 100, _phase, MC4_CONTROLE_DIST, round _az, (if (_trouve) then {1} else {0}),
            (date select 3) + ((date select 4) / 60), moonIntensity,
            ((_hommes apply { hmd _x }) joinString ","), count _hommes]) call MC4_LOG;
    };
    if ((MC4_CONTROLE_PERCEPTION in [2, 3]) && { count _hommes > 0 }) then {
        private _chef = leader (group (_hommes select 0));
        private _p = if (MC4_CONTROLE_PERCEPTION == 2) then { _chef getPos [1500, (getDir _chef) + 180] } else { _chef getPos [150, getDir _chef] };
        private _g = ([east, 4] call MULTI_fnc_groupe);
        {
            private _u = _g createUnit [_x, _p, [], 5, "NONE"];
            _u disableAI "AUTOTARGET"; _u disableAI "TARGET"; _u disableAI "MOVE";
            _u setUnitPos (if (MC4_CONTROLE_PERCEPTION == 3) then {"UP"} else {"MIDDLE"});
        } forEach ["O_Soldier_TL_F", "O_Soldier_F", "O_Soldier_F"];
        _g setCombatMode "BLUE";
        MC4_MENACES pushBack [_phase, "CONTROLE_PERCEPTION", _g];
        (format ["CHACAL|E|controle_perception|%1|phase|%2|mode|%3|distance|%4|posture|%5", round (time * 100) / 100, _phase,
            MC4_CONTROLE_PERCEPTION, round (_chef distance2D _p), (if (MC4_CONTROLE_PERCEPTION == 3) then {"UP"} else {"MIDDLE"})]) call MC4_LOG;
    };
    (format ["CHACAL|E|observation|%1|phase|%2|debut|duree_prevue|%3%4", round (time * 100) / 100, _phase, MC4_OBSERVATION,
        call MC4_fnc_perceptionMenace]) call MC4_LOG;
    MC4_MOTEUR_FENETRE = 0;   // la memoire du son commence avec la fenetre
    if (MC4_SONDE_PERCEPTION == 1) then {
        [_phase] spawn {
            params ["_ph"];
            private _t0 = time;
            while { !MC4_FIN && { (time - _t0) < (600 * MC4_ECHELLE) } } do {
                (format ["CHACAL|E|sonde_decision|%1|phase|%2|depuis|%3%4", round (time * 100) / 100, _ph,
                    round (time - _t0), call MC4_fnc_perceptionMenace]) call MC4_LOG;
                sleep (10 * MC4_ECHELLE);
            };
        };
    };
    (format ["CHACAL|E|reglage_observation|%1|phase|%2|avant|%3|balayage|%4|distance_secteur|%5|azimuts|%6", round (time * 100) / 100, _phase,
        MC4_AVANT, MC4_BALAYAGE, (if (count _secteur > 0) then { round (((_hommes select 0) distance2D _secteur)) } else { -1 }), count _regards]) call MC4_LOG;
    ...
    waitUntil {
        ...
        if ((count _regards > 0) && { time >= _tRegard }) then {
            ...
            // ! levier MC4_BALAYAGE : 1 = chaque changement d azimut TOURNE les hommes ( setDir ) avant le doWatch ; 0 = origine
            {
                if (MC4_BALAYAGE == 1) then { _x setDir (_x getDir (_regards select _iRegard)) };
                _x doWatch (_regards select _iRegard);
            } forEach (MC4_FS select { alive _x });
        };
        if ((MC4_SONDE > 0) && { time >= _prochain }) then {
            _prochain = time + 5;
            private _angles = [];
            {
                private _t = _x;
                private _a = 999;
                { private _r = abs ((((_x getDir _t) - (getDir _x) + 540) % 360) - 180); if (_r < _a) then { _a = _r } } forEach (MC4_FS select { alive _x });
        ...
        MC4_FIN || ((time - _t0) >= (MC4_OBSERVATION * MC4_ECHELLE))
    };
...
MC4_fnc_decision = {
    ...
    private _def = (if (isNil "MC4_EST_SITE") then {[]} else {MC4_EST_SITE}) select { alive _x };
    (format ["CHACAL|E|decision|%1|phase|%2|point|%3|options|%4|choix|%5|decideur|%6|alarme|%7|depuis_alarme|%8|compromis|%9|vivants|%10|defenseurs_connus|%11|verite_defenseurs|%12|situation|%13%14",
        ...
        (if (MC4_ALARME) then {1} else {0}),
        (if (MC4_T_ALARME >= 0) then { round (time - MC4_T_ALARME) } else { -1 }),
        (if (MC4_COMPROMIS) then {1} else {0}),
        count (MC4_FS select { alive _x }),
        { (west knowsAbout _x) > 1.4 } count _def,
        count _def, MC4_SITUATION, _extra + (call MC4_fnc_perceptionMenace)]) call MC4_LOG;
};
// Interprete des formules EvoGP ( codes prefixes ). ">" vaut +1 si a > b, sinon -1, comme dans EvoGP ( mesure le 17/09 ).
MC4_F_CONST = [-1, -0.5, -0.25, 0, 0.25, 0.5, 1];
MC4_fnc_evalNoeud = {
    params ["_i", "_obs"];
    private _c = MC4_F select _i;
    if (_c >= 301) exitWith { [MC4_F_CONST select (_c - 301), _i + 1] };
    if (_c >= 201) exitWith { [_obs select (_c - 201), _i + 1] };
    if (_c == 106) exitWith { private _m = [_i + 1, _obs] call MC4_fnc_evalNoeud; [-(_m select 0), _m select 1] };
    private _g = [_i + 1, _obs] call MC4_fnc_evalNoeud;
    private _d = [_g select 1, _obs] call MC4_fnc_evalNoeud;
    ...
    private _r = switch (_c) do {
        ...
        default { (format ["CHACAL|ERREUR|formule|code_inconnu|%1", _c]) call MC4_LOG; 0 };
    };
    [_r, _d select 1]
};
MC4_fnc_finPhase = {
    params ["_n", "_nom", "_issue"];
    if (MC4_FIN && { _n > MC4_ARRET }) exitWith {};
    (format ["CHACAL|PH|%1|%2|fin|%3|%4|vivants|%5|compromis|%6|alarme|%7", _n, _nom, round (time * 100) / 100, _issue,
        count (MC4_FS select { alive _x }),
        (if (MC4_COMPROMIS) then {1} else {0}), (if (MC4_ALARME) then {1} else {0})]) call MC4_LOG;
    // ! LA VIGNETTE SE FERME ICI, et par le VERDICT : 70_verdict attend MC4_FIN,
    ...
    if (_n >= MC4_ARRET) then {
        (format ["CHACAL|OK|vignette|arret|%1|issue|%2", _n, _issue]) call MC4_LOG;
        MC4_FIN = true;
    };
};
...
MC4_fnc_ordreAller = {
    ...
    // On libere la posture ET le regard : `doWatch MC4_SITE` pose sur l appui
    ...
    (format ["CHACAL|E|ordre|%1|%2|%3|%4|pourquoi|%5", round (time * 100) / 100,
        (_g getVariable ["chacal_element", "DETACHEMENT"]), _p, MC4_PHASE, _pourquoi]) call MC4_LOG;
};
...
MC4_fnc_arrive = {
    ...
    private _c2 = _v2 call MC4_fnc_centre;
...
MC4_fnc_rejoindre = {
    ...
    if (isNull _g) exitWith { "ABSENT" };
    [_g, _p, "NORMAL", "AWARE", "WEDGE", _nom + "_JAMBE_RAPIDE"] call MC4_fnc_ordreAller;
            ...
            if (count _c > 0) then {
                ...
                if (time - _tLog >= 30) then {
                    ...
                    (format ["CHACAL|E|progression|%1|%2|reste|%3|vitesse|%4|jambe|%5|hommes|%6",
                        ...
                        (if (_lente) then {"DISCRETE"} else {"RAPIDE"}), count _v]) call MC4_LOG;
                };
                ...
                if ((_vm / (count _v)) < 0.5) then {
                    ...
                    if (_immobile == 8) then {
                        (format ["CHACAL|E|deblocage|%1|%2|essai|RELEVER|reste|%3", round (time * 100) / 100, _nom, round _d]) call MC4_LOG;
                        [_g, _p, "NORMAL", "AWARE", "WEDGE", _nom + "_DEBLOCAGE_RELEVER"] call MC4_fnc_ordreAller;
                    ...
                    if (_immobile == 20) then {
                        private _neuf = [_p getPos [70, 60 + (240 call MC4_fnc_al)], 50] call MC4_fnc_plat;
                        (format ["CHACAL|E|deblocage|%1|%2|essai|CONTOURNER|vers|%3", round (time * 100) / 100, _nom, _neuf]) call MC4_LOG;
                        _p = _neuf;
                        [_g, _p, "NORMAL", "AWARE", "WEDGE", _nom + "_DEBLOCAGE_CONTOURNER"] call MC4_fnc_ordreAller;
                    ...
                    if (_immobile == 5) then {
                        ...
                        (format ["CHACAL|E|blocage|%1|%2|reste|%3|commande|%4|pret|%5|waypoints|%6|courant|%7|eau|%8|posture|%9|comport|%10|obstacles|%11|pente|%12",
                            ...
                            round (atan (((getTerrainHeightASL ((getPosATL _l) getPos [10, (getPosATL _l) getDir _p])) - (getTerrainHeightASL (getPosATL _l))) / 10))]) call MC4_LOG;
                    };
                } else { _immobile = 0 };
                ...
                // 800 m pendant 2000 s. On la journalise pour savoir si MC4_ACCESSIBLE la
                ...
                if (unitReady _l2 && { (currentCommand _l2) == "" } && { (currentWaypoint _g) >= (count (waypoints _g)) } && { _d > _ray }) then {
                    if (_immobile == 5) then {
                        (format ["CHACAL|E|moteur_dit_fini|%1|%2|reste|%3|rayon|%4|pente_trajet|%5",
                            ...
                            round ([getPosATL _l2, _p] call MC4_fnc_penteTrajet)]) call MC4_LOG;
                    };
                };
                if (_d < _ray) then { _fini = true; _res = "ATTEINT" }
                    ...
                    if (!_lente && { _d < 300 }) then {
                        _lente = true;
                        [_g, _p, "LIMITED", "STEALTH", "WEDGE", _nom + "_JAMBE_DISCRETE"] call MC4_fnc_ordreAller;
                    };
                    // une fois degage de quarante metres, on se recouche
                    if (_releve && { _d < _dReleve - 40 } && { _d < 300 }) then {
                        _releve = false;
                        [_g, _p, "LIMITED", "STEALTH", "WEDGE", _nom + "_RETOUR_JAMBE_DISCRETE"] call MC4_fnc_ordreAller;
                    };
                    if (_d < _dRef - 25) then { _dRef = _d; _tRef = time };
    ...
    if (_res == "") then { _res = if (MC4_SAUT) then {"COMPROMIS"} else {"PLAFOND"} };
    _res
};
...
MC4_fnc_rejoindreItineraire = {
    params ["_g", "_p", "_ray", "_plafond", "_nom"];
    if (isNull _g) exitWith { "ABSENT" };
    if (MC4_ITINERAIRE == 2) then {
        private _v = (units _g) select { alive _x };
        if (count _v > 0) then {
            private _c = _v call MC4_fnc_centre;
            ...
            _wp set [2, 0];
            private _t0 = time;
            [_g, _wp, "NORMAL", "AWARE", "WEDGE", _nom + "_DETOUR"] call MC4_fnc_ordreAller;
            private _rD = [_g, _wp, 80, ([_g, _wp] call MC4_fnc_budgetDeuxJambes)] call MC4_fnc_arrive;
            (format ["CHACAL|E|choix_joue|%1|point|ITINERAIRE|choix|2|detail|DETOUR|element|%2|cote|%3|point_detour|%4|issue|%5|duree|%6",
                round (time * 100) / 100, _nom, _cote, _wp, _rD, round (time - _t0)]) call MC4_LOG;
            _plafond = _plafond - (time - _t0);
        };
    } else {
        if (MC4_ITINERAIRE == 1) then {
            (format ["CHACAL|E|choix_joue|%1|point|ITINERAIRE|choix|1|detail|DIRECT|element|%2", round (time * 100) / 100, _nom]) call MC4_LOG;
        };
    };
    [_g, _p, _ray, (_plafond max 60), _nom] call MC4_fnc_rejoindre
};
...
MC4_fnc_pointDePose = {
    ...
    private _r = (_o call MC4_fnc_emprise) + 4;
    private _az0 = if (_o isEqualTo MC4_PC) then { _pO getDir MC4_OUV_CHOISIE } else { _pO getDir MC4_SITE };
    private _best = _pO getPos [_r, _az0]; private _sc = -1;
    {
        private _p = _pO getPos [_r, _az0 + _x]; _p set [2, 0];
        if ((_p distance2D MC4_SITE) < (MC4_RAYON - 5)) then {
            private _dmin = 8;
...
MC4_fnc_exploiter = {
    private _l = leader MC4_gAssaut;
    private _cand = (units MC4_gAssaut) select { alive _x && { _x != _l } };
    ...
    MC4_EXPLOITANT = _cand select 0;
    private _h = MC4_EXPLOITANT;
    (format ["CHACAL|E|ordre|%1|EXPLOITATION|%2|%3|pourquoi|OPORD_TERMINAL|par|%4", round (time * 100) / 100,
        getPosATL MC4_PORTABLE, MC4_PHASE, (_h getVariable ["chacal_role", ""])]) call MC4_LOG;
    _h doMove (getPosATL MC4_PORTABLE);
    private _t = time; private _dedans = 0; private _tOrdre = time;
    while { !MC4_INTEL && { alive _h } && (time - _t < 100 * MC4_ECHELLE) && !MC4_FIN } do {
        if ((_h distance2D MC4_PORTABLE) < 4) then { _dedans = _dedans + 2 } else {
            _dedans = 0;
            if (time - _tOrdre > 20) then { _h doMove (getPosATL MC4_PORTABLE); _tOrdre = time };
        };
        if (_dedans >= (60 * MC4_ECHELLE)) then {
            MC4_INTEL = true;
            (format ["CHACAL|E|exploitation|%1|reussie|par|%2", round (time * 100) / 100, (_h getVariable ["chacal_role", ""])]) call MC4_LOG;
        };
        sleep 2;
    };
    if (!MC4_INTEL) then {
        (format ["CHACAL|E|exploitation|%1|manquee|distance|%2", round (time * 100) / 100, round (_h distance2D MC4_PORTABLE)]) call MC4_LOG;
    };
    if (alive _h) then { _h doFollow (leader MC4_gAssaut) };
};
...
MC4_fnc_miseAFeu = {
    params [["_degagement", "-"]];
    if (count MC4_CHARGES_OBJ == 0) exitWith {
        (format ["CHACAL|E|mise_a_feu|%1|aucune_charge|degagement|%2", round (time * 100) / 100, _degagement]) call MC4_LOG;
    };
    private _fnProches = {
        private _n = 0;
        {
            private _c = _x select 1;
            if (!isNull _c) then { _n = _n + ({ alive _x && { (_x distance2D _c) < MC4_SECURITE_FEU } } count MC4_FS) };
        } forEach MC4_CHARGES_OBJ;
        _n
    };
    private _t = time; private _proches = call _fnProches;
    while { _proches > 0 && (time - _t < 90 * MC4_ECHELLE) && !MC4_FIN } do { sleep 3; _proches = call _fnProches };
    if (_proches > 0) exitWith {
        (format ["CHACAL|E|mise_a_feu|%1|REFUSEE|hommes_a_portee|%2|attente|%3|degagement|%4", round (time * 100) / 100,
            _proches, round (time - _t), _degagement]) call MC4_LOG;
    };
    (format ["CHACAL|E|mise_a_feu|%1|charges|%2|attente|%3|degagement|%4", round (time * 100) / 100,
        count MC4_CHARGES_OBJ, round (time - _t), _degagement]) call MC4_LOG;
    { private _c = _x select 1; if (!isNull _c) then { _c setDamage 1 } } forEach MC4_CHARGES_OBJ;
    sleep 2;
    {
        private _o = _x select 0;
        if (alive _o) then { _o setDamage 1 };
        MC4_DETRUITS pushBackUnique _o;
        (format ["CHACAL|E|destruction|%1|%2|effet_scripte|total|%3", round (time * 100) / 100, typeOf _o, count MC4_DETRUITS]) call MC4_LOG;
    } forEach MC4_CHARGES_OBJ;
};
...
// =====================================================================
MC4_fnc_compromettre = {
    params ["_cause"];
    if (MC4_COMPROMIS) exitWith {};
    MC4_COMPROMIS = true; MC4_T_COMPROMIS = time;
    (format ["CHACAL|E|compromis|%1|cause|%2|phase|%3|latence_oracle|%4", round (time * 100) / 100,
        _cause, MC4_PHASE,
        (if (MC4_T_ALARME < 0) then {-1} else { round ((time - MC4_T_ALARME) * 100) / 100 })]) call MC4_LOG;
    if (MC4_PHASE < 5) then { MC4_SAUT = (MC4_TENIR == 0) };
    ...
    private _bl = if (MC4_PHASE < 5) then { MC4_FS select { alive _x } } else { [] };
    if (count _bl > 0) then {
        private _c = _bl call MC4_fnc_centre;
        private _menace = [];
        {
            private _e = _x;
            if ({ [_x, _e, 900] call MC4_fnc_voit } count _bl > 0) exitWith { _menace = getPosATL _e };
        } forEach ((allUnits + vehicles) select {
            alive _x && { side (if (_x isKindOf "CAManBase") then {_x} else {effectiveCommander _x}) == east } });
        private _az = if (count _menace > 0) then { _menace getDir _c } else { MC4_SITE getDir _c };
        private _fuite = _c getPos [400, _az];
        {
            if (!isNull _x) then {
                _x setBehaviour "COMBAT"; _x setCombatMode "YELLOW"; _x setSpeedMode "FULL";
                [_x, _fuite, "FULL", "COMBAT", "WEDGE", "RUPTURE_CONTACT"] call MC4_fnc_ordreAller;
            };
        } forEach ([MC4_gFS, MC4_gReco, MC4_gAppui, MC4_gAssaut, MC4_gBouchon] select { !isNull _x });
        (format ["CHACAL|E|rupture|%1|vers|%2|menace_percue|%3", round (time * 100) / 100,
            _fuite, (if (count _menace > 0) then {1} else {0})]) call MC4_LOG;
    };
};
...
// les trois sens du detachement
{
    _x addEventHandler ["FiredNear", {
        ...
        if (_dist < 45 && { side _tireur != west }) then { ["FEU_PROCHE"] call MC4_fnc_compromettre };
    }];
    _x addEventHandler ["Hit", {
        params ["_u", "_source"];
        if (!isNull _source && { side _source != west }) then { ["COUP_RECU"] call MC4_fnc_compromettre };
    }];
} forEach MC4_FS;

// et le troisieme : un rouge que NOUS voyons, et qui vient de passer en combat
[] spawn {
    while { !MC4_COMPROMIS && !MC4_FIN } do {
        private _bl = MC4_FS select { alive _x };
        if (count _bl > 0) then {
            {
                private _e = _x;
                if ({ [_x, _e, 700] call MC4_fnc_voit } count _bl > 0) exitWith { ["ENNEMI_VU_EN_COMBAT"] call MC4_fnc_compromettre };
            } forEach (MC4_EST_SITE select { alive _x && { behaviour _x == "COMBAT" } });
        };
        sleep 2;
    };
};
...
[] spawn {
    ...
    while { !MC4_FIN && MC4_PHASE < 4 } do {
        ...
        if (count _bl > 0) then {
            ...
            private _vu = false;
            { private _e = _x; if ({ [_x, _e, 500] call MC4_fnc_voit } count _bl > 0) exitWith { _vu = true } } forEach _rouges;
            if (_vu && !MC4_GEL && !MC4_SAUT) then {
                MC4_GEL = true;
                { doStop _x; _x setUnitPos "DOWN" } forEach _bl;
                (format ["CHACAL|E|reflexe|%1|gel|phase|%2", round (time * 100) / 100, MC4_PHASE]) call MC4_LOG;
                private _t = time;
                while { MC4_GEL && !MC4_FIN && !MC4_SAUT } do {
                    sleep 3;
                    private _b2 = MC4_FS select { alive _x };
                    private _encore = false;
                    { private _e = _x; if ({ [_x, _e, 500] call MC4_fnc_voit } count _b2 > 0) exitWith { _encore = true } } forEach _rouges;
                    if (_encore) then { _t = time };
                    if (time - _t > 30) then { MC4_GEL = false };
                };
                { if (alive _x) then { _x setUnitPos "AUTO"; _x doFollow (leader (group _x)) } } forEach (MC4_FS select { alive _x });
                (format ["CHACAL|E|reflexe|%1|degel|phase|%2", round (time * 100) / 100, MC4_PHASE]) call MC4_LOG;
            };
        };
        sleep 3;
    };
...
[] spawn {
    while { !MC4_ALARME && !MC4_FIN } do {
        private _m = 0;
        { private _k = east knowsAbout _x; if (_k > _m) then { _m = _k } } forEach (MC4_FS select { alive _x });
        if (_m > 1.4) then { MC4_ALARME = true; MC4_T_ALARME = time };
        sleep 1;
    };
    if (MC4_FIN) exitWith {};
    (format ["CHACAL|E|oracle_alarme|%1|phase|%2", round (time * 100) / 100, MC4_PHASE]) call MC4_LOG;
    { if (!isNull _x) then { _x setBehaviour "COMBAT"; _x setCombatMode "RED"; _x setSpeedMode "FULL" } } forEach MC4_GROUPES_EST;
    ...
    sleep (MC4_PAL_DELAI + (30 call MC4_fnc_al));
    MC4_QRF_PARTIE = true;
    (format ["CHACAL|E|qrf_partie|%1", round (time * 100) / 100]) call MC4_LOG;
    MC4_gQrf setBehaviour "AWARE"; MC4_gQrf setCombatMode "RED"; MC4_gQrf setSpeedMode "FULL";
    while { count (waypoints MC4_gQrf) > 0 } do { deleteWaypoint ((waypoints MC4_gQrf) select 0) };
    private _w = MC4_gQrf addWaypoint [MC4_SITE, 0];
    ...
    if (MC4_LAMBS) then { [MC4_gQrf, 450, 30, [], MC4_SITE] spawn lambs_wp_fnc_taskHunt };
};
...
sleep 3;
if (MC4_ISSUE == "VOID") exitWith { MC4_FIN = true };
...
MC4_OPORD = [MC4_SITE, MC4_OP, MC4_ROUTE, MC4_QRF_BASE, MC4_LZ, MC4_PZ, MC4_RALLY];
(format ["CHACAL|E|opord|%1|site|%2|crete|%3|route|%4|reserve|%5|depose|%6|secours|%7|rally|%8|ouvertures|%9",
    round (time * 100) / 100, MC4_SITE, MC4_OP, MC4_ROUTE, MC4_QRF_BASE,
    MC4_LZ, MC4_PZ, MC4_RALLY, MC4_OUV_AZ]) call MC4_LOG;
...
// la crete voit-elle a 800 m de nuit - n avait aucune reponse. `MC4_DEPART`
// pose le detachement au regroupement. C est un DIAGNOSTIC : l episode est
// marque hors corpus, son approche n ayant pas eu lieu.
if (MC4_DEPART >= 3) then {
    (format ["CHACAL|AVERT|hors_corpus|depart|%1|approche_non_jouee", MC4_DEPART]) call MC4_LOG;
    { if (alive _x) then { _x setPosATL (MC4_RALLY getPos [8 + (14 call MC4_fnc_al), 360 call MC4_fnc_al]) } } forEach MC4_FS;
    MC4_gFS setBehaviour "STEALTH"; MC4_gFS setCombatMode "GREEN";
    MC4_gFS setSpeedMode "LIMITED"; MC4_gFS setFormation "FILE";
    (format ["CHACAL|E|depart_direct|%1|au_rally|%2", round (time * 100) / 100, MC4_RALLY]) call MC4_LOG;
    sleep 5;
} else {
...
if (MC4_DEPART == 2) then {
    "CHACAL|AVERT|hors_corpus|depart|2|insertion_non_jouee" call MC4_LOG;
    private _pt = MC4_ROUTE getPos [MC4_AVANT + 70, MC4_ROUTE getDir MC4_LZ];   // 70 m derriere le point d observation, comme a l origine ( 260 + 70 = 330 )
    private _k = 0;
    { if (alive _x) then { _x setPosATL (_pt getPos [10, _k * 36]); _k = _k + 1 } } forEach MC4_FS;
    MC4_gFS setBehaviour "STEALTH"; MC4_gFS setCombatMode "GREEN";
    MC4_gFS setSpeedMode "LIMITED"; MC4_gFS setFormation "FILE";
    (format ["CHACAL|E|depart_direct|%1|a_la_route|%2", round (time * 100) / 100, _pt]) call MC4_LOG;
    sleep 5;
} else {
_plafond = 1 call MC4_fnc_duree;
...
if (MC4_GEOMETRIE == 1) exitWith {
    (format ["CHACAL|GEO|%1|graine|%2|site|%3|crete|%4|route|%5|lz|%6|qrf|%7|pz|%8|rally|%9|ouvertures|%10|az_site|%11|gain_crete|%12",
        round (time * 100) / 100, MC4_GRAINE, MC4_SITE, MC4_OP, MC4_ROUTE,
        MC4_LZ, MC4_QRF_BASE, MC4_PZ, MC4_RALLY,
        (if (isNil "MC4_OUVERTURES") then {[]} else {MC4_OUVERTURES apply { round (MC4_SITE getDir _x) }}),
        (if (isNil "MC4_AZ") then {-1} else {round MC4_AZ}), round MC4_OP_GAIN]) call MC4_LOG;
    MC4_ISSUE = "VOID"; MC4_CAUSE = "GEOMETRIE_SEULE"; MC4_FIN = true;
};

[1, "INSERTION", _plafond] call MC4_fnc_debutPhase;

MC4_HELO = objNull;
if (["B_Heli_Transport_01_F"] call MC4_fnc_has) then {
    private _dep = MC4_LZ getPos [3400, (MC4_LZ getDir MC4_SITE) + 180];
    MC4_HELO = createVehicle ["B_Heli_Transport_01_F", [_dep select 0, _dep select 1, 140], [], 0, "FLY"];
    createVehicleCrew MC4_HELO;
    call MC4_fnc_recenser;   // un equipage neuf ne doit pas passer 2 s sans identite
    MC4_HELO flyInHeight 60;
    private _gh = group (driver MC4_HELO);
    _gh setBehaviour "CARELESS"; _gh setCombatMode "BLUE";
    { _x assignAsCargo MC4_HELO; _x moveInCargo MC4_HELO } forEach MC4_FS;
    private _w = _gh addWaypoint [MC4_LZ, 0];
    _w setWaypointType "MOVE"; _w setWaypointSpeed "FULL";
};

// Les plafonds physiques ne sont PAS a l echelle : un vol de 3,4 km est une
// duree du MONDE. MC4_ECHELLE raccourcit les plafonds TACTIQUES et rien d autre.
private _pose = false;
if (!isNull MC4_HELO) then {
    private _tv = time;
    waitUntil { sleep 2; (MC4_HELO distance2D MC4_LZ < 250) || (time - _tv > 300) || MC4_FIN };
    if (MC4_HELO distance2D MC4_LZ < 600) then {
        MC4_HELO land "GET OUT";
        private _tl = time;
        waitUntil { sleep 1; (((getPosATL MC4_HELO) select 2) < 1.5 && { (speed MC4_HELO) < 3 }) || (time - _tl > 120) || MC4_FIN };
    };
    _pose = (((getPosATL MC4_HELO) select 2) < 2.5) && { (speed MC4_HELO) < 5 };
    if (!_pose) then {
        MC4_INSERTION_FORCEE = true;
        MC4_HELO setPosATL [MC4_LZ select 0, MC4_LZ select 1, 0.3];
        MC4_HELO setVelocity [0, 0, 0];
        sleep 2;
        (format ["CHACAL|AVERT|insertion_forcee|%1|appareil_pose_a_la_main", round (time * 100) / 100]) call MC4_LOG;
    };
    ...
    private _k = 0;
    {
        if (alive _x) then {
            ...
            _x setPosATL (MC4_HELO getPos [15, _k * 36]);
            _k = _k + 1;
        };
    } forEach MC4_FS;
    sleep 3;
    {
        if (alive _x && { vehicle _x != _x }) then {
            unassignVehicle _x; moveOut _x;
            _x setPosATL (MC4_HELO getPos [15, _k * 36]); _k = _k + 1;
        };
    } forEach MC4_FS;
    sleep 2;
};
...
private _auSol = MC4_FS select { alive _x && { vehicle _x == _x } };
private _ecart = if (count _auSol > 0) then { round ((_auSol call MC4_fnc_centre) distance2D MC4_LZ) } else { -1 };
(format ["CHACAL|E|debarquement|%1|au_sol|%2|ecart_lz|%3|helico_pose|%4|methode|ANNEAU_SCRIPTE", round (time * 100) / 100,
    count _auSol, _ecart, (if (_pose) then {1} else {0})]) call MC4_LOG;

private _morts = MC4_EFFECTIF - (count (MC4_FS select { alive _x }));
...
private _parEnnemi = { !alive _x && { _x getVariable ["chacal_tue_par_est", false] } } count MC4_FS;
if (_parEnnemi > 0) then {
    (format ["CHACAL|E|insertion_sous_le_feu|%1|morts_par_ennemi|%2|morts_total|%3", round (time * 100) / 100, _parEnnemi, _morts]) call MC4_LOG;
};
if ((_morts - _parEnnemi) > 0) exitWith {
    ...
    MC4_ISSUE = "VOID"; MC4_CAUSE = "INSERTION_MORTELLE";
    (format ["CHACAL|VOID|insertion|%1|morts|%2|au_sol|%3", round (time * 100) / 100, _morts, count _auSol]) call MC4_LOG;
    MC4_FIN = true;
};
if (count _auSol < (MC4_EFFECTIF - _parEnnemi)) exitWith {
    MC4_ISSUE = "VOID"; MC4_CAUSE = "INSERTION_INCOMPLETE";
    (format ["CHACAL|VOID|insertion|%1|au_sol|%2", round (time * 100) / 100, count _auSol]) call MC4_LOG;
    MC4_FIN = true;
};
_issue = if (MC4_INSERTION_FORCEE) then {"FORCEE"} else {"ATTEINT"};

if (!isNull MC4_HELO) then {
    private _gh = group (driver MC4_HELO);
    if (!isNull _gh) then {
        while { count (waypoints _gh) > 0 } do { deleteWaypoint ((waypoints _gh) select 0) };
        private _rr = _gh addWaypoint [MC4_LZ getPos [5000, (MC4_LZ getDir MC4_SITE) + 180], 0];
    ...
    [] spawn {
        ...
        if (!isNull MC4_HELO) then {
            { (format ["CHACAL|E|retrait|%1|%2|equipage_helicoptere", round (time * 100) / 100,
                (_x getVariable ["chacal_id", -1])]) call MC4_LOG; deleteVehicle _x } forEach (crew MC4_HELO);
            deleteVehicle MC4_HELO;
        };
    };
};
...
if (MC4_P1_ATTENTE > 0) then {
    [1] call MC4_fnc_fenetreObservation;   // menace visible : regarder avant de choisir ( 0 = origine )
    [1, "INSERTION_ATTENTE", [1, 2], MC4_P1_ATTENTE, "IMPOSE"] call MC4_fnc_decision;
    if (MC4_P1_ATTENTE == 1) then {
        private _dest = MC4_LZ getPos [300, MC4_LZ getDir MC4_ROUTE];
        [MC4_gFS, _dest, "LIMITED", "STEALTH", "FILE", "CHOIX_P1_PARTIR"] call MC4_fnc_ordreAller;
        (format ["CHACAL|E|choix_joue|%1|point|INSERTION_ATTENTE|choix|1|detail|PARTIR|vers|%2", round (time * 100) / 100, _dest]) call MC4_LOG;
    } else {
        { if (alive _x) then { doStop _x; _x setUnitPos "DOWN" } } forEach (units MC4_gFS);
        (format ["CHACAL|E|choix_joue|%1|point|INSERTION_ATTENTE|choix|2|detail|SE_TERRER", round (time * 100) / 100]) call MC4_LOG;
    };
    private _tC = time;
    waitUntil { sleep 2; ((time - _tC) > 180) || MC4_FIN };
    { if (alive _x) then { _x setUnitPos "AUTO" } } forEach (units MC4_gFS);
} else {
    sleep (60 * MC4_ECHELLE);
};
[1, "INSERTION", _issue] call MC4_fnc_finPhase;
};   // fin de l alternative DEPART = 2

// ---------------------------------------------------------------------
// PHASE 2 - APPROCHE, et le franchissement de la route
// ---------------------------------------------------------------------
if (MC4_FIN) exitWith {};   // vignette close a la phase 1
_plafond = 2 call MC4_fnc_duree;
[2, "APPROCHE", _plafond] call MC4_fnc_debutPhase;

private _fr = MC4_ROUTE;
private _avant = _fr getPos [MC4_AVANT, _fr getDir MC4_LZ];   // levier : d ou l on observe la route ( origine 260 m )
...
private _b1 = [MC4_gFS, _avant, 0.7] call MC4_fnc_budget;
(format ["CHACAL|E|budget|%1|etape|APPROCHE_ROUTE|distance|%2|plafond|%3", round (time * 100) / 100,
    round (((units MC4_gFS) call MC4_fnc_centre) distance2D _avant), round _b1]) call MC4_LOG;
[MC4_gFS, _avant, "NORMAL", "AWARE", "FILE", "OPORD_ROUTE_JAMBE_LOINTAINE"] call MC4_fnc_ordreAller;
_r = [MC4_gFS, _avant, 60, _b1] call MC4_fnc_arrive;

// ! LA FENETRE SE DECIDE SUR CE QUE LA FILE PERCOIT, PAS SUR LA POSITION VRAIE.
// Le premier jet lisait `MC4_VEH_ROUTE distance2D _avant`, que personne dans
...
if ((_r in ["ATTEINT", "ENLISE"]) && !MC4_SAUT) then {
    { doStop _x } forEach (units MC4_gFS);
    ...
    if (MC4_TRAVERSEE > 0) then {
        [2] call MC4_fnc_fenetreObservation;   // menace visible : regarder avant de choisir ( 0 = origine )
        private _vD = if (isNil "MC4_VEH_ROUTE") then { objNull } else { MC4_VEH_ROUTE };
        private _vuD = 0;
        if (!isNull _vD && { alive _vD }) then {
            if (({ [_x, _vD, 800, 70] call MC4_fnc_voit } count ((units MC4_gFS) select { alive _x })) > 0) then { _vuD = 1 };
        };
        [2, "TRAVERSEE", [1, 2], MC4_TRAVERSEE, "IMPOSE", format ["|vehicule_vu|%1", _vuD]] call MC4_fnc_decision;
    };
    private _tf = time;
    private _dejaVu = false; private _dernierVu = -1; private _traverse = false; private _cause = "";
    while { !_traverse && (time - _tf < 420 * MC4_ECHELLE) && ((call MC4_fnc_reste) > 0) && !MC4_SAUT && !MC4_FIN } do {
        if (MC4_TRAVERSEE == 1) then { _traverse = true; _cause = "IMPOSE_TOUT_DE_SUITE" } else {
            private _v = MC4_VEH_ROUTE;
            private _vu = false;
            if (!isNull _v && { alive _v }) then {
                private _hommes = (units MC4_gFS) select { alive _x };
                _vu = ({ [_x, _v, 800, 70] call MC4_fnc_voit } count _hommes) > 0;
            };
            if (_vu) then { _dejaVu = true; _dernierVu = time };
            if (_dejaVu && { !_vu } && { (time - _dernierVu) > (45 * MC4_ECHELLE) }) then { _traverse = true; _cause = "FENETRE_OBSERVEE" };
            if (!_dejaVu && { (time - _tf) > (240 * MC4_ECHELLE) }) then { _traverse = true; _cause = "TRAVERSEE_AVEUGLE" };
            if ((MC4_TRAVERSEE != 2) && { isNull _v || { !alive _v } }) then { _traverse = true; _cause = "PATROUILLE_ABSENTE" };
            sleep 3;
        };
    };
    if (!_traverse) then { _cause = "PLAFOND_FENETRE" };
    (format ["CHACAL|E|fenetre|%1|%2|attente|%3|vehicule_vu|%4", round (time * 100) / 100,
        _cause, round (time - _tf), (if (_dejaVu) then {1} else {0})]) call MC4_LOG;
    if (MC4_TRAVERSEE > 0) then {
        (format ["CHACAL|E|choix_joue|%1|point|TRAVERSEE|choix|%2|detail|%3|attente|%4", round (time * 100) / 100,
            MC4_TRAVERSEE, _cause, round (time - _tf)]) call MC4_LOG;
    };
    { _x doFollow (leader MC4_gFS) } forEach (units MC4_gFS);
    ...
    private _perp = (MC4_ROUTE_A getDir MC4_ROUTE_B) + 90;
    if ((abs (((_perp - (_fr getDir MC4_SITE)) + 540) % 360 - 180)) > 90) then { _perp = _perp + 180 };
    private _degage = _fr getPos [750, _perp];
    [MC4_gFS, _degage, "NORMAL", "AWARE", "WEDGE", "SORTIE_DE_COULOIR"] call MC4_fnc_ordreAller;
    (format ["CHACAL|E|couloir|%1|axe|%2|sortie|%3", round (time * 100) / 100, round _perp, _degage]) call MC4_LOG;
    [MC4_gFS, _degage, 90, [MC4_gFS, _degage, 1.1] call MC4_fnc_budget] call MC4_fnc_arrive;
};
...
if (!MC4_SAUT && { !(MC4_DEPART == 2 && MC4_ARRET == 2) }) then {
    private _b2 = [MC4_gFS, MC4_RALLY, 0.5] call MC4_fnc_budget;
    (format ["CHACAL|E|budget|%1|etape|APPROCHE_RALLY|distance|%2|plafond|%3", round (time * 100) / 100,
        round (((units MC4_gFS) call MC4_fnc_centre) distance2D MC4_RALLY), round _b2]) call MC4_LOG;
    [MC4_gFS, MC4_RALLY, "LIMITED", "STEALTH", "FILE", "OPORD_RALLY"] call MC4_fnc_ordreAller;
    _r = [MC4_gFS, MC4_RALLY, 70, _b2] call MC4_fnc_arrive;
};
[2, "APPROCHE", (if (MC4_SAUT) then {"COMPROMIS"} else {_r})] call MC4_fnc_finPhase;
...
// ! 20/09 : quand la patrouille tue les dix, la liste des vivants est VIDE et MC4_fnc_centre erre.
// Le defaut dormait depuis toujours : aucune campagne n avait encore aneanti le detachement.
if (MC4_TENIR == 0 && MC4_COMPROMIS && { count (MC4_FS select { alive _x }) > 0 }
    && { ((MC4_FS select { alive _x }) call MC4_fnc_centre) distance2D MC4_SITE > 1200 }) then {
    MC4_ABANDON = true; MC4_SAUT = false;
    MC4_CAUSE_ABANDON = "COMPROMIS_LOIN";
    (format ["CHACAL|E|abandon|%1|distance|%2", round (time * 100) / 100,
        round (((MC4_FS select { alive _x }) call MC4_fnc_centre) distance2D MC4_SITE)]) call MC4_LOG;
};
...
// invendable.
if (MC4_BRAS == "NUL" && !MC4_SAUT && !MC4_ABANDON) then {
    MC4_OUV_CHOISIE = MC4_OUVERTURES select 0;
    MC4_POS_ASSAUT = [MC4_SITE getPos [230, MC4_SITE getDir MC4_OUV_CHOISIE], 60] call MC4_fnc_plat;
    [MC4_gFS, MC4_POS_ASSAUT, "NORMAL", "AWARE", "WEDGE", "BRAS_NUL_DROIT_SUR_L_OBJECTIF"] call MC4_fnc_ordreAller;
    (format ["CHACAL|E|bras_nul|%1|vers|%2", round (time * 100) / 100, MC4_POS_ASSAUT]) call MC4_LOG;
    [MC4_gFS, MC4_POS_ASSAUT, 90, [MC4_gFS, MC4_POS_ASSAUT, 0.9] call MC4_fnc_budget] call MC4_fnc_arrive;
};

// ---------------------------------------------------------------------
// PHASE 3 - POINT D OBSERVATION : deux hommes montent, huit se terrent
// ---------------------------------------------------------------------
if (!MC4_FIN && !MC4_SAUT && !MC4_ABANDON && { MC4_BRAS != "NUL" } && { MC4_DEPART < 4 }) then {
    _plafond = 3 call MC4_fnc_duree;
    [3, "OBSERVATION", _plafond] call MC4_fnc_debutPhase;
    // ! COUPURE ASSUMEE ET ECRITE. Voir verdicts/phase3-porte-hors-datteinte.md.
    if (MC4_OBS == 0) exitWith {
        (format ["CHACAL|AVERT|hors_corpus|observation_coupee|1|seuil|%1|jamais_atteint|1",
            MC4_SEUIL_RENS]) call MC4_LOG;
        (format ["CHACAL|E|observation_coupee|%1|economie_s|%2", round (time * 100) / 100,
            round _plafond]) call MC4_LOG;
        [3, "OBSERVATION", "COUPEE"] call MC4_fnc_finPhase;
    };

    MC4_gReco = [MC4_RECO, "RECHERCHE"] call MC4_fnc_detacher;
    MC4_gFS setVariable ["chacal_element", "GROS", true];
    [MC4_gReco, MC4_OP, "LIMITED", "STEALTH", "FILE", "OPORD_CRETE"] call MC4_fnc_ordreAller;
    private _cache = [MC4_RALLY getPos [120, MC4_RALLY getDir MC4_SITE], 50] call MC4_fnc_plat;
    [MC4_gFS, _cache, "LIMITED", "STEALTH", "WEDGE", "TENIR_HORS_DE_VUE"] call MC4_fnc_ordreAller;

    private _rArr = [MC4_gReco, MC4_OP, 60, [MC4_gReco, MC4_OP, 0.5] call MC4_fnc_budget] call MC4_fnc_arrive;
    (format ["CHACAL|E|montee|%1|issue|%2|reco|%3|distance|%4", round (time * 100) / 100,
        _rArr, count ((units MC4_gReco) select { alive _x }),
        (if (count ((units MC4_gReco) select { alive _x }) > 0)
         then { round ((((units MC4_gReco) select { alive _x }) call MC4_fnc_centre) distance2D MC4_OP) }
         else { -1 })]) call MC4_LOG;
    ...
    // et redimensionne toute la mission autour d une fausse cause.
    {
        if (alive _x) then {
            _x setUnitPos "DOWN";
            _x setDir (_x getDir MC4_SITE);
            _x doWatch MC4_SITE;
        };
    } forEach (units MC4_gReco);
    (format ["CHACAL|E|orientation|%1|reco|%2|azimut_site|%3|distance|%4", round (time * 100) / 100,
        count ((units MC4_gReco) select { alive _x }),
        round (MC4_OP getDir MC4_SITE), round (MC4_OP distance2D MC4_SITE)]) call MC4_LOG;
    { if (alive _x) then { _x setUnitPos "MIDDLE" } } forEach (units MC4_gFS);

    // LE RENSEIGNEMENT EST RETENU, pas seulement compte : ce qui entre dans
    // MC4_VUES est ce que la reco a vu GEOMETRIQUEMENT au moins une fois.
    ...
    if (MC4_OBS_DUREE > 0) then {
        _dureeObs = MC4_OBS_DUREE;
        [3, "OBS_DUREE", [120, 480], MC4_OBS_DUREE, "IMPOSE", format ["|reco_vivants|%1", count ((units MC4_gReco) select { alive _x })]] call MC4_fnc_decision;
    };
    private _tObs = time;
    while { (time - _tObs) < _dureeObs && ((call MC4_fnc_reste) > 0) && !MC4_SAUT && !MC4_FIN } do {
        private _obs = (units MC4_gReco) select { alive _x };
        if (count _obs > 0) then {
            {
                private _e = _x;
                if ({ [_x, _e, 900] call MC4_fnc_voit } count _obs > 0) then {
                    private _k = 0;
                    { private _kk = _x knowsAbout _e; if (_kk > _k) then { _k = _kk } } forEach _obs;
                    private _i = MC4_VUES findIf { (_x select 0) isEqualTo _e };
                    if (_i < 0) then { MC4_VUES pushBack [_e, _k] }
                    else { if (_k > ((MC4_VUES select _i) select 1)) then { (MC4_VUES select _i) set [1, _k] } };
                };
            } forEach (MC4_EST_SITE select { alive _x });
            (format ["CHACAL|E|renseignement|%1|localisees|%2|sur|%3|seuil|%4", round (time * 100) / 100,
                count MC4_VUES, count (MC4_EST_SITE select { alive _x }), MC4_SEUIL_RENS]) call MC4_LOG;
        };
        sleep (60 * MC4_ECHELLE);
    };
    ...
    if (MC4_OBS_DUREE > 0) then {
        (format ["CHACAL|E|choix_joue|%1|point|OBS_DUREE|choix|%2|detail|OBSERVE|duree_reelle|%3|localisees|%4|rapprochement|0",
            round (time * 100) / 100, MC4_OBS_DUREE, round (time - _tObs), count MC4_VUES]) call MC4_LOG;
    };
    if (!MC4_SAUT && { MC4_OBS_DUREE == 0 } && { count MC4_VUES < MC4_SEUIL_RENS }) then {
        private _second = [MC4_SITE getPos [250, MC4_SITE getDir MC4_OP], 60] call MC4_fnc_plat;
        (format ["CHACAL|E|rapprochement|%1|localisees|%2|seuil|%3|vers|%4", round (time * 100) / 100,
            count MC4_VUES, MC4_SEUIL_RENS, _second]) call MC4_LOG;
        [MC4_gReco, _second, 60, ([MC4_gReco, _second] call MC4_fnc_budgetDeuxJambes) min 600, "RECO_SECOND_POSTE"] call MC4_fnc_rejoindre;
        { if (alive _x) then { _x setUnitPos "DOWN"; _x setDir (_x getDir MC4_SITE); _x doWatch MC4_SITE } } forEach (units MC4_gReco);
        private _t2 = time;
        while { (time - _t2) < (300 * MC4_ECHELLE) && !MC4_SAUT && !MC4_FIN } do {
            private _obs2 = (units MC4_gReco) select { alive _x };
            if (count _obs2 > 0) then {
                {
                    private _e = _x;
                    if ({ [_x, _e, 600] call MC4_fnc_voit } count _obs2 > 0) then {
                        private _k = 0;
        ...
        if (count MC4_VUES < MC4_SEUIL_RENS) then {
            (format ["CHACAL|E|renseignement_pauvre|%1|localisees|%2|l_assaut_part_quand_meme", round (time * 100) / 100, count MC4_VUES]) call MC4_LOG;
        };
    };
    [3, "OBSERVATION", (if (MC4_SAUT) then {"COMPROMIS"} else {
        if (count MC4_VUES >= MC4_SEUIL_RENS) then {"ATTEINT"} else {"RENSEIGNEMENT_PAUVRE"} })] call MC4_fnc_finPhase;
};

// ---------------------------------------------------------------------
// PHASE 4 - MISE EN PLACE : appui, assaut, bouchon
// ---------------------------------------------------------------------
if (!MC4_FIN && !MC4_SAUT && !MC4_ABANDON && { MC4_BRAS != "NUL" }) then {
    _plafond = 4 call MC4_fnc_duree;
    [4, "MISE_EN_PLACE", _plafond] call MC4_fnc_debutPhase;

    MC4_gAppui   = [MC4_APPUI,   "APPUI"]   call MC4_fnc_detacher;
    MC4_gAssaut  = [MC4_ASSAUT,  "ASSAUT"]  call MC4_fnc_detacher;
    MC4_gBouchon = [MC4_BOUCHON, "BOUCHON"] call MC4_fnc_detacher;
    ...
    // definir MC4_PAL_QRF : d ou le garde isNil ci-dessous, sans lequel cette ligne
    ...
    (format ["CHACAL|E|partage|%1|regle|%2|assaut|%3|bouchon|%4|appui|%5|qrf_vehicules|%6|qrf_hommes|%7|delai_qrf|%8",
        round (time * 100) / 100,
        (if (MC4_PARTAGE == 1) then {"SEPT_UN"} else {"CINQ_TROIS"}),
        count (units MC4_gAssaut), count (units MC4_gBouchon), count (units MC4_gAppui),
        (if (isNil "MC4_PAL_QRF") then {0} else {MC4_PAL_QRF}), count MC4_QRF, MC4_PAL_DELAI]) call MC4_LOG;
    ...
    // ! L ORACLE COMPLET ( MC4_ORACLE = 2, revue de Fable 11/09 ) : les defenseurs sont reveles et
    // inscrits comme vus AVANT le choix de la porte, qui se fait alors en les connaissant.
    if (MC4_ORACLE == 2) then {
        private _defO = MC4_EST_SITE select { alive _x };
        { private _u = _x; { _u reveal [_x, 4] } forEach _defO } forEach (MC4_FS select { alive _x });
        MC4_VUES = _defO apply { [_x, 4] };
        (format ["CHACAL|E|oracle|%1|reveles|%2|hommes|%3|phase|4|avant_choix_porte|1", round (time * 100) / 100,
            count _defO, count (MC4_FS select { alive _x })]) call MC4_LOG;
    };
    private _dep = ((units MC4_gAssaut) select { alive _x }) call MC4_fnc_centre;
    if (count _dep == 0) then { _dep = MC4_RALLY };
    ...
    // PLUS PROCHE. Discriminant par construction des que MC4_VUES n est pas vide.
    ...
    private _connus = (MC4_VUES apply { _x select 0 }) select { !isNull _x };
    private _aff  = MC4_OUVERTURES apply { 0 };      // defenseurs connus attribues a l ouverture
    private _dgar = MC4_OUVERTURES apply { -1 };     // distance du defenseur connu le plus proche
    {
    ...
    MC4_OUV_CHOISIE = MC4_OUVERTURES select _meilleure;
    // ! LA COUTURE DE L AZIMUT. Douze candidats publies, un decideur designe par MC4_AZIMUT.
    // Le mode 0 ne touche a rien : MC4_OUV_CHOISIE garde la valeur que le score vient de poser.
    ...
    MC4_AZIMUT_CHOISI = round (MC4_SITE getDir MC4_OUV_CHOISIE);
    private _qui = "SCRIPT";
    if (MC4_AZIMUT == 1) then {
        ...
        private _k = floor ((call MC4_fnc_rndTemoin) * (count _candidats));
        if (_k >= count _candidats) then { _k = (count _candidats) - 1 };
        MC4_AZIMUT_CHOISI = _candidats select _k; _qui = "HASARD";
    };
    if (MC4_AZIMUT == 2) then {
        MC4_AZIMUT_CHOISI = ((round MC4_AZIMUT_VAL) + 360) % 360; _qui = "IMPOSE";
    };
    if (MC4_AZIMUT > 0) then {
        MC4_OUV_CHOISIE = MC4_SITE getPos [MC4_RAYON, MC4_AZIMUT_CHOISI];
        MC4_OUV_CHOISIE set [2, 0];
    };
    (format ["CHACAL|E|couture_azimut|%1|mode|%2|decideur|%3|candidats|%4|choisi|%5|point|%6|az_ouvertures|%7",
        round (time * 100) / 100, MC4_AZIMUT, _qui, count _candidats, MC4_AZIMUT_CHOISI,
        MC4_OUV_CHOISIE, (MC4_OUVERTURES apply { round (MC4_SITE getDir _x) })]) call MC4_LOG;
    ...
    (format ["CHACAL|E|choix_ouverture|%1|indice|%2|gardes|%3|distances|%4|score|%5|renseignement|%6|dgarde|%7|regle|PLUS_PROCHE",
        ...
        round (_minScore * 100) / 100, count MC4_VUES,
        str (_dgar apply { if (_x < 0) then { -1 } else { round _x } })]) call MC4_LOG;

    // ! L APPUI N EST PLUS L OBSERVATOIRE ( sous parametre ). A 0, on garde la ligne d origine.
    MC4_POS_APPUI   = MC4_OP;
    // ! Le point de depart est passe depuis l element concerne : c est la seule facon de savoir
    // si le trajet est praticable. Sans MC4_ACCESSIBLE, l argument est ignore.
    private _depAssaut  = ((units MC4_gAssaut)  select { alive _x }) call MC4_fnc_centre;
    private _depBouchon = ((units MC4_gBouchon) select { alive _x }) call MC4_fnc_centre;
    MC4_POS_ASSAUT  = [MC4_SITE getPos [230, MC4_SITE getDir MC4_OUV_CHOISIE], 60, 220, _depAssaut] call MC4_fnc_plat;
    ...
    MC4_POS_BOUCHON = [MC4_SITE getPos [400, MC4_SITE getDir MC4_QRF_BASE], 80, 220, _depBouchon] call MC4_fnc_plat;
    if (MC4_APPUI_FEU == 1) then {
        MC4_POS_APPUI = [MC4_SITE, MC4_OUV_CHOISIE, MC4_POS_ASSAUT, MC4_OP] call MC4_fnc_positionAppui;
    };
    ...
    if (MC4_APPUI_FIXE == 1) then {
        [] spawn {
            waitUntil { sleep 2; MC4_FIN || { !isNull MC4_gAppui && { [MC4_gAppui, MC4_POS_APPUI, 70] call MC4_fnc_enPlace } } };
            if (MC4_FIN || { isNull MC4_gAppui }) exitWith {};
            MC4_gAppui setCombatMode "YELLOW";
            MC4_gAppui setVariable ["lambs_danger_disableGroupAI", true, true];
            { if (alive _x) then { _x disableAI "PATH"; _x setVariable ["lambs_danger_disableAI", true, true] } } forEach (units MC4_gAppui);
            (format ["CHACAL|E|appui_fixe|%1|position|%2|compromis|%3", round (time * 100) / 100, MC4_POS_APPUI,
                (if (MC4_COMPROMIS) then {1} else {0})]) call MC4_LOG;
        };
    };
    ...
    private _b1 = [MC4_gAppui,   MC4_POS_APPUI]   call MC4_fnc_budgetDeuxJambes;
    private _b2 = [MC4_gAssaut,  MC4_POS_ASSAUT]  call MC4_fnc_budgetDeuxJambes;
    private _b3 = [MC4_gBouchon, MC4_POS_BOUCHON] call MC4_fnc_budgetDeuxJambes;
    ...
    if (MC4_ITINERAIRE > 0) then { _plafond = _plafond * 2 };
    MC4_TPHASE = time; MC4_PLAFOND_COURANT = _plafond;
    (format ["CHACAL|E|budget|%1|etape|MISE_EN_PLACE|appui|%2|assaut|%3|bouchon|%4|plafond|%5",
        round (time * 100) / 100, round _b1, round _b2, round _b3, round _plafond]) call MC4_LOG;
    ...
    if (MC4_ORACLE >= 1) then {
        private _def = MC4_EST_SITE select { alive _x };
        { private _u = _x; { _u reveal [_x, 4] } forEach _def } forEach (MC4_FS select { alive _x });
        MC4_VUES = _def apply { [_x, 4] };
        (format ["CHACAL|E|oracle|%1|reveles|%2|hommes|%3|phase|4", round (time * 100) / 100, count _def,
            count (MC4_FS select { alive _x })]) call MC4_LOG;
    };
    if (MC4_DEPART >= 4) then {
        "CHACAL|AVERT|hors_corpus|depart|4|articulation_non_jouee" call MC4_LOG;
        {
            _x params ["_g", "_p"];
            if (!isNull _g) then {
                { if (alive _x) then { _x setPosATL (_p getPos [4 + (10 call MC4_fnc_al), 360 call MC4_fnc_al]) } } forEach (units _g);
            };
        } forEach [[MC4_gAppui, MC4_POS_APPUI], [MC4_gAssaut, MC4_POS_ASSAUT], [MC4_gBouchon, MC4_POS_BOUCHON]];
        sleep 5;
    } else {
        if (MC4_ITINERAIRE > 0) then { [4] call MC4_fnc_fenetreObservation; [4, "ITINERAIRE", [1, 2], MC4_ITINERAIRE, "IMPOSE"] call MC4_fnc_decision };
        private _h1 = [MC4_gAppui,   MC4_POS_APPUI,   70, _plafond, "APPUI"]   spawn MC4_fnc_rejoindreItineraire;
        private _h2 = [MC4_gAssaut,  MC4_POS_ASSAUT,  70, _plafond, "ASSAUT"]  spawn MC4_fnc_rejoindreItineraire;
        private _h3 = [MC4_gBouchon, MC4_POS_BOUCHON, 80, _plafond, "BOUCHON"] spawn MC4_fnc_rejoindreItineraire;
        waitUntil { sleep 3; (scriptDone _h1 && scriptDone _h2 && scriptDone _h3) || MC4_FIN || MC4_SAUT };
    };
    _r1 = if ([MC4_gAppui,   MC4_POS_APPUI,   70] call MC4_fnc_enPlace) then {"ATTEINT"} else {"NON"};
    _r2 = if ([MC4_gAssaut,  MC4_POS_ASSAUT,  70] call MC4_fnc_enPlace) then {"ATTEINT"} else {"NON"};
    _r3 = if ([MC4_gBouchon, MC4_POS_BOUCHON, 80] call MC4_fnc_enPlace) then {"ATTEINT"} else {"NON"};
    { if (alive _x) then { _x setUnitPos "DOWN"; _x setDir (_x getDir MC4_SITE); _x doWatch MC4_SITE } } forEach (units MC4_gAppui);
    (format ["CHACAL|E|articulation|%1|appui|%2|assaut|%3|bouchon|%4", round (time * 100) / 100, _r1, _r2, _r3]) call MC4_LOG;
    [4, "MISE_EN_PLACE", (if (_r1 == "ATTEINT" && _r2 == "ATTEINT" && _r3 == "ATTEINT") then {"ATTEINT"}
        else { if (MC4_SAUT) then {"COMPROMIS"} else {"PLAFOND"} })] call MC4_fnc_finPhase;
};

// ---------------------------------------------------------------------
// PHASE 5 - ACTION SUR OBJECTIF
// ---------------------------------------------------------------------
if (!MC4_ABANDON && !MC4_FIN) then {
MC4_SAUT = false;
...
if (MC4_ATTENTE_TEST > 0) then {
    ...
    // L appui deja fige par MC4_APPUI_FIXE n est pas touche : lui rendre PATH le ferait partir, et seulement
    // dans les bras qui attendent.
    private _fige = (MC4_FS select { alive _x }) select { !(MC4_APPUI_FIXE == 1 && { group _x == MC4_gAppui }) };
    ...
    (format ["CHACAL|E|attente_test|%1|debut|duree_prevue|%2|vivants|%3|alarme|%4|figes|%5", round (time * 100) / 100,
        MC4_ATTENTE_TEST, count (MC4_FS select { alive _x }), (if (MC4_ALARME) then {1} else {0}),
        count _fige]) call MC4_LOG;
    waitUntil { sleep 2; MC4_FIN || ((time - _tA) >= (MC4_ATTENTE_TEST * MC4_ECHELLE)) };
    private _derive = 0;
    ...
    (format ["CHACAL|E|attente_test|%1|fin|duree|%2|vivants|%3|alarme|%4|compromis|%5|derive|%6", round (time * 100) / 100,
        round (time - _tA), count (MC4_FS select { alive _x }), (if (MC4_ALARME) then {1} else {0}),
        (if (MC4_COMPROMIS) then {1} else {0}), round _derive]) call MC4_LOG;
};
_plafond = 5 call MC4_fnc_duree;
[5, "ASSAUT", _plafond] call MC4_fnc_debutPhase;

if (isNull MC4_gAssaut) then {
    MC4_gAssaut = MC4_gFS; MC4_gAssaut setVariable ["chacal_element", "ASSAUT_BLOC", true];
    if (count MC4_OUV_CHOISIE == 0) then { MC4_OUV_CHOISIE = MC4_OUVERTURES select 0 };
};
...
if (!isNil "MC4_POS_ASSAUT") then {
    private _a1 = [MC4_gAppui,   MC4_POS_APPUI,   130] call MC4_fnc_enPlace;
    private _a2 = [MC4_gAssaut,  MC4_POS_ASSAUT,  130] call MC4_fnc_enPlace;
    private _a3 = [MC4_gBouchon, MC4_POS_BOUCHON, 160] call MC4_fnc_enPlace;
    ...
    if (!_a2 || { !_a1 }) then {
        ...
        private _uA = (units MC4_gAssaut) select { alive _x };
        private _dA = if (count _uA > 0) then { (_uA call MC4_fnc_centre) distance2D MC4_POS_ASSAUT } else { 9999 };
        if (_dA > 400) exitWith {
            (format ["CHACAL|E|rearticulation|%1|refusee|assaut_a|%2|trop_loin", round (time * 100) / 100, round _dA]) call MC4_LOG;
        };
        (format ["CHACAL|E|rearticulation|%1|cause|DISPOSITIF_ECLATE|assaut_a|%2", round (time * 100) / 100, round _dA]) call MC4_LOG;
        private _b = ([MC4_gAssaut, MC4_POS_ASSAUT] call MC4_fnc_budgetDeuxJambes) min (_plafond * 0.45);
        private _hA = [MC4_gAssaut, MC4_POS_ASSAUT, 130, _b, "ASSAUT_REPRISE"] spawn MC4_fnc_rejoindre;
        [MC4_gAppui,   MC4_POS_APPUI,   "NORMAL", "AWARE", "LINE",  "REARTICULATION"] call MC4_fnc_ordreAller;
        [MC4_gBouchon, MC4_POS_BOUCHON, "NORMAL", "AWARE", "WEDGE", "REARTICULATION"] call MC4_fnc_ordreAller;
        waitUntil { sleep 3; scriptDone _hA || MC4_FIN };
        _a1 = [MC4_gAppui,   MC4_POS_APPUI,   130] call MC4_fnc_enPlace;
        _a2 = [MC4_gAssaut,  MC4_POS_ASSAUT,  130] call MC4_fnc_enPlace;
        _a3 = [MC4_gBouchon, MC4_POS_BOUCHON, 160] call MC4_fnc_enPlace;
        (format ["CHACAL|E|dispositif|%1|apres_rearticulation|appui|%2|assaut|%3|bouchon|%4|vivants|%5",
            ...
            (if (_a3) then {1} else {0}), count (MC4_FS select { alive _x })]) call MC4_LOG;
    };

    if (!_a2) then {
        _renonce = true; MC4_ABANDON = true;
        MC4_CAUSE_ABANDON = "ARTICULATION_ROMPUE";
...
if (MC4_ORACLE >= 1) then {
    private _def = MC4_EST_SITE select { alive _x };
    { private _u = _x; { _u reveal [_x, 4] } forEach _def } forEach (MC4_FS select { alive _x });
    (format ["CHACAL|E|oracle|%1|reveles|%2|phase|5", round (time * 100) / 100, count _def]) call MC4_LOG;
};
private _recoHommes = MC4_RECO call MC4_fnc_role;
private _restitue = 0;
{
    _x params ["_cible", "_k"];
...
(format ["CHACAL|E|restitution|%1|restituees|%2|localisees|%3|destinataires|%4", round (time * 100) / 100,
    _restitue, count MC4_VUES, count _recoHommes]) call MC4_LOG;
...
if (_restitue == 0 && !_renonce) then {
    (format ["CHACAL|E|renseignement_pauvre|%1|restituees|0|l_assaut_part_par_l_ouverture_par_defaut", round (time * 100) / 100]) call MC4_LOG;
};

if (_renonce) then {
    [5, "ASSAUT", MC4_CAUSE_ABANDON] call MC4_fnc_finPhase;
} else {

if (!isNull MC4_gAppui) then {
    MC4_gAppui setBehaviour "COMBAT"; MC4_gAppui setCombatMode (if (MC4_APPUI_FIXE == 1) then {"YELLOW"} else {"RED"}); MC4_gAppui setSpeedMode "LIMITED";
    private _cibles = (MC4_VUES apply { _x select 0 }) select { alive _x };
    { if (alive _x && { count _cibles > 0 }) then { _x doTarget (_cibles select 0); _x doFire (_cibles select 0) } } forEach (units MC4_gAppui);
};
...
if (MC4_BANC_APPUI > 0) exitWith {
    call MC4_fnc_bancAppui;
    [5, "ASSAUT", "BANC_APPUI"] call MC4_fnc_finPhase;
    MC4_FIN = true;
};
// ! LE SOCLE ET LA TACTIQUE ( document du 11/09 ). Le socle repare l execution, la tactique conduit l appui.
if (MC4_SOCLE == 1) then { call MC4_fnc_socleAssaut };
if (MC4_TACTIQUE == 5) then {
    // T5 : personne ne tire tant que rien ne nous tire dessus. A la premiere balle, on bascule en T1.
    { if (!isNull _x) then { _x setCombatMode "GREEN" } } forEach [MC4_gAppui, MC4_gAssaut, MC4_gBouchon];
    MC4_gAssaut setBehaviour "STEALTH";
    [] spawn {
        waitUntil { sleep 2; MC4_FIN || { MC4_PHASE != 5 } || { count MC4_CONNUS > 0 } || MC4_COMPROMIS };
        if (MC4_FIN || { MC4_PHASE != 5 }) exitWith {};
        MC4_TACTIQUE = 1;
        { if (!isNull _x) then { _x setCombatMode "RED" } } forEach [MC4_gAppui, MC4_gBouchon];
        MC4_gAssaut setBehaviour "AWARE"; MC4_gAssaut setCombatMode "YELLOW";
        (format ["CHACAL|E|bascule_t5_vers_t1|%1|connus|%2|compromis|%3", round (time * 100) / 100,
            count MC4_CONNUS, (if (MC4_COMPROMIS) then {1} else {0})]) call MC4_LOG;
    };
};
if (MC4_TACTIQUE > 0) then { [] spawn MC4_fnc_tactiqueAppui };
if (MC4_SOCLE == 1 && { !(1 call MC4_fnc_sans) }) then { [] spawn MC4_fnc_chienDeGarde };
// L assaut attend : T1 le premier coup de l appui ( 120 s au plus ), T2 qu il ne reste au plus qu un defenseur ( 8 min ).
if (MC4_TACTIQUE == 1) then {
    private _tA = time;
    waitUntil { sleep 1; (MC4_T_PREMIER_TIR_APPUI > 0) || { time - _tA > 120 } || MC4_FIN };
    sleep 5;
    (format ["CHACAL|E|t1_depart|%1|premier_tir_appui|%2|attente|%3", round (time * 100) / 100,
        round MC4_T_PREMIER_TIR_APPUI, round (time - _tA)]) call MC4_LOG;
};
if (MC4_TACTIQUE == 2) then {
    private _tA = time;
    waitUntil { sleep 2; (count (MC4_EST_SITE select { alive _x }) <= 1) || { time - _tA > 480 } || MC4_FIN };
    (format ["CHACAL|E|t2_depart|%1|defenseurs_restants|%2|attente|%3", round (time * 100) / 100,
        count (MC4_EST_SITE select { alive _x }), round (time - _tA)]) call MC4_LOG;
};
...
MC4_TIRS_APPUI = 0; MC4_T_PREMIER_TIR = -1; MC4_CIBLE_ASSAUT = []; MC4_RELANCES = 0;
if (!isNull MC4_gAppui) then {
    {
        _x addEventHandler ["Fired", {
            MC4_TIRS_APPUI = MC4_TIRS_APPUI + 1;
            if (MC4_T_PREMIER_TIR < 0) then { MC4_T_PREMIER_TIR = time };
        }];
    } forEach ((units MC4_gAppui) select { alive _x });
};
if (MC4_FEU_AVANT == 1 && { !isNull MC4_gAppui }) then {
    [] spawn {
        while { !MC4_FIN && { MC4_PHASE == 5 } } do {
            private _def = MC4_EST_SITE select { alive _x };
            private _ref = if (count MC4_CIBLE_ASSAUT > 0) then { MC4_CIBLE_ASSAUT } else { MC4_OUV_CHOISIE };
            if (count _def > 0) then {
                ...
                } forEach (units MC4_gAppui);
            };
            sleep 5;
        };
    };
    private _tG = time;
    waitUntil { sleep 1; (MC4_TIRS_APPUI > 0) || { (time - _tG) > (120 * MC4_ECHELLE) } || { MC4_FIN } };
    (format ["CHACAL|E|feu_avant|%1|premier_tir|%2|attente|%3|defenseurs_vivants|%4", round (time * 100) / 100,
        (if (MC4_T_PREMIER_TIR < 0) then {-1} else {round (MC4_T_PREMIER_TIR * 100) / 100}),
        round (time - _tG), count (MC4_EST_SITE select { alive _x })]) call MC4_LOG;
};
sleep (8 * MC4_ECHELLE);

if (!isNull MC4_gBouchon) then {
    MC4_gBouchon setBehaviour "COMBAT"; MC4_gBouchon setCombatMode "RED";
    { if (alive _x) then { _x setUnitPos "MIDDLE" } } forEach (units MC4_gBouchon);
};
...
if (MC4_MG_ASSAUT == 1) then {
    private _mg = (units MC4_gAssaut) select { alive _x && { (_x getVariable ["chacal_role", ""]) == "ADJOINT" } };
    if (count _mg == 0) then { _mg = (units MC4_gAssaut) select { alive _x && { !((_x getVariable ["chacal_role", ""]) in ["DEMO_1", "DEMO_2"]) } } };
    if (count _mg > 0) then {
        ...
        (format ["CHACAL|E|mg_assaut|%1|role|%2|arme|%3|munitions_chargees|%4|chargeurs_sac|%5", round (time * 100) / 100,
            ...
            { _x == "200Rnd_65x39_cased_Box" } count (magazines _u)]) call MC4_LOG;
    } else { "CHACAL|AVERT|mg_assaut|aucun_tireur_disponible" call MC4_LOG };
};
...
if (MC4_MENACE_P5 in [2, 3]) then {
    private _tDec = time;
    waitUntil { sleep 0.5; (!isNil "MC4_SITUATION_P5_FAITE") || MC4_FIN || ((time - _tDec) > 10) };
};
if ((MC4_DELAI_MODE == 1) && { MC4_F_LEN > 0 }) then {
    // memes perceptions, memes calculs et meme instant que la ligne de decision ecrite juste apres
    private _defF = (if (isNil "MC4_EST_SITE") then {[]} else {MC4_EST_SITE}) select { alive _x };
    private _obsF = [
        (if (MC4_ALARME) then {1} else {0}),
        (if (MC4_T_ALARME >= 0) then { round (time - MC4_T_ALARME) } else { -1 }),
        (if (MC4_COMPROMIS) then {1} else {0}),
    ...
    private _res = [0, _obsF] call MC4_fnc_evalNoeud;
    private _valF = _res select 0;
    MC4_DELAI_JOUE = if (_valF > 0) then {180} else {45};
    [5, "DELAI_PORTEUR", [45, 180], MC4_DELAI_JOUE, "FORMULE", format ["|valeur_formule|%1|codes_lus|%2|codes_attendus|%3", _valF, _res select 1, MC4_F_LEN]] call MC4_fnc_decision;
} else {
    [5, "DELAI_PORTEUR", [45, 180], MC4_DELAI_PORTEUR, "IMPOSE"] call MC4_fnc_decision;
};
...
(format ["CHACAL|E|premier_pas_assaut|%1|tirs_appui_avant|%2|feu_avant|%3", round (time * 100) / 100,
    MC4_TIRS_APPUI, MC4_FEU_AVANT]) call MC4_LOG;
...
MC4_COMP_ASSAUT = if (MC4_FEU_AVANT == 1 || { MC4_TACTIQUE > 0 }) then {"AWARE"} else {"COMBAT"};
MC4_gAssaut setBehaviour MC4_COMP_ASSAUT; MC4_gAssaut setCombatMode "RED"; MC4_gAssaut setSpeedMode "FULL";
MC4_CIBLE_ASSAUT = +MC4_OUV_CHOISIE;
[MC4_gAssaut, MC4_OUV_CHOISIE, "FULL", MC4_COMP_ASSAUT, "WEDGE", "OUVERTURE_CHOISIE"] call MC4_fnc_ordreAller;
if (MC4_FEU_AVANT == 1 || { MC4_TACTIQUE > 0 }) then {
    [] spawn {
        while { !MC4_FIN && { MC4_PHASE == 5 } } do {
            sleep 10;
            if (count MC4_CIBLE_ASSAUT > 0 && { !isNull MC4_gAssaut }) then {
                MC4_gAssaut setBehaviour "AWARE";
                { if (alive _x) then { _x doMove MC4_CIBLE_ASSAUT } } forEach (units MC4_gAssaut);
                MC4_RELANCES = MC4_RELANCES + 1;
            };
        };
    };
};
...
private _rO = [MC4_gAssaut, MC4_OUV_CHOISIE, 55, [MC4_gAssaut, MC4_OUV_CHOISIE, 1.2] call MC4_fnc_budget] call MC4_fnc_arrive;
(format ["CHACAL|E|ouverture|%1|issue|%2|distance|%3|reste|%4", round (time * 100) / 100, _rO,
    round ((((units MC4_gAssaut) select { alive _x }) call MC4_fnc_centre) distance2D MC4_OUV_CHOISIE),
    round (call MC4_fnc_reste)]) call MC4_LOG;
...
// =====================================================================
private _ordre = MC4_OBJETS select { _x != MC4_PC };
_ordre = [_ordre, [], { _x distance2D MC4_OUV_CHOISIE }, "ASCEND"] call BIS_fnc_sortBy;
if (!isNull MC4_PC) then { _ordre pushBack MC4_PC };
...
(format ["CHACAL|E|file_objectifs|%1|%2|origines_z|%3", round (time * 100) / 100,
    ...
    (_ordre apply { round ((((boundingBoxReal _x) select 0) select 2) * 10) / 10 })]) call MC4_LOG;
...
"CHACAL|E|cqb|differe|le_nettoyage_est_une_etape_separee" call MC4_LOG;

private _hExpl = scriptNull;
{
    private _o = _x;
    if (MC4_FIN || { !alive _o }) then { continue };
    if (count ((units MC4_gAssaut) select { alive _x }) == 0) then {
        (format ["CHACAL|E|file_interrompue|%1|restants|0", round (time * 100) / 100]) call MC4_LOG;
        continue
    };
    ...
    private _cap = (180 * MC4_ECHELLE) min ((call MC4_fnc_reste) - MC4_RESERVE_FEU);
    if (_cap <= 0) then {
        (format ["CHACAL|E|charge_manquee|%1|%2|cause|PLAFOND_PHASE", round (time * 100) / 100, typeOf _o]) call MC4_LOG;
        continue
    };
    private _pt = [_o] call MC4_fnc_pointDePose;
    (format ["CHACAL|E|objectif|%1|%2|point_de_pose|%3|emprise|%4|cap|%5", round (time * 100) / 100,
        typeOf _o, _pt, round (_o call MC4_fnc_emprise), round _cap]) call MC4_LOG;
    MC4_CIBLE_ASSAUT = +_pt;
    [MC4_gAssaut, _pt, "FULL", MC4_COMP_ASSAUT, "WEDGE", "OBJECTIF_" + (typeOf _o)] call MC4_fnc_ordreAller;
    private _r = [MC4_gAssaut, _pt, 22, _cap, 120] call MC4_fnc_arrive;
    private _c0 = ((units MC4_gAssaut) select { alive _x }) call MC4_fnc_centre;
    ...
    if (_dG > 40) then {
        (format ["CHACAL|E|charge_manquee|%1|%2|cause|ELEMENT_N_ARRIVE_PAS|issue|%3|distance|%4",
            round (time * 100) / 100, typeOf _o, _r, _dG]) call MC4_LOG;
        continue
    };
    ...
    if (_o isEqualTo MC4_PC) then {
        "CHACAL|E|exploitation|differee|controle_positif_manquant" call MC4_LOG;
    };

    private _porteurs = (units MC4_gAssaut) select { alive _x && { "DemoCharge_Remote_Mag" in (magazines _x) } };
    if (count _porteurs == 0) then {
        (format ["CHACAL|E|charge_manquee|%1|%2|cause|AUCUN_PORTEUR_VIVANT", round (time * 100) / 100, typeOf _o]) call MC4_LOG;
        continue
    };
    ...
    _h doMove _pt;
    private _t = time;
    waitUntil { sleep 1; ((_h distance2D _pt) < 5) || !(alive _h) || (time - _t > MC4_DELAI_JOUE * MC4_ECHELLE) || MC4_FIN };
    private _dH = round (_h distance2D _pt);
    (format ["CHACAL|E|porteur|%1|%2|%3|temps|%4|distance|%5|delai|%6", round (time * 100) / 100, typeOf _o,
        (_h getVariable ["chacal_role", ""]), round (time - _t), _dH, MC4_DELAI_JOUE]) call MC4_LOG;
    if (!alive _h || { _dH > 12 }) then {
        (format ["CHACAL|E|charge_manquee|%1|%2|cause|PORTEUR_N_ARRIVE_PAS|%3|distance|%4", round (time * 100) / 100,
            typeOf _o, (_h getVariable ["chacal_role", ""]), _dH]) call MC4_LOG;
        if (alive _h && { _h != leader MC4_gAssaut }) then { _h doFollow (leader MC4_gAssaut) };
        continue
    };
    sleep (4 * MC4_ECHELLE);   // le geste a une duree
    ...
    MC4_CHARGES pushBackUnique _o;
    MC4_CHARGES_OBJ pushBack [_o, _c];
    (format ["CHACAL|E|charge_posee|%1|%2|par|%3|distance|%4|actes|%5|sur|%6", round (time * 100) / 100,
        typeOf _o, (_h getVariable ["chacal_role", ""]), _dH, count MC4_CHARGES, count MC4_OBJETS]) call MC4_LOG;
    if (_h != leader MC4_gAssaut) then { _h doFollow (leader MC4_gAssaut) };
} forEach _ordre;

// --- l exploitation est coupee AVANT le degagement ---
if (!isNull _hExpl) then {
    private _tX = time;
    waitUntil { sleep 2; scriptDone _hExpl || (time - _tX > 90 * MC4_ECHELLE) || MC4_FIN };
    if (!scriptDone _hExpl) then { terminate _hExpl };
    if (alive MC4_EXPLOITANT) then { MC4_EXPLOITANT doFollow (leader MC4_gAssaut) };
};

// --- UN SEUL degagement, puis TOUT saute ensemble ---
private _abri = MC4_OUV_CHOISIE getPos [70, MC4_SITE getDir MC4_OUV_CHOISIE];
MC4_CIBLE_ASSAUT = +_abri;
[MC4_gAssaut, _abri, "FULL", MC4_COMP_ASSAUT, "WEDGE", "DEGAGEMENT_AVANT_MISE_A_FEU"] call MC4_fnc_ordreAller;
(format ["CHACAL|E|degagement|%1|vers|%2|charges|%3", round (time * 100) / 100, _abri, count MC4_CHARGES_OBJ]) call MC4_LOG;
private _rD = [MC4_gAssaut, _abri, 30, ((call MC4_fnc_reste) - 60) max 45, 60] call MC4_fnc_arrive;
[_rD] call MC4_fnc_miseAFeu;
...
private _iss5 = if (count MC4_CHARGES >= count MC4_OBJETS) then {"ATTEINT"} else {
    if (count (MC4_FS select { alive _x }) == 0) then {"DETRUIT"} else {
    if ((call MC4_fnc_reste) <= 0) then {"PLAFOND"} else {"INCOMPLET"} } };
(format ["CHACAL|E|feu_appui_total|%1|tirs_appui|%2|relances_assaut|%3|feu_avant|%4", round (time * 100) / 100,
    MC4_TIRS_APPUI, MC4_RELANCES, MC4_FEU_AVANT]) call MC4_LOG;
[5, "ASSAUT", _iss5] call MC4_fnc_finPhase;
};
};
...
// la position REELLE des SURVIVANTS - `MC4_gFS` est vide depuis la scission,
// et la fonction rendait alors son defaut de 120 s pour 4 km a parcourir.
if (MC4_FIN) exitWith {};   // vignette close avant l exfiltration
MC4_EXFIL_POINT = if (MC4_ABANDON) then { MC4_LZ } else { MC4_PZ };
private _vitBudget = if (MC4_EXFIL == 2) then {1.2} else {1.8};
_plafond = [(MC4_FS select { alive _x }), MC4_EXFIL_POINT, _vitBudget] call MC4_fnc_budget;
[6, "EXFILTRATION", _plafond] call MC4_fnc_debutPhase;
...
if (MC4_EXFIL in [0, 1]) then {
    private _vD6 = MC4_FS select { alive _x };
    private _dD6 = if (count _vD6 > 0) then { round ((_vD6 call MC4_fnc_centre) distance2D MC4_EXFIL_POINT) } else { -1 };
    [6, "EXFIL_ALLURE", [0, 1], MC4_EXFIL, "IMPOSE", format ["|distance_point|%1", _dD6]] call MC4_fnc_decision;
};
(format ["CHACAL|E|budget|%1|etape|EXFIL|vers|%2|distance|%3|plafond|%4", round (time * 100) / 100,
    (if (MC4_ABANDON) then {"LZ"} else {"PZ"}),
    round (((MC4_FS select { alive _x }) call MC4_fnc_centre) distance2D MC4_EXFIL_POINT),
    round _plafond]) call MC4_LOG;

private _pourquoi = if (MC4_ABANDON) then {"ABANDON_REPLI_PAR_LA_LZ"} else {"OBJECTIFS_TRAITES"};
...
// L ancienne ligne ne liberait l appui QUE si MC4_APPUI_FIXE valait 1. Or le socle le cloue par sa
...
} forEach [MC4_gAssaut, MC4_gAppui, MC4_gBouchon, MC4_gReco, MC4_gFS];
...
MC4_SANS_JAMBES = 0;
{ if (alive _x && { !(_x checkAIFeature "PATH") }) then { MC4_SANS_JAMBES = MC4_SANS_JAMBES + 1 } } forEach MC4_FS;
(format ["CHACAL|E|exfil_jambes|%1|vivants|%2|sans_path|%3|seuil_exige|%4", round (time * 100) / 100,
    count (MC4_FS select { alive _x }), MC4_SANS_JAMBES, round (0.6 * MC4_EFFECTIF)]) call MC4_LOG;
if (MC4_SANS_JAMBES > 0) then {
    (format ["CHACAL|AVERT|exfil|hommes_vivants_sans_jambes|%1", MC4_SANS_JAMBES]) call MC4_LOG;
};
private _compExf = if (MC4_COMPROMIS) then {"COMBAT"} else {"AWARE"};
if (MC4_EXFIL in [0, 1]) then {
    (format ["CHACAL|E|choix_joue|%1|point|EXFIL_ALLURE|choix|%2|detail|%3|comportement_initial|%4", round (time * 100) / 100,
        MC4_EXFIL, (if (MC4_EXFIL == 1) then {"RAPIDE"} else {"PRUDENT"}), _compExf]) call MC4_LOG;
};
...
if (MC4_EXFIL == 1) then {
    [] spawn {
        private _t0 = time;
        waitUntil { sleep 5;
            private _v = MC4_FS select { alive _x };
            (count _v == 0) || MC4_FIN || ((time - _t0) > 180) ||
            ((_v call MC4_fnc_centre) distance2D MC4_SITE > 200) };
        if (MC4_FIN) exitWith {};
        private _v = MC4_FS select { alive _x };
        ...
        (format ["CHACAL|E|exfil_degage|%1|distance_site|%2|vivants|%3|comportement|AWARE",
            round (time * 100) / 100, round ((_v call MC4_fnc_centre) distance2D MC4_SITE),
            count _v]) call MC4_LOG;
    };
};
{
    if (!isNull _x) then {
        ...
        { if (alive _x) then { _x setUnitPos "AUTO" } } forEach (units _x);
        [_x, MC4_EXFIL_POINT, "FULL", _compExf, "WEDGE", _pourquoi] call MC4_fnc_ordreAller;
    };
} forEach [MC4_gAssaut, MC4_gAppui, MC4_gBouchon, MC4_gReco, MC4_gFS];

private _tE = time;
private _seuilExf = round (0.6 * MC4_EFFECTIF);
...
waitUntil { sleep 3;
    (count (MC4_FS select { alive _x && { (_x distance2D MC4_EXFIL_POINT) < 90 } }) >= _seuilExf) ||
    (count (MC4_FS select { alive _x }) < _seuilExf) || (time - _tE > _plafond) || MC4_FIN };
private _vivExf = count (MC4_FS select { alive _x });
if (_vivExf < _seuilExf) then {
    (format ["CHACAL|E|exfil_impossible|%1|vivants|%2|seuil|%3|temps_ecoule|%4", round (time * 100) / 100,
        _vivExf, _seuilExf, round (time - _tE)]) call MC4_LOG;
};
MC4_EXFILTRES = count (MC4_FS select { alive _x && { (_x distance2D MC4_EXFIL_POINT) < 90 } });
[6, "EXFILTRATION", (if (MC4_EXFILTRES >= _seuilExf) then {"ATTEINT"} else {
    if (count (MC4_FS select { alive _x }) == 0) then {"DETRUIT"} else {
    if (count (MC4_FS select { alive _x }) < _seuilExf) then {"PERTES_EXCESSIVES"} else {"PLAFOND"} } })] call MC4_fnc_finPhase;
