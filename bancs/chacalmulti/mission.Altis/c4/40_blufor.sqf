...
MC4_gFS = ([west, 4] call MULTI_fnc_groupe);
MC4_FS = [];
...
private _rolesTous = [];
for "_p" from 1 to (round (MC4_EFFECTIF / 10)) do { _rolesTous append _rolesBase };

{
    _x params ["_cls", "_role"];
    private _u = MC4_gFS createUnit [_cls, MC4_LZ getPos [12 call MC4_fnc_al, 360 call MC4_fnc_al], [], 0, "NONE"];
    [_u, _role] call MC4_fnc_kitOuest;
    MC4_FS pushBack _u;
} forEach _rolesTous;if (count MC4_FS < MC4_EFFECTIF) exitWith {
    MC4_ISSUE = "VOID"; MC4_CAUSE = "DETACHEMENT_INCOMPLET";
    (format ["CHACAL|VOID|blufor|%1", count MC4_FS]) call MC4_LOG;
};

MC4_gFS selectLeader (MC4_FS select 0);
MC4_gFS setBehaviour "STEALTH";
MC4_gFS setCombatMode "GREEN";      // ne tire que si on lui tire dessus
MC4_gFS setSpeedMode "LIMITED";
MC4_gFS setFormation "FILE";        // la formation de nuit hors piste
MC4_gFS allowFleeing 0;
...
{ for "_i" from 1 to 3 do { _x addMagazine "DemoCharge_Remote_Mag" }; }
    forEach (MC4_FS select { (_x getVariable ["chacal_role",""]) in ["DEMO_1","DEMO_2"] });
{ _x addMagazine "DemoCharge_Remote_Mag"; }
    forEach (MC4_FS select { (_x getVariable ["chacal_role",""]) in ["CHEF","ADJOINT"] });

MC4_fnc_role = {
    private _r = _this;
    MC4_FS select { alive _x && { (_x getVariable ["chacal_role",""]) in _r } }
};
...
MC4_RECO    = ["TIREUR_1","TIREUR_2"];
MC4_APPUI   = ["TIREUR_1","TIREUR_2"];
MC4_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN"];
MC4_BOUCHON = ["AT","FUSILIER_1","FUSILIER_2"];
...
if (MC4_PARTAGE == 1) then {
    MC4_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN","FUSILIER_1","FUSILIER_2"];
    MC4_BOUCHON = ["AT"];
};

MC4_gAppui = grpNull; MC4_gAssaut = grpNull;
MC4_gBouchon = grpNull; MC4_gReco = grpNull; MC4_gDemo = grpNull;
...
MC4_fnc_detacher = {
    params ["_roles", "_nom"];
    private _u = _roles call MC4_fnc_role;
    if (count _u == 0) exitWith { grpNull };
    private _g = ([west, 4] call MULTI_fnc_groupe);
    _u joinSilent _g;
    _g selectLeader (_u select 0);
    _g setVariable ["chacal_element", _nom, true];
    _g allowFleeing 0;
    (format ["CHACAL|E|scission|%1|%2|%3", round (time * 100) / 100, _nom, count _u]) call MC4_LOG;
    _g
};

if (MC4_IMMORTEL == 1) then {
    { _x allowDamage false } forEach MC4_FS;
    "CHACAL|AVERT|hors_corpus|immortel|1|couture_sous_le_feu" call MC4_LOG;
};

(format ["CHACAL|OK|blufor|%1|roles|%2", count MC4_FS,
    (MC4_FS apply { _x getVariable ["chacal_role", "?"] })]) call MC4_LOG;
