...
MC4_GRAINE  = (["MC4_GRAINE", 0] call MULTI_fnc_param)
               + 32 * (["MC4_GRAINE_HAUT", 0] call MULTI_fnc_param);
MC4_ECHELLE = (["MC4_ECHELLE", 100] call MULTI_fnc_param) / 100;
MC4_DT      = (["MC4_DTCS", 100] call MULTI_fnc_param) / 100;
if (MC4_ECHELLE <= 0) then { MC4_ECHELLE = 1 };
if (MC4_DT <= 0) then { MC4_DT = 1 };
// 0 = le PLAN en six phases, 1 = le TEMOIN qui marche droit
MC4_BRAS   = if ((["MC4_BRAS", 0] call MULTI_fnc_param) == 1) then {"NUL"} else {"PLAN"};
MC4_JOUR   = ["MC4_JOUR", 0] call MULTI_fnc_param;
MC4_DEPART = ["MC4_DEPART", 1] call MULTI_fnc_param;
MC4_ARRET  = ["MC4_ARRET", 6] call MULTI_fnc_param;   // vignette : derniere phase jouee
MC4_PALIER = ["MC4_PALIER", 3] call MULTI_fnc_param;
MC4_IMMORTEL = ["MC4_IMMORTEL", 0] call MULTI_fnc_param;
...
// ! MC4_TENIR : poursuivre le plan malgre la compromission (0 = regle prudente d origine).
// La compromission reste JOURNALISEE a l identique ; seule la REACTION change.
// ! MC4_ACCESSIBLE : exiger qu une position soit ATTEIGNABLE, pas seulement plate.
// 0 = comportement d origine, conserve pour que tout ce qui precede reste comparable.
// ! MC4_EFFECTIF : taille du detachement. 10 = d origine.
// A 20, la liste des roles est jouee deux fois : chaque element double, et le corpus
// continue de porter les memes noms de role.
// ! MC4_APPUI_FEU : donner a l appui une position choisie pour TIRER, distincte de
// l observatoire. 0 = comportement d origine, ou l appui reste a l observatoire.
MC4_APPUI_FEU = ["MC4_APPUI_FEU", 0] call MULTI_fnc_param;
// ! MC4_FEU_AVANT ( Fable, 10/09 ) : l appui connait et vise les defenseurs, l assaut attend son
// premier coup, puis marche en AWARE avec un ordre relance toutes les 10 s. 0 = comportement d origine.
MC4_FEU_AVANT = ["MC4_FEU_AVANT", 0] call MULTI_fnc_param;
// ! CAMPAGNE GRAINE 8 ( Fable, 11/09 ) - trois leviers, 0 / 45 = comportement d origine.
MC4_MG_ASSAUT = ["MC4_MG_ASSAUT", 0] call MULTI_fnc_param;
MC4_DELAI_PORTEUR = ["MC4_DELAI_PORTEUR", 45] call MULTI_fnc_param;
// ! BOUCLE EVOGP ( equation/PROTOCOLE_BOUCLE_EVOGP_P5.md, 17/09 ) : le delai du porteur peut etre decide par une formule
// apprise par EvoGP, passee en codes entiers ( MC4_F0..F31, ordre prefixe, equation/formule.py ). Mode 0 = impose par
// le job ( origine ). MC4_DELAI_PORTEUR reste la valeur du job ( ligne FINI ) ; MC4_DELAI_JOUE est le delai joue.
MC4_DELAI_MODE = ["MC4_DELAI_MODE", 0] call MULTI_fnc_param;
MC4_F_LEN = ["MC4_F_LEN", 0] call MULTI_fnc_param;
MC4_F = [];
for "_k" from 0 to 31 do { MC4_F pushBack (["MC4_F" + str _k, 0] call MULTI_fnc_param) };
MC4_F resize MC4_F_LEN;
MC4_DELAI_JOUE = MC4_DELAI_PORTEUR;
MC4_APPUI_FIXE = ["MC4_APPUI_FIXE", 0] call MULTI_fnc_param;
// ! MC4_ORACLE ( Fable, 11/09 ) : les defenseurs reveles a tous et inscrits comme vus. 0 = origine.
MC4_ORACLE = ["MC4_ORACLE", 0] call MULTI_fnc_param;
// ! LE SOCLE ET LES TACTIQUES ( 11/09 ). 0 = comportement d origine dans les deux cas.
MC4_SOCLE = ["MC4_SOCLE", 0] call MULTI_fnc_param;
...
MC4_AZIMUT = ["MC4_AZIMUT", 0] call MULTI_fnc_param;
...
// ou on la reparera.
MC4_OBS = ["MC4_OBS", 1] call MULTI_fnc_param;
// ! MC4_GEOMETRIE : rendre la geometrie du site SANS jouer la mission.
...
MC4_GEOMETRIE = ["MC4_GEOMETRIE", 0] call MULTI_fnc_param;
MC4_AZIMUT_VAL = ["MC4_AZIMUT_VAL", 0] call MULTI_fnc_param;
MC4_AZIMUT_CHOISI = -1;   // ce qui a REELLEMENT ete joue, ecrit dans la ligne FINI
// ! MC4_EXFIL : ce qui fait echouer l exfiltration. 12 episodes sur 12 finissent en PLAFOND
...
MC4_EXFIL = ["MC4_EXFIL", 0] call MULTI_fnc_param;
MC4_TACTIQUE = ["MC4_TACTIQUE", 0] call MULTI_fnc_param;
...
MC4_PLACEUR = ["MC4_PLACEUR", 0] call MULTI_fnc_param;
MC4_COUV = -1;   // part des points interieurs battus depuis la position retenue
MC4_BANC_APPUI = ["MC4_BANC_APPUI", 0] call MULTI_fnc_param;
MC4_TIRS_BANC = 0;
...
MC4_ABLATION = ["MC4_ABLATION", 0] call MULTI_fnc_param;
MC4_fnc_sans = { (floor (MC4_ABLATION / _this)) % 2 == 1 };   // _this = le bit teste
MC4_CONNUS = [];          // defenseurs qui se sont trahis en tirant
MC4_T_PREMIER_TIR_APPUI = -1;
MC4_RELANCES_SOCLE = 0; MC4_FUMIGENES = 0; MC4_ZONES = 0;
MC4_SANS_JAMBES = 0;   // hommes vivants incapables de marcher a l exfiltration - la garde du 13/09
MC4_CIBLE_ASSAUT = [];   // ! posee ici : le chien de garde demarrait avant son initialisation
MC4_EFFECTIF = ["MC4_EFFECTIF", 10] call MULTI_fnc_param;
MC4_ACCESSIBLE = ["MC4_ACCESSIBLE", 0] call MULTI_fnc_param;
MC4_TENIR = ["MC4_TENIR", 0] call MULTI_fnc_param;
MC4_HMG_FORCE = ["MC4_HMG", -1] call MULTI_fnc_param;
MC4_ASSAUT_X  = (["MC4_ASSAUT_X", 100] call MULTI_fnc_param) / 100;
...
MC4_PARTAGE = ["MC4_PARTAGE", 0] call MULTI_fnc_param;
...
MC4_QRF_N     = ["MC4_QRF_N", -1] call MULTI_fnc_param;
MC4_QRF_DELAI = ["MC4_QRF_DELAI", -1] call MULTI_fnc_param;
// Distance imposee de la base de reserve, appliquee dans 10_monde.sqf APRES le tirage.
MC4_QRF_DIST  = ["MC4_QRF_DIST", -1] call MULTI_fnc_param;
MC4_ACC       = ["MC4_ACC", 1] call MULTI_fnc_param;
// La situation : voir 35_menaces.sqf. Tout a 0 par defaut.
MC4_SITUATION = ["MC4_SITUATION", 0] call MULTI_fnc_param;
MC4_MENACE_P1 = ["MC4_MENACE_P1", 0] call MULTI_fnc_param;
MC4_MENACE_P2 = ["MC4_MENACE_P2", 0] call MULTI_fnc_param;
MC4_MENACE_P3 = ["MC4_MENACE_P3", 0] call MULTI_fnc_param;
MC4_MENACE_P4 = ["MC4_MENACE_P4", 0] call MULTI_fnc_param;
MC4_MENACE_P5 = ["MC4_MENACE_P5", 0] call MULTI_fnc_param;
MC4_MENACE_P6 = ["MC4_MENACE_P6", 0] call MULTI_fnc_param;
// ! LES CHOIX DES VIGNETTES ( plans/plan-choix-par-vignette.md, 17/09 ). 0 = la regle d origine, inchangee.
MC4_P1_ATTENTE = ["MC4_P1_ATTENTE", 0] call MULTI_fnc_param;   // 1 partir tout de suite, 2 se terrer 3 min
MC4_TRAVERSEE  = ["MC4_TRAVERSEE", 0] call MULTI_fnc_param;    // 1 traverser tout de suite, 2 attendre la patrouille
MC4_OBS_DUREE  = ["MC4_OBS_DUREE", 0] call MULTI_fnc_param;    // secondes d observation : 120 ou 480
MC4_ITINERAIRE = ["MC4_ITINERAIRE", 0] call MULTI_fnc_param;
...
MC4_OBSERVATION = ["MC4_OBSERVATION", 0] call MULTI_fnc_param;
MC4_CONTROLE_PERCEPTION = ["MC4_CONTROLE_PERCEPTION", 0] call MULTI_fnc_param;   // 3 = 150 m devant, DEBOUT
MC4_SONDE = ["MC4_SONDE", 0] call MULTI_fnc_param;
// ! L ORACLE COMMANDANT ( plan a139c63 ). 0 = temoin, le decor d aujourd hui. 1 = il cherche.
MC4_PORTEE_SON = ["MC4_PORTEE_SON", 600] call MULTI_fnc_param;   // portee du moteur entendu, en metres
// ! 1 = une ligne sonde_decision toutes les 10 s pendant la fenetre : n importe quelle duree de fenetre se relit apres coup
MC4_SONDE_PERCEPTION = ["MC4_SONDE_PERCEPTION", 0] call MULTI_fnc_param;   // portee du moteur entendu, en metres
MC4_ORACLE_CMD   = ["MC4_ORACLE_CMD", 0] call MULTI_fnc_param;
MC4_ORACLE_B     = ["MC4_ORACLE_B", 6] call MULTI_fnc_param;      // budget : nombre de deplacements de patrouille
MC4_ORACLE_CTRL  = ["MC4_ORACLE_CTRL", 0] call MULTI_fnc_param;   // controle de l Oracle : 0 aucun, 1 positif, 3 non-triche
MC4_ORACLE_NU    = ["MC4_ORACLE_NU", 15] call MULTI_fnc_param;    // doute, en pourcent : il se trompe
MC4_ORACLE_EPS   = ["MC4_ORACLE_EPS", 15] call MULTI_fnc_param;   // erreur volontaire, en pourcent
MC4_ORACLE_DELTA = ["MC4_ORACLE_DELTA", 60] call MULTI_fnc_param; // periode de decision, en secondes
// ! PRIX DU TEMPS ( saisine de Fable, 18/09 ) : attente imposee AVANT la phase 5, hors plafond de phase. 0 = origine.
MC4_ATTENTE_TEST = ["MC4_ATTENTE_TEST", 0] call MULTI_fnc_param;
MC4_AVANT = ["MC4_AVANT", 260] call MULTI_fnc_param;   // phase 2 : d ou l on observe la route ( origine 260 m )
MC4_BALAYAGE = ["MC4_BALAYAGE", 0] call MULTI_fnc_param;   // fenetre : 0 balayage d origine, 1 balayage repare
MC4_CONTROLE_DIST = ["MC4_CONTROLE_DIST", 150] call MULTI_fnc_param;   // banc de perception : distance de la cible   // 1 : une ligne sonde_perception toutes les 5 s pendant la fenetre   // 1 direct, 2 detour de 350 m
...
MC4_RNG = ((MC4_GRAINE * 7919) + 104729) % 65537;
if (MC4_RNG == 0) then { MC4_RNG = 1 };
// ! UN TEMOIN QUI NE TIRE QU UNE VALEUR N EST PAS UN TEMOIN ( 14/09 ).
// MC4_fnc_rnd est seme par la graine, et c est voulu : le MONDE doit etre reproductible.
...
MC4_RNG_T = ((MC4_GRAINE * 2654435761) + (round (serverTime * 1000)) + (round (diag_tickTime * 997))) % 65537;
if (MC4_RNG_T == 0) then { MC4_RNG_T = 1 };
MC4_fnc_rndTemoin = {
    MC4_RNG_T = ((MC4_RNG_T * 75) + 74) % 65537;
    MC4_RNG_T / 65537
};

MC4_fnc_rnd = {
    MC4_RNG = ((MC4_RNG * 75) + 74) % 65537;
    MC4_RNG / 65537
};
MC4_fnc_al = { (call MC4_fnc_rnd) * _this };      // uniforme sur [0 ; _this[
// On brule les premiers tirages : deux graines voisines commenceraient sinon
// au meme endroit.
for "_i" from 1 to 20 do { call MC4_fnc_rnd };

// --- etat de l episode ---
MC4_PHASE      = 0;
MC4_PHASE_NOM  = "AUCUNE";
MC4_ALARME     = false;      // le camp EST a compris - irreversible
MC4_COMPROMIS  = false;      // NOUS nous en sommes rendu compte
MC4_FIN        = false;
...
if (MC4_ACC != 1) then {
    setAccTime MC4_ACC;
    (format ["CHACAL|AVERT|hors_corpus|acc|%1", MC4_ACC]) call MC4_LOG;
    [] spawn {
        while { !MC4_FIN } do {
            (format ["CHACAL|E|horloge|%1|reel|%2|acc_demande|%3|acc_moteur|%4",
                round (time * 100) / 100, round (diag_tickTime * 100) / 100, MC4_ACC, accTime]) call MC4_LOG;
            sleep 30;
        };
    };
};
...
MC4_DUREES = [900, 1800, 1500, 900, 900, 600];
MC4_fnc_duree = { (MC4_DUREES select (_this - 1)) * MC4_ECHELLE };

// L HORLOGE DE PHASE. Toute attente s y refere : une sous-etape ne peut pas
// emprunter du temps a la phase suivante.
MC4_TPHASE = 0;
MC4_PLAFOND_COURANT = 1e9;
MC4_fnc_reste = { (MC4_TPHASE + MC4_PLAFOND_COURANT) - time };

MC4_fnc_has = { isClass (configFile >> "CfgVehicles" >> (_this select 0)) };
...
// La phase 6 demandait le budget du repli a MC4_gFS - un groupe VIDE depuis
...
MC4_fnc_budget = {
    ...
    if (count _u == 0) exitWith {
        (format ["CHACAL|AVERT|budget_defaut|%1|aucun_homme|vers|%2", round (time * 100) / 100, _p]) call MC4_LOG;
        120
    };
    private _c = _u call MC4_fnc_centre;
    if (count _c == 0) exitWith {
        (format ["CHACAL|AVERT|budget_defaut|%1|centre_vide|vers|%2", round (time * 100) / 100, _p]) call MC4_LOG;
        120
    };
...
MC4_fnc_pose = {
    ...
    if (!([_cls] call MC4_fnc_has)) exitWith { MC4_MANQUANTES pushBackUnique _cls; objNull };
...
MC4_fnc_poseP = {
    ...
    private _p = _c getPos [_dist, MC4_AZ + _gis];
    [_cls, _c, (_p select 0) - (_c select 0), (_p select 1) - (_c select 1), MC4_AZ + _dirRel] call MC4_fnc_pose
};
...
// MC4_ACCESSIBLE vaut 0, la fonction se comporte exactement comme avant.
MC4_fnc_penteTrajet = {
    params ["_a", "_b"];
...
MC4_fnc_plancher = {
    private _R = 45;
    if (count MC4_OBJETS > 0) then {
        { _R = _R max ((_x distance2D MC4_SITE) + 15) } forEach MC4_OBJETS;
    };
    _R = (_R max 30) min 80;
    private _pts = [];
    private _c = +MC4_SITE; _c set [2, (getTerrainHeightASL MC4_SITE) + 1.1];
    _pts pushBack _c;
    {
        _x params ["_r", "_n", "_dec"];
        for "_i" from 0 to (_n - 1) do {
            private _p = MC4_SITE getPos [_R * _r, _dec + (_i * 360 / _n)];
...
MC4_fnc_positionAppui = {
    ...
    if (MC4_PLACEUR == 1) exitWith {
        ...
        private _plancher = call MC4_fnc_plancher;
        ...
        while { _passe < 3 && { count _best == 0 } } do {
            ...
            for "_a" from 0 to 35 do {
                for "_k" from 3 to 10 do {
                    ...
                    private _c = [_p, _plancher] call MC4_fnc_couverture;
            ...
            // ! SONDES : quatre points fixes a 150 m, pour NOMMER ce qui arrete les rayons.
            {
                ...
                (format ["CHACAL|E|appui_sonde|%1|passe|%2|azimut|%3|libres|%4|sur|%5|terrain|%6|objets|%7|gain|%8",
                    ...
                    round ((getTerrainHeightASL _ps) - _solSite)]) call MC4_LOG;
            ...
            (format ["CHACAL|E|appui_balayage|%1|passe|%2|temoin|%3|balayes|%4|refus_eau|%5|refus_pente|%6|refus_axe|%7|meilleure_couverture|%8|retenue|%9",
                ...
                _refus select 2, round (_meilleurBrut * 1000) / 1000, count _best]) call MC4_LOG;
        };
        MC4_COUV = round (_couv * 1000) / 1000;
        if (count _best == 0 || { _couv < 0.25 }) exitWith {
            (format ["CHACAL|AVERT|appui_feu|aucune_position_battante|meilleure_couverture|%1|temoin|%2|passes|%3",
                round (_meilleurBrut * 1000) / 1000, round (_temoin * 1000) / 1000, _passe]) call MC4_LOG;
            (format ["CHACAL|E|appui_feu|%1|placeur|1|couverture|%2|temoin|%3|refuse|1",
                round (time * 100) / 100, MC4_COUV, round (_temoin * 1000) / 1000]) call MC4_LOG;
            _repli
        };
        _best set [2, 0];
        (format ["CHACAL|E|appui_feu|%1|placeur|1|position|%2|couverture|%3|temoin|%4|passes|%5|points|%6|dist_site|%7|gain_sol_site|%8|score|%9|azimut|%10",
            round (time * 100) / 100, _best, MC4_COUV, round (_temoin * 1000) / 1000, _passe, _np,
            round (_best distance2D _site), round ((getTerrainHeightASL _best) - _solSite),
            round (_sc * 100) / 100, round (_site getDir _best)]) call MC4_LOG;
        _best
    };
    ...
    for "_i" from 1 to 160 do {
        ...
        private _cote = if ((call MC4_fnc_rnd) < 0.5) then {1} else {-1};
        private _dec = _cote * (40 + (80 call MC4_fnc_al));
        private _dist = 160 + (190 call MC4_fnc_al);
    ...
    if (count _best == 0) exitWith {
        (format ["CHACAL|AVERT|appui_feu|aucune_position|repli_observatoire"]) call MC4_LOG;
        _repli
    };
    _best set [2, 0];
    (format ["CHACAL|OK|appui_feu|position|%1|dist_ouverture|%2|gain|%3|score|%4|decal_axe|%5",
        ...
        round (abs ((_site getDir _best) - (_site getDir _axeAssaut)))]) call MC4_LOG;
    _best
};
...
MC4_fnc_socleAssaut = {
    private _ass = (units MC4_gAssaut) select { alive _x };
    ...
    if (!isNull MC4_gAppui && { !(4 call MC4_fnc_sans) }) then {
        MC4_gAppui setBehaviour "COMBAT"; MC4_gAppui setCombatMode "RED";
        MC4_gAppui setVariable ["lambs_danger_disableGroupAI", true, true];
        {
            if (alive _x) then {
                ...
                _x addEventHandler ["Fired", {
                    if (MC4_T_PREMIER_TIR_APPUI < 0) then { MC4_T_PREMIER_TIR_APPUI = time };
                }];
            };
    ...
    {
        if (alive _x && { !(8 call MC4_fnc_sans) }) then {
            _x addEventHandler ["Fired", {
                params ["_t"];
                if (!(_t in MC4_CONNUS)) then {
                    MC4_CONNUS pushBack _t;
                    { _x reveal [_t, 4] } forEach ((units MC4_gAppui) + (units MC4_gAssaut));
                    (format ["CHACAL|E|trahi_par_son_tir|%1|%2|connus|%3", round (time * 100) / 100,
                        (_t getVariable ["chacal_id", -1]), count MC4_CONNUS]) call MC4_LOG;
                };
            }];
        };
    } forEach MC4_EST_SITE;
    (format ["CHACAL|E|socle|%1|porteurs|%2|appui_cloue|%3|ablation|%4|sans_chien|%5|sans_porteurs|%6|sans_appui|%7|sans_revelation|%8",
        ...
        (if (isNull MC4_gAppui || { 4 call MC4_fnc_sans }) then {0} else {count (units MC4_gAppui)}),
        MC4_ABLATION,
        (if (1 call MC4_fnc_sans) then {1} else {0}), (if (2 call MC4_fnc_sans) then {1} else {0}),
        (if (4 call MC4_fnc_sans) then {1} else {0}), (if (8 call MC4_fnc_sans) then {1} else {0})]) call MC4_LOG;
};

// La cible que l appui doit prendre : le fusilier-mitrailleur d abord, puis le chef, puis le reste.
MC4_fnc_ciblePrio = {
    private _viv = (MC4_EST_SITE select { alive _x });
...
MC4_fnc_tactiqueAppui = {
    private _t0 = time; private _derniere = objNull; private _tCible = 0; private _tSupp = -99;
    while { !MC4_FIN && { MC4_PHASE == 5 } && { !isNull MC4_gAppui } } do {
        private _app = (units MC4_gAppui) select { alive _x };
        ...
        if (MC4_TACTIQUE == 2) then {
            ...
            if (isNull _derniere || { !alive _derniere } || { time - _tCible > 60 }) then {
                _derniere = call MC4_fnc_ciblePrio; _tCible = time;
                if (!isNull _derniere) then {
                    if (!MC4_FIN) then {
                        (format ["CHACAL|E|cible_designee|%1|%2|restants|%3", round (time * 100) / 100,
                            (_derniere getVariable ["chacal_id", -1]), count _viv]) call MC4_LOG;
                    };
                };
            };
            if (!isNull _derniere) then {
            ...
            private _cn = MC4_CONNUS select { alive _x };
            if (count _cn == 0) then {
                ...
                if (time - _tSupp > 30) then {
                    _tSupp = time; MC4_ZONES = MC4_ZONES + 1;
                    private _z = if (!isNull MC4_PC) then { getPosATL MC4_PC } else { MC4_SITE };
                    { _x doWatch _z; _x doSuppressiveFire _z } forEach _app;
                    (format ["CHACAL|E|suppression_de_zone|%1|vers|%2|n|%3", round (time * 100) / 100, str _z, MC4_ZONES]) call MC4_LOG;
                };
            };
            if (count _cn > 0) then {
                private _ca = ((units MC4_gAssaut) select { alive _x }) call MC4_fnc_centre;
                ...
                if (!isNull _cible && { time - _tSupp > 30 }) then {
                    ...
                    if (!MC4_FIN) then {
                        (format ["CHACAL|E|suppression|%1|%2|distance_assaut|%3", round (time * 100) / 100,
                            (_cible getVariable ["chacal_id", -1]), round _dmax]) call MC4_LOG;
                    };
                };
            };
        };
...
MC4_fnc_chienDeGarde = {
    ...
    while { !MC4_FIN && { MC4_PHASE == 5 } } do {
        ...
        // apres le debut de la phase 6. C est le bug corrige le 12/09 pour MC4_FIN, dans cette meme
        ...
        // Quand MC4_FIN est pose, le verdict a DEJA ecrit sa ligne CHACAL|FINI| ; le chien, lui,
        ...
        if (MC4_FIN) exitWith {};
        if (MC4_PHASE != 5) exitWith {
            (format ["CHACAL|E|chien_de_garde|%1|phase_quittee|%2|relances|%3", round (time * 100) / 100,
                MC4_PHASE, MC4_RELANCES_SOCLE]) call MC4_LOG;
        };
        if (count MC4_CIBLE_ASSAUT > 0 && { !isNull MC4_gAssaut }) then {
            private _v = (units MC4_gAssaut) select { alive _x };
            if (count _v > 0) then {
                private _c = _v call MC4_fnc_centre;
                private _d = _c distance2D MC4_CIBLE_ASSAUT;
                if (_d < _dRef - 5) then { _dRef = _d; _tRef = time; _rates = 0 }
                else {
                    if (time - _tRef > 30) then {
                        _tRef = time; _rates = _rates + 1; MC4_RELANCES_SOCLE = MC4_RELANCES_SOCLE + 1;
                        MC4_gAssaut setBehaviour "AWARE";
                        { if (alive _x) then { _x doMove MC4_CIBLE_ASSAUT } } forEach _v;
                            if (MC4_FIN) exitWith {};   // ! rien apres la ligne FINI : la porte de lecture refuse l episode
                        (format ["CHACAL|E|chien_de_garde|%1|relance|%2|reste|%3", round (time * 100) / 100,
                            _rates, round _d]) call MC4_LOG;
                        if (_rates >= 2) then {
                            private _cn = MC4_CONNUS select { alive _x };
                            private _vers = if (count _cn > 0) then { getPosATL (_cn select 0) } else { MC4_CIBLE_ASSAUT };
                            private _p = _c getPos [25, _c getDir _vers];
                            createVehicle ["SmokeShell", _p, [], 0, "CAN_COLLIDE"];
                            MC4_FUMIGENES = MC4_FUMIGENES + 1; _rates = 0;
                            (format ["CHACAL|E|fumigene|%1|entre|%2|et|%3", round (time * 100) / 100, str _p, str _vers]) call MC4_LOG;
                        };
                    };
                };
            };
...
MC4_fnc_bancAppui = {
    ...
    if (count _app == 0 || { count _def == 0 }) exitWith {
        (format ["CHACAL|E|banc_appui_fin|%1|coups|0|tireurs|%2|defenseurs|%3|cause|RIEN_A_MESURER",
            round (time * 100) / 100, count _app, count _def]) call MC4_LOG;
    };
    "CHACAL|AVERT|hors_corpus|banc_appui|episode_de_certification" call MC4_LOG;
    ...
    private _cible = _tri select 0;
    {
        ...
        if (MC4_BANC_APPUI == 1) then {
            _u disableAI "PATH";
            _u setVariable ["lambs_danger_disableAI", true, true];
        };
        { _u reveal [_x, 4] } forEach _def;
        _u addEventHandler ["Fired", { MC4_TIRS_BANC = MC4_TIRS_BANC + 1; (_this select 0) setVariable ["banc_tirs", ((_this select 0) getVariable ["banc_tirs", 0]) + 1] }];
        ...
        (format ["CHACAL|E|banc_appui_poste|%1|%2|role|%3|distance_cible|%4|defenseurs_vus|%5|sur|%6|connait|%7|occulteur|%8|mode|%9|couverture|%10",
            ...
            (if (_occulteur == "") then {"aucun"} else {_occulteur}), (combatMode (group _u)), MC4_COUV]) call MC4_LOG;
    } forEach _app;
    MC4_gAppui setBehaviour "COMBAT"; MC4_gAppui setCombatMode "RED";
    if (MC4_BANC_APPUI == 1) then { MC4_gAppui setVariable ["lambs_danger_disableGroupAI", true, true] };
    { if (alive _x) then { _x doWatch _cible; _x doTarget _cible; _x doFire _cible } } forEach _app;
    private _t0 = time; private _morts0 = { !alive _x } count MC4_EST_SITE;
    while { (time - _t0) < (300 * MC4_ECHELLE) && { !MC4_FIN } } do {
        sleep 10;
        private _viv = MC4_EST_SITE select { alive _x };
        private _a = (units MC4_gAppui) select { alive _x };
        ...
        // s ouvrir ensuite. On ecrit toutes les 10 s ce que chaque tireur atteint reellement.
        {
            ...
            (format ["CHACAL|E|banc_appui_vue|%1|%2|vus|%3|sur|%4|coups|%5|mode|%6", round (time * 100) / 100,
                ...
                (combatMode (group _u))]) call MC4_LOG;
        } forEach _a;
    };
    private _detail = "";
    ...
    (format ["CHACAL|E|banc_appui_fin|%1|coups|%2|par_tireur|%3|defenseurs_tues|%4|duree|%5|appui_vivant|%6|cloue|%7",
        round (time * 100) / 100, MC4_TIRS_BANC, _detail,
        ({ !alive _x } count MC4_EST_SITE) - _morts0, round (time - _t0),
        count ((units MC4_gAppui) select { alive _x }), MC4_BANC_APPUI]) call MC4_LOG;
};

MC4_fnc_plat = {
    params ["_c", "_r", ["_essais", 220], ["_depuis", []]];
    private _best = +_c; private _sc = 1e9;
    private _garde = (MC4_ACCESSIBLE == 1) && { count _depuis > 0 };
    for "_i" from 1 to _essais do {
        private _p = _c getPos [sqrt(call MC4_fnc_rnd) * _r, 360 call MC4_fnc_al];
        if (!surfaceIsWater _p) then {
            ...
            if (_garde) then {
                private _pt = [_depuis, _p] call MC4_fnc_penteTrajet;
    ...
    if (_garde) then {
        (format ["CHACAL|OK|plat|accessible|1|depuis|%1|choisi|%2|pente_trajet|%3|score|%4",
            _depuis, _best, round ([_depuis, _best] call MC4_fnc_penteTrajet), round _sc]) call MC4_LOG;
    };
    _best set [2, 0]; _best
};
...
MC4_LAMBS = !(isNil "lambs_wp_fnc_taskPatrol") && { !(isNil "lambs_danger_fnc_tactics") };
...
(format ["CHACAL|EMPREINTE|version|%1|addons|%2|somme|%3|lune|%4|radioShout|%5|radioBackpack|%6|maxRange|%7|cqbRange|%8|manoeuvres|%9",
    productVersion select 2, count activatedAddons, call MC4_fnc_empreinte,
    round ((moonPhase date) * 1000) / 1000,
    ("lambs_main_radioShout" call MC4_fnc_reglage), ("lambs_main_radioBackpack" call MC4_fnc_reglage),
    ("lambs_danger_maxRange" call MC4_fnc_reglage), ("lambs_danger_cqbRange" call MC4_fnc_reglage),
    ("lambs_danger_disableAIAutonomousManoeuvres" call MC4_fnc_reglage)]) call MC4_LOG;

(format ["CHACAL|OK|socle|%1|graine|%2|echelle|%3|dt|%4|lambs|%5|bras|%6|depart|%7|jour|%8|palier|%9|immortel|%10|tenir|%11|arret|%12",
    MC4_VERSION, MC4_GRAINE, MC4_ECHELLE, MC4_DT,
    (if (MC4_LAMBS) then {1} else {0}), MC4_BRAS, MC4_DEPART, MC4_JOUR, MC4_PALIER, MC4_IMMORTEL, MC4_TENIR, MC4_ARRET]) call MC4_LOG;
