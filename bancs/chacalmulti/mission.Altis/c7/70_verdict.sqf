...
[] spawn {
    ...
    private _rngSauve = MC7_RNG;
    MC7_RNG = ((MC7_GRAINE * 2749) + 40507) % 65537;
    if (MC7_RNG == 0) then { MC7_RNG = 1 };
    for "_i" from 1 to 12 do { call MC7_fnc_rnd };
    private _p = []; private _pt = []; private _essaisT = 0; private _trouveT = false;
    for "_k" from 1 to 14 do {
        private _c0 = [MC7_SITE getPos [2400 + (1200 call MC7_fnc_al), 360 call MC7_fnc_al], 150] call MC7_fnc_plat;
        if (surfaceIsWater _c0 || { !([_c0, 7] call MULTI_fnc_loin) }) then { continue };
        private _ha = [(_c0 select 0), (_c0 select 1), (getTerrainHeightASL _c0) + 1.5];
        for "_j" from 1 to 18 do {
            _essaisT = _essaisT + 1;
            private _c1 = _c0 getPos [30 + (30 call MC7_fnc_al), _j * 20];
            _c1 set [2, 0];
    ...
    if (!_trouveT) then {
        (format ["CHACAL|AVERT|temoin_sans_vue|%1|aucun_couple_franc_en_%2_essais", round (time * 100) / 100, _essaisT]) call MC7_LOG;
        _p = [MC7_SITE getPos [3000, 360 call MC7_fnc_al], 150] call MC7_fnc_plat;
        { private _c9 = [MC7_SITE getPos [3000, _x], 150] call MC7_fnc_plat; if (!surfaceIsWater _c9 && { [_c9, 7] call MULTI_fnc_loin }) exitWith { _p = _c9 } } forEach [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330];
        _pt = _p getPos [40, 0];
    };
    (format ["CHACAL|CANARI|couple|%1|essais|%2|franc|%3|distance|%4", round (time * 100) / 100,
        _essaisT, (if (_trouveT) then {1} else {0}), round (_pt distance2D _p)]) call MC7_LOG;
    MC7_RNG = _rngSauve;   // le generateur de la mission est rendu intact

    private _g = ([east, 7] call MULTI_fnc_groupe);
    MC7_CANARI = _g createUnit ["O_Soldier_F", _p, [], 0, "NONE"];
    MC7_CANARI setVariable ["chacal_role", "CANARI", true];
    MC7_CANARI call MC7_fnc_identifier;   // l instrument ne doit pas subir son propre defaut
    _g setBehaviour "CARELESS"; _g allowFleeing 0;
    doStop MC7_CANARI;
    sleep 4;
    private _id = MC7_CANARI getVariable ["chacal_id", -1];
    (format ["CHACAL|CANARI|pose|%1|id|%2|pos|%3", round (time * 100) / 100, _id, _p]) call MC7_LOG;
    ...
    MC7_CANARI setBehaviour "AWARE";
    private _arme = primaryWeapon MC7_CANARI;
    MC7_CANARI selectWeapon _arme;
    sleep 2;
    (format ["CHACAL|CANARI|arme|%1|muzzle|%2|mode|%3", _arme,
        currentMuzzle MC7_CANARI, currentWeaponMode MC7_CANARI]) call MC7_LOG;
    (format ["CHACAL|CANARI|tire|%1|id|%2|coups|3", round (time * 100) / 100, _id]) call MC7_LOG;
    for "_i" from 1 to 3 do {
        if (alive MC7_CANARI) then {
            private _m = currentMuzzle MC7_CANARI;
            if (_m == "") then { _m = _arme };
            MC7_CANARI forceWeaponFire [_m, currentWeaponMode MC7_CANARI];
        };
        sleep 1;
    };
    ...
    private _gt = ([west, 7] call MULTI_fnc_groupe);
    MC7_TEMOIN = _gt createUnit ["B_Soldier_F", _pt, [], 0, "NONE"];
    MC7_TEMOIN setVariable ["chacal_role", "TEMOIN_VUE", true];
    MC7_TEMOIN call MC7_fnc_identifier;
    MC7_TEMOIN setDir (MC7_TEMOIN getDir MC7_CANARI);
    ...
    for "_j" from 1 to 24 do {
        _essaisV = _j;
        if ([MC7_TEMOIN, MC7_CANARI] call MC7_fnc_voit) exitWith { _okVue = true };
        private _cand = _p getPos [28 + (34 call MC7_fnc_al), _j * 15];
        _cand set [2, 0];
        if (!surfaceIsWater _cand) then {
            MC7_TEMOIN setPosATL _cand;
            MC7_TEMOIN setDir (MC7_TEMOIN getDir MC7_CANARI);
            sleep 0.5;
        };
    };
    (format ["CHACAL|CANARI|placement_temoin|%1|essais|%2|vue|%3|distance|%4", round (time * 100) / 100,
        _essaisV, (if (_okVue) then {1} else {0}), round (MC7_TEMOIN distance MC7_CANARI)]) call MC7_LOG;
    if (!_okVue) then {
        (format ["CHACAL|AVERT|temoin_sans_vue|%1|aucune_position_mesuree_franche_en_%2_essais", round (time * 100) / 100, _essaisV]) call MC7_LOG;
    };
    ...
    // AWARE ; on lui `reveal` un ennemi a 34 m, il l abat, et `MC7_fnc_voit`
    // sortait alors sur sa toute premiere garde - `!alive _u`.
    MC7_TEMOIN allowDamage false;
    MC7_CANARI setCombatMode "BLUE";
    MC7_CANARI disableAI "AUTOTARGET";
    MC7_CANARI disableAI "TARGET";
    // On ne coupe JAMAIS "PATH" : mesure du 11/08, ca retire les jambes.
    MC7_CANARI reveal [MC7_TEMOIN, 4];
    MC7_TEMOIN reveal [MC7_CANARI, 4];
    MC7_TEMOIN doWatch MC7_CANARI;
    sleep 5;
    private _rel = abs ((((MC7_TEMOIN getDir MC7_CANARI) - (getDir MC7_TEMOIN) + 540) % 360) - 180);
    private _a = (getPosASL MC7_TEMOIN) vectorAdd [0,0,1.5];
    private _b = (getPosASL MC7_CANARI) vectorAdd [0,0,1.2];
    private _los = (count (lineIntersectsSurfaces [_a, _b, MC7_TEMOIN, MC7_CANARI, true, 1, "VIEW", "VIEW"])) == 0;
    (format ["CHACAL|CANARI|temoin|%1|id|%2|distance|%3|vue|%4|ecart_regard|%5|ligne_de_vue|%6|posture|%7|vivant|%8",
        round (time * 100) / 100, (MC7_TEMOIN getVariable ["chacal_id", -1]),
        round (MC7_TEMOIN distance MC7_CANARI),
        (if ([MC7_TEMOIN, MC7_CANARI] call MC7_fnc_voit) then {1} else {0}),
        round _rel, (if (_los) then {1} else {0}), stance MC7_TEMOIN,
        (if (alive MC7_TEMOIN) then {1} else {0})]) call MC7_LOG;

    // --- CANAL MORT ---
    sleep 45;
    (format ["CHACAL|CANARI|tue|%1|id|%2", round (time * 100) / 100, _id]) call MC7_LOG;
    MC7_CANARI setDamage 1;
};
...
// =====================================================================
[] spawn {
    waitUntil { sleep 2; MC7_FIN };
    sleep 3;

    private _actes = count MC7_CHARGES;
    private _objTotal = count MC7_OBJETS;
    private _vivants = count (MC7_FS select { alive _x });
    private _exf = if (isNil "MC7_EXFILTRES") then {0} else {MC7_EXFILTRES};
    private _pertesEst = (count MC7_EST_SITE) - (count (MC7_EST_SITE select { alive _x }));
    private _lat = if (MC7_T_ALARME < 0 || MC7_T_COMPROMIS < 0) then {-999}
                   else { round ((MC7_T_COMPROMIS - MC7_T_ALARME) * 100) / 100 };

    if (MC7_ISSUE != "VOID" && !MC7_LAMBS) then { MC7_ISSUE = "VOID"; MC7_CAUSE = "LAMBS_ABSENT" };
    if (MC7_ISSUE != "VOID") then {
        if (MC7_ABANDON && _actes == 0) then {
            MC7_ISSUE = "ABANDON";
            ...
            MC7_CAUSE = if (!isNil "MC7_CAUSE_ABANDON" && { MC7_CAUSE_ABANDON != "" })
                           then {MC7_CAUSE_ABANDON} else {"ABANDON_SANS_CAUSE"};
        } else {
            if (_actes >= _objTotal && _exf >= (round (0.6 * MC7_EFFECTIF))) then { MC7_ISSUE = "SUCCES"; MC7_CAUSE = "CHARGES_ET_EXFIL" }
            else {
                MC7_ISSUE = "ECHEC";
                ...
                MC7_CAUSE = if (_vivants == 0) then {"DETACHEMENT_DETRUIT"}
                    else { if (_actes < _objTotal) then {"CHARGES_INCOMPLETES"} else {
                    if (_vivants < (round (0.6 * MC7_EFFECTIF))) then {"PERTES_EXCESSIVES"} else {"EXFIL_MANQUEE"} } };
            };
        };
    };
    ...
    (format ["CHACAL|FINI|%1|%2|graine|%3|echelle|%4|dt|%5|bras|%6|palier|%24|phase_max|%7|charges|%8|sur|%9|detruits_scriptes|%10|intel|%11|exfiltres|%12|vivants|%13|pertes_est|%14|renseignement|%15|alarme|%16|compromis|%17|latence_surprise|%18|abandon|%19|insertion_forcee|%20|lambs|%21|ticks|%22|duree|%23|tenir|%25|arret|%26|depart|%27|effectif|%28|appui_feu|%29|accessible|%30|feu_avant|%31|mg_assaut|%32|delai_porteur|%33|appui_fixe|%34|oracle|%35|socle|%36|tactique|%37|relances|%38|fumigenes|%39|zones|%40|ablation|%41|banc_appui|%42|tirs_banc|%43|placeur|%44|couverture|%45|exfil|%46|azimut|%47|azimut_joue|%48|sans_jambes|%49|obs|%50",
        MC7_ISSUE, MC7_CAUSE, MC7_GRAINE, MC7_ECHELLE, MC7_DT, MC7_BRAS, MC7_PHASE,
        _actes, _objTotal, count MC7_DETRUITS, (if (MC7_INTEL) then {1} else {0}),
        _exf, _vivants, _pertesEst, count MC7_VUES,
        (if (MC7_ALARME) then {1} else {0}), (if (MC7_COMPROMIS) then {1} else {0}), _lat,
        (if (MC7_ABANDON) then {1} else {0}), (if (MC7_INSERTION_FORCEE) then {1} else {0}),
        (if (MC7_LAMBS) then {1} else {0}), MC7_TICK, round time, MC7_PALIER, MC7_TENIR, MC7_ARRET, MC7_DEPART, MC7_EFFECTIF, MC7_APPUI_FEU, MC7_ACCESSIBLE, MC7_FEU_AVANT, MC7_MG_ASSAUT, MC7_DELAI_PORTEUR, MC7_APPUI_FIXE, MC7_ORACLE, MC7_SOCLE, MC7_TACTIQUE, MC7_RELANCES_SOCLE, MC7_FUMIGENES, MC7_ZONES, MC7_ABLATION, MC7_BANC_APPUI, MC7_TIRS_BANC, MC7_PLACEUR, MC7_COUV, MC7_EXFIL, MC7_AZIMUT, MC7_AZIMUT_CHOISI, MC7_SANS_JAMBES, MC7_OBS]) call MC7_LOG;
    MC7_MUET = true;   // MULTI : la cellule se tait, comme un serveur arrete
    ...
    if (MC7_ECHELLE != 1) then {
        (format ["CHACAL|AVERT|hors_corpus|echelle|%1", MC7_ECHELLE]) call MC7_LOG;
    };

    call MC7_fnc_arreterCapture;
    ...
    "CHACAL|OK|serveur_au_repos|le_harnais_peut_relancer" call MC7_LOG;
};
