...
MC7_SITE = []; MC7_OP = []; MC7_LZ = []; MC7_PZ = [];
MC7_ROUTE = []; MC7_QRF_BASE = []; MC7_OP_GAIN = -1e9; MC7_OP_VUE = false;
MC7_ROUTE_A = []; MC7_ROUTE_B = []; MC7_RALLY = [];

private _essais = 0; private _trouve = false;
// Un refus qui ne nomme pas sa cause envoie chercher au hasard : on compte.
MC7_REFUS = ["ancre",0,"replat",0,"crete",0,"vue",0,"route",0,"lz",0,"qrf",0,"pz",0];
MC7_fnc_refus = {
    private _i = MC7_REFUS find _this;
    if (_i >= 0) then { MC7_REFUS set [_i + 1, (MC7_REFUS select (_i + 1)) + 1] };
};
...
while { _essais < 120 && !_trouve } do {
    ...
    for "_k" from 1 to 80 do {
        private _p = [4000 + (22000 call MC7_fnc_al), 4000 + (22000 call MC7_fnc_al), 0];
        if (!surfaceIsWater _p && { (getTerrainHeightASL _p) > 12 }
            && { count (nearestObjects [_p, ["House"], 90]) <= 2 }) exitWith { _a = _p };
    };
    if (count _a == 0) then { "ancre" call MC7_fnc_refus; continue };

    private _site = [_a, 90, 200] call MC7_fnc_plat;
    if (surfaceIsWater _site) then { "replat" call MC7_fnc_refus; continue };
    ...
    for "_i" from 1 to 500 do {
        private _p = _site getPos [480 + (340 call MC7_fnc_al), 360 call MC7_fnc_al];
        _p set [2, 0];
    ...
    if (count _op == 0 || { _gain < 18 }) then { "crete" call MC7_fnc_refus; continue };
    _vue = [_op, _site] call MC7_fnc_libre;
    if (!_vue) then {
        ...
        for "_i" from 1 to 220 do {
            private _q = _op getPos [90 call MC7_fnc_al, 360 call MC7_fnc_al]; _q set [2, 0];
            if (!surfaceIsWater _q && { [_q, _site] call MC7_fnc_libre }) exitWith { _op = _q; _vue = true };
        };
    };
    if (!_vue) then { "vue" call MC7_fnc_refus; continue };
    ...
    if (count _rs == 0) then { "route" call MC7_fnc_refus; continue };
    ...
    for "_j" from 1 to 30 do {
        private _c1 = [_site getPos [3500 + (1000 call MC7_fnc_al), _azLz + (60 call MC7_fnc_al) - 30], 130, 160] call MC7_fnc_plat;
    ...
    if (count _lz == 0) then { "lz" call MC7_fnc_refus; continue };
    ...
    if (count _qrs == 0) then { "qrf" call MC7_fnc_refus; continue };
    private _qcles = _qrs apply { [round ((getPosATL _x) select 0) * 100000 + round ((getPosATL _x) select 1), _x] };
    _qcles sort true;
    private _qb = getPosATL ((_qcles select (floor ((count _qcles) call MC7_fnc_al))) select 1);
    _qb set [2, 0];

    // --- 6. le point d extraction de secours ---
    private _pz = [];
    for "_j" from 1 to 15 do {
        private _c2 = [_site getPos [1200 + (400 call MC7_fnc_al), _azLz + 120 + (80 call MC7_fnc_al)], 110, 160] call MC7_fnc_plat;
        if (!surfaceIsWater _c2) exitWith { _pz = _c2 };
    };
    if (count _pz == 0) then { "pz" call MC7_fnc_refus; continue };

    MC7_SITE = _site; MC7_OP = _op; MC7_LZ = _lz; MC7_PZ = _pz;
    MC7_ROUTE = _route; MC7_QRF_BASE = _qb;
    MC7_OP_GAIN = _gain; MC7_OP_VUE = _vue;
    _trouve = true;
};

if (!_trouve) exitWith {
    MC7_ISSUE = "VOID"; MC7_CAUSE = "SITE_INTROUVABLE";
    (format ["CHACAL|VOID|site|essais|%1|refus|%2", _essais, MC7_REFUS]) call MC7_LOG;
};
(format ["CHACAL|OK|tirage|essais|%1|refus|%2", _essais, MC7_REFUS]) call MC7_LOG;
...
// MC7_fnc_rnd est un cycle unique, deux marches differentes rendent deux mondes ).
...
if (MC7_QRF_DIST > 0) then {
    private _d0 = round (MC7_SITE distance2D MC7_QRF_BASE);
    private _az = MC7_SITE getDir MC7_QRF_BASE;
    private _cible = MC7_SITE getPos [MC7_QRF_DIST, _az];
    ...
    (format ["CHACAL|E|qrf_rapprochee|%1|dist_tiree|%2|dist_imposee|%3|dist_reelle|%4|sur_route|%5|azimut|%6|base|%7",
        round (time * 100) / 100, _d0, MC7_QRF_DIST, round (MC7_SITE distance2D _base),
        _surRoute, round _az, _base]) call MC7_LOG;
    (format ["CHACAL|AVERT|hors_corpus|qrf_dist|%1", MC7_QRF_DIST]) call MC7_LOG;
};

// les deux bouts du circuit patrouille
private _ext = MC7_ROUTE nearRoads 900;
MC7_ROUTE_A = MC7_ROUTE; MC7_ROUTE_B = MC7_ROUTE;
if (count _ext > 3) then {
    private _far = 0;
    { private _d = _x distance2D MC7_ROUTE; if (_d > _far) then { _far = _d; MC7_ROUTE_B = getPosATL _x } } forEach _ext;
    MC7_ROUTE_B set [2, 0];
    private _far2 = 0;
    { private _d = _x distance2D MC7_ROUTE_B; if (_d > _far2) then { _far2 = _d; MC7_ROUTE_A = getPosATL _x } } forEach _ext;
    MC7_ROUTE_A set [2, 0];
};
...
MC7_RALLY = [MC7_SITE getPos [900, (MC7_SITE getDir MC7_OP) + 25], 90] call MC7_fnc_plat;
...
MC7_VIT_INFIL = 0.7;
MC7_VIT_REPLI = 1.8;
private _dApp = MC7_LZ distance2D MC7_RALLY;
private _dObs = MC7_RALLY distance2D MC7_OP;
private _dMep = (MC7_SITE distance2D MC7_QRF_BASE) min 900;
private _dExf = (MC7_SITE distance2D MC7_PZ) + 400;
MC7_DUREES = [
    480,                                    // 1 insertion : vol + mise en ordre
    (_dApp / MC7_VIT_INFIL) + 600,       // 2 approche + attente de la fenetre
    (_dObs / MC7_VIT_INFIL) + 1200,      // 3 montee + 20 min d observation
    (_dMep / MC7_VIT_INFIL) + 420,       // 4 articulation des trois elements
    900 * MC7_ASSAUT_X,                  // 5 action sur objectif (seul plafond CONSTANT,
                                            //   d ou le multiplicateur : les autres derivent
                                            //   de la geometrie, celui-ci non)
    (_dExf / MC7_VIT_REPLI) + 300        // 6 rupture jusqu au point de secours
];
(format ["CHACAL|OK|plafonds|%1|total_prevu_min|%2", (MC7_DUREES apply { round _x }),
    round (((MC7_DUREES select 0) + (MC7_DUREES select 1) + (MC7_DUREES select 2)
    + (MC7_DUREES select 3) + (MC7_DUREES select 4) + (MC7_DUREES select 5)) / 60)]) call MC7_LOG;
...
if (MC7_JOUR == 1) then {
    ...
    (format ["CHACAL|AVERT|hors_corpus|jour|1|controle_positif_observation|heure|%1", date select 3]) call MC7_LOG;
};

(format ["CHACAL|OK|monde|essais|%1|site|%2|alt|%3|op|%4|gain|%5|vue|%6|lz|%7|route|%8|qrf|%9|pz|%10|rally|%11",
    _essais, MC7_SITE, round (getTerrainHeightASL MC7_SITE), MC7_OP,
    round MC7_OP_GAIN, (if (MC7_OP_VUE) then {1} else {0}),
    MC7_LZ, MC7_ROUTE, MC7_QRF_BASE, MC7_PZ, MC7_RALLY]) call MC7_LOG;
(format ["CHACAL|OK|distances|lz_site|%1|op_site|%2|route_site|%3|qrf_site|%4|pz_site|%5|lz_route|%6",
    round (MC7_LZ distance2D MC7_SITE), round (MC7_OP distance2D MC7_SITE),
    round (MC7_ROUTE distance2D MC7_SITE), round (MC7_QRF_BASE distance2D MC7_SITE),
    round (MC7_PZ distance2D MC7_SITE), round (MC7_LZ distance2D MC7_ROUTE)]) call MC7_LOG;
