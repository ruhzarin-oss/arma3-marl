...
MC7_CAP_VERSION = 1;
if (isNil "MC7_NEXT_ID") then { MC7_NEXT_ID = 1 };
MC7_TICK = 0; MC7_LAST = -1;
MC7_MAXCAR = 850;
MC7_CHRONO = [0, 0];
MC7_DTV = 2;                                   // la perception se rejoue moins vite que la position

MC7_TOUS = { (allUnits + allDeadMen) select { private _c = _x getVariable ["multi_c", -1]; if (_c < 0) then { _c = _x call MULTI_fnc_cellule }; _c == 7 } };
MC7_fnc_side = {
    private _s = _this;
...
MC7_fnc_identifier = {
    ...
    _u setVariable ["chacal_id", MC7_NEXT_ID];
    _u setVariable ["chacal_side", (side _u) call MC7_fnc_side];
    _u setVariable ["chacal_fire", 0];
    (format ["CHACAL|E|spawn|%1|%2|%3|%4|%5", round (time * 100) / 100, MC7_NEXT_ID,
        (side _u) call MC7_fnc_side, typeOf _u,
        (_u getVariable ["chacal_role", ""])]) call MC7_LOG;
    _u addEventHandler ["Fired", {
        ...
        (format ["CHACAL|E|tir|%1|%2|%3|%4", round (time * 100) / 100,
            ...
            MC7_PHASE]) call MC7_LOG;
    }];
    MC7_NEXT_ID = MC7_NEXT_ID + 1;
    MC7_NEXT_ID - 1
};
...
MC7_fnc_identifierVehicule = {
    ...
    _v addEventHandler ["Fired", {
        ...
        (format ["CHACAL|E|tir|%1|%2|%3|%4|bord|%5", round (time * 100) / 100,
            ...
            MC7_PHASE, typeOf _veh]) call MC7_LOG;
    }];
};

MC7_fnc_recenser = {
    { _x call MC7_fnc_identifier } forEach (call MC7_TOUS);
    { if ((count (crew _x) > 0) && { ((crew _x select 0) call MULTI_fnc_cellule) == 7 }) then { _x setVariable ["multi_c", 7]; _x call MC7_fnc_identifierVehicule } } forEach vehicles;
};
call MC7_fnc_recenser;
[] spawn { while { !MC7_FIN } do { call MC7_fnc_recenser; sleep 2 } };
(format ["CHACAL|OK|recenseur|1|au_premier_passage|%1", MC7_NEXT_ID - 1]) call MC7_LOG;
...
MC7_EH_FRAME = addMissionEventHandler ["EachFrame", {
    if (MC7_CAP_VERSION != 1) exitWith {};
    private _t = time;
    if (_t - MC7_LAST < MC7_DT) exitWith {};
    MC7_LAST = _t;
    private _d0 = diag_tickTime;
    MC7_TICK = MC7_TICK + 1;
    private _tr = round (_t * 100) / 100;
    private _tous = call MC7_TOUS;
    private _n = count _tous;
    private _bouts = []; private _cour = "";
    {
        private _u = _x;
        private _id = _u getVariable ["chacal_id", -1];
        if (_id < 0) then { _id = _u call MC7_fnc_identifier };
        ...
        private _e = str [
            ...
            (_u call MC7_fnc_posture),
            (_u call MC7_fnc_comport),
            round (speed _u),
            (_u getVariable ["chacal_fire", 0])
        ];
        if ((_u getVariable ["chacal_fire", 0]) == 1) then { _u setVariable ["chacal_fire", 0] };
        if ((count _cour) + (count _e) + 1 > MC7_MAXCAR) then { _bouts pushBack _cour; _cour = "" };
        _cour = if (_cour == "") then { _e } else { _cour + "," + _e };
    } forEach _tous;
    _bouts pushBack _cour;
    { (format ["CHACAL|S|%1|%2|%3|%4|%5|[%6]", MC7_TICK, _forEachIndex, _tr, _n, MC7_PHASE, _x]) call MC7_LOG; } forEach _bouts;

    MC7_CHRONO set [0, (MC7_CHRONO select 0) + (diag_tickTime - _d0)];
    MC7_CHRONO set [1, (MC7_CHRONO select 1) + 1];
    if ((MC7_CHRONO select 1) % 400 == 0) then {
        (format ["CHACAL|C|cout_ms|%1|ticks|%2|unites|%3|fps|%4",
            round (1000 * (MC7_CHRONO select 0) / (MC7_CHRONO select 1) * 100) / 100,
            (MC7_CHRONO select 1), _n, round diag_fps]) call MC7_LOG;
        MC7_CHRONO = [0, 0];
    };
}];
"CHACAL|OK|emetteur|1" call MC7_LOG;

// ======================= 3. LES TROIS VUES =======================
MC7_T_VUES = time - (7 * 0.37);
MC7_EH_VUES = addMissionEventHandler ["EachFrame", {
    if (MC7_FIN || { MC7_CAP_VERSION != 1 } || { (time - MC7_T_VUES) < MC7_DTV }) exitWith {};
    MC7_T_VUES = time;
    call {
        private _tr = round (time * 100) / 100;
        private _vivants = (call MC7_TOUS) select { alive _x };
        ...
        (format ["CHACAL|VC|%1|%2|est_sur_ouest|%3", _tr, MC7_PHASE, str _cb]) call MC7_LOG;
        (format ["CHACAL|VC|%1|%2|ouest_sur_est|%3", _tr, MC7_PHASE, str _co]) call MC7_LOG;
        ...
        private _fnc = {
            ...
            (format ["CHACAL|VH|%1|%2|%3|%4", _tr, MC7_PHASE, _etiq, str _lignes]) call MC7_LOG;
        };
        ...
        private _vg = [];
        {
            ...
            {
                if ([_u, _x] call MC7_fnc_voit) then {
        ...
        (format ["CHACAL|VG|%1|%2|ouest|%3", _tr, MC7_PHASE, str _vg]) call MC7_LOG;

    };
}];
"CHACAL|OK|vues|1" call MC7_LOG;
...
[] spawn { while { !MC7_FIN } do { sleep 2; (format ["CHACAL|C|pouls|%1", round (time * 100) / 100]) call MC7_LOG } };

// ======================= 4. LES MORTS =======================
MC7_EH_MORT = addMissionEventHandler ["EntityKilled", {
    params ["_vic", "_tueur", ["_instig", objNull]];
    if ((_vic call MULTI_fnc_cellule) != 7) exitWith {};   // MULTI : une mort d une autre cellule ne s ecrit pas ici
    ...
    private _t = _tueur call MC7_fnc_idTueur;
    (format ["CHACAL|E|mort|%1|%2|%3|%4|%5|par|%6", round (time * 100) / 100,
        (_vic getVariable ["chacal_id", -1]),
        (_t select 0), MC7_PHASE,
        (_vic getVariable ["chacal_role", ""]), (_t select 1)]) call MC7_LOG;
}];
"CHACAL|OK|morts|1" call MC7_LOG;
...
MC7_fnc_arreterCapture = {
    if (MC7_CAP_VERSION == 0) exitWith {};
    MC7_CAP_VERSION = 0;
    removeMissionEventHandler ["EachFrame", MC7_EH_FRAME];
    removeMissionEventHandler ["EachFrame", MC7_EH_VUES];
    removeMissionEventHandler ["EntityKilled", MC7_EH_MORT];
    { _x removeAllEventHandlers "Fired" } forEach ((call MC7_TOUS) + (vehicles select { (_x getVariable ["multi_c", -1]) == 7 }));
    (format ["CHACAL|OK|capture_arretee|ticks|%1|t|%2", MC7_TICK, round (time * 100) / 100]) call MC7_LOG;
};

(format ["CHACAL|OK|capture|%1|dt|%2|dtv|%3", MC7_CAP_VERSION, MC7_DT, MC7_DTV]) call MC7_LOG;
