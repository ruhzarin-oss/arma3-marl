...
[] spawn {
    ...
    private _rngSauve = MC1_RNG;
    MC1_RNG = ((MC1_GRAINE * 2749) + 40507) % 65537;
    if (MC1_RNG == 0) then { MC1_RNG = 1 };
    for "_i" from 1 to 12 do { call MC1_fnc_rnd };
    private _p = []; private _pt = []; private _essaisT = 0; private _trouveT = false;
    for "_k" from 1 to 14 do {
        private _c0 = [MC1_SITE getPos [2400 + (1200 call MC1_fnc_al), 360 call MC1_fnc_al], 150] call MC1_fnc_plat;
        if (surfaceIsWater _c0 || { !([_c0, 1] call MULTI_fnc_loin) }) then { continue };
        private _ha = [(_c0 select 0), (_c0 select 1), (getTerrainHeightASL _c0) + 1.5];
        for "_j" from 1 to 18 do {
            _essaisT = _essaisT + 1;
            private _c1 = _c0 getPos [30 + (30 call MC1_fnc_al), _j * 20];
            _c1 set [2, 0];
    ...
    if (!_trouveT) then {
        (format ["CHACAL|AVERT|temoin_sans_vue|%1|aucun_couple_franc_en_%2_essais", round (time * 100) / 100, _essaisT]) call MC1_LOG;
        _p = [MC1_SITE getPos [3000, 360 call MC1_fnc_al], 150] call MC1_fnc_plat;
        { private _c9 = [MC1_SITE getPos [3000, _x], 150] call MC1_fnc_plat; if (!surfaceIsWater _c9 && { [_c9, 1] call MULTI_fnc_loin }) exitWith { _p = _c9 } } forEach [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330];
        _pt = _p getPos [40, 0];
    };
    (format ["CHACAL|CANARI|couple|%1|essais|%2|franc|%3|distance|%4", round (time * 100) / 100,
        _essaisT, (if (_trouveT) then {1} else {0}), round (_pt distance2D _p)]) call MC1_LOG;
    MC1_RNG = _rngSauve;   // le generateur de la mission est rendu intact

    private _g = ([east, 1] call MULTI_fnc_groupe);
    MC1_CANARI = _g createUnit ["O_Soldier_F", _p, [], 0, "NONE"];
    MC1_CANARI setVariable ["chacal_role", "CANARI", true];
    MC1_CANARI call MC1_fnc_identifier;   // l instrument ne doit pas subir son propre defaut
    _g setBehaviour "CARELESS"; _g allowFleeing 0;
    doStop MC1_CANARI;
    sleep 4;
    private _id = MC1_CANARI getVariable ["chacal_id", -1];
    (format ["CHACAL|CANARI|pose|%1|id|%2|pos|%3", round (time * 100) / 100, _id, _p]) call MC1_LOG;
    ...
    MC1_CANARI setBehaviour "AWARE";
    private _arme = primaryWeapon MC1_CANARI;
    MC1_CANARI selectWeapon _arme;
    sleep 2;
    (format ["CHACAL|CANARI|arme|%1|muzzle|%2|mode|%3", _arme,
        currentMuzzle MC1_CANARI, currentWeaponMode MC1_CANARI]) call MC1_LOG;
    (format ["CHACAL|CANARI|tire|%1|id|%2|coups|3", round (time * 100) / 100, _id]) call MC1_LOG;
    for "_i" from 1 to 3 do {
        if (alive MC1_CANARI) then {
            private _m = currentMuzzle MC1_CANARI;
            if (_m == "") then { _m = _arme };
            MC1_CANARI forceWeaponFire [_m, currentWeaponMode MC1_CANARI];
        };
        sleep 1;
    };
    ...
    private _gt = ([west, 1] call MULTI_fnc_groupe);
    MC1_TEMOIN = _gt createUnit ["B_Soldier_F", _pt, [], 0, "NONE"];
    MC1_TEMOIN setVariable ["chacal_role", "TEMOIN_VUE", true];
    MC1_TEMOIN call MC1_fnc_identifier;
    MC1_TEMOIN setDir (MC1_TEMOIN getDir MC1_CANARI);
    ...
    for "_j" from 1 to 24 do {
        _essaisV = _j;
        if ([MC1_TEMOIN, MC1_CANARI] call MC1_fnc_voit) exitWith { _okVue = true };
        private _cand = _p getPos [28 + (34 call MC1_fnc_al), _j * 15];
        _cand set [2, 0];
        if (!surfaceIsWater _cand) then {
            MC1_TEMOIN setPosATL _cand;
            MC1_TEMOIN setDir (MC1_TEMOIN getDir MC1_CANARI);
            sleep 0.5;
        };
    };
    (format ["CHACAL|CANARI|placement_temoin|%1|essais|%2|vue|%3|distance|%4", round (time * 100) / 100,
        _essaisV, (if (_okVue) then {1} else {0}), round (MC1_TEMOIN distance MC1_CANARI)]) call MC1_LOG;
    if (!_okVue) then {
        (format ["CHACAL|AVERT|temoin_sans_vue|%1|aucune_position_mesuree_franche_en_%2_essais", round (time * 100) / 100, _essaisV]) call MC1_LOG;
    };
    ...
    // AWARE ; on lui `reveal` un ennemi a 34 m, il l abat, et `MC1_fnc_voit`
    // sortait alors sur sa toute premiere garde - `!alive _u`.
    MC1_TEMOIN allowDamage false;
    MC1_CANARI setCombatMode "BLUE";
    MC1_CANARI disableAI "AUTOTARGET";
    MC1_CANARI disableAI "TARGET";
    // On ne coupe JAMAIS "PATH" : mesure du 11/08, ca retire les jambes.
    MC1_CANARI reveal [MC1_TEMOIN, 4];
    MC1_TEMOIN reveal [MC1_CANARI, 4];
    MC1_TEMOIN doWatch MC1_CANARI;
    sleep 5;
    private _rel = abs ((((MC1_TEMOIN getDir MC1_CANARI) - (getDir MC1_TEMOIN) + 540) % 360) - 180);
    private _a = (getPosASL MC1_TEMOIN) vectorAdd [0,0,1.5];
    private _b = (getPosASL MC1_CANARI) vectorAdd [0,0,1.2];
    private _los = (count (lineIntersectsSurfaces [_a, _b, MC1_TEMOIN, MC1_CANARI, true, 1, "VIEW", "VIEW"])) == 0;
    (format ["CHACAL|CANARI|temoin|%1|id|%2|distance|%3|vue|%4|ecart_regard|%5|ligne_de_vue|%6|posture|%7|vivant|%8",
        round (time * 100) / 100, (MC1_TEMOIN getVariable ["chacal_id", -1]),
        round (MC1_TEMOIN distance MC1_CANARI),
        (if ([MC1_TEMOIN, MC1_CANARI] call MC1_fnc_voit) then {1} else {0}),
        round _rel, (if (_los) then {1} else {0}), stance MC1_TEMOIN,
        (if (alive MC1_TEMOIN) then {1} else {0})]) call MC1_LOG;

    // --- CANAL MORT ---
    sleep 45;
    (format ["CHACAL|CANARI|tue|%1|id|%2", round (time * 100) / 100, _id]) call MC1_LOG;
    MC1_CANARI setDamage 1;
};
...
// =====================================================================
[] spawn {
    waitUntil { sleep 2; MC1_FIN };
    sleep 3;

    private _actes = count MC1_CHARGES;
    private _objTotal = count MC1_OBJETS;
    private _vivants = count (MC1_FS select { alive _x });
    private _exf = if (isNil "MC1_EXFILTRES") then {0} else {MC1_EXFILTRES};
    private _pertesEst = (count MC1_EST_SITE) - (count (MC1_EST_SITE select { alive _x }));
    private _lat = if (MC1_T_ALARME < 0 || MC1_T_COMPROMIS < 0) then {-999}
                   else { round ((MC1_T_COMPROMIS - MC1_T_ALARME) * 100) / 100 };

    if (MC1_ISSUE != "VOID" && !MC1_LAMBS) then { MC1_ISSUE = "VOID"; MC1_CAUSE = "LAMBS_ABSENT" };
    if (MC1_ISSUE != "VOID") then {
        if (MC1_ABANDON && _actes == 0) then {
            MC1_ISSUE = "ABANDON";
            ...
            MC1_CAUSE = if (!isNil "MC1_CAUSE_ABANDON" && { MC1_CAUSE_ABANDON != "" })
                           then {MC1_CAUSE_ABANDON} else {"ABANDON_SANS_CAUSE"};
        } else {
            if (_actes >= _objTotal && _exf >= (round (0.6 * MC1_EFFECTIF))) then { MC1_ISSUE = "SUCCES"; MC1_CAUSE = "CHARGES_ET_EXFIL" }
            else {
                MC1_ISSUE = "ECHEC";
                ...
                MC1_CAUSE = if (_vivants == 0) then {"DETACHEMENT_DETRUIT"}
                    else { if (_actes < _objTotal) then {"CHARGES_INCOMPLETES"} else {
                    if (_vivants < (round (0.6 * MC1_EFFECTIF))) then {"PERTES_EXCESSIVES"} else {"EXFIL_MANQUEE"} } };
            };
        };
    };
    ...
    (format ["CHACAL|FINI|%1|%2|graine|%3|echelle|%4|dt|%5|bras|%6|palier|%24|phase_max|%7|charges|%8|sur|%9|detruits_scriptes|%10|intel|%11|exfiltres|%12|vivants|%13|pertes_est|%14|renseignement|%15|alarme|%16|compromis|%17|latence_surprise|%18|abandon|%19|insertion_forcee|%20|lambs|%21|ticks|%22|duree|%23|tenir|%25|arret|%26|depart|%27|effectif|%28|appui_feu|%29|accessible|%30|feu_avant|%31|mg_assaut|%32|delai_porteur|%33|appui_fixe|%34|oracle|%35|socle|%36|tactique|%37|relances|%38|fumigenes|%39|zones|%40|ablation|%41|banc_appui|%42|tirs_banc|%43|placeur|%44|couverture|%45|exfil|%46|azimut|%47|azimut_joue|%48|sans_jambes|%49|obs|%50",
        MC1_ISSUE, MC1_CAUSE, MC1_GRAINE, MC1_ECHELLE, MC1_DT, MC1_BRAS, MC1_PHASE,
        _actes, _objTotal, count MC1_DETRUITS, (if (MC1_INTEL) then {1} else {0}),
        _exf, _vivants, _pertesEst, count MC1_VUES,
        (if (MC1_ALARME) then {1} else {0}), (if (MC1_COMPROMIS) then {1} else {0}), _lat,
        (if (MC1_ABANDON) then {1} else {0}), (if (MC1_INSERTION_FORCEE) then {1} else {0}),
        (if (MC1_LAMBS) then {1} else {0}), MC1_TICK, round time, MC1_PALIER, MC1_TENIR, MC1_ARRET, MC1_DEPART, MC1_EFFECTIF, MC1_APPUI_FEU, MC1_ACCESSIBLE, MC1_FEU_AVANT, MC1_MG_ASSAUT, MC1_DELAI_PORTEUR, MC1_APPUI_FIXE, MC1_ORACLE, MC1_SOCLE, MC1_TACTIQUE, MC1_RELANCES_SOCLE, MC1_FUMIGENES, MC1_ZONES, MC1_ABLATION, MC1_BANC_APPUI, MC1_TIRS_BANC, MC1_PLACEUR, MC1_COUV, MC1_EXFIL, MC1_AZIMUT, MC1_AZIMUT_CHOISI, MC1_SANS_JAMBES, MC1_OBS]) call MC1_LOG;
    MC1_MUET = true;   // MULTI : la cellule se tait, comme un serveur arrete
    ...
    if (MC1_ECHELLE != 1) then {
        (format ["CHACAL|AVERT|hors_corpus|echelle|%1", MC1_ECHELLE]) call MC1_LOG;
    };

    call MC1_fnc_arreterCapture;
    ...
    "CHACAL|OK|serveur_au_repos|le_harnais_peut_relancer" call MC1_LOG;
};
