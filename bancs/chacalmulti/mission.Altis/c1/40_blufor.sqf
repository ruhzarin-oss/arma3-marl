...
MC1_gFS = ([west, 1] call MULTI_fnc_groupe);
MC1_FS = [];
...
private _rolesTous = [];
for "_p" from 1 to (round (MC1_EFFECTIF / 10)) do { _rolesTous append _rolesBase };

{
    _x params ["_cls", "_role"];
    private _u = MC1_gFS createUnit [_cls, MC1_LZ getPos [12 call MC1_fnc_al, 360 call MC1_fnc_al], [], 0, "NONE"];
    [_u, _role] call MC1_fnc_kitOuest;
    MC1_FS pushBack _u;
} forEach _rolesTous;if (count MC1_FS < MC1_EFFECTIF) exitWith {
    MC1_ISSUE = "VOID"; MC1_CAUSE = "DETACHEMENT_INCOMPLET";
    (format ["CHACAL|VOID|blufor|%1", count MC1_FS]) call MC1_LOG;
};

MC1_gFS selectLeader (MC1_FS select 0);
MC1_gFS setBehaviour "STEALTH";
MC1_gFS setCombatMode "GREEN";      // ne tire que si on lui tire dessus
MC1_gFS setSpeedMode "LIMITED";
MC1_gFS setFormation "FILE";        // la formation de nuit hors piste
MC1_gFS allowFleeing 0;
...
{ for "_i" from 1 to 3 do { _x addMagazine "DemoCharge_Remote_Mag" }; }
    forEach (MC1_FS select { (_x getVariable ["chacal_role",""]) in ["DEMO_1","DEMO_2"] });
{ _x addMagazine "DemoCharge_Remote_Mag"; }
    forEach (MC1_FS select { (_x getVariable ["chacal_role",""]) in ["CHEF","ADJOINT"] });

MC1_fnc_role = {
    private _r = _this;
    MC1_FS select { alive _x && { (_x getVariable ["chacal_role",""]) in _r } }
};
...
MC1_RECO    = ["TIREUR_1","TIREUR_2"];
MC1_APPUI   = ["TIREUR_1","TIREUR_2"];
MC1_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN"];
MC1_BOUCHON = ["AT","FUSILIER_1","FUSILIER_2"];
...
if (MC1_PARTAGE == 1) then {
    MC1_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN","FUSILIER_1","FUSILIER_2"];
    MC1_BOUCHON = ["AT"];
};

MC1_gAppui = grpNull; MC1_gAssaut = grpNull;
MC1_gBouchon = grpNull; MC1_gReco = grpNull; MC1_gDemo = grpNull;
...
MC1_fnc_detacher = {
    params ["_roles", "_nom"];
    private _u = _roles call MC1_fnc_role;
    if (count _u == 0) exitWith { grpNull };
    private _g = ([west, 1] call MULTI_fnc_groupe);
    _u joinSilent _g;
    _g selectLeader (_u select 0);
    _g setVariable ["chacal_element", _nom, true];
    _g allowFleeing 0;
    (format ["CHACAL|E|scission|%1|%2|%3", round (time * 100) / 100, _nom, count _u]) call MC1_LOG;
    _g
};

if (MC1_IMMORTEL == 1) then {
    { _x allowDamage false } forEach MC1_FS;
    "CHACAL|AVERT|hors_corpus|immortel|1|couture_sous_le_feu" call MC1_LOG;
};

(format ["CHACAL|OK|blufor|%1|roles|%2", count MC1_FS,
    (MC1_FS apply { _x getVariable ["chacal_role", "?"] })]) call MC1_LOG;
