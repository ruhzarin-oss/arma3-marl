...
// ne vaut plus rien.
// =====================================================================
if (MC1_ORACLE_CMD <= 0) exitWith {};

MC1_O_BUDGET = MC1_ORACLE_B;
MC1_O_CIBLE = -1;
...
private _lz = MC1_LZ; private _rt = MC1_ROUTE; private _op = MC1_OP; private _st = MC1_SITE;
private _ra = if (count MC1_RALLY > 1) then { MC1_RALLY } else { _lz };
MC1_O_CASES = [
    ["POSER",   _lz],
...
MC1_O_B = _poids apply { _x / _sp };
MC1_O_PRIOR = +MC1_O_B;   // la doctrine, gardee a part : c est vers elle que revient le doute
// les cases que la patrouille de route peut atteindre, et le troncon de route le plus proche de chacune
MC1_O_RTE = MC1_O_CASES apply {
...
private _carte = [];
{
    ...
    _carte pushBack [(_x select 0), (if (count (MC1_O_RTE select _forEachIndex) > 0) then {1} else {0}), _d2];
} forEach MC1_O_CASES;
(format ["CHACAL|O|carte|%1|cases_routieres|%2|detail|%3", round (time * 100) / 100,
    count (MC1_O_RTE select { count _x > 0 }), str _carte]) call MC1_LOG;
...
MC1_O_fnc_yeux = {
    ...
    { if (!isNull _x) then { { if (alive _x) then { _y pushBack _x } } forEach (units _x) } } forEach
        ((MC1_GROUPES_EST + [MC1_gRoute]) select { !isNull _x });
    { if (alive _x && { !(_x in _y) }) then { _y pushBack _x } } forEach MC1_EST_SITE;
    _y
};

MC1_O_fnc_caseLaPlusProche = {
    params ["_p"];
    private _d = MC1_O_CASES apply { (_x select 1) distance2D _p };
...
MC1_O_fnc_croire = {
    ...
    for "_i" from 0 to (_n - 1) do {
        private _v = 0.70 * (MC1_O_B select _i);
        if (_i > 0) then { _v = _v + (0.30 * (MC1_O_B select (_i - 1))) } else { _v = _v + (0.30 * (MC1_O_B select _i)) };
        _bp pushBack _v;
    };
        ...
        if (!isNull _chef && { alive _chef }) then {
            {
                ...
                if (_kn select 0) then {
                    ...
                    if (_age < _fraicheur) then { _fraicheur = _age; _vu = [(_kn select 6)] call MC1_O_fnc_caseLaPlusProche };
                };
            } forEach MC1_FS;                                  // on parcourt la liste, on ne lit QUE sa connaissance
        };
    } forEach ((MC1_GROUPES_EST + [MC1_gRoute]) select { !isNull _x });
    // 3. ce qu il n a pas vu : une case regardee et vide devient moins probable
    private _yeux = call MC1_O_fnc_yeux;
    private _l = [];
    for "_i" from 0 to (_n - 1) do {
        private _p = (MC1_O_CASES select _i) select 1;
        private _q = 1;
        { _q = _q * (1 - ([_x distance2D _p] call MC1_O_fnc_pd)) } forEach _yeux;
        _l pushBack _q;
    };
    ...
    for "_i" from 0 to (_n - 1) do { _bp set [_i, ((1 - _nu) * ((_bp select _i) / _s)) + (_nu * (MC1_O_PRIOR select _i))] };
    MC1_O_B = _bp;
    _vu
};
...
MC1_O_fnc_agir = {
    params ["_cible"];
    private _p = (MC1_O_CASES select _cible) select 1;
    ...
    private _bR = MC1_O_B apply { 0 };
    { if (count (MC1_O_RTE select _forEachIndex) > 0) then { _bR set [_forEachIndex, _x] } } forEach MC1_O_B;
    private _mxR = selectMax _bR;
    private _candR = []; { if ((_x > 0) && { _x >= (_mxR - 0.01) }) then { _candR pushBack _forEachIndex } } forEach _bR;
    if (MC1_O_BUDGET > 0 && { count _candR > 0 } && { !isNull MC1_gRoute }
        && { ({ alive _x } count (units MC1_gRoute)) > 0 }) then {
        private _cibleR = if (MC1_O_CIBLE in _candR) then { MC1_O_CIBLE } else { selectRandom _candR };
        if (_cibleR != MC1_O_CIBLE) then {
            private _a = MC1_O_RTE select _cibleR;
            private _r2 = (_a nearRoads 350) select { (_x distance2D _a) > 250 };
            private _b2 = if (count _r2 > 0) then { getPosATL (_r2 select 0) } else { _a };
            while { (count (waypoints MC1_gRoute)) > 0 } do { deleteWaypoint ((waypoints MC1_gRoute) select 0) };
            private _w = MC1_gRoute addWaypoint [_a, 15];  _w setWaypointType "MOVE";  _w setWaypointSpeed "NORMAL";
            private _w2 = MC1_gRoute addWaypoint [_b2, 15]; _w2 setWaypointType "MOVE"; _w2 setWaypointSpeed "LIMITED";
            (MC1_gRoute addWaypoint [_a, 15]) setWaypointType "CYCLE";
            ...
            MC1_gRoute setCurrentWaypoint _w;
            MC1_O_BUDGET = MC1_O_BUDGET - 1;
            MC1_O_CIBLE = _cibleR;
            _action = format ["PATROUILLE_%1", (MC1_O_CASES select _cibleR) select 0];
        };
    };
    _action
};
...
[] spawn {
    waitUntil { sleep 2; MC1_FIN || { !isNil "MC1_TPHASE" } };
    while { !MC1_FIN } do {
        private _vu = call MC1_O_fnc_croire;
        private _b = +MC1_O_B;
        ...
        if ((random 1) < (MC1_ORACLE_EPS / 100)) then {
            private _c = +_b; _c set [_cible, -1];
            _cible = _c find (selectMax _c);
        };
        private _action = [_cible] call MC1_O_fnc_agir;
        private _dPat = if ((MC1_O_CIBLE >= 0) && { !isNull MC1_gRoute } && { !isNull (leader MC1_gRoute) }) then {
            round ((leader MC1_gRoute) distance2D (MC1_O_RTE select MC1_O_CIBLE)) } else { -1 };
        (format ["CHACAL|O|decision|%1|action|%2|case|%3|p|%4|vu|%5|budget|%6|croyance|%7|cible_patrouille|%8|patrouille_a|%9|vitesse_patrouille|%10",
            round (time * 100) / 100, _action, (MC1_O_CASES select _cible) select 0,
            round ((_b select _cible) * 100) / 100, (if (_vu >= 0) then { (MC1_O_CASES select _vu) select 0 } else { "RIEN" }),
            MC1_O_BUDGET, str (_b apply { round (_x * 100) }),
            (if (MC1_O_CIBLE >= 0) then { (MC1_O_CASES select MC1_O_CIBLE) select 0 } else { "AUCUNE" }), _dPat,
            (if (!isNull MC1_VEH_ROUTE) then { round (speed MC1_VEH_ROUTE) } else { -1 })]) call MC1_LOG;
        sleep (MC1_ORACLE_DELTA * MC1_ECHELLE);
    };
};
