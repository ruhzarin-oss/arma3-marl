...
MC3_CAP_VERSION = 1;
if (isNil "MC3_NEXT_ID") then { MC3_NEXT_ID = 1 };
MC3_TICK = 0; MC3_LAST = -1;
MC3_MAXCAR = 850;
MC3_CHRONO = [0, 0];
...
MC3_fnc_identifier = {
    ...
    _u setVariable ["chacal_id", MC3_NEXT_ID];
    _u setVariable ["chacal_side", (side _u) call MC3_fnc_side];
    _u setVariable ["chacal_fire", 0];
    (format ["CHACAL|E|spawn|%1|%2|%3|%4|%5", round (time * 100) / 100, MC3_NEXT_ID,
        (side _u) call MC3_fnc_side, typeOf _u,
        (_u getVariable ["chacal_role", ""])]) call MC3_LOG;
    _u addEventHandler ["Fired", {
        ...
        (format ["CHACAL|E|tir|%1|%2|%3|%4", round (time * 100) / 100,
            ...
            MC3_PHASE]) call MC3_LOG;
    }];
    MC3_NEXT_ID = MC3_NEXT_ID + 1;
    MC3_NEXT_ID - 1
};
...
MC3_fnc_identifierVehicule = {
    ...
    _v addEventHandler ["Fired", {
        ...
        (format ["CHACAL|E|tir|%1|%2|%3|%4|bord|%5", round (time * 100) / 100,
            ...
            MC3_PHASE, typeOf _veh]) call MC3_LOG;
    }];
};

MC3_fnc_recenser = {
    { _x call MC3_fnc_identifier } forEach (call MC3_TOUS);
    { if ((count (crew _x) > 0) && { ((crew _x select 0) call MULTI_fnc_cellule) == 3 }) then { _x setVariable ["multi_c", 3]; _x call MC3_fnc_identifierVehicule } } forEach vehicles;
};
call MC3_fnc_recenser;
[] spawn { while { !MC3_FIN } do { call MC3_fnc_recenser; sleep 2 } };
(format ["CHACAL|OK|recenseur|1|au_premier_passage|%1", MC3_NEXT_ID - 1]) call MC3_LOG;
...
MC3_EH_FRAME = addMissionEventHandler ["EachFrame", {
    if (MC3_CAP_VERSION != 1) exitWith {};
    private _t = time;
    if (_t - MC3_LAST < MC3_DT) exitWith {};
    MC3_LAST = _t;
    private _d0 = diag_tickTime;
    MC3_TICK = MC3_TICK + 1;
    private _tr = round (_t * 100) / 100;
    private _tous = call MC3_TOUS;
    private _n = count _tous;
    private _bouts = []; private _cour = "";
    {
        private _u = _x;
        private _id = _u getVariable ["chacal_id", -1];
        if (_id < 0) then { _id = _u call MC3_fnc_identifier };
        ...
        private _e = str [
            ...
            (_u call MC3_fnc_posture),
            (_u call MC3_fnc_comport),
            round (speed _u),
            (_u getVariable ["chacal_fire", 0])
        ];
        if ((_u getVariable ["chacal_fire", 0]) == 1) then { _u setVariable ["chacal_fire", 0] };
        if ((count _cour) + (count _e) + 1 > MC3_MAXCAR) then { _bouts pushBack _cour; _cour = "" };
        _cour = if (_cour == "") then { _e } else { _cour + "," + _e };
    } forEach _tous;
    _bouts pushBack _cour;
    { (format ["CHACAL|S|%1|%2|%3|%4|%5|[%6]", MC3_TICK, _forEachIndex, _tr, _n, MC3_PHASE, _x]) call MC3_LOG; } forEach _bouts;

    MC3_CHRONO set [0, (MC3_CHRONO select 0) + (diag_tickTime - _d0)];
    MC3_CHRONO set [1, (MC3_CHRONO select 1) + 1];
    if ((MC3_CHRONO select 1) % 400 == 0) then {
        (format ["CHACAL|C|cout_ms|%1|ticks|%2|unites|%3|fps|%4",
            round (1000 * (MC3_CHRONO select 0) / (MC3_CHRONO select 1) * 100) / 100,
            (MC3_CHRONO select 1), _n, round diag_fps]) call MC3_LOG;
        MC3_CHRONO = [0, 0];
    };
}];
...
MC3_EH_VUES = addMissionEventHandler ["EachFrame", {
    ...
    call {
        ...
        (format ["CHACAL|VC|%1|%2|est_sur_ouest|%3", _tr, MC3_PHASE, str _cb]) call MC3_LOG;
        (format ["CHACAL|VC|%1|%2|ouest_sur_est|%3", _tr, MC3_PHASE, str _co]) call MC3_LOG;
        ...
        private _fnc = {
            ...
            (format ["CHACAL|VH|%1|%2|%3|%4", _tr, MC3_PHASE, _etiq, str _lignes]) call MC3_LOG;
        };
        ...
        private _vg = [];
        {
            ...
            {
                if ([_u, _x] call MC3_fnc_voit) then {
...
MC3_EH_MORT = addMissionEventHandler ["EntityKilled", {
    ...
    if ((_vic call MULTI_fnc_cellule) != 3) exitWith {};   // MULTI : une mort d une autre cellule ne s ecrit pas ici
    ...
    private _t = _tueur call MC3_fnc_idTueur;
    (format ["CHACAL|E|mort|%1|%2|%3|%4|%5|par|%6", round (time * 100) / 100,
        (_vic getVariable ["chacal_id", -1]),
        (_t select 0), MC3_PHASE,
        (_vic getVariable ["chacal_role", ""]), (_t select 1)]) call MC3_LOG;
}];
"CHACAL|OK|morts|1" call MC3_LOG;
...
(format ["CHACAL|OK|capture|%1|dt|%2|dtv|%3", MC3_CAP_VERSION, MC3_DT, MC3_DTV]) call MC3_LOG;
