...
MC3_gFS = ([west, 3] call MULTI_fnc_groupe);
MC3_FS = [];
...
private _rolesTous = [];
for "_p" from 1 to (round (MC3_EFFECTIF / 10)) do { _rolesTous append _rolesBase };

{
    _x params ["_cls", "_role"];
    private _u = MC3_gFS createUnit [_cls, MC3_LZ getPos [12 call MC3_fnc_al, 360 call MC3_fnc_al], [], 0, "NONE"];
    [_u, _role] call MC3_fnc_kitOuest;
    MC3_FS pushBack _u;
} forEach _rolesTous;if (count MC3_FS < MC3_EFFECTIF) exitWith {
    MC3_ISSUE = "VOID"; MC3_CAUSE = "DETACHEMENT_INCOMPLET";
    (format ["CHACAL|VOID|blufor|%1", count MC3_FS]) call MC3_LOG;
};

MC3_gFS selectLeader (MC3_FS select 0);
MC3_gFS setBehaviour "STEALTH";
MC3_gFS setCombatMode "GREEN";      // ne tire que si on lui tire dessus
MC3_gFS setSpeedMode "LIMITED";
MC3_gFS setFormation "FILE";        // la formation de nuit hors piste
MC3_gFS allowFleeing 0;
...
{ for "_i" from 1 to 3 do { _x addMagazine "DemoCharge_Remote_Mag" }; }
    forEach (MC3_FS select { (_x getVariable ["chacal_role",""]) in ["DEMO_1","DEMO_2"] });
{ _x addMagazine "DemoCharge_Remote_Mag"; }
    forEach (MC3_FS select { (_x getVariable ["chacal_role",""]) in ["CHEF","ADJOINT"] });

MC3_fnc_role = {
    private _r = _this;
    MC3_FS select { alive _x && { (_x getVariable ["chacal_role",""]) in _r } }
};
...
MC3_RECO    = ["TIREUR_1","TIREUR_2"];
MC3_APPUI   = ["TIREUR_1","TIREUR_2"];
MC3_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN"];
MC3_BOUCHON = ["AT","FUSILIER_1","FUSILIER_2"];
...
if (MC3_PARTAGE == 1) then {
    MC3_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN","FUSILIER_1","FUSILIER_2"];
    MC3_BOUCHON = ["AT"];
};

MC3_gAppui = grpNull; MC3_gAssaut = grpNull;
MC3_gBouchon = grpNull; MC3_gReco = grpNull; MC3_gDemo = grpNull;
...
MC3_fnc_detacher = {
    params ["_roles", "_nom"];
    private _u = _roles call MC3_fnc_role;
    if (count _u == 0) exitWith { grpNull };
    private _g = ([west, 3] call MULTI_fnc_groupe);
    _u joinSilent _g;
    _g selectLeader (_u select 0);
    _g setVariable ["chacal_element", _nom, true];
    _g allowFleeing 0;
    (format ["CHACAL|E|scission|%1|%2|%3", round (time * 100) / 100, _nom, count _u]) call MC3_LOG;
    _g
};

if (MC3_IMMORTEL == 1) then {
    { _x allowDamage false } forEach MC3_FS;
    "CHACAL|AVERT|hors_corpus|immortel|1|couture_sous_le_feu" call MC3_LOG;
};

(format ["CHACAL|OK|blufor|%1|roles|%2", count MC3_FS,
    (MC3_FS apply { _x getVariable ["chacal_role", "?"] })]) call MC3_LOG;
