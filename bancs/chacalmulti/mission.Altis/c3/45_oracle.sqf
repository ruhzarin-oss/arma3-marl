...
// ne vaut plus rien.
// =====================================================================
if (MC3_ORACLE_CMD <= 0) exitWith {};

MC3_O_BUDGET = MC3_ORACLE_B;
MC3_O_CIBLE = -1;
...
private _lz = MC3_LZ; private _rt = MC3_ROUTE; private _op = MC3_OP; private _st = MC3_SITE;
private _ra = if (count MC3_RALLY > 1) then { MC3_RALLY } else { _lz };
MC3_O_CASES = [
    ["POSER",   _lz],
...
MC3_O_B = _poids apply { _x / _sp };
MC3_O_PRIOR = +MC3_O_B;   // la doctrine, gardee a part : c est vers elle que revient le doute
// les cases que la patrouille de route peut atteindre, et le troncon de route le plus proche de chacune
MC3_O_RTE = MC3_O_CASES apply {
...
private _carte = [];
{
    ...
    _carte pushBack [(_x select 0), (if (count (MC3_O_RTE select _forEachIndex) > 0) then {1} else {0}), _d2];
} forEach MC3_O_CASES;
(format ["CHACAL|O|carte|%1|cases_routieres|%2|detail|%3", round (time * 100) / 100,
    count (MC3_O_RTE select { count _x > 0 }), str _carte]) call MC3_LOG;
...
MC3_O_fnc_yeux = {
    ...
    { if (!isNull _x) then { { if (alive _x) then { _y pushBack _x } } forEach (units _x) } } forEach
        ((MC3_GROUPES_EST + [MC3_gRoute]) select { !isNull _x });
    { if (alive _x && { !(_x in _y) }) then { _y pushBack _x } } forEach MC3_EST_SITE;
    _y
};

MC3_O_fnc_caseLaPlusProche = {
    params ["_p"];
    private _d = MC3_O_CASES apply { (_x select 1) distance2D _p };
...
MC3_O_fnc_croire = {
    ...
    for "_i" from 0 to (_n - 1) do {
        private _v = 0.70 * (MC3_O_B select _i);
        if (_i > 0) then { _v = _v + (0.30 * (MC3_O_B select (_i - 1))) } else { _v = _v + (0.30 * (MC3_O_B select _i)) };
        _bp pushBack _v;
    };
        ...
        if (!isNull _chef && { alive _chef }) then {
            {
                ...
                if (_kn select 0) then {
                    ...
                    if (_age < _fraicheur) then { _fraicheur = _age; _vu = [(_kn select 6)] call MC3_O_fnc_caseLaPlusProche };
                };
            } forEach MC3_FS;                                  // on parcourt la liste, on ne lit QUE sa connaissance
        };
    } forEach ((MC3_GROUPES_EST + [MC3_gRoute]) select { !isNull _x });
    // 3. ce qu il n a pas vu : une case regardee et vide devient moins probable
    private _yeux = call MC3_O_fnc_yeux;
    private _l = [];
    for "_i" from 0 to (_n - 1) do {
        private _p = (MC3_O_CASES select _i) select 1;
        private _q = 1;
        { _q = _q * (1 - ([_x distance2D _p] call MC3_O_fnc_pd)) } forEach _yeux;
        _l pushBack _q;
    };
    ...
    for "_i" from 0 to (_n - 1) do { _bp set [_i, ((1 - _nu) * ((_bp select _i) / _s)) + (_nu * (MC3_O_PRIOR select _i))] };
    MC3_O_B = _bp;
    _vu
};
...
MC3_O_fnc_agir = {
    params ["_cible"];
    private _p = (MC3_O_CASES select _cible) select 1;
    ...
    private _bR = MC3_O_B apply { 0 };
    { if (count (MC3_O_RTE select _forEachIndex) > 0) then { _bR set [_forEachIndex, _x] } } forEach MC3_O_B;
    private _mxR = selectMax _bR;
    private _candR = []; { if ((_x > 0) && { _x >= (_mxR - 0.01) }) then { _candR pushBack _forEachIndex } } forEach _bR;
    if (MC3_O_BUDGET > 0 && { count _candR > 0 } && { !isNull MC3_gRoute }
        && { ({ alive _x } count (units MC3_gRoute)) > 0 }) then {
        private _cibleR = if (MC3_O_CIBLE in _candR) then { MC3_O_CIBLE } else { selectRandom _candR };
        if (_cibleR != MC3_O_CIBLE) then {
            private _a = MC3_O_RTE select _cibleR;
            private _r2 = (_a nearRoads 350) select { (_x distance2D _a) > 250 };
            private _b2 = if (count _r2 > 0) then { getPosATL (_r2 select 0) } else { _a };
            while { (count (waypoints MC3_gRoute)) > 0 } do { deleteWaypoint ((waypoints MC3_gRoute) select 0) };
            private _w = MC3_gRoute addWaypoint [_a, 15];  _w setWaypointType "MOVE";  _w setWaypointSpeed "NORMAL";
            private _w2 = MC3_gRoute addWaypoint [_b2, 15]; _w2 setWaypointType "MOVE"; _w2 setWaypointSpeed "LIMITED";
            (MC3_gRoute addWaypoint [_a, 15]) setWaypointType "CYCLE";
            ...
            MC3_gRoute setCurrentWaypoint _w;
            MC3_O_BUDGET = MC3_O_BUDGET - 1;
            MC3_O_CIBLE = _cibleR;
            _action = format ["PATROUILLE_%1", (MC3_O_CASES select _cibleR) select 0];
        };
    };
    _action
};
...
[] spawn {
    ...
    while { !MC3_FIN } do {
        ...
        if ((random 1) < (MC3_ORACLE_EPS / 100)) then {
            private _c = +_b; _c set [_cible, -1];
            _cible = _c find (selectMax _c);
        };
        private _action = [_cible] call MC3_O_fnc_agir;
        private _dPat = if ((MC3_O_CIBLE >= 0) && { !isNull MC3_gRoute } && { !isNull (leader MC3_gRoute) }) then {
            round ((leader MC3_gRoute) distance2D (MC3_O_RTE select MC3_O_CIBLE)) } else { -1 };
        (format ["CHACAL|O|decision|%1|action|%2|case|%3|p|%4|vu|%5|budget|%6|croyance|%7|cible_patrouille|%8|patrouille_a|%9|vitesse_patrouille|%10",
            round (time * 100) / 100, _action, (MC3_O_CASES select _cible) select 0,
            round ((_b select _cible) * 100) / 100, (if (_vu >= 0) then { (MC3_O_CASES select _vu) select 0 } else { "RIEN" }),
            MC3_O_BUDGET, str (_b apply { round (_x * 100) }),
            (if (MC3_O_CIBLE >= 0) then { (MC3_O_CASES select MC3_O_CIBLE) select 0 } else { "AUCUNE" }), _dPat,
            (if (!isNull MC3_VEH_ROUTE) then { round (speed MC3_VEH_ROUTE) } else { -1 })]) call MC3_LOG;
        sleep (MC3_ORACLE_DELTA * MC3_ECHELLE);
    };
};
