...
MC8_gFS = ([west, 8] call MULTI_fnc_groupe);
MC8_FS = [];
...
private _rolesTous = [];
for "_p" from 1 to (round (MC8_EFFECTIF / 10)) do { _rolesTous append _rolesBase };

{
    _x params ["_cls", "_role"];
    private _u = MC8_gFS createUnit [_cls, MC8_LZ getPos [12 call MC8_fnc_al, 360 call MC8_fnc_al], [], 0, "NONE"];
    [_u, _role] call MC8_fnc_kitOuest;
    MC8_FS pushBack _u;
} forEach _rolesTous;if (count MC8_FS < MC8_EFFECTIF) exitWith {
    MC8_ISSUE = "VOID"; MC8_CAUSE = "DETACHEMENT_INCOMPLET";
    (format ["CHACAL|VOID|blufor|%1", count MC8_FS]) call MC8_LOG;
};

MC8_gFS selectLeader (MC8_FS select 0);
MC8_gFS setBehaviour "STEALTH";
MC8_gFS setCombatMode "GREEN";      // ne tire que si on lui tire dessus
MC8_gFS setSpeedMode "LIMITED";
MC8_gFS setFormation "FILE";        // la formation de nuit hors piste
MC8_gFS allowFleeing 0;
...
{ for "_i" from 1 to 3 do { _x addMagazine "DemoCharge_Remote_Mag" }; }
    forEach (MC8_FS select { (_x getVariable ["chacal_role",""]) in ["DEMO_1","DEMO_2"] });
{ _x addMagazine "DemoCharge_Remote_Mag"; }
    forEach (MC8_FS select { (_x getVariable ["chacal_role",""]) in ["CHEF","ADJOINT"] });

MC8_fnc_role = {
    private _r = _this;
    MC8_FS select { alive _x && { (_x getVariable ["chacal_role",""]) in _r } }
};
...
MC8_RECO    = ["TIREUR_1","TIREUR_2"];
MC8_APPUI   = ["TIREUR_1","TIREUR_2"];
MC8_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN"];
MC8_BOUCHON = ["AT","FUSILIER_1","FUSILIER_2"];
...
if (MC8_PARTAGE == 1) then {
    MC8_ASSAUT  = ["CHEF","ADJOINT","DEMO_1","DEMO_2","MEDECIN","FUSILIER_1","FUSILIER_2"];
    MC8_BOUCHON = ["AT"];
};

MC8_gAppui = grpNull; MC8_gAssaut = grpNull;
MC8_gBouchon = grpNull; MC8_gReco = grpNull; MC8_gDemo = grpNull;
...
MC8_fnc_detacher = {
    params ["_roles", "_nom"];
    private _u = _roles call MC8_fnc_role;
    if (count _u == 0) exitWith { grpNull };
    private _g = ([west, 8] call MULTI_fnc_groupe);
    _u joinSilent _g;
    _g selectLeader (_u select 0);
    _g setVariable ["chacal_element", _nom, true];
    _g allowFleeing 0;
    (format ["CHACAL|E|scission|%1|%2|%3", round (time * 100) / 100, _nom, count _u]) call MC8_LOG;
    _g
};

if (MC8_IMMORTEL == 1) then {
    { _x allowDamage false } forEach MC8_FS;
    "CHACAL|AVERT|hors_corpus|immortel|1|couture_sous_le_feu" call MC8_LOG;
};

(format ["CHACAL|OK|blufor|%1|roles|%2", count MC8_FS,
    (MC8_FS apply { _x getVariable ["chacal_role", "?"] })]) call MC8_LOG;
