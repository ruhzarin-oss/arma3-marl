...
MC8_CAUSE_ABANDON = "";
MC8_INTEL = false;
MC8_CHARGES = [];              // ACTES : objectifs sur lesquels une charge a ete posee
MC8_DETRUITS = [];             // EFFET SCRIPTE : hors jugement tactique
MC8_VUES = [];                 // [unite, k] reellement observees par la reco
MC8_INSERTION_FORCEE = false;
MC8_QRF_PARTIE = false;
MC8_OUV_CHOISIE = [];
...
MC8_CHARGES_OBJ = [];          // [objectif, charge] : ce qui reste a faire sauter
MC8_RESERVE_FEU = 180;         // s gardees en fin de phase : un degagement, une mise a feu
MC8_SECURITE_FEU = 35;         // m : personne a moins de ca d une charge quand elle part
MC8_EXPLOITANT = objNull;
// ! MARQUEUR-SEUIL-RENS-DERIVE - SEUIL DERIVE DE LA MESURE, PAS CHOISI ( 15/09 ).
// Distribution de count MC8_VUES en fin de phase 3, sur les 148 episodes des journaux qui
...
// ! CE CORRECTIF NE FAIT PAS JOUER LA PHASE 3. Elle ne tourne que si MC8_DEPART < 4 et
// MC8_OBS = 1 ; c est un reglage de job, pas de code.
MC8_SEUIL_RENS = 1;            // sous ce compte, la crete n a rien vu
MC8_T_ALARME = -1; MC8_T_COMPROMIS = -1;
MC8_EXFIL_POINT = [];
MC8_GEL = false;

MC8_fnc_debutPhase = {
    params ["_n", "_nom", ["_plafond", 1e9]];
    // ! VIGNETTE : une phase posterieure a MC8_ARRET n ouvre rien et n ecrit
    // rien. Sinon le RPT porterait des phases jamais jouees et `phase_max` mentirait.
    if (MC8_FIN) exitWith {};
    MC8_PHASE = _n; MC8_PHASE_NOM = _nom;
    MC8_TPHASE = time; MC8_PLAFOND_COURANT = _plafond;
    (format ["CHACAL|PH|%1|%2|debut|%3|vivants|%4|compromis|%5|alarme|%6", _n, _nom, round (time * 100) / 100,
        count (MC8_FS select { alive _x }),
        (if (MC8_COMPROMIS) then {1} else {0}), (if (MC8_ALARME) then {1} else {0})]) call MC8_LOG;
};
...
"CHACAL|OK|decision|version|3" call MC8_LOG;   // 3 : perceptions de la menace en fin de ligne ( plans/plan-menace-visible.md )
// ! LA MENACE VISIBLE ( 17/09 ). Deux canaux : ce qu un homme du detachement VOIT maintenant ( MC8_fnc_voit, geometrie ) et
...
MC8_fnc_unitesMenace = {
    private _t = [];
    { private _g = _x select 2; if (!isNull _g) then { _t append ((units _g) select { alive _x }) } } forEach MC8_MENACES;
    _t
};
MC8_fnc_chefsDetachement = {
    private _g = [];
    { if (alive _x) then { _g pushBackUnique (group _x) } } forEach MC8_FS;
    (_g select { !isNull _x && { alive (leader _x) } }) apply { leader _x }
};
MC8_fnc_suiviMenaces = {
    while { !MC8_FIN } do {
        private _chefs = call MC8_fnc_chefsDetachement;
        private _hommesS = MC8_FS select { alive _x };
        {
            ...
            if (({ [_x, _t, 800, 70] call MC8_fnc_voit } count _hommesS) > 0) then {
        ...
        } forEach (call MC8_fnc_unitesMenace);
        sleep 5;
    };
};
[] spawn MC8_fnc_suiviMenaces;
MC8_fnc_perceptionMenace = {
    ...
    private _menaces = call MC8_fnc_unitesMenace;
    private _hommes = MC8_FS select { alive _x };
    private _chefs = call MC8_fnc_chefsDetachement;
    ...
    {
        private _t = _x;
        private _vu = ({ [_x, _t, 800, 70] call MC8_fnc_voit } count _hommes) > 0;
    ...
    private _veh = 0;
    if ((!isNil "MC8_VEH_ROUTE") && { !isNull MC8_VEH_ROUTE } && { ({ (_x select 1) == "PATROUILLE_ROUTE" } count MC8_MENACES) > 0 }) then {
        if (({ (_x targetKnowledge MC8_VEH_ROUTE) select 0 } count _chefs) > 0) then { _veh = 1 };
    };
    ...
    private _moteur = 0; private _dMoteur = -1; private _moteurAllume = 0; private _vueVeh = 0;
    {
        ...
        if ((_v != _x) && { isEngineOn _v }) then {
            ...
            if ((_dMoteur >= 0) && { _dMoteur < MC8_PORTEE_SON }) then { _moteur = 1; MC8_MOTEUR_FENETRE = 1 };
            if (({ [_x, _v, 1200, 70] call MC8_fnc_voit } count _hommes) > 0) then { _vueVeh = 1 };
        };
    } forEach _menaces;
    private _vDist = -1;
    ...
    format ["|menace_percue|%1|menaces_vues|%2|menaces_connues|%3|menaces_camp|%4|menaces_homme|%5|distance_menace|%6|erreur_position|%7|menace_mobile|%8|vue_depuis|%9|vehicule_connu|%10|menace_mobile_vue|%11|moteur_entendu|%12|distance_moteur|%13|moteur_allume|%14|vue_vehicule|%15|n_vues_menace|%16|moteur_depuis_fenetre|%17|verite_menaces|%18|verite_distance_menace|%19|azimut_chef|%20",
        ...
        (missionNamespace getVariable ["MC8_MOTEUR_FENETRE", 0]),
        count _menaces, round _vDist, (if (count _chefs > 0) then { round (getDir (_chefs select 0)) } else { -1 })]
};
MC8_fnc_secteurPhase = {
    params ["_phase"];
    switch (_phase) do {
        case 1: { if (isNil "MC8_LZ") then { [] } else { MC8_LZ } };
        case 2: { if (isNil "MC8_ROUTE") then { [] } else { MC8_ROUTE } };
        case 4: { if (isNil "MC8_SITE") then { [] } else { MC8_SITE } };
        default { [] };
    };
};
MC8_fnc_fenetreObservation = {
    params ["_phase"];
    if (MC8_OBSERVATION <= 0) exitWith {};
    private _t0 = time;
    private _hommes = MC8_FS select { alive _x };
    private _secteur = [_phase] call MC8_fnc_secteurPhase;
    ...
    if ((MC8_CONTROLE_PERCEPTION in [1, 5]) && { count _hommes > 0 }) then {
        ...
        {
            private _p = _chef getPos [MC8_CONTROLE_DIST, _x];
        ...
        if (!_trouve) exitWith {
            ...
            (format ["CHACAL|E|banc_refuse|%1|phase|%2|distance|%3|cause|AUCUNE_LIGNE_DE_VUE", round (time * 100) / 100,
                _phase, MC8_CONTROLE_DIST]) call MC8_LOG;
        };
        private _p = _chef getPos [MC8_CONTROLE_DIST, _az];
        ...
        MC8_MENACES pushBack [_phase, "BANC_PERCEPTION", _g];
        if (MC8_CONTROLE_PERCEPTION == 5) then {
        ...
        (format ["CHACAL|E|banc_perception|%1|phase|%2|distance|%3|azimut|%4|ligne_de_vue|%5|heure|%6|lune|%7|jumelles|%8|hommes|%9",
            round (time * 100) / 100, _phase, MC8_CONTROLE_DIST, round _az, (if (_trouve) then {1} else {0}),
            (date select 3) + ((date select 4) / 60), moonIntensity,
            ((_hommes apply { hmd _x }) joinString ","), count _hommes]) call MC8_LOG;
    };
    if ((MC8_CONTROLE_PERCEPTION in [2, 3]) && { count _hommes > 0 }) then {
        private _chef = leader (group (_hommes select 0));
        private _p = if (MC8_CONTROLE_PERCEPTION == 2) then { _chef getPos [1500, (getDir _chef) + 180] } else { _chef getPos [150, getDir _chef] };
        private _g = ([east, 8] call MULTI_fnc_groupe);
        {
            private _u = _g createUnit [_x, _p, [], 5, "NONE"];
            _u disableAI "AUTOTARGET"; _u disableAI "TARGET"; _u disableAI "MOVE";
            _u setUnitPos (if (MC8_CONTROLE_PERCEPTION == 3) then {"UP"} else {"MIDDLE"});
        } forEach ["O_Soldier_TL_F", "O_Soldier_F", "O_Soldier_F"];
        _g setCombatMode "BLUE";
        MC8_MENACES pushBack [_phase, "CONTROLE_PERCEPTION", _g];
        (format ["CHACAL|E|controle_perception|%1|phase|%2|mode|%3|distance|%4|posture|%5", round (time * 100) / 100, _phase,
            MC8_CONTROLE_PERCEPTION, round (_chef distance2D _p), (if (MC8_CONTROLE_PERCEPTION == 3) then {"UP"} else {"MIDDLE"})]) call MC8_LOG;
    };
    (format ["CHACAL|E|observation|%1|phase|%2|debut|duree_prevue|%3%4", round (time * 100) / 100, _phase, MC8_OBSERVATION,
        call MC8_fnc_perceptionMenace]) call MC8_LOG;
    MC8_MOTEUR_FENETRE = 0;   // la memoire du son commence avec la fenetre
    if (MC8_SONDE_PERCEPTION == 1) then {
        [_phase] spawn {
            params ["_ph"];
            private _t0 = time;
            while { !MC8_FIN && { (time - _t0) < (600 * MC8_ECHELLE) } } do {
                (format ["CHACAL|E|sonde_decision|%1|phase|%2|depuis|%3%4", round (time * 100) / 100, _ph,
                    round (time - _t0), call MC8_fnc_perceptionMenace]) call MC8_LOG;
                sleep (10 * MC8_ECHELLE);
            };
        };
    };
    (format ["CHACAL|E|reglage_observation|%1|phase|%2|avant|%3|balayage|%4|distance_secteur|%5|azimuts|%6", round (time * 100) / 100, _phase,
        MC8_AVANT, MC8_BALAYAGE, (if (count _secteur > 0) then { round (((_hommes select 0) distance2D _secteur)) } else { -1 }), count _regards]) call MC8_LOG;
    ...
    waitUntil {
        ...
        if ((count _regards > 0) && { time >= _tRegard }) then {
            ...
            // ! levier MC8_BALAYAGE : 1 = chaque changement d azimut TOURNE les hommes ( setDir ) avant le doWatch ; 0 = origine
            {
                if (MC8_BALAYAGE == 1) then { _x setDir (_x getDir (_regards select _iRegard)) };
                _x doWatch (_regards select _iRegard);
            } forEach (MC8_FS select { alive _x });
        };
        if ((MC8_SONDE > 0) && { time >= _prochain }) then {
            _prochain = time + 5;
            private _angles = [];
            {
                private _t = _x;
                private _a = 999;
                { private _r = abs ((((_x getDir _t) - (getDir _x) + 540) % 360) - 180); if (_r < _a) then { _a = _r } } forEach (MC8_FS select { alive _x });
        ...
        MC8_FIN || ((time - _t0) >= (MC8_OBSERVATION * MC8_ECHELLE))
    };
...
MC8_fnc_decision = {
    ...
    private _def = (if (isNil "MC8_EST_SITE") then {[]} else {MC8_EST_SITE}) select { alive _x };
    (format ["CHACAL|E|decision|%1|phase|%2|point|%3|options|%4|choix|%5|decideur|%6|alarme|%7|depuis_alarme|%8|compromis|%9|vivants|%10|defenseurs_connus|%11|verite_defenseurs|%12|situation|%13%14",
        ...
        (if (MC8_ALARME) then {1} else {0}),
        (if (MC8_T_ALARME >= 0) then { round (time - MC8_T_ALARME) } else { -1 }),
        (if (MC8_COMPROMIS) then {1} else {0}),
        count (MC8_FS select { alive _x }),
        { (west knowsAbout _x) > 1.4 } count _def,
        count _def, MC8_SITUATION, _extra + (call MC8_fnc_perceptionMenace)]) call MC8_LOG;
};
// Interprete des formules EvoGP ( codes prefixes ). ">" vaut +1 si a > b, sinon -1, comme dans EvoGP ( mesure le 17/09 ).
MC8_F_CONST = [-1, -0.5, -0.25, 0, 0.25, 0.5, 1];
MC8_fnc_evalNoeud = {
    params ["_i", "_obs"];
    private _c = MC8_F select _i;
    if (_c >= 301) exitWith { [MC8_F_CONST select (_c - 301), _i + 1] };
    if (_c >= 201) exitWith { [_obs select (_c - 201), _i + 1] };
    if (_c == 106) exitWith { private _m = [_i + 1, _obs] call MC8_fnc_evalNoeud; [-(_m select 0), _m select 1] };
    private _g = [_i + 1, _obs] call MC8_fnc_evalNoeud;
    private _d = [_g select 1, _obs] call MC8_fnc_evalNoeud;
    ...
    private _r = switch (_c) do {
        ...
        default { (format ["CHACAL|ERREUR|formule|code_inconnu|%1", _c]) call MC8_LOG; 0 };
    };
    [_r, _d select 1]
};
MC8_fnc_finPhase = {
    params ["_n", "_nom", "_issue"];
    if (MC8_FIN && { _n > MC8_ARRET }) exitWith {};
    (format ["CHACAL|PH|%1|%2|fin|%3|%4|vivants|%5|compromis|%6|alarme|%7", _n, _nom, round (time * 100) / 100, _issue,
        count (MC8_FS select { alive _x }),
        (if (MC8_COMPROMIS) then {1} else {0}), (if (MC8_ALARME) then {1} else {0})]) call MC8_LOG;
    // ! LA VIGNETTE SE FERME ICI, et par le VERDICT : 70_verdict attend MC8_FIN,
    ...
    if (_n >= MC8_ARRET) then {
        (format ["CHACAL|OK|vignette|arret|%1|issue|%2", _n, _issue]) call MC8_LOG;
        MC8_FIN = true;
    };
};
...
MC8_fnc_ordreAller = {
    ...
    // On libere la posture ET le regard : `doWatch MC8_SITE` pose sur l appui
    ...
    (format ["CHACAL|E|ordre|%1|%2|%3|%4|pourquoi|%5", round (time * 100) / 100,
        (_g getVariable ["chacal_element", "DETACHEMENT"]), _p, MC8_PHASE, _pourquoi]) call MC8_LOG;
};
...
MC8_fnc_arrive = {
    ...
    private _c2 = _v2 call MC8_fnc_centre;
...
MC8_fnc_rejoindre = {
    ...
    if (isNull _g) exitWith { "ABSENT" };
    [_g, _p, "NORMAL", "AWARE", "WEDGE", _nom + "_JAMBE_RAPIDE"] call MC8_fnc_ordreAller;
            ...
            if (count _c > 0) then {
                ...
                if (time - _tLog >= 30) then {
                    ...
                    (format ["CHACAL|E|progression|%1|%2|reste|%3|vitesse|%4|jambe|%5|hommes|%6",
                        ...
                        (if (_lente) then {"DISCRETE"} else {"RAPIDE"}), count _v]) call MC8_LOG;
                };
                ...
                if ((_vm / (count _v)) < 0.5) then {
                    ...
                    if (_immobile == 8) then {
                        (format ["CHACAL|E|deblocage|%1|%2|essai|RELEVER|reste|%3", round (time * 100) / 100, _nom, round _d]) call MC8_LOG;
                        [_g, _p, "NORMAL", "AWARE", "WEDGE", _nom + "_DEBLOCAGE_RELEVER"] call MC8_fnc_ordreAller;
                    ...
                    if (_immobile == 20) then {
                        private _neuf = [_p getPos [70, 60 + (240 call MC8_fnc_al)], 50] call MC8_fnc_plat;
                        (format ["CHACAL|E|deblocage|%1|%2|essai|CONTOURNER|vers|%3", round (time * 100) / 100, _nom, _neuf]) call MC8_LOG;
                        _p = _neuf;
                        [_g, _p, "NORMAL", "AWARE", "WEDGE", _nom + "_DEBLOCAGE_CONTOURNER"] call MC8_fnc_ordreAller;
                    ...
                    if (_immobile == 5) then {
                        ...
                        (format ["CHACAL|E|blocage|%1|%2|reste|%3|commande|%4|pret|%5|waypoints|%6|courant|%7|eau|%8|posture|%9|comport|%10|obstacles|%11|pente|%12",
                            ...
                            round (atan (((getTerrainHeightASL ((getPosATL _l) getPos [10, (getPosATL _l) getDir _p])) - (getTerrainHeightASL (getPosATL _l))) / 10))]) call MC8_LOG;
                    };
                } else { _immobile = 0 };
                ...
                // 800 m pendant 2000 s. On la journalise pour savoir si MC8_ACCESSIBLE la
                ...
                if (unitReady _l2 && { (currentCommand _l2) == "" } && { (currentWaypoint _g) >= (count (waypoints _g)) } && { _d > _ray }) then {
                    if (_immobile == 5) then {
                        (format ["CHACAL|E|moteur_dit_fini|%1|%2|reste|%3|rayon|%4|pente_trajet|%5",
                            ...
                            round ([getPosATL _l2, _p] call MC8_fnc_penteTrajet)]) call MC8_LOG;
                    };
                };
                if (_d < _ray) then { _fini = true; _res = "ATTEINT" }
                    ...
                    if (!_lente && { _d < 300 }) then {
                        _lente = true;
                        [_g, _p, "LIMITED", "STEALTH", "WEDGE", _nom + "_JAMBE_DISCRETE"] call MC8_fnc_ordreAller;
                    };
                    // une fois degage de quarante metres, on se recouche
                    if (_releve && { _d < _dReleve - 40 } && { _d < 300 }) then {
                        _releve = false;
                        [_g, _p, "LIMITED", "STEALTH", "WEDGE", _nom + "_RETOUR_JAMBE_DISCRETE"] call MC8_fnc_ordreAller;
                    };
                    if (_d < _dRef - 25) then { _dRef = _d; _tRef = time };
    ...
    if (_res == "") then { _res = if (MC8_SAUT) then {"COMPROMIS"} else {"PLAFOND"} };
    _res
};
...
MC8_fnc_rejoindreItineraire = {
    params ["_g", "_p", "_ray", "_plafond", "_nom"];
    if (isNull _g) exitWith { "ABSENT" };
    if (MC8_ITINERAIRE == 2) then {
        private _v = (units _g) select { alive _x };
        if (count _v > 0) then {
            private _c = _v call MC8_fnc_centre;
            ...
            _wp set [2, 0];
            private _t0 = time;
            [_g, _wp, "NORMAL", "AWARE", "WEDGE", _nom + "_DETOUR"] call MC8_fnc_ordreAller;
            private _rD = [_g, _wp, 80, ([_g, _wp] call MC8_fnc_budgetDeuxJambes)] call MC8_fnc_arrive;
            (format ["CHACAL|E|choix_joue|%1|point|ITINERAIRE|choix|2|detail|DETOUR|element|%2|cote|%3|point_detour|%4|issue|%5|duree|%6",
                round (time * 100) / 100, _nom, _cote, _wp, _rD, round (time - _t0)]) call MC8_LOG;
            _plafond = _plafond - (time - _t0);
        };
    } else {
        if (MC8_ITINERAIRE == 1) then {
            (format ["CHACAL|E|choix_joue|%1|point|ITINERAIRE|choix|1|detail|DIRECT|element|%2", round (time * 100) / 100, _nom]) call MC8_LOG;
        };
    };
    [_g, _p, _ray, (_plafond max 60), _nom] call MC8_fnc_rejoindre
};
...
MC8_fnc_pointDePose = {
    ...
    private _r = (_o call MC8_fnc_emprise) + 4;
    private _az0 = if (_o isEqualTo MC8_PC) then { _pO getDir MC8_OUV_CHOISIE } else { _pO getDir MC8_SITE };
    private _best = _pO getPos [_r, _az0]; private _sc = -1;
    {
        private _p = _pO getPos [_r, _az0 + _x]; _p set [2, 0];
        if ((_p distance2D MC8_SITE) < (MC8_RAYON - 5)) then {
            private _dmin = 8;
...
MC8_fnc_exploiter = {
    private _l = leader MC8_gAssaut;
    private _cand = (units MC8_gAssaut) select { alive _x && { _x != _l } };
    ...
    MC8_EXPLOITANT = _cand select 0;
    private _h = MC8_EXPLOITANT;
    (format ["CHACAL|E|ordre|%1|EXPLOITATION|%2|%3|pourquoi|OPORD_TERMINAL|par|%4", round (time * 100) / 100,
        getPosATL MC8_PORTABLE, MC8_PHASE, (_h getVariable ["chacal_role", ""])]) call MC8_LOG;
    _h doMove (getPosATL MC8_PORTABLE);
    private _t = time; private _dedans = 0; private _tOrdre = time;
    while { !MC8_INTEL && { alive _h } && (time - _t < 100 * MC8_ECHELLE) && !MC8_FIN } do {
        if ((_h distance2D MC8_PORTABLE) < 4) then { _dedans = _dedans + 2 } else {
            _dedans = 0;
            if (time - _tOrdre > 20) then { _h doMove (getPosATL MC8_PORTABLE); _tOrdre = time };
        };
        if (_dedans >= (60 * MC8_ECHELLE)) then {
            MC8_INTEL = true;
            (format ["CHACAL|E|exploitation|%1|reussie|par|%2", round (time * 100) / 100, (_h getVariable ["chacal_role", ""])]) call MC8_LOG;
        };
        sleep 2;
    };
    if (!MC8_INTEL) then {
        (format ["CHACAL|E|exploitation|%1|manquee|distance|%2", round (time * 100) / 100, round (_h distance2D MC8_PORTABLE)]) call MC8_LOG;
    };
    if (alive _h) then { _h doFollow (leader MC8_gAssaut) };
};
...
MC8_fnc_miseAFeu = {
    params [["_degagement", "-"]];
    if (count MC8_CHARGES_OBJ == 0) exitWith {
        (format ["CHACAL|E|mise_a_feu|%1|aucune_charge|degagement|%2", round (time * 100) / 100, _degagement]) call MC8_LOG;
    };
    private _fnProches = {
        private _n = 0;
        {
            private _c = _x select 1;
            if (!isNull _c) then { _n = _n + ({ alive _x && { (_x distance2D _c) < MC8_SECURITE_FEU } } count MC8_FS) };
        } forEach MC8_CHARGES_OBJ;
        _n
    };
    private _t = time; private _proches = call _fnProches;
    while { _proches > 0 && (time - _t < 90 * MC8_ECHELLE) && !MC8_FIN } do { sleep 3; _proches = call _fnProches };
    if (_proches > 0) exitWith {
        (format ["CHACAL|E|mise_a_feu|%1|REFUSEE|hommes_a_portee|%2|attente|%3|degagement|%4", round (time * 100) / 100,
            _proches, round (time - _t), _degagement]) call MC8_LOG;
    };
    (format ["CHACAL|E|mise_a_feu|%1|charges|%2|attente|%3|degagement|%4", round (time * 100) / 100,
        count MC8_CHARGES_OBJ, round (time - _t), _degagement]) call MC8_LOG;
    { private _c = _x select 1; if (!isNull _c) then { _c setDamage 1 } } forEach MC8_CHARGES_OBJ;
    sleep 2;
    {
        private _o = _x select 0;
        if (alive _o) then { _o setDamage 1 };
        MC8_DETRUITS pushBackUnique _o;
        (format ["CHACAL|E|destruction|%1|%2|effet_scripte|total|%3", round (time * 100) / 100, typeOf _o, count MC8_DETRUITS]) call MC8_LOG;
    } forEach MC8_CHARGES_OBJ;
};
...
// =====================================================================
MC8_fnc_compromettre = {
    params ["_cause"];
    if (MC8_COMPROMIS) exitWith {};
    MC8_COMPROMIS = true; MC8_T_COMPROMIS = time;
    (format ["CHACAL|E|compromis|%1|cause|%2|phase|%3|latence_oracle|%4", round (time * 100) / 100,
        _cause, MC8_PHASE,
        (if (MC8_T_ALARME < 0) then {-1} else { round ((time - MC8_T_ALARME) * 100) / 100 })]) call MC8_LOG;
    if (MC8_PHASE < 5) then { MC8_SAUT = (MC8_TENIR == 0) };
    ...
    private _bl = if (MC8_PHASE < 5) then { MC8_FS select { alive _x } } else { [] };
    if (count _bl > 0) then {
        private _c = _bl call MC8_fnc_centre;
        private _menace = [];
        {
            private _e = _x;
            if ({ [_x, _e, 900] call MC8_fnc_voit } count _bl > 0) exitWith { _menace = getPosATL _e };
        } forEach ((allUnits + vehicles) select {
            alive _x && { side (if (_x isKindOf "CAManBase") then {_x} else {effectiveCommander _x}) == east } });
        private _az = if (count _menace > 0) then { _menace getDir _c } else { MC8_SITE getDir _c };
        private _fuite = _c getPos [400, _az];
        {
            if (!isNull _x) then {
                _x setBehaviour "COMBAT"; _x setCombatMode "YELLOW"; _x setSpeedMode "FULL";
                [_x, _fuite, "FULL", "COMBAT", "WEDGE", "RUPTURE_CONTACT"] call MC8_fnc_ordreAller;
            };
        } forEach ([MC8_gFS, MC8_gReco, MC8_gAppui, MC8_gAssaut, MC8_gBouchon] select { !isNull _x });
        (format ["CHACAL|E|rupture|%1|vers|%2|menace_percue|%3", round (time * 100) / 100,
            _fuite, (if (count _menace > 0) then {1} else {0})]) call MC8_LOG;
    };
};
...
// les trois sens du detachement
{
    _x addEventHandler ["FiredNear", {
        ...
        if (_dist < 45 && { side _tireur != west }) then { ["FEU_PROCHE"] call MC8_fnc_compromettre };
    }];
    _x addEventHandler ["Hit", {
        params ["_u", "_source"];
        if (!isNull _source && { side _source != west }) then { ["COUP_RECU"] call MC8_fnc_compromettre };
    }];
} forEach MC8_FS;

// et le troisieme : un rouge que NOUS voyons, et qui vient de passer en combat
[] spawn {
    while { !MC8_COMPROMIS && !MC8_FIN } do {
        private _bl = MC8_FS select { alive _x };
        if (count _bl > 0) then {
            {
                private _e = _x;
                if ({ [_x, _e, 700] call MC8_fnc_voit } count _bl > 0) exitWith { ["ENNEMI_VU_EN_COMBAT"] call MC8_fnc_compromettre };
            } forEach (MC8_EST_SITE select { alive _x && { behaviour _x == "COMBAT" } });
        };
        sleep 2;
    };
};
...
[] spawn {
    ...
    while { !MC8_FIN && MC8_PHASE < 4 } do {
        ...
        if (count _bl > 0) then {
            ...
            private _vu = false;
            { private _e = _x; if ({ [_x, _e, 500] call MC8_fnc_voit } count _bl > 0) exitWith { _vu = true } } forEach _rouges;
            if (_vu && !MC8_GEL && !MC8_SAUT) then {
                MC8_GEL = true;
                { doStop _x; _x setUnitPos "DOWN" } forEach _bl;
                (format ["CHACAL|E|reflexe|%1|gel|phase|%2", round (time * 100) / 100, MC8_PHASE]) call MC8_LOG;
                private _t = time;
                while { MC8_GEL && !MC8_FIN && !MC8_SAUT } do {
                    sleep 3;
                    private _b2 = MC8_FS select { alive _x };
                    private _encore = false;
                    { private _e = _x; if ({ [_x, _e, 500] call MC8_fnc_voit } count _b2 > 0) exitWith { _encore = true } } forEach _rouges;
                    if (_encore) then { _t = time };
                    if (time - _t > 30) then { MC8_GEL = false };
                };
                { if (alive _x) then { _x setUnitPos "AUTO"; _x doFollow (leader (group _x)) } } forEach (MC8_FS select { alive _x });
                (format ["CHACAL|E|reflexe|%1|degel|phase|%2", round (time * 100) / 100, MC8_PHASE]) call MC8_LOG;
            };
        };
        sleep 3;
    };
...
[] spawn {
    while { !MC8_ALARME && !MC8_FIN } do {
        private _m = 0;
        { private _k = east knowsAbout _x; if (_k > _m) then { _m = _k } } forEach (MC8_FS select { alive _x });
        if (_m > 1.4) then { MC8_ALARME = true; MC8_T_ALARME = time };
        sleep 1;
    };
    if (MC8_FIN) exitWith {};
    (format ["CHACAL|E|oracle_alarme|%1|phase|%2", round (time * 100) / 100, MC8_PHASE]) call MC8_LOG;
    { if (!isNull _x) then { _x setBehaviour "COMBAT"; _x setCombatMode "RED"; _x setSpeedMode "FULL" } } forEach MC8_GROUPES_EST;
    ...
    sleep (MC8_PAL_DELAI + (30 call MC8_fnc_al));
    MC8_QRF_PARTIE = true;
    (format ["CHACAL|E|qrf_partie|%1", round (time * 100) / 100]) call MC8_LOG;
    MC8_gQrf setBehaviour "AWARE"; MC8_gQrf setCombatMode "RED"; MC8_gQrf setSpeedMode "FULL";
    while { count (waypoints MC8_gQrf) > 0 } do { deleteWaypoint ((waypoints MC8_gQrf) select 0) };
    private _w = MC8_gQrf addWaypoint [MC8_SITE, 0];
    ...
    if (MC8_LAMBS) then { [MC8_gQrf, 450, 30, [], MC8_SITE] spawn lambs_wp_fnc_taskHunt };
};
...
sleep 3;
if (MC8_ISSUE == "VOID") exitWith { MC8_FIN = true };
...
MC8_OPORD = [MC8_SITE, MC8_OP, MC8_ROUTE, MC8_QRF_BASE, MC8_LZ, MC8_PZ, MC8_RALLY];
(format ["CHACAL|E|opord|%1|site|%2|crete|%3|route|%4|reserve|%5|depose|%6|secours|%7|rally|%8|ouvertures|%9",
    round (time * 100) / 100, MC8_SITE, MC8_OP, MC8_ROUTE, MC8_QRF_BASE,
    MC8_LZ, MC8_PZ, MC8_RALLY, MC8_OUV_AZ]) call MC8_LOG;
...
// la crete voit-elle a 800 m de nuit - n avait aucune reponse. `MC8_DEPART`
// pose le detachement au regroupement. C est un DIAGNOSTIC : l episode est
// marque hors corpus, son approche n ayant pas eu lieu.
if (MC8_DEPART >= 3) then {
    (format ["CHACAL|AVERT|hors_corpus|depart|%1|approche_non_jouee", MC8_DEPART]) call MC8_LOG;
    { if (alive _x) then { _x setPosATL (MC8_RALLY getPos [8 + (14 call MC8_fnc_al), 360 call MC8_fnc_al]) } } forEach MC8_FS;
    MC8_gFS setBehaviour "STEALTH"; MC8_gFS setCombatMode "GREEN";
    MC8_gFS setSpeedMode "LIMITED"; MC8_gFS setFormation "FILE";
    (format ["CHACAL|E|depart_direct|%1|au_rally|%2", round (time * 100) / 100, MC8_RALLY]) call MC8_LOG;
    sleep 5;
} else {
...
if (MC8_DEPART == 2) then {
    "CHACAL|AVERT|hors_corpus|depart|2|insertion_non_jouee" call MC8_LOG;
    private _pt = MC8_ROUTE getPos [MC8_AVANT + 70, MC8_ROUTE getDir MC8_LZ];   // 70 m derriere le point d observation, comme a l origine ( 260 + 70 = 330 )
    private _k = 0;
    { if (alive _x) then { _x setPosATL (_pt getPos [10, _k * 36]); _k = _k + 1 } } forEach MC8_FS;
    MC8_gFS setBehaviour "STEALTH"; MC8_gFS setCombatMode "GREEN";
    MC8_gFS setSpeedMode "LIMITED"; MC8_gFS setFormation "FILE";
    (format ["CHACAL|E|depart_direct|%1|a_la_route|%2", round (time * 100) / 100, _pt]) call MC8_LOG;
    sleep 5;
} else {
_plafond = 1 call MC8_fnc_duree;
...
if (MC8_GEOMETRIE == 1) exitWith {
    (format ["CHACAL|GEO|%1|graine|%2|site|%3|crete|%4|route|%5|lz|%6|qrf|%7|pz|%8|rally|%9|ouvertures|%10|az_site|%11|gain_crete|%12",
        round (time * 100) / 100, MC8_GRAINE, MC8_SITE, MC8_OP, MC8_ROUTE,
        MC8_LZ, MC8_QRF_BASE, MC8_PZ, MC8_RALLY,
        (if (isNil "MC8_OUVERTURES") then {[]} else {MC8_OUVERTURES apply { round (MC8_SITE getDir _x) }}),
        (if (isNil "MC8_AZ") then {-1} else {round MC8_AZ}), round MC8_OP_GAIN]) call MC8_LOG;
    MC8_ISSUE = "VOID"; MC8_CAUSE = "GEOMETRIE_SEULE"; MC8_FIN = true;
};

[1, "INSERTION", _plafond] call MC8_fnc_debutPhase;

MC8_HELO = objNull;
if (["B_Heli_Transport_01_F"] call MC8_fnc_has) then {
    private _dep = MC8_LZ getPos [3400, (MC8_LZ getDir MC8_SITE) + 180];
    MC8_HELO = createVehicle ["B_Heli_Transport_01_F", [_dep select 0, _dep select 1, 140], [], 0, "FLY"];
    createVehicleCrew MC8_HELO;
    call MC8_fnc_recenser;   // un equipage neuf ne doit pas passer 2 s sans identite
    MC8_HELO flyInHeight 60;
    private _gh = group (driver MC8_HELO);
    _gh setBehaviour "CARELESS"; _gh setCombatMode "BLUE";
    { _x assignAsCargo MC8_HELO; _x moveInCargo MC8_HELO } forEach MC8_FS;
    private _w = _gh addWaypoint [MC8_LZ, 0];
    _w setWaypointType "MOVE"; _w setWaypointSpeed "FULL";
};

// Les plafonds physiques ne sont PAS a l echelle : un vol de 3,4 km est une
// duree du MONDE. MC8_ECHELLE raccourcit les plafonds TACTIQUES et rien d autre.
private _pose = false;
if (!isNull MC8_HELO) then {
    private _tv = time;
    waitUntil { sleep 2; (MC8_HELO distance2D MC8_LZ < 250) || (time - _tv > 300) || MC8_FIN };
    if (MC8_HELO distance2D MC8_LZ < 600) then {
        MC8_HELO land "GET OUT";
        private _tl = time;
        waitUntil { sleep 1; (((getPosATL MC8_HELO) select 2) < 1.5 && { (speed MC8_HELO) < 3 }) || (time - _tl > 120) || MC8_FIN };
    };
    _pose = (((getPosATL MC8_HELO) select 2) < 2.5) && { (speed MC8_HELO) < 5 };
    if (!_pose) then {
        MC8_INSERTION_FORCEE = true;
        MC8_HELO setPosATL [MC8_LZ select 0, MC8_LZ select 1, 0.3];
        MC8_HELO setVelocity [0, 0, 0];
        sleep 2;
        (format ["CHACAL|AVERT|insertion_forcee|%1|appareil_pose_a_la_main", round (time * 100) / 100]) call MC8_LOG;
    };
    ...
    private _k = 0;
    {
        if (alive _x) then {
            ...
            _x setPosATL (MC8_HELO getPos [15, _k * 36]);
            _k = _k + 1;
        };
    } forEach MC8_FS;
    sleep 3;
    {
        if (alive _x && { vehicle _x != _x }) then {
            unassignVehicle _x; moveOut _x;
            _x setPosATL (MC8_HELO getPos [15, _k * 36]); _k = _k + 1;
        };
    } forEach MC8_FS;
    sleep 2;
};
...
private _auSol = MC8_FS select { alive _x && { vehicle _x == _x } };
private _ecart = if (count _auSol > 0) then { round ((_auSol call MC8_fnc_centre) distance2D MC8_LZ) } else { -1 };
(format ["CHACAL|E|debarquement|%1|au_sol|%2|ecart_lz|%3|helico_pose|%4|methode|ANNEAU_SCRIPTE", round (time * 100) / 100,
    count _auSol, _ecart, (if (_pose) then {1} else {0})]) call MC8_LOG;

private _morts = MC8_EFFECTIF - (count (MC8_FS select { alive _x }));
...
private _parEnnemi = { !alive _x && { _x getVariable ["chacal_tue_par_est", false] } } count MC8_FS;
if (_parEnnemi > 0) then {
    (format ["CHACAL|E|insertion_sous_le_feu|%1|morts_par_ennemi|%2|morts_total|%3", round (time * 100) / 100, _parEnnemi, _morts]) call MC8_LOG;
};
if ((_morts - _parEnnemi) > 0) exitWith {
    ...
    MC8_ISSUE = "VOID"; MC8_CAUSE = "INSERTION_MORTELLE";
    (format ["CHACAL|VOID|insertion|%1|morts|%2|au_sol|%3", round (time * 100) / 100, _morts, count _auSol]) call MC8_LOG;
    MC8_FIN = true;
};
if (count _auSol < (MC8_EFFECTIF - _parEnnemi)) exitWith {
    MC8_ISSUE = "VOID"; MC8_CAUSE = "INSERTION_INCOMPLETE";
    (format ["CHACAL|VOID|insertion|%1|au_sol|%2", round (time * 100) / 100, count _auSol]) call MC8_LOG;
    MC8_FIN = true;
};
_issue = if (MC8_INSERTION_FORCEE) then {"FORCEE"} else {"ATTEINT"};

if (!isNull MC8_HELO) then {
    private _gh = group (driver MC8_HELO);
    if (!isNull _gh) then {
        while { count (waypoints _gh) > 0 } do { deleteWaypoint ((waypoints _gh) select 0) };
        private _rr = _gh addWaypoint [MC8_LZ getPos [5000, (MC8_LZ getDir MC8_SITE) + 180], 0];
    ...
    [] spawn {
        ...
        if (!isNull MC8_HELO) then {
            { (format ["CHACAL|E|retrait|%1|%2|equipage_helicoptere", round (time * 100) / 100,
                (_x getVariable ["chacal_id", -1])]) call MC8_LOG; deleteVehicle _x } forEach (crew MC8_HELO);
            deleteVehicle MC8_HELO;
        };
    };
};
...
if (MC8_P1_ATTENTE > 0) then {
    [1] call MC8_fnc_fenetreObservation;   // menace visible : regarder avant de choisir ( 0 = origine )
    [1, "INSERTION_ATTENTE", [1, 2], MC8_P1_ATTENTE, "IMPOSE"] call MC8_fnc_decision;
    if (MC8_P1_ATTENTE == 1) then {
        private _dest = MC8_LZ getPos [300, MC8_LZ getDir MC8_ROUTE];
        [MC8_gFS, _dest, "LIMITED", "STEALTH", "FILE", "CHOIX_P1_PARTIR"] call MC8_fnc_ordreAller;
        (format ["CHACAL|E|choix_joue|%1|point|INSERTION_ATTENTE|choix|1|detail|PARTIR|vers|%2", round (time * 100) / 100, _dest]) call MC8_LOG;
    } else {
        { if (alive _x) then { doStop _x; _x setUnitPos "DOWN" } } forEach (units MC8_gFS);
        (format ["CHACAL|E|choix_joue|%1|point|INSERTION_ATTENTE|choix|2|detail|SE_TERRER", round (time * 100) / 100]) call MC8_LOG;
    };
    private _tC = time;
    waitUntil { sleep 2; ((time - _tC) > 180) || MC8_FIN };
    { if (alive _x) then { _x setUnitPos "AUTO" } } forEach (units MC8_gFS);
} else {
    sleep (60 * MC8_ECHELLE);
};
[1, "INSERTION", _issue] call MC8_fnc_finPhase;
};   // fin de l alternative DEPART = 2

// ---------------------------------------------------------------------
// PHASE 2 - APPROCHE, et le franchissement de la route
// ---------------------------------------------------------------------
if (MC8_FIN) exitWith {};   // vignette close a la phase 1
_plafond = 2 call MC8_fnc_duree;
[2, "APPROCHE", _plafond] call MC8_fnc_debutPhase;

private _fr = MC8_ROUTE;
private _avant = _fr getPos [MC8_AVANT, _fr getDir MC8_LZ];   // levier : d ou l on observe la route ( origine 260 m )
...
private _b1 = [MC8_gFS, _avant, 0.7] call MC8_fnc_budget;
(format ["CHACAL|E|budget|%1|etape|APPROCHE_ROUTE|distance|%2|plafond|%3", round (time * 100) / 100,
    round (((units MC8_gFS) call MC8_fnc_centre) distance2D _avant), round _b1]) call MC8_LOG;
[MC8_gFS, _avant, "NORMAL", "AWARE", "FILE", "OPORD_ROUTE_JAMBE_LOINTAINE"] call MC8_fnc_ordreAller;
_r = [MC8_gFS, _avant, 60, _b1] call MC8_fnc_arrive;

// ! LA FENETRE SE DECIDE SUR CE QUE LA FILE PERCOIT, PAS SUR LA POSITION VRAIE.
// Le premier jet lisait `MC8_VEH_ROUTE distance2D _avant`, que personne dans
...
if ((_r in ["ATTEINT", "ENLISE"]) && !MC8_SAUT) then {
    { doStop _x } forEach (units MC8_gFS);
    ...
    if (MC8_TRAVERSEE > 0) then {
        [2] call MC8_fnc_fenetreObservation;   // menace visible : regarder avant de choisir ( 0 = origine )
        private _vD = if (isNil "MC8_VEH_ROUTE") then { objNull } else { MC8_VEH_ROUTE };
        private _vuD = 0;
        if (!isNull _vD && { alive _vD }) then {
            if (({ [_x, _vD, 800, 70] call MC8_fnc_voit } count ((units MC8_gFS) select { alive _x })) > 0) then { _vuD = 1 };
        };
        [2, "TRAVERSEE", [1, 2], MC8_TRAVERSEE, "IMPOSE", format ["|vehicule_vu|%1", _vuD]] call MC8_fnc_decision;
    };
    private _tf = time;
    private _dejaVu = false; private _dernierVu = -1; private _traverse = false; private _cause = "";
    while { !_traverse && (time - _tf < 420 * MC8_ECHELLE) && ((call MC8_fnc_reste) > 0) && !MC8_SAUT && !MC8_FIN } do {
        if (MC8_TRAVERSEE == 1) then { _traverse = true; _cause = "IMPOSE_TOUT_DE_SUITE" } else {
            private _v = MC8_VEH_ROUTE;
            private _vu = false;
            if (!isNull _v && { alive _v }) then {
                private _hommes = (units MC8_gFS) select { alive _x };
                _vu = ({ [_x, _v, 800, 70] call MC8_fnc_voit } count _hommes) > 0;
            };
            if (_vu) then { _dejaVu = true; _dernierVu = time };
            if (_dejaVu && { !_vu } && { (time - _dernierVu) > (45 * MC8_ECHELLE) }) then { _traverse = true; _cause = "FENETRE_OBSERVEE" };
            if (!_dejaVu && { (time - _tf) > (240 * MC8_ECHELLE) }) then { _traverse = true; _cause = "TRAVERSEE_AVEUGLE" };
            if ((MC8_TRAVERSEE != 2) && { isNull _v || { !alive _v } }) then { _traverse = true; _cause = "PATROUILLE_ABSENTE" };
            sleep 3;
        };
    };
    if (!_traverse) then { _cause = "PLAFOND_FENETRE" };
    (format ["CHACAL|E|fenetre|%1|%2|attente|%3|vehicule_vu|%4", round (time * 100) / 100,
        _cause, round (time - _tf), (if (_dejaVu) then {1} else {0})]) call MC8_LOG;
    if (MC8_TRAVERSEE > 0) then {
        (format ["CHACAL|E|choix_joue|%1|point|TRAVERSEE|choix|%2|detail|%3|attente|%4", round (time * 100) / 100,
            MC8_TRAVERSEE, _cause, round (time - _tf)]) call MC8_LOG;
    };
    { _x doFollow (leader MC8_gFS) } forEach (units MC8_gFS);
    ...
    private _perp = (MC8_ROUTE_A getDir MC8_ROUTE_B) + 90;
    if ((abs (((_perp - (_fr getDir MC8_SITE)) + 540) % 360 - 180)) > 90) then { _perp = _perp + 180 };
    private _degage = _fr getPos [750, _perp];
    [MC8_gFS, _degage, "NORMAL", "AWARE", "WEDGE", "SORTIE_DE_COULOIR"] call MC8_fnc_ordreAller;
    (format ["CHACAL|E|couloir|%1|axe|%2|sortie|%3", round (time * 100) / 100, round _perp, _degage]) call MC8_LOG;
    [MC8_gFS, _degage, 90, [MC8_gFS, _degage, 1.1] call MC8_fnc_budget] call MC8_fnc_arrive;
};
...
if (!MC8_SAUT && { !(MC8_DEPART == 2 && MC8_ARRET == 2) }) then {
    private _b2 = [MC8_gFS, MC8_RALLY, 0.5] call MC8_fnc_budget;
    (format ["CHACAL|E|budget|%1|etape|APPROCHE_RALLY|distance|%2|plafond|%3", round (time * 100) / 100,
        round (((units MC8_gFS) call MC8_fnc_centre) distance2D MC8_RALLY), round _b2]) call MC8_LOG;
    [MC8_gFS, MC8_RALLY, "LIMITED", "STEALTH", "FILE", "OPORD_RALLY"] call MC8_fnc_ordreAller;
    _r = [MC8_gFS, MC8_RALLY, 70, _b2] call MC8_fnc_arrive;
};
[2, "APPROCHE", (if (MC8_SAUT) then {"COMPROMIS"} else {_r})] call MC8_fnc_finPhase;
...
// ! 20/09 : quand la patrouille tue les dix, la liste des vivants est VIDE et MC8_fnc_centre erre.
// Le defaut dormait depuis toujours : aucune campagne n avait encore aneanti le detachement.
if (MC8_TENIR == 0 && MC8_COMPROMIS && { count (MC8_FS select { alive _x }) > 0 }
    && { ((MC8_FS select { alive _x }) call MC8_fnc_centre) distance2D MC8_SITE > 1200 }) then {
    MC8_ABANDON = true; MC8_SAUT = false;
    MC8_CAUSE_ABANDON = "COMPROMIS_LOIN";
    (format ["CHACAL|E|abandon|%1|distance|%2", round (time * 100) / 100,
        round (((MC8_FS select { alive _x }) call MC8_fnc_centre) distance2D MC8_SITE)]) call MC8_LOG;
};
...
// invendable.
if (MC8_BRAS == "NUL" && !MC8_SAUT && !MC8_ABANDON) then {
    MC8_OUV_CHOISIE = MC8_OUVERTURES select 0;
    MC8_POS_ASSAUT = [MC8_SITE getPos [230, MC8_SITE getDir MC8_OUV_CHOISIE], 60] call MC8_fnc_plat;
    [MC8_gFS, MC8_POS_ASSAUT, "NORMAL", "AWARE", "WEDGE", "BRAS_NUL_DROIT_SUR_L_OBJECTIF"] call MC8_fnc_ordreAller;
    (format ["CHACAL|E|bras_nul|%1|vers|%2", round (time * 100) / 100, MC8_POS_ASSAUT]) call MC8_LOG;
    [MC8_gFS, MC8_POS_ASSAUT, 90, [MC8_gFS, MC8_POS_ASSAUT, 0.9] call MC8_fnc_budget] call MC8_fnc_arrive;
};

// ---------------------------------------------------------------------
// PHASE 3 - POINT D OBSERVATION : deux hommes montent, huit se terrent
// ---------------------------------------------------------------------
if (!MC8_FIN && !MC8_SAUT && !MC8_ABANDON && { MC8_BRAS != "NUL" } && { MC8_DEPART < 4 }) then {
    _plafond = 3 call MC8_fnc_duree;
    [3, "OBSERVATION", _plafond] call MC8_fnc_debutPhase;
    // ! COUPURE ASSUMEE ET ECRITE. Voir verdicts/phase3-porte-hors-datteinte.md.
    if (MC8_OBS == 0) exitWith {
        (format ["CHACAL|AVERT|hors_corpus|observation_coupee|1|seuil|%1|jamais_atteint|1",
            MC8_SEUIL_RENS]) call MC8_LOG;
        (format ["CHACAL|E|observation_coupee|%1|economie_s|%2", round (time * 100) / 100,
            round _plafond]) call MC8_LOG;
        [3, "OBSERVATION", "COUPEE"] call MC8_fnc_finPhase;
    };

    MC8_gReco = [MC8_RECO, "RECHERCHE"] call MC8_fnc_detacher;
    MC8_gFS setVariable ["chacal_element", "GROS", true];
    [MC8_gReco, MC8_OP, "LIMITED", "STEALTH", "FILE", "OPORD_CRETE"] call MC8_fnc_ordreAller;
    private _cache = [MC8_RALLY getPos [120, MC8_RALLY getDir MC8_SITE], 50] call MC8_fnc_plat;
    [MC8_gFS, _cache, "LIMITED", "STEALTH", "WEDGE", "TENIR_HORS_DE_VUE"] call MC8_fnc_ordreAller;

    private _rArr = [MC8_gReco, MC8_OP, 60, [MC8_gReco, MC8_OP, 0.5] call MC8_fnc_budget] call MC8_fnc_arrive;
    (format ["CHACAL|E|montee|%1|issue|%2|reco|%3|distance|%4", round (time * 100) / 100,
        _rArr, count ((units MC8_gReco) select { alive _x }),
        (if (count ((units MC8_gReco) select { alive _x }) > 0)
         then { round ((((units MC8_gReco) select { alive _x }) call MC8_fnc_centre) distance2D MC8_OP) }
         else { -1 })]) call MC8_LOG;
    ...
    // et redimensionne toute la mission autour d une fausse cause.
    {
        if (alive _x) then {
            _x setUnitPos "DOWN";
            _x setDir (_x getDir MC8_SITE);
            _x doWatch MC8_SITE;
        };
    } forEach (units MC8_gReco);
    (format ["CHACAL|E|orientation|%1|reco|%2|azimut_site|%3|distance|%4", round (time * 100) / 100,
        count ((units MC8_gReco) select { alive _x }),
        round (MC8_OP getDir MC8_SITE), round (MC8_OP distance2D MC8_SITE)]) call MC8_LOG;
    { if (alive _x) then { _x setUnitPos "MIDDLE" } } forEach (units MC8_gFS);

    // LE RENSEIGNEMENT EST RETENU, pas seulement compte : ce qui entre dans
    // MC8_VUES est ce que la reco a vu GEOMETRIQUEMENT au moins une fois.
    ...
    if (MC8_OBS_DUREE > 0) then {
        _dureeObs = MC8_OBS_DUREE;
        [3, "OBS_DUREE", [120, 480], MC8_OBS_DUREE, "IMPOSE", format ["|reco_vivants|%1", count ((units MC8_gReco) select { alive _x })]] call MC8_fnc_decision;
    };
    private _tObs = time;
    while { (time - _tObs) < _dureeObs && ((call MC8_fnc_reste) > 0) && !MC8_SAUT && !MC8_FIN } do {
        private _obs = (units MC8_gReco) select { alive _x };
        if (count _obs > 0) then {
            {
                private _e = _x;
                if ({ [_x, _e, 900] call MC8_fnc_voit } count _obs > 0) then {
                    private _k = 0;
                    { private _kk = _x knowsAbout _e; if (_kk > _k) then { _k = _kk } } forEach _obs;
                    private _i = MC8_VUES findIf { (_x select 0) isEqualTo _e };
                    if (_i < 0) then { MC8_VUES pushBack [_e, _k] }
                    else { if (_k > ((MC8_VUES select _i) select 1)) then { (MC8_VUES select _i) set [1, _k] } };
                };
            } forEach (MC8_EST_SITE select { alive _x });
            (format ["CHACAL|E|renseignement|%1|localisees|%2|sur|%3|seuil|%4", round (time * 100) / 100,
                count MC8_VUES, count (MC8_EST_SITE select { alive _x }), MC8_SEUIL_RENS]) call MC8_LOG;
        };
        sleep (60 * MC8_ECHELLE);
    };
    ...
    if (MC8_OBS_DUREE > 0) then {
        (format ["CHACAL|E|choix_joue|%1|point|OBS_DUREE|choix|%2|detail|OBSERVE|duree_reelle|%3|localisees|%4|rapprochement|0",
            round (time * 100) / 100, MC8_OBS_DUREE, round (time - _tObs), count MC8_VUES]) call MC8_LOG;
    };
    if (!MC8_SAUT && { MC8_OBS_DUREE == 0 } && { count MC8_VUES < MC8_SEUIL_RENS }) then {
        private _second = [MC8_SITE getPos [250, MC8_SITE getDir MC8_OP], 60] call MC8_fnc_plat;
        (format ["CHACAL|E|rapprochement|%1|localisees|%2|seuil|%3|vers|%4", round (time * 100) / 100,
            count MC8_VUES, MC8_SEUIL_RENS, _second]) call MC8_LOG;
        [MC8_gReco, _second, 60, ([MC8_gReco, _second] call MC8_fnc_budgetDeuxJambes) min 600, "RECO_SECOND_POSTE"] call MC8_fnc_rejoindre;
        { if (alive _x) then { _x setUnitPos "DOWN"; _x setDir (_x getDir MC8_SITE); _x doWatch MC8_SITE } } forEach (units MC8_gReco);
        private _t2 = time;
        while { (time - _t2) < (300 * MC8_ECHELLE) && !MC8_SAUT && !MC8_FIN } do {
            private _obs2 = (units MC8_gReco) select { alive _x };
            if (count _obs2 > 0) then {
                {
                    private _e = _x;
                    if ({ [_x, _e, 600] call MC8_fnc_voit } count _obs2 > 0) then {
                        private _k = 0;
        ...
        if (count MC8_VUES < MC8_SEUIL_RENS) then {
            (format ["CHACAL|E|renseignement_pauvre|%1|localisees|%2|l_assaut_part_quand_meme", round (time * 100) / 100, count MC8_VUES]) call MC8_LOG;
        };
    };
    [3, "OBSERVATION", (if (MC8_SAUT) then {"COMPROMIS"} else {
        if (count MC8_VUES >= MC8_SEUIL_RENS) then {"ATTEINT"} else {"RENSEIGNEMENT_PAUVRE"} })] call MC8_fnc_finPhase;
};

// ---------------------------------------------------------------------
// PHASE 4 - MISE EN PLACE : appui, assaut, bouchon
// ---------------------------------------------------------------------
if (!MC8_FIN && !MC8_SAUT && !MC8_ABANDON && { MC8_BRAS != "NUL" }) then {
    _plafond = 4 call MC8_fnc_duree;
    [4, "MISE_EN_PLACE", _plafond] call MC8_fnc_debutPhase;

    MC8_gAppui   = [MC8_APPUI,   "APPUI"]   call MC8_fnc_detacher;
    MC8_gAssaut  = [MC8_ASSAUT,  "ASSAUT"]  call MC8_fnc_detacher;
    MC8_gBouchon = [MC8_BOUCHON, "BOUCHON"] call MC8_fnc_detacher;
    ...
    // definir MC8_PAL_QRF : d ou le garde isNil ci-dessous, sans lequel cette ligne
    ...
    (format ["CHACAL|E|partage|%1|regle|%2|assaut|%3|bouchon|%4|appui|%5|qrf_vehicules|%6|qrf_hommes|%7|delai_qrf|%8",
        round (time * 100) / 100,
        (if (MC8_PARTAGE == 1) then {"SEPT_UN"} else {"CINQ_TROIS"}),
        count (units MC8_gAssaut), count (units MC8_gBouchon), count (units MC8_gAppui),
        (if (isNil "MC8_PAL_QRF") then {0} else {MC8_PAL_QRF}), count MC8_QRF, MC8_PAL_DELAI]) call MC8_LOG;
    ...
    // ! L ORACLE COMPLET ( MC8_ORACLE = 2, revue de Fable 11/09 ) : les defenseurs sont reveles et
    // inscrits comme vus AVANT le choix de la porte, qui se fait alors en les connaissant.
    if (MC8_ORACLE == 2) then {
        private _defO = MC8_EST_SITE select { alive _x };
        { private _u = _x; { _u reveal [_x, 4] } forEach _defO } forEach (MC8_FS select { alive _x });
        MC8_VUES = _defO apply { [_x, 4] };
        (format ["CHACAL|E|oracle|%1|reveles|%2|hommes|%3|phase|4|avant_choix_porte|1", round (time * 100) / 100,
            count _defO, count (MC8_FS select { alive _x })]) call MC8_LOG;
    };
    private _dep = ((units MC8_gAssaut) select { alive _x }) call MC8_fnc_centre;
    if (count _dep == 0) then { _dep = MC8_RALLY };
    ...
    // PLUS PROCHE. Discriminant par construction des que MC8_VUES n est pas vide.
    ...
    private _connus = (MC8_VUES apply { _x select 0 }) select { !isNull _x };
    private _aff  = MC8_OUVERTURES apply { 0 };      // defenseurs connus attribues a l ouverture
    private _dgar = MC8_OUVERTURES apply { -1 };     // distance du defenseur connu le plus proche
    {
    ...
    MC8_OUV_CHOISIE = MC8_OUVERTURES select _meilleure;
    // ! LA COUTURE DE L AZIMUT. Douze candidats publies, un decideur designe par MC8_AZIMUT.
    // Le mode 0 ne touche a rien : MC8_OUV_CHOISIE garde la valeur que le score vient de poser.
    ...
    MC8_AZIMUT_CHOISI = round (MC8_SITE getDir MC8_OUV_CHOISIE);
    private _qui = "SCRIPT";
    if (MC8_AZIMUT == 1) then {
        ...
        private _k = floor ((call MC8_fnc_rndTemoin) * (count _candidats));
        if (_k >= count _candidats) then { _k = (count _candidats) - 1 };
        MC8_AZIMUT_CHOISI = _candidats select _k; _qui = "HASARD";
    };
    if (MC8_AZIMUT == 2) then {
        MC8_AZIMUT_CHOISI = ((round MC8_AZIMUT_VAL) + 360) % 360; _qui = "IMPOSE";
    };
    if (MC8_AZIMUT > 0) then {
        MC8_OUV_CHOISIE = MC8_SITE getPos [MC8_RAYON, MC8_AZIMUT_CHOISI];
        MC8_OUV_CHOISIE set [2, 0];
    };
    (format ["CHACAL|E|couture_azimut|%1|mode|%2|decideur|%3|candidats|%4|choisi|%5|point|%6|az_ouvertures|%7",
        round (time * 100) / 100, MC8_AZIMUT, _qui, count _candidats, MC8_AZIMUT_CHOISI,
        MC8_OUV_CHOISIE, (MC8_OUVERTURES apply { round (MC8_SITE getDir _x) })]) call MC8_LOG;
    ...
    (format ["CHACAL|E|choix_ouverture|%1|indice|%2|gardes|%3|distances|%4|score|%5|renseignement|%6|dgarde|%7|regle|PLUS_PROCHE",
        ...
        round (_minScore * 100) / 100, count MC8_VUES,
        str (_dgar apply { if (_x < 0) then { -1 } else { round _x } })]) call MC8_LOG;

    // ! L APPUI N EST PLUS L OBSERVATOIRE ( sous parametre ). A 0, on garde la ligne d origine.
    MC8_POS_APPUI   = MC8_OP;
    // ! Le point de depart est passe depuis l element concerne : c est la seule facon de savoir
    // si le trajet est praticable. Sans MC8_ACCESSIBLE, l argument est ignore.
    private _depAssaut  = ((units MC8_gAssaut)  select { alive _x }) call MC8_fnc_centre;
    private _depBouchon = ((units MC8_gBouchon) select { alive _x }) call MC8_fnc_centre;
    MC8_POS_ASSAUT  = [MC8_SITE getPos [230, MC8_SITE getDir MC8_OUV_CHOISIE], 60, 220, _depAssaut] call MC8_fnc_plat;
    ...
    MC8_POS_BOUCHON = [MC8_SITE getPos [400, MC8_SITE getDir MC8_QRF_BASE], 80, 220, _depBouchon] call MC8_fnc_plat;
    if (MC8_APPUI_FEU == 1) then {
        MC8_POS_APPUI = [MC8_SITE, MC8_OUV_CHOISIE, MC8_POS_ASSAUT, MC8_OP] call MC8_fnc_positionAppui;
    };
    ...
    if (MC8_APPUI_FIXE == 1) then {
        [] spawn {
            waitUntil { sleep 2; MC8_FIN || { !isNull MC8_gAppui && { [MC8_gAppui, MC8_POS_APPUI, 70] call MC8_fnc_enPlace } } };
            if (MC8_FIN || { isNull MC8_gAppui }) exitWith {};
            MC8_gAppui setCombatMode "YELLOW";
            MC8_gAppui setVariable ["lambs_danger_disableGroupAI", true, true];
            { if (alive _x) then { _x disableAI "PATH"; _x setVariable ["lambs_danger_disableAI", true, true] } } forEach (units MC8_gAppui);
            (format ["CHACAL|E|appui_fixe|%1|position|%2|compromis|%3", round (time * 100) / 100, MC8_POS_APPUI,
                (if (MC8_COMPROMIS) then {1} else {0})]) call MC8_LOG;
        };
    };
    ...
    private _b1 = [MC8_gAppui,   MC8_POS_APPUI]   call MC8_fnc_budgetDeuxJambes;
    private _b2 = [MC8_gAssaut,  MC8_POS_ASSAUT]  call MC8_fnc_budgetDeuxJambes;
    private _b3 = [MC8_gBouchon, MC8_POS_BOUCHON] call MC8_fnc_budgetDeuxJambes;
    ...
    if (MC8_ITINERAIRE > 0) then { _plafond = _plafond * 2 };
    MC8_TPHASE = time; MC8_PLAFOND_COURANT = _plafond;
    (format ["CHACAL|E|budget|%1|etape|MISE_EN_PLACE|appui|%2|assaut|%3|bouchon|%4|plafond|%5",
        round (time * 100) / 100, round _b1, round _b2, round _b3, round _plafond]) call MC8_LOG;
    ...
    if (MC8_ORACLE >= 1) then {
        private _def = MC8_EST_SITE select { alive _x };
        { private _u = _x; { _u reveal [_x, 4] } forEach _def } forEach (MC8_FS select { alive _x });
        MC8_VUES = _def apply { [_x, 4] };
        (format ["CHACAL|E|oracle|%1|reveles|%2|hommes|%3|phase|4", round (time * 100) / 100, count _def,
            count (MC8_FS select { alive _x })]) call MC8_LOG;
    };
    if (MC8_DEPART >= 4) then {
        "CHACAL|AVERT|hors_corpus|depart|4|articulation_non_jouee" call MC8_LOG;
        {
            _x params ["_g", "_p"];
            if (!isNull _g) then {
                { if (alive _x) then { _x setPosATL (_p getPos [4 + (10 call MC8_fnc_al), 360 call MC8_fnc_al]) } } forEach (units _g);
            };
        } forEach [[MC8_gAppui, MC8_POS_APPUI], [MC8_gAssaut, MC8_POS_ASSAUT], [MC8_gBouchon, MC8_POS_BOUCHON]];
        sleep 5;
    } else {
        if (MC8_ITINERAIRE > 0) then { [4] call MC8_fnc_fenetreObservation; [4, "ITINERAIRE", [1, 2], MC8_ITINERAIRE, "IMPOSE"] call MC8_fnc_decision };
        private _h1 = [MC8_gAppui,   MC8_POS_APPUI,   70, _plafond, "APPUI"]   spawn MC8_fnc_rejoindreItineraire;
        private _h2 = [MC8_gAssaut,  MC8_POS_ASSAUT,  70, _plafond, "ASSAUT"]  spawn MC8_fnc_rejoindreItineraire;
        private _h3 = [MC8_gBouchon, MC8_POS_BOUCHON, 80, _plafond, "BOUCHON"] spawn MC8_fnc_rejoindreItineraire;
        waitUntil { sleep 3; (scriptDone _h1 && scriptDone _h2 && scriptDone _h3) || MC8_FIN || MC8_SAUT };
    };
    _r1 = if ([MC8_gAppui,   MC8_POS_APPUI,   70] call MC8_fnc_enPlace) then {"ATTEINT"} else {"NON"};
    _r2 = if ([MC8_gAssaut,  MC8_POS_ASSAUT,  70] call MC8_fnc_enPlace) then {"ATTEINT"} else {"NON"};
    _r3 = if ([MC8_gBouchon, MC8_POS_BOUCHON, 80] call MC8_fnc_enPlace) then {"ATTEINT"} else {"NON"};
    { if (alive _x) then { _x setUnitPos "DOWN"; _x setDir (_x getDir MC8_SITE); _x doWatch MC8_SITE } } forEach (units MC8_gAppui);
    (format ["CHACAL|E|articulation|%1|appui|%2|assaut|%3|bouchon|%4", round (time * 100) / 100, _r1, _r2, _r3]) call MC8_LOG;
    [4, "MISE_EN_PLACE", (if (_r1 == "ATTEINT" && _r2 == "ATTEINT" && _r3 == "ATTEINT") then {"ATTEINT"}
        else { if (MC8_SAUT) then {"COMPROMIS"} else {"PLAFOND"} })] call MC8_fnc_finPhase;
};

// ---------------------------------------------------------------------
// PHASE 5 - ACTION SUR OBJECTIF
// ---------------------------------------------------------------------
if (!MC8_ABANDON && !MC8_FIN) then {
MC8_SAUT = false;
...
if (MC8_ATTENTE_TEST > 0) then {
    ...
    // L appui deja fige par MC8_APPUI_FIXE n est pas touche : lui rendre PATH le ferait partir, et seulement
    // dans les bras qui attendent.
    private _fige = (MC8_FS select { alive _x }) select { !(MC8_APPUI_FIXE == 1 && { group _x == MC8_gAppui }) };
    ...
    (format ["CHACAL|E|attente_test|%1|debut|duree_prevue|%2|vivants|%3|alarme|%4|figes|%5", round (time * 100) / 100,
        MC8_ATTENTE_TEST, count (MC8_FS select { alive _x }), (if (MC8_ALARME) then {1} else {0}),
        count _fige]) call MC8_LOG;
    waitUntil { sleep 2; MC8_FIN || ((time - _tA) >= (MC8_ATTENTE_TEST * MC8_ECHELLE)) };
    private _derive = 0;
    ...
    (format ["CHACAL|E|attente_test|%1|fin|duree|%2|vivants|%3|alarme|%4|compromis|%5|derive|%6", round (time * 100) / 100,
        round (time - _tA), count (MC8_FS select { alive _x }), (if (MC8_ALARME) then {1} else {0}),
        (if (MC8_COMPROMIS) then {1} else {0}), round _derive]) call MC8_LOG;
};
_plafond = 5 call MC8_fnc_duree;
[5, "ASSAUT", _plafond] call MC8_fnc_debutPhase;

if (isNull MC8_gAssaut) then {
    MC8_gAssaut = MC8_gFS; MC8_gAssaut setVariable ["chacal_element", "ASSAUT_BLOC", true];
    if (count MC8_OUV_CHOISIE == 0) then { MC8_OUV_CHOISIE = MC8_OUVERTURES select 0 };
};
...
if (!isNil "MC8_POS_ASSAUT") then {
    private _a1 = [MC8_gAppui,   MC8_POS_APPUI,   130] call MC8_fnc_enPlace;
    private _a2 = [MC8_gAssaut,  MC8_POS_ASSAUT,  130] call MC8_fnc_enPlace;
    private _a3 = [MC8_gBouchon, MC8_POS_BOUCHON, 160] call MC8_fnc_enPlace;
    ...
    if (!_a2 || { !_a1 }) then {
        ...
        private _uA = (units MC8_gAssaut) select { alive _x };
        private _dA = if (count _uA > 0) then { (_uA call MC8_fnc_centre) distance2D MC8_POS_ASSAUT } else { 9999 };
        if (_dA > 400) exitWith {
            (format ["CHACAL|E|rearticulation|%1|refusee|assaut_a|%2|trop_loin", round (time * 100) / 100, round _dA]) call MC8_LOG;
        };
        (format ["CHACAL|E|rearticulation|%1|cause|DISPOSITIF_ECLATE|assaut_a|%2", round (time * 100) / 100, round _dA]) call MC8_LOG;
        private _b = ([MC8_gAssaut, MC8_POS_ASSAUT] call MC8_fnc_budgetDeuxJambes) min (_plafond * 0.45);
        private _hA = [MC8_gAssaut, MC8_POS_ASSAUT, 130, _b, "ASSAUT_REPRISE"] spawn MC8_fnc_rejoindre;
        [MC8_gAppui,   MC8_POS_APPUI,   "NORMAL", "AWARE", "LINE",  "REARTICULATION"] call MC8_fnc_ordreAller;
        [MC8_gBouchon, MC8_POS_BOUCHON, "NORMAL", "AWARE", "WEDGE", "REARTICULATION"] call MC8_fnc_ordreAller;
        waitUntil { sleep 3; scriptDone _hA || MC8_FIN };
        _a1 = [MC8_gAppui,   MC8_POS_APPUI,   130] call MC8_fnc_enPlace;
        _a2 = [MC8_gAssaut,  MC8_POS_ASSAUT,  130] call MC8_fnc_enPlace;
        _a3 = [MC8_gBouchon, MC8_POS_BOUCHON, 160] call MC8_fnc_enPlace;
        (format ["CHACAL|E|dispositif|%1|apres_rearticulation|appui|%2|assaut|%3|bouchon|%4|vivants|%5",
            ...
            (if (_a3) then {1} else {0}), count (MC8_FS select { alive _x })]) call MC8_LOG;
    };

    if (!_a2) then {
        _renonce = true; MC8_ABANDON = true;
        MC8_CAUSE_ABANDON = "ARTICULATION_ROMPUE";
...
if (MC8_ORACLE >= 1) then {
    private _def = MC8_EST_SITE select { alive _x };
    { private _u = _x; { _u reveal [_x, 4] } forEach _def } forEach (MC8_FS select { alive _x });
    (format ["CHACAL|E|oracle|%1|reveles|%2|phase|5", round (time * 100) / 100, count _def]) call MC8_LOG;
};
private _recoHommes = MC8_RECO call MC8_fnc_role;
private _restitue = 0;
{
    _x params ["_cible", "_k"];
...
(format ["CHACAL|E|restitution|%1|restituees|%2|localisees|%3|destinataires|%4", round (time * 100) / 100,
    _restitue, count MC8_VUES, count _recoHommes]) call MC8_LOG;
...
if (_restitue == 0 && !_renonce) then {
    (format ["CHACAL|E|renseignement_pauvre|%1|restituees|0|l_assaut_part_par_l_ouverture_par_defaut", round (time * 100) / 100]) call MC8_LOG;
};

if (_renonce) then {
    [5, "ASSAUT", MC8_CAUSE_ABANDON] call MC8_fnc_finPhase;
} else {

if (!isNull MC8_gAppui) then {
    MC8_gAppui setBehaviour "COMBAT"; MC8_gAppui setCombatMode (if (MC8_APPUI_FIXE == 1) then {"YELLOW"} else {"RED"}); MC8_gAppui setSpeedMode "LIMITED";
    private _cibles = (MC8_VUES apply { _x select 0 }) select { alive _x };
    { if (alive _x && { count _cibles > 0 }) then { _x doTarget (_cibles select 0); _x doFire (_cibles select 0) } } forEach (units MC8_gAppui);
};
...
if (MC8_BANC_APPUI > 0) exitWith {
    call MC8_fnc_bancAppui;
    [5, "ASSAUT", "BANC_APPUI"] call MC8_fnc_finPhase;
    MC8_FIN = true;
};
// ! LE SOCLE ET LA TACTIQUE ( document du 11/09 ). Le socle repare l execution, la tactique conduit l appui.
if (MC8_SOCLE == 1) then { call MC8_fnc_socleAssaut };
if (MC8_TACTIQUE == 5) then {
    // T5 : personne ne tire tant que rien ne nous tire dessus. A la premiere balle, on bascule en T1.
    { if (!isNull _x) then { _x setCombatMode "GREEN" } } forEach [MC8_gAppui, MC8_gAssaut, MC8_gBouchon];
    MC8_gAssaut setBehaviour "STEALTH";
    [] spawn {
        waitUntil { sleep 2; MC8_FIN || { MC8_PHASE != 5 } || { count MC8_CONNUS > 0 } || MC8_COMPROMIS };
        if (MC8_FIN || { MC8_PHASE != 5 }) exitWith {};
        MC8_TACTIQUE = 1;
        { if (!isNull _x) then { _x setCombatMode "RED" } } forEach [MC8_gAppui, MC8_gBouchon];
        MC8_gAssaut setBehaviour "AWARE"; MC8_gAssaut setCombatMode "YELLOW";
        (format ["CHACAL|E|bascule_t5_vers_t1|%1|connus|%2|compromis|%3", round (time * 100) / 100,
            count MC8_CONNUS, (if (MC8_COMPROMIS) then {1} else {0})]) call MC8_LOG;
    };
};
if (MC8_TACTIQUE > 0) then { [] spawn MC8_fnc_tactiqueAppui };
if (MC8_SOCLE == 1 && { !(1 call MC8_fnc_sans) }) then { [] spawn MC8_fnc_chienDeGarde };
// L assaut attend : T1 le premier coup de l appui ( 120 s au plus ), T2 qu il ne reste au plus qu un defenseur ( 8 min ).
if (MC8_TACTIQUE == 1) then {
    private _tA = time;
    waitUntil { sleep 1; (MC8_T_PREMIER_TIR_APPUI > 0) || { time - _tA > 120 } || MC8_FIN };
    sleep 5;
    (format ["CHACAL|E|t1_depart|%1|premier_tir_appui|%2|attente|%3", round (time * 100) / 100,
        round MC8_T_PREMIER_TIR_APPUI, round (time - _tA)]) call MC8_LOG;
};
if (MC8_TACTIQUE == 2) then {
    private _tA = time;
    waitUntil { sleep 2; (count (MC8_EST_SITE select { alive _x }) <= 1) || { time - _tA > 480 } || MC8_FIN };
    (format ["CHACAL|E|t2_depart|%1|defenseurs_restants|%2|attente|%3", round (time * 100) / 100,
        count (MC8_EST_SITE select { alive _x }), round (time - _tA)]) call MC8_LOG;
};
...
MC8_TIRS_APPUI = 0; MC8_T_PREMIER_TIR = -1; MC8_CIBLE_ASSAUT = []; MC8_RELANCES = 0;
if (!isNull MC8_gAppui) then {
    {
        _x addEventHandler ["Fired", {
            MC8_TIRS_APPUI = MC8_TIRS_APPUI + 1;
            if (MC8_T_PREMIER_TIR < 0) then { MC8_T_PREMIER_TIR = time };
        }];
    } forEach ((units MC8_gAppui) select { alive _x });
};
if (MC8_FEU_AVANT == 1 && { !isNull MC8_gAppui }) then {
    [] spawn {
        while { !MC8_FIN && { MC8_PHASE == 5 } } do {
            private _def = MC8_EST_SITE select { alive _x };
            private _ref = if (count MC8_CIBLE_ASSAUT > 0) then { MC8_CIBLE_ASSAUT } else { MC8_OUV_CHOISIE };
    ...
    private _tG = time;
    waitUntil { sleep 1; (MC8_TIRS_APPUI > 0) || { (time - _tG) > (120 * MC8_ECHELLE) } || { MC8_FIN } };
    (format ["CHACAL|E|feu_avant|%1|premier_tir|%2|attente|%3|defenseurs_vivants|%4", round (time * 100) / 100,
        (if (MC8_T_PREMIER_TIR < 0) then {-1} else {round (MC8_T_PREMIER_TIR * 100) / 100}),
        round (time - _tG), count (MC8_EST_SITE select { alive _x })]) call MC8_LOG;
};
sleep (8 * MC8_ECHELLE);

if (!isNull MC8_gBouchon) then {
    MC8_gBouchon setBehaviour "COMBAT"; MC8_gBouchon setCombatMode "RED";
    { if (alive _x) then { _x setUnitPos "MIDDLE" } } forEach (units MC8_gBouchon);
};
...
if (MC8_MG_ASSAUT == 1) then {
    private _mg = (units MC8_gAssaut) select { alive _x && { (_x getVariable ["chacal_role", ""]) == "ADJOINT" } };
    if (count _mg == 0) then { _mg = (units MC8_gAssaut) select { alive _x && { !((_x getVariable ["chacal_role", ""]) in ["DEMO_1", "DEMO_2"]) } } };
    if (count _mg > 0) then {
        ...
        (format ["CHACAL|E|mg_assaut|%1|role|%2|arme|%3|munitions_chargees|%4|chargeurs_sac|%5", round (time * 100) / 100,
            ...
            { _x == "200Rnd_65x39_cased_Box" } count (magazines _u)]) call MC8_LOG;
    } else { "CHACAL|AVERT|mg_assaut|aucun_tireur_disponible" call MC8_LOG };
};
...
if (MC8_MENACE_P5 in [2, 3]) then {
    private _tDec = time;
    waitUntil { sleep 0.5; (!isNil "MC8_SITUATION_P5_FAITE") || MC8_FIN || ((time - _tDec) > 10) };
};
if ((MC8_DELAI_MODE == 1) && { MC8_F_LEN > 0 }) then {
    // memes perceptions, memes calculs et meme instant que la ligne de decision ecrite juste apres
    private _defF = (if (isNil "MC8_EST_SITE") then {[]} else {MC8_EST_SITE}) select { alive _x };
    private _obsF = [
        (if (MC8_ALARME) then {1} else {0}),
        (if (MC8_T_ALARME >= 0) then { round (time - MC8_T_ALARME) } else { -1 }),
        (if (MC8_COMPROMIS) then {1} else {0}),
    ...
    private _res = [0, _obsF] call MC8_fnc_evalNoeud;
    private _valF = _res select 0;
    MC8_DELAI_JOUE = if (_valF > 0) then {180} else {45};
    [5, "DELAI_PORTEUR", [45, 180], MC8_DELAI_JOUE, "FORMULE", format ["|valeur_formule|%1|codes_lus|%2|codes_attendus|%3", _valF, _res select 1, MC8_F_LEN]] call MC8_fnc_decision;
} else {
    [5, "DELAI_PORTEUR", [45, 180], MC8_DELAI_PORTEUR, "IMPOSE"] call MC8_fnc_decision;
};
...
(format ["CHACAL|E|premier_pas_assaut|%1|tirs_appui_avant|%2|feu_avant|%3", round (time * 100) / 100,
    MC8_TIRS_APPUI, MC8_FEU_AVANT]) call MC8_LOG;
...
MC8_COMP_ASSAUT = if (MC8_FEU_AVANT == 1 || { MC8_TACTIQUE > 0 }) then {"AWARE"} else {"COMBAT"};
MC8_gAssaut setBehaviour MC8_COMP_ASSAUT; MC8_gAssaut setCombatMode "RED"; MC8_gAssaut setSpeedMode "FULL";
MC8_CIBLE_ASSAUT = +MC8_OUV_CHOISIE;
[MC8_gAssaut, MC8_OUV_CHOISIE, "FULL", MC8_COMP_ASSAUT, "WEDGE", "OUVERTURE_CHOISIE"] call MC8_fnc_ordreAller;
if (MC8_FEU_AVANT == 1 || { MC8_TACTIQUE > 0 }) then {
    [] spawn {
        while { !MC8_FIN && { MC8_PHASE == 5 } } do {
            sleep 10;
            if (count MC8_CIBLE_ASSAUT > 0 && { !isNull MC8_gAssaut }) then {
                MC8_gAssaut setBehaviour "AWARE";
                { if (alive _x) then { _x doMove MC8_CIBLE_ASSAUT } } forEach (units MC8_gAssaut);
                MC8_RELANCES = MC8_RELANCES + 1;
            };
        };
    };
};
...
private _rO = [MC8_gAssaut, MC8_OUV_CHOISIE, 55, [MC8_gAssaut, MC8_OUV_CHOISIE, 1.2] call MC8_fnc_budget] call MC8_fnc_arrive;
(format ["CHACAL|E|ouverture|%1|issue|%2|distance|%3|reste|%4", round (time * 100) / 100, _rO,
    round ((((units MC8_gAssaut) select { alive _x }) call MC8_fnc_centre) distance2D MC8_OUV_CHOISIE),
    round (call MC8_fnc_reste)]) call MC8_LOG;
...
// =====================================================================
private _ordre = MC8_OBJETS select { _x != MC8_PC };
_ordre = [_ordre, [], { _x distance2D MC8_OUV_CHOISIE }, "ASCEND"] call BIS_fnc_sortBy;
if (!isNull MC8_PC) then { _ordre pushBack MC8_PC };
...
(format ["CHACAL|E|file_objectifs|%1|%2|origines_z|%3", round (time * 100) / 100,
    ...
    (_ordre apply { round ((((boundingBoxReal _x) select 0) select 2) * 10) / 10 })]) call MC8_LOG;
...
"CHACAL|E|cqb|differe|le_nettoyage_est_une_etape_separee" call MC8_LOG;

private _hExpl = scriptNull;
{
    private _o = _x;
    if (MC8_FIN || { !alive _o }) then { continue };
    if (count ((units MC8_gAssaut) select { alive _x }) == 0) then {
        (format ["CHACAL|E|file_interrompue|%1|restants|0", round (time * 100) / 100]) call MC8_LOG;
        continue
    };
    ...
    private _cap = (180 * MC8_ECHELLE) min ((call MC8_fnc_reste) - MC8_RESERVE_FEU);
    if (_cap <= 0) then {
        (format ["CHACAL|E|charge_manquee|%1|%2|cause|PLAFOND_PHASE", round (time * 100) / 100, typeOf _o]) call MC8_LOG;
        continue
    };
    private _pt = [_o] call MC8_fnc_pointDePose;
    (format ["CHACAL|E|objectif|%1|%2|point_de_pose|%3|emprise|%4|cap|%5", round (time * 100) / 100,
        typeOf _o, _pt, round (_o call MC8_fnc_emprise), round _cap]) call MC8_LOG;
    MC8_CIBLE_ASSAUT = +_pt;
    [MC8_gAssaut, _pt, "FULL", MC8_COMP_ASSAUT, "WEDGE", "OBJECTIF_" + (typeOf _o)] call MC8_fnc_ordreAller;
    private _r = [MC8_gAssaut, _pt, 22, _cap, 120] call MC8_fnc_arrive;
    private _c0 = ((units MC8_gAssaut) select { alive _x }) call MC8_fnc_centre;
    ...
    if (_dG > 40) then {
        (format ["CHACAL|E|charge_manquee|%1|%2|cause|ELEMENT_N_ARRIVE_PAS|issue|%3|distance|%4",
            round (time * 100) / 100, typeOf _o, _r, _dG]) call MC8_LOG;
        continue
    };
    ...
    if (_o isEqualTo MC8_PC) then {
        "CHACAL|E|exploitation|differee|controle_positif_manquant" call MC8_LOG;
    };

    private _porteurs = (units MC8_gAssaut) select { alive _x && { "DemoCharge_Remote_Mag" in (magazines _x) } };
    if (count _porteurs == 0) then {
        (format ["CHACAL|E|charge_manquee|%1|%2|cause|AUCUN_PORTEUR_VIVANT", round (time * 100) / 100, typeOf _o]) call MC8_LOG;
        continue
    };
    ...
    _h doMove _pt;
    private _t = time;
    waitUntil { sleep 1; ((_h distance2D _pt) < 5) || !(alive _h) || (time - _t > MC8_DELAI_JOUE * MC8_ECHELLE) || MC8_FIN };
    private _dH = round (_h distance2D _pt);
    (format ["CHACAL|E|porteur|%1|%2|%3|temps|%4|distance|%5|delai|%6", round (time * 100) / 100, typeOf _o,
        (_h getVariable ["chacal_role", ""]), round (time - _t), _dH, MC8_DELAI_JOUE]) call MC8_LOG;
    if (!alive _h || { _dH > 12 }) then {
        (format ["CHACAL|E|charge_manquee|%1|%2|cause|PORTEUR_N_ARRIVE_PAS|%3|distance|%4", round (time * 100) / 100,
            typeOf _o, (_h getVariable ["chacal_role", ""]), _dH]) call MC8_LOG;
        if (alive _h && { _h != leader MC8_gAssaut }) then { _h doFollow (leader MC8_gAssaut) };
        continue
    };
    sleep (4 * MC8_ECHELLE);   // le geste a une duree
    ...
    MC8_CHARGES pushBackUnique _o;
    MC8_CHARGES_OBJ pushBack [_o, _c];
    (format ["CHACAL|E|charge_posee|%1|%2|par|%3|distance|%4|actes|%5|sur|%6", round (time * 100) / 100,
        typeOf _o, (_h getVariable ["chacal_role", ""]), _dH, count MC8_CHARGES, count MC8_OBJETS]) call MC8_LOG;
    if (_h != leader MC8_gAssaut) then { _h doFollow (leader MC8_gAssaut) };
} forEach _ordre;

// --- l exploitation est coupee AVANT le degagement ---
if (!isNull _hExpl) then {
    private _tX = time;
    waitUntil { sleep 2; scriptDone _hExpl || (time - _tX > 90 * MC8_ECHELLE) || MC8_FIN };
    if (!scriptDone _hExpl) then { terminate _hExpl };
    if (alive MC8_EXPLOITANT) then { MC8_EXPLOITANT doFollow (leader MC8_gAssaut) };
};

// --- UN SEUL degagement, puis TOUT saute ensemble ---
private _abri = MC8_OUV_CHOISIE getPos [70, MC8_SITE getDir MC8_OUV_CHOISIE];
MC8_CIBLE_ASSAUT = +_abri;
[MC8_gAssaut, _abri, "FULL", MC8_COMP_ASSAUT, "WEDGE", "DEGAGEMENT_AVANT_MISE_A_FEU"] call MC8_fnc_ordreAller;
(format ["CHACAL|E|degagement|%1|vers|%2|charges|%3", round (time * 100) / 100, _abri, count MC8_CHARGES_OBJ]) call MC8_LOG;
private _rD = [MC8_gAssaut, _abri, 30, ((call MC8_fnc_reste) - 60) max 45, 60] call MC8_fnc_arrive;
[_rD] call MC8_fnc_miseAFeu;
...
private _iss5 = if (count MC8_CHARGES >= count MC8_OBJETS) then {"ATTEINT"} else {
    if (count (MC8_FS select { alive _x }) == 0) then {"DETRUIT"} else {
    if ((call MC8_fnc_reste) <= 0) then {"PLAFOND"} else {"INCOMPLET"} } };
(format ["CHACAL|E|feu_appui_total|%1|tirs_appui|%2|relances_assaut|%3|feu_avant|%4", round (time * 100) / 100,
    MC8_TIRS_APPUI, MC8_RELANCES, MC8_FEU_AVANT]) call MC8_LOG;
[5, "ASSAUT", _iss5] call MC8_fnc_finPhase;
};
};
...
// la position REELLE des SURVIVANTS - `MC8_gFS` est vide depuis la scission,
// et la fonction rendait alors son defaut de 120 s pour 4 km a parcourir.
if (MC8_FIN) exitWith {};   // vignette close avant l exfiltration
MC8_EXFIL_POINT = if (MC8_ABANDON) then { MC8_LZ } else { MC8_PZ };
private _vitBudget = if (MC8_EXFIL == 2) then {1.2} else {1.8};
_plafond = [(MC8_FS select { alive _x }), MC8_EXFIL_POINT, _vitBudget] call MC8_fnc_budget;
[6, "EXFILTRATION", _plafond] call MC8_fnc_debutPhase;
...
if (MC8_EXFIL in [0, 1]) then {
    private _vD6 = MC8_FS select { alive _x };
    private _dD6 = if (count _vD6 > 0) then { round ((_vD6 call MC8_fnc_centre) distance2D MC8_EXFIL_POINT) } else { -1 };
    [6, "EXFIL_ALLURE", [0, 1], MC8_EXFIL, "IMPOSE", format ["|distance_point|%1", _dD6]] call MC8_fnc_decision;
};
(format ["CHACAL|E|budget|%1|etape|EXFIL|vers|%2|distance|%3|plafond|%4", round (time * 100) / 100,
    (if (MC8_ABANDON) then {"LZ"} else {"PZ"}),
    round (((MC8_FS select { alive _x }) call MC8_fnc_centre) distance2D MC8_EXFIL_POINT),
    round _plafond]) call MC8_LOG;

private _pourquoi = if (MC8_ABANDON) then {"ABANDON_REPLI_PAR_LA_LZ"} else {"OBJECTIFS_TRAITES"};
...
// L ancienne ligne ne liberait l appui QUE si MC8_APPUI_FIXE valait 1. Or le socle le cloue par sa
...
} forEach [MC8_gAssaut, MC8_gAppui, MC8_gBouchon, MC8_gReco, MC8_gFS];
...
MC8_SANS_JAMBES = 0;
{ if (alive _x && { !(_x checkAIFeature "PATH") }) then { MC8_SANS_JAMBES = MC8_SANS_JAMBES + 1 } } forEach MC8_FS;
(format ["CHACAL|E|exfil_jambes|%1|vivants|%2|sans_path|%3|seuil_exige|%4", round (time * 100) / 100,
    count (MC8_FS select { alive _x }), MC8_SANS_JAMBES, round (0.6 * MC8_EFFECTIF)]) call MC8_LOG;
if (MC8_SANS_JAMBES > 0) then {
    (format ["CHACAL|AVERT|exfil|hommes_vivants_sans_jambes|%1", MC8_SANS_JAMBES]) call MC8_LOG;
};
private _compExf = if (MC8_COMPROMIS) then {"COMBAT"} else {"AWARE"};
if (MC8_EXFIL in [0, 1]) then {
    (format ["CHACAL|E|choix_joue|%1|point|EXFIL_ALLURE|choix|%2|detail|%3|comportement_initial|%4", round (time * 100) / 100,
        MC8_EXFIL, (if (MC8_EXFIL == 1) then {"RAPIDE"} else {"PRUDENT"}), _compExf]) call MC8_LOG;
};
...
if (MC8_EXFIL == 1) then {
    [] spawn {
        private _t0 = time;
        waitUntil { sleep 5;
            private _v = MC8_FS select { alive _x };
            (count _v == 0) || MC8_FIN || ((time - _t0) > 180) ||
            ((_v call MC8_fnc_centre) distance2D MC8_SITE > 200) };
        if (MC8_FIN) exitWith {};
        private _v = MC8_FS select { alive _x };
        ...
        (format ["CHACAL|E|exfil_degage|%1|distance_site|%2|vivants|%3|comportement|AWARE",
            round (time * 100) / 100, round ((_v call MC8_fnc_centre) distance2D MC8_SITE),
            count _v]) call MC8_LOG;
    };
};
{
    if (!isNull _x) then {
        ...
        { if (alive _x) then { _x setUnitPos "AUTO" } } forEach (units _x);
        [_x, MC8_EXFIL_POINT, "FULL", _compExf, "WEDGE", _pourquoi] call MC8_fnc_ordreAller;
    };
} forEach [MC8_gAssaut, MC8_gAppui, MC8_gBouchon, MC8_gReco, MC8_gFS];

private _tE = time;
private _seuilExf = round (0.6 * MC8_EFFECTIF);
...
waitUntil { sleep 3;
    (count (MC8_FS select { alive _x && { (_x distance2D MC8_EXFIL_POINT) < 90 } }) >= _seuilExf) ||
    (count (MC8_FS select { alive _x }) < _seuilExf) || (time - _tE > _plafond) || MC8_FIN };
private _vivExf = count (MC8_FS select { alive _x });
if (_vivExf < _seuilExf) then {
    (format ["CHACAL|E|exfil_impossible|%1|vivants|%2|seuil|%3|temps_ecoule|%4", round (time * 100) / 100,
        _vivExf, _seuilExf, round (time - _tE)]) call MC8_LOG;
};
MC8_EXFILTRES = count (MC8_FS select { alive _x && { (_x distance2D MC8_EXFIL_POINT) < 90 } });
[6, "EXFILTRATION", (if (MC8_EXFILTRES >= _seuilExf) then {"ATTEINT"} else {
    if (count (MC8_FS select { alive _x }) == 0) then {"DETRUIT"} else {
    if (count (MC8_FS select { alive _x }) < _seuilExf) then {"PERTES_EXCESSIVES"} else {"PLAFOND"} } })] call MC8_fnc_finPhase;
