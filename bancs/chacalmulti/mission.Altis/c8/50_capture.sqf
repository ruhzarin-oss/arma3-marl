...
MC8_CAP_VERSION = 1;
if (isNil "MC8_NEXT_ID") then { MC8_NEXT_ID = 1 };
MC8_TICK = 0; MC8_LAST = -1;
MC8_MAXCAR = 850;
MC8_CHRONO = [0, 0];
MC8_DTV = 2;                                   // la perception se rejoue moins vite que la position

MC8_TOUS = { (allUnits + allDeadMen) select { private _c = _x getVariable ["multi_c", -1]; if (_c < 0) then { _c = _x call MULTI_fnc_cellule }; _c == 8 } };
MC8_fnc_side = {
    private _s = _this;
...
MC8_fnc_identifier = {
    ...
    _u setVariable ["chacal_id", MC8_NEXT_ID];
    _u setVariable ["chacal_side", (side _u) call MC8_fnc_side];
    _u setVariable ["chacal_fire", 0];
    (format ["CHACAL|E|spawn|%1|%2|%3|%4|%5", round (time * 100) / 100, MC8_NEXT_ID,
        (side _u) call MC8_fnc_side, typeOf _u,
        (_u getVariable ["chacal_role", ""])]) call MC8_LOG;
    _u addEventHandler ["Fired", {
        ...
        (format ["CHACAL|E|tir|%1|%2|%3|%4", round (time * 100) / 100,
            ...
            MC8_PHASE]) call MC8_LOG;
    }];
    MC8_NEXT_ID = MC8_NEXT_ID + 1;
    MC8_NEXT_ID - 1
};
...
MC8_fnc_identifierVehicule = {
    ...
    _v addEventHandler ["Fired", {
        ...
        (format ["CHACAL|E|tir|%1|%2|%3|%4|bord|%5", round (time * 100) / 100,
            ...
            MC8_PHASE, typeOf _veh]) call MC8_LOG;
    }];
};

MC8_fnc_recenser = {
    { _x call MC8_fnc_identifier } forEach (call MC8_TOUS);
    { if ((count (crew _x) > 0) && { ((crew _x select 0) call MULTI_fnc_cellule) == 8 }) then { _x setVariable ["multi_c", 8]; _x call MC8_fnc_identifierVehicule } } forEach vehicles;
};
call MC8_fnc_recenser;
[] spawn { while { !MC8_FIN } do { call MC8_fnc_recenser; sleep 2 } };
(format ["CHACAL|OK|recenseur|1|au_premier_passage|%1", MC8_NEXT_ID - 1]) call MC8_LOG;
...
MC8_EH_FRAME = addMissionEventHandler ["EachFrame", {
    if (MC8_CAP_VERSION != 1) exitWith {};
    private _t = time;
    if (_t - MC8_LAST < MC8_DT) exitWith {};
    MC8_LAST = _t;
    private _d0 = diag_tickTime;
    MC8_TICK = MC8_TICK + 1;
    private _tr = round (_t * 100) / 100;
    private _tous = call MC8_TOUS;
    private _n = count _tous;
    private _bouts = []; private _cour = "";
    {
        private _u = _x;
        private _id = _u getVariable ["chacal_id", -1];
        if (_id < 0) then { _id = _u call MC8_fnc_identifier };
        ...
        private _e = str [
            ...
            (_u call MC8_fnc_posture),
            (_u call MC8_fnc_comport),
            round (speed _u),
            (_u getVariable ["chacal_fire", 0])
        ];
        if ((_u getVariable ["chacal_fire", 0]) == 1) then { _u setVariable ["chacal_fire", 0] };
        if ((count _cour) + (count _e) + 1 > MC8_MAXCAR) then { _bouts pushBack _cour; _cour = "" };
        _cour = if (_cour == "") then { _e } else { _cour + "," + _e };
    } forEach _tous;
    _bouts pushBack _cour;
    { (format ["CHACAL|S|%1|%2|%3|%4|%5|[%6]", MC8_TICK, _forEachIndex, _tr, _n, MC8_PHASE, _x]) call MC8_LOG; } forEach _bouts;

    MC8_CHRONO set [0, (MC8_CHRONO select 0) + (diag_tickTime - _d0)];
    MC8_CHRONO set [1, (MC8_CHRONO select 1) + 1];
    if ((MC8_CHRONO select 1) % 400 == 0) then {
        (format ["CHACAL|C|cout_ms|%1|ticks|%2|unites|%3|fps|%4",
            round (1000 * (MC8_CHRONO select 0) / (MC8_CHRONO select 1) * 100) / 100,
            (MC8_CHRONO select 1), _n, round diag_fps]) call MC8_LOG;
        MC8_CHRONO = [0, 0];
    };
}];
"CHACAL|OK|emetteur|1" call MC8_LOG;

// ======================= 3. LES TROIS VUES =======================
MC8_T_VUES = time - (8 * 0.37);
MC8_EH_VUES = addMissionEventHandler ["EachFrame", {
    if (MC8_FIN || { MC8_CAP_VERSION != 1 } || { (time - MC8_T_VUES) < MC8_DTV }) exitWith {};
    MC8_T_VUES = time;
    call {
        private _tr = round (time * 100) / 100;
        private _vivants = (call MC8_TOUS) select { alive _x };
        ...
        (format ["CHACAL|VC|%1|%2|est_sur_ouest|%3", _tr, MC8_PHASE, str _cb]) call MC8_LOG;
        (format ["CHACAL|VC|%1|%2|ouest_sur_est|%3", _tr, MC8_PHASE, str _co]) call MC8_LOG;
        ...
        private _fnc = {
            ...
            (format ["CHACAL|VH|%1|%2|%3|%4", _tr, MC8_PHASE, _etiq, str _lignes]) call MC8_LOG;
        };
        ...
        private _vg = [];
        {
            ...
            {
                if ([_u, _x] call MC8_fnc_voit) then {
        ...
        (format ["CHACAL|VG|%1|%2|ouest|%3", _tr, MC8_PHASE, str _vg]) call MC8_LOG;

    };
}];
"CHACAL|OK|vues|1" call MC8_LOG;
...
[] spawn { while { !MC8_FIN } do { sleep 2; (format ["CHACAL|C|pouls|%1", round (time * 100) / 100]) call MC8_LOG } };

// ======================= 4. LES MORTS =======================
MC8_EH_MORT = addMissionEventHandler ["EntityKilled", {
    params ["_vic", "_tueur", ["_instig", objNull]];
    if ((_vic call MULTI_fnc_cellule) != 8) exitWith {};   // MULTI : une mort d une autre cellule ne s ecrit pas ici
    ...
    private _t = _tueur call MC8_fnc_idTueur;
    (format ["CHACAL|E|mort|%1|%2|%3|%4|%5|par|%6", round (time * 100) / 100,
        (_vic getVariable ["chacal_id", -1]),
        (_t select 0), MC8_PHASE,
        (_vic getVariable ["chacal_role", ""]), (_t select 1)]) call MC8_LOG;
}];
"CHACAL|OK|morts|1" call MC8_LOG;
...
MC8_fnc_arreterCapture = {
    if (MC8_CAP_VERSION == 0) exitWith {};
    MC8_CAP_VERSION = 0;
    removeMissionEventHandler ["EachFrame", MC8_EH_FRAME];
    removeMissionEventHandler ["EachFrame", MC8_EH_VUES];
    removeMissionEventHandler ["EntityKilled", MC8_EH_MORT];
    { _x removeAllEventHandlers "Fired" } forEach ((call MC8_TOUS) + (vehicles select { (_x getVariable ["multi_c", -1]) == 8 }));
    (format ["CHACAL|OK|capture_arretee|ticks|%1|t|%2", MC8_TICK, round (time * 100) / 100]) call MC8_LOG;
};

(format ["CHACAL|OK|capture|%1|dt|%2|dtv|%3", MC8_CAP_VERSION, MC8_DT, MC8_DTV]) call MC8_LOG;
