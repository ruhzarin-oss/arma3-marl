...
// ne vaut plus rien.
// =====================================================================
if (MC8_ORACLE_CMD <= 0) exitWith {};

MC8_O_BUDGET = MC8_ORACLE_B;
MC8_O_CIBLE = -1;
...
private _lz = MC8_LZ; private _rt = MC8_ROUTE; private _op = MC8_OP; private _st = MC8_SITE;
private _ra = if (count MC8_RALLY > 1) then { MC8_RALLY } else { _lz };
MC8_O_CASES = [
    ["POSER",   _lz],
...
MC8_O_B = _poids apply { _x / _sp };
MC8_O_PRIOR = +MC8_O_B;   // la doctrine, gardee a part : c est vers elle que revient le doute
// les cases que la patrouille de route peut atteindre, et le troncon de route le plus proche de chacune
MC8_O_RTE = MC8_O_CASES apply {
...
private _carte = [];
{
    ...
    _carte pushBack [(_x select 0), (if (count (MC8_O_RTE select _forEachIndex) > 0) then {1} else {0}), _d2];
} forEach MC8_O_CASES;
(format ["CHACAL|O|carte|%1|cases_routieres|%2|detail|%3", round (time * 100) / 100,
    count (MC8_O_RTE select { count _x > 0 }), str _carte]) call MC8_LOG;
...
MC8_O_fnc_yeux = {
    ...
    { if (!isNull _x) then { { if (alive _x) then { _y pushBack _x } } forEach (units _x) } } forEach
        ((MC8_GROUPES_EST + [MC8_gRoute]) select { !isNull _x });
    { if (alive _x && { !(_x in _y) }) then { _y pushBack _x } } forEach MC8_EST_SITE;
    _y
};

MC8_O_fnc_caseLaPlusProche = {
    params ["_p"];
    private _d = MC8_O_CASES apply { (_x select 1) distance2D _p };
...
MC8_O_fnc_croire = {
    ...
    for "_i" from 0 to (_n - 1) do {
        private _v = 0.70 * (MC8_O_B select _i);
        if (_i > 0) then { _v = _v + (0.30 * (MC8_O_B select (_i - 1))) } else { _v = _v + (0.30 * (MC8_O_B select _i)) };
        _bp pushBack _v;
    };
        ...
        if (!isNull _chef && { alive _chef }) then {
            {
                ...
                if (_kn select 0) then {
                    ...
                    if (_age < _fraicheur) then { _fraicheur = _age; _vu = [(_kn select 6)] call MC8_O_fnc_caseLaPlusProche };
                };
            } forEach MC8_FS;                                  // on parcourt la liste, on ne lit QUE sa connaissance
        };
    } forEach ((MC8_GROUPES_EST + [MC8_gRoute]) select { !isNull _x });
    // 3. ce qu il n a pas vu : une case regardee et vide devient moins probable
    private _yeux = call MC8_O_fnc_yeux;
    private _l = [];
    for "_i" from 0 to (_n - 1) do {
        private _p = (MC8_O_CASES select _i) select 1;
        private _q = 1;
        { _q = _q * (1 - ([_x distance2D _p] call MC8_O_fnc_pd)) } forEach _yeux;
        _l pushBack _q;
    };
    ...
    for "_i" from 0 to (_n - 1) do { _bp set [_i, ((1 - _nu) * ((_bp select _i) / _s)) + (_nu * (MC8_O_PRIOR select _i))] };
    MC8_O_B = _bp;
    _vu
};
...
MC8_O_fnc_agir = {
    params ["_cible"];
    private _p = (MC8_O_CASES select _cible) select 1;
    ...
    private _bR = MC8_O_B apply { 0 };
    { if (count (MC8_O_RTE select _forEachIndex) > 0) then { _bR set [_forEachIndex, _x] } } forEach MC8_O_B;
    private _mxR = selectMax _bR;
    private _candR = []; { if ((_x > 0) && { _x >= (_mxR - 0.01) }) then { _candR pushBack _forEachIndex } } forEach _bR;
    if (MC8_O_BUDGET > 0 && { count _candR > 0 } && { !isNull MC8_gRoute }
        && { ({ alive _x } count (units MC8_gRoute)) > 0 }) then {
        private _cibleR = if (MC8_O_CIBLE in _candR) then { MC8_O_CIBLE } else { selectRandom _candR };
        if (_cibleR != MC8_O_CIBLE) then {
            private _a = MC8_O_RTE select _cibleR;
            private _r2 = (_a nearRoads 350) select { (_x distance2D _a) > 250 };
            private _b2 = if (count _r2 > 0) then { getPosATL (_r2 select 0) } else { _a };
            while { (count (waypoints MC8_gRoute)) > 0 } do { deleteWaypoint ((waypoints MC8_gRoute) select 0) };
            private _w = MC8_gRoute addWaypoint [_a, 15];  _w setWaypointType "MOVE";  _w setWaypointSpeed "NORMAL";
            private _w2 = MC8_gRoute addWaypoint [_b2, 15]; _w2 setWaypointType "MOVE"; _w2 setWaypointSpeed "LIMITED";
            (MC8_gRoute addWaypoint [_a, 15]) setWaypointType "CYCLE";
            ...
            MC8_gRoute setCurrentWaypoint _w;
            MC8_O_BUDGET = MC8_O_BUDGET - 1;
            MC8_O_CIBLE = _cibleR;
            _action = format ["PATROUILLE_%1", (MC8_O_CASES select _cibleR) select 0];
        };
    };
    _action
};
...
[] spawn {
    waitUntil { sleep 2; MC8_FIN || { !isNil "MC8_TPHASE" } };
    while { !MC8_FIN } do {
        private _vu = call MC8_O_fnc_croire;
        private _b = +MC8_O_B;
        ...
        if ((random 1) < (MC8_ORACLE_EPS / 100)) then {
            private _c = +_b; _c set [_cible, -1];
            _cible = _c find (selectMax _c);
        };
        private _action = [_cible] call MC8_O_fnc_agir;
        private _dPat = if ((MC8_O_CIBLE >= 0) && { !isNull MC8_gRoute } && { !isNull (leader MC8_gRoute) }) then {
            round ((leader MC8_gRoute) distance2D (MC8_O_RTE select MC8_O_CIBLE)) } else { -1 };
        (format ["CHACAL|O|decision|%1|action|%2|case|%3|p|%4|vu|%5|budget|%6|croyance|%7|cible_patrouille|%8|patrouille_a|%9|vitesse_patrouille|%10",
            round (time * 100) / 100, _action, (MC8_O_CASES select _cible) select 0,
            round ((_b select _cible) * 100) / 100, (if (_vu >= 0) then { (MC8_O_CASES select _vu) select 0 } else { "RIEN" }),
            MC8_O_BUDGET, str (_b apply { round (_x * 100) }),
            (if (MC8_O_CIBLE >= 0) then { (MC8_O_CASES select MC8_O_CIBLE) select 0 } else { "AUCUNE" }), _dPat,
            (if (!isNull MC8_VEH_ROUTE) then { round (speed MC8_VEH_ROUTE) } else { -1 })]) call MC8_LOG;
        sleep (MC8_ORACLE_DELTA * MC8_ECHELLE);
    };
};
