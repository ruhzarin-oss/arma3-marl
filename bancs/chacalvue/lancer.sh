...
# ! COPIE DU BANC ( fumee de patch_banc_vue_fine, 18/09 ) : config serveur A PART, server_vue.cfg, refaite a chaque
# lancement depuis le server.cfg de l instance. Le server.cfg partage n est JAMAIS ecrit par ce lanceur.
SRC_CFG="$PROFIL/server.cfg"; CFG="$PROFIL/server_vue.cfg"
[ -f "$SRC_CFG" ] || { echo "profil hmtech$INST sans server.cfg"; exit 1; }
cp "$SRC_CFG" "$CFG"; sed -i 's/template = "CHACAL.Altis";/template = "CHACALVUE.Altis";/' "$CFG"
grep -q 'template = "CHACALVUE.Altis";' "$CFG" || { echo "server_vue.cfg : template non bascule"; exit 1; }
...
DEP=/mnt/data/hmt/depot/bancs/chacalvue/mission.Altis
MPM="/mnt/c/Program Files (x86)/Steam/steamapps/common/Arma 3/MPMissions/CHACALVUE.Altis"
