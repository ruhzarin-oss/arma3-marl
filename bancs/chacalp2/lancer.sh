...
# ! COPIE DU BANC : config serveur A PART ( server_p2.cfg ), refaite a chaque lancement depuis le server.cfg de l instance.
# Le server.cfg partage n est JAMAIS ecrit par ce lanceur.
SRC_CFG="$PROFIL/server.cfg"; CFG="$PROFIL/server_p2.cfg"
[ -f "$SRC_CFG" ] || { echo "profil hmtech$INST sans server.cfg"; exit 1; }
cp "$SRC_CFG" "$CFG"; sed -i 's/template = "CHACAL.Altis";/template = "CHACALP2.Altis";/' "$CFG"
grep -q 'template = "CHACALP2.Altis";' "$CFG" || { echo "server_p2.cfg : template non bascule"; exit 1; }
...
DEP=/mnt/data/hmt/depot/bancs/chacalp2/mission.Altis
MPM="/mnt/c/Program Files (x86)/Steam/steamapps/common/Arma 3/MPMissions/CHACALP2.Altis"
