...
EFFECTIF=$(lit effectif 10); ACCESSIBLE=$(lit accessible 0); TENIR=$(lit tenir 0); HMG=$(lit hmg -1); ASSAUT_X=$(lit assaut_x 100)   # -1 = valeur du palier ; 100 = plafond inchange
...
ecrire_param EFFECTIF "$EFFECTIF"; ecrire_param ACCESSIBLE "$ACCESSIBLE"; ecrire_param TENIR "$TENIR"; ecrire_param HMG "$HMG"; ecrire_param ASSAUT_X "$ASSAUT_X"; ecrire_param ARRET "$ARRET"
