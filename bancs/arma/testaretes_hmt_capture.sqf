...
HMT_KMAX = 10;                   // arêtes retenues par homme, les plus proches d'abord
    ...
    private _lignes = [];
    {
        if ((_forEachIndex % HMT_NTRANCHES) == HMT_TRANCHE) then {
            ...
            if (alive _a) then {
                ...
                // TRI SPATIAL, et surtout PLAFOND. Premier jet mesuré : 26 239 paires
                // distinctes, 110 ms par passage, serveur à 3,9 FPS. Dans un affrontement
                // serré tout le monde voit tout le monde, donc filtrer sur « le lien existe »
                // ne filtre RIEN — la ligne de vue à 51,8 µs se payait 26 000 fois.
                // Un soldat n'a pas besoin de 130 liens : ses plus proches ennemis portent la
                // décision. nearestObjects rend DÉJÀ trié par distance, on coupe dans l'ordre.
                private _nk = 0;                     // arêtes retenues pour cet homme
                private _nv = 0;                     // dont sondées géométriquement
                {
                    private _b = _x;
                    if (_nk >= HMT_KMAX) exitWith {};
                    if (alive _b && {(_b getVariable ["hmt_side",3]) != _sa}) then {
                        private _k = _a knowsAbout _b;
                        if (_k > 0.05) then {
                            _nk = _nk + 1;
                            // la géométrie ne se paie que sur les plus proches : c'est là
                            // qu'un couvert change une décision.
                            private _vue = -1;
                            ...
                            _lignes pushBack (format ["%1,%2,%3,%4",
                                _ida, (_b getVariable ["hmt_id",-1]),
                                (round (_k*100))/100, _vue]);
                        };
                    };
