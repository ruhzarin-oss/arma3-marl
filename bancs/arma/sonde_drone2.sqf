// ═══════════════════════════════════════════════════════════════════════════
//  sonde_drone2.sqf — CONTROLE POSITIF du lien drone -> soldat, TENTATIVE N 2.
//  Criteres deposes AVANT lancement : CONTROLE_POSITIF_DRONE_2.md
//  Tentative n 1 : ECHEC (P2 a 3/20 pour un seuil de <=2/20), elle reste un echec au registre.
//
...
HMT_DR_JOUER = {
    ...
    // ⚠️ 5 s, ET C EST LE SEUL CHANGEMENT DE LA TENTATIVE N 2. L attente valait 30 s pour
    // laisser l observateur aerien acquerir sa cible. Elle DOUBLAIT l exposition de l escouade
    // (30 s -> 60 s) et c est elle qui a fait tomber P2 : les 3 fautives portaient toutes un
    // temoin d ouverture NON NUL, elles avaient detecte PENDANT l attente. L observateur etant
    // retire, l attente n a plus d objet — il ne reste que le temps de poser les hommes.
    sleep 5;
    ...
    (format ["HMT|DR2|rep|%1|bras|%2|az|%3|tirs|%4|t1|%5|vue|%6|saitdrone|%7|saitgrp|%8|saitcamp|%9|tirsenn|%10|void|%11|saitgrp0|%12",
...
[] spawn {
    ...
    (format ["HMT|DR2|debut|reps|%1|fenetre|%2|dist|%3|socle|%4",
    ...
    for "_rep" from 1 to HMT_DR_REPS do {
        ...
        if (_az < 0) then {
            (format ["HMT|DR2|rep|%1|bras|AUCUN|az|-1|tirs|-1|t1|-1|vue|0|saitdrone|0|saitgrp|0|saitcamp|0|tirsenn|0|void|1|saitgrp0|0",
                     _rep]) call HMT_LOG;
        } else {
            // DEUX BRAS, PLUS AUCUN AERONEF. Le bras A1 est RETIRE : il existait pour
            // attraper une fuite native du DRONE vers le sol, or cette question est deja
            // repondue — pour un drone ce canal n existe pas (UAV knowsAbout = 0, meme
            // cloue a 148 m au-dessus de la cible). Tel que joue, A1 mesurait la fuite d un
            // HELICOPTERE A EQUIPAGE, qui n est pas l objet.
            [_rep, "A0", _az, objNull] call HMT_DR_JOUER;
            [_rep, "B",  _az, objNull] call HMT_DR_JOUER;
        };
    };

    "HMT|DR2|TERMINE" call HMT_LOG;
};
