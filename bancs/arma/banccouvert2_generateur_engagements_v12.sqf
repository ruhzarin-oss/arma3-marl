        ...
        if (count HMT_ACC < _maxAcc && {(random 1) < (5 / _periode)}) then {
            ...
            if (count _c > 0) then {
                ...
                if (_calib) then {
                    ...
                    if (missionNamespace getVariable ["HMT_COUVERT3", false]) then {
                        // GESTE N°3 : le professeur se jette a couvert au premier tir proche,
                        // se couche, repart apres 10 s ; le temoin continue DEBOUT, meme axe,
                        // meme lieu, meme effectif. UN SEUL axe des deux cotes : on mesure le
                        // GESTE, pas la manoeuvre.
                        // ⚠️ CETTE BRANCHE A ETE REECRITE LE 11/08. Elle avait disparu avec le
                        // generateur du n°3, ecrase le 10/08 par une copie de celui du n°2.
                        // Sans elle, le banc tirait au sort parmi QUATRE bras generiques
                        // (frontal / deux axes / figes) — il ne jouait plus le geste du tout.
                        _nAxes = 1; _fige = false;
                        HMT_C3_CE = (_d < 0.5);
                        if (missionNamespace getVariable ["HMT_CALIBRE", false]) then {
                            HMT_C3_CE = false;
                        };
                        // le bras est publie PAR ACCROCHAGE : c est `couvert_c3.sqf` qui
                        // applique le comportement, et il doit savoir a qui.
                        if (isNil "HMT_C3_BRAS") then { HMT_C3_BRAS = createHashMap };
                    } else {
                    if (missionNamespace getVariable ["HMT_BOND", false]) then {
                ...
                if (!isNil "HMT_C3_BRAS") then { HMT_C3_BRAS set [_n, missionNamespace getVariable ["HMT_C3_CE", false]] };
