...
// ===== ENUMERATION EXHAUSTIVE DES COMMANDES DU MOTEUR =====
if (isServer) then {
    ...
    [] spawn {
        sleep 25;
        private _all = supportInfo "";
        diag_log format ["HMT|CMD|TOTAL|%1", count _all];
        // on decoupe en paquets : diag_log tronque les lignes longues
        private _n = 0;
        {
            _n = _n + 1;
            diag_log format ["HMT|CMD|%1|%2", _n, _x];
        } forEach _all;
        diag_log "HMT|CMD|FIN";
    };
};
