// banc_escouade2.sqf — LE BANC D'ESCOUADE, REFAIT SUR CE QUE LE SMOKE TEST A MESURE.
//
...
if (isNil "HMT_DATA") exitWith { "HMT|ESC2|ECHEC|donnees_non_chargees" call HMT_LOG };
HMT_JALONS = [120, 90, 60, 40];
(format ["HMT|ESC2|debut|%1|configs", count HMT_DATA]) call HMT_LOG;
...
[] spawn {
    ...
    if (count _base == 0) exitWith { "HMT|ESC2|ECHEC|aucun_terrain" call HMT_LOG };
    (format ["HMT|ESC2|terrain|%1|%2|denivele|%3", round (_base select 0), round (_base select 1), _tol]) call HMT_LOG;
    ...
    HMT_REJOUE = {
        ...
        (format ["HMT|ESC2|essai|%1|%2|man|%3|fix|%4|jalons|%5|vus|%6|sur|%7|pas|%8|tirs|%9|arme_vers_man|%10|regard_vers_man|%11",
                 _cfg, _lib, (round (_kMan*100))/100, (round (_kFix*100))/100, str _jal,
                 _vus, count _man, _nmax, HMT_TIRS,
                 _armeMan call _moy, _regMan call _moy]) call HMT_LOG;
