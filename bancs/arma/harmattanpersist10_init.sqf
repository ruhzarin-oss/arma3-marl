...
// ===== greffe HARMATTAN : pont de controle + CAPTURE du corpus (ajoute par persist_lancer.sh) =====
if (isServer) then {
    if (fileExists "harmattan_actuator.sqf") then {
        call compile preprocessFileLineNumbers "harmattan_actuator.sqf";
    };
    private _v = "hmt_native" callExtension "version";
    if (_v != "") then {
        call compile preprocessFileLineNumbers "harmattan_actuator_native.sqf";
        HMT_EMIT = { diag_log _this; "hmt_native" callExtension ("o|" + _this); };
        diag_log format ["HARMATTAN_EXT %1 -> pont de controle actif", _v];
    } else {
        diag_log "HARMATTAN_EXT absente -> pas de pont, la capture tourne quand meme";
    };
    [] spawn { sleep 20; [10, 0.2] execVM "capture\hmt_capture.sqf";
               sleep 15; [1, 600, 1] execVM "capture\generateur_engagements.sqf"; };
};
