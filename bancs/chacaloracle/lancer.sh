...
grep -q 'template = "CHACALORACLE.Altis";' "$CFG" || { echo "server_oracle.cfg : template non bascule"; exit 1; }
ARMA='C:\Program Files (x86)\Steam\steamapps\common\Arma 3\arma3server_x64.exe'
[ -f "$CFG" ] || { echo "profil hmtech$INST sans server.cfg"; exit 1; }
...
ecrire_param() {   # ecrit la cle si elle existe, l'ajoute sinon
  local k=$1 v=$2
  if grep -q "CHACAL_$k = " "$CFG"; then
    sed -i "s/CHACAL_$k = [-0-9]*;/CHACAL_$k = ${v};/" "$CFG"
  else
    sed -i "0,/class Params/s//class Params/" "$CFG"
    sed -i "s/CHACAL_GRAINE = \([0-9]*\);/CHACAL_GRAINE = \1;\n            CHACAL_$k = ${v};/" "$CFG"
  fi
}
grep -q CHACAL_GRAINE "$CFG" || { echo "server.cfg de hmtech$INST sans CHACAL_GRAINE"; exit 1; }
...
ecrire_param PLACEUR "$PLACEUR"; ecrire_param BANC_APPUI "$BANC_APPUI"; ecrire_param ABLATION "$ABLATION"; ecrire_param SOCLE "$SOCLE"; ecrire_param AZIMUT "$AZIMUT"; ecrire_param GEOMETRIE "$GEOMETRIE"; ecrire_param OBS "$OBS"; ecrire_param AZIMUT_VAL "$AZIMUT_VAL"; ecrire_param EXFIL "$EXFIL"; ecrire_param TACTIQUE "$TACTIQUE"; ecrire_param ORACLE "$ORACLE"; ecrire_param MG_ASSAUT "$MG_ASSAUT"; ecrire_param DELAI_PORTEUR "$DELAI_PORTEUR"; ecrire_param APPUI_FIXE "$APPUI_FIXE"; ecrire_param FEU_AVANT "$FEU_AVANT"; ecrire_param APPUI_FEU "$APPUI_FEU"; ecrire_param EFFECTIF "$EFFECTIF"; ecrire_param ACCESSIBLE "$ACCESSIBLE"; ecrire_param TENIR "$TENIR"; ecrire_param HMG "$HMG"; ecrire_param ASSAUT_X "$ASSAUT_X"; ecrire_param ARRET "$ARRET"
...
ecrire_param OBSERVATION "$OBSERVATION"; ecrire_param CONTROLE_PERCEPTION "$CONTROLE_PERCEPTION"; ecrire_param SONDE "$SONDE"; ecrire_param ORACLE_CMD "$ORACLE_CMD"; ecrire_param ORACLE_B "$ORACLE_B"; ecrire_param ORACLE_NU "$ORACLE_NU"; ecrire_param ORACLE_EPS "$ORACLE_EPS"; ecrire_param ORACLE_DELTA "$ORACLE_DELTA"; ecrire_param ATTENTE_TEST "$ATTENTE_TEST"; ecrire_param CONTROLE_DIST "$CONTROLE_DIST"
...
echo "server.cfg : $(grep -o 'CHACAL_[A-Z_]* = [-0-9]*' "$CFG" | tr '\n' ' ') (bras=$BRAS_NOM)"
...
DEP=/mnt/data/hmt/depot/bancs/chacaloracle/mission.Altis
MPM="/mnt/c/Program Files (x86)/Steam/steamapps/common/Arma 3/MPMissions/CHACALORACLE.Altis"
...
LEVIERS_CONTROLES = ("depart","effectif","appui_feu","accessible","feu_avant","mg_assaut",
                     ...
                     "banc_appui","placeur","exfil","azimut","obs")
controles=[]
