...
CHACAL_O_fnc_agir = {
    ...
    private _bR = CHACAL_O_B apply { 0 };
    { if (count (CHACAL_O_RTE select _forEachIndex) > 0) then { _bR set [_forEachIndex, _x] } } forEach CHACAL_O_B;
    private _mxR = selectMax _bR;
    private _candR = []; { if ((_x > 0) && { _x >= (_mxR - 0.01) }) then { _candR pushBack _forEachIndex } } forEach _bR;
    if (CHACAL_O_BUDGET > 0 && { count _candR > 0 } && { !isNull CHACAL_gRoute }
        && { ({ alive _x } count (units CHACAL_gRoute)) > 0 }) then {
        private _cibleR = if (CHACAL_O_CIBLE in _candR) then { CHACAL_O_CIBLE } else { selectRandom _candR };
        if (_cibleR != CHACAL_O_CIBLE) then {
            private _a = CHACAL_O_RTE select _cibleR;
