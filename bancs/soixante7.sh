...
mkdir -p $L; rm -f $L/*
echo "=== 67 EPISODES — taille DERIVEE (9 morts/20 ep, il en faut 30) ==="
for i in $(seq 1 67); do
  rm -f /tmp/releve_live.npz
  bash lancer_live.sh > /dev/null 2>&1
  ok=0
  for w in $(seq 1 200); do [ -f /tmp/releve_live.npz ] && { ok=1; break; }; sleep 5; done
  if [ "$ok" = 1 ]; then
    ...
    cp /tmp/banc_live.txt $L/ep_${i}_ECHEC.txt 2>/dev/null; echo "  episode $i  ECHEC"
  fi
done
for p in $(pgrep -f arma3server_x64); do kill $p 2>/dev/null; done
pkill -f "[b]anc_live\.py" 2>/dev/null
echo "=== TERMINE : $(ls $L/ep_*.npz 2>/dev/null | wc -l) releves sur 67 ==="
