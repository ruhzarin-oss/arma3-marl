#!/usr/bin/env bash
# lancer_sonde_drone2.sh — monte la mission SondeDrone2.Stratis et lance SON instance.
# N installe rien sur une instance qui mesure : port, profil, cfg et log dedies.
# Usage : lancer_sonde_drone2.sh [reps]     (defaut 20 ; mettre 2 pour le smoke-test)
set -uo pipefail
SB=/mnt/data/harmattan-sandbox
REPO=/home/younes/arma3-marl
REPS=${1:-20}

MIS="$SB/arma3server/mpmissions/SondeDrone2.Stratis"
...
pkill -9 -f "staging/serverDR2\.cfg" 2>/dev/null; sleep 3
...
cp "$REPO/bancs/arma/sonde_drone2.sqf"   "$MIS/"
cat > "$MIS/init.sqf" <<EOF
if (isServer) then { [] spawn { sleep 5; HMT_DR_REPS = $REPS; execVM "sonde_drone2.sqf"; }; };
EOF

cat > "$SB/staging/serverDR2.cfg" <<'EOF'
hostname = "Harmattan Sonde Drone 2";
maxPlayers = 1;
persistent = 1;
class Missions { class M1 { template = "SondeDrone2.Stratis"; difficulty = "veteran"; }; };
EOF

mkdir -p "$SB/profilesDR2"; : > "$LOG"
...
HMT_EXT_PORT=5842 LD_LIBRARY_PATH=.:./linux64 nohup setsid ./"$BIN" \
  -config="$SB/staging/serverDR2.cfg" -profiles="$SB/profilesDR2" \
  -port=6084 -world=Stratis -autoInit -mod="$MODS" -serverMod="@LAMBS_Danger" \
  >> "$LOG" 2>&1 < /dev/null &
disown
