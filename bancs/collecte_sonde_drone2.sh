#!/usr/bin/env bash
# collecte_sonde_drone2.sh — collecte N repetitions COMPLETES en RELANCANT le serveur.
#
...
#
# Usage : collecte_sonde_drone2.sh [cible] [lot]      defauts : 20 reps, 1 par lot
set -uo pipefail
CIBLE=${1:-20}
LOT=${2:-1}
SB=/mnt/data/harmattan-sandbox
LOG=$SB/logs/sonde_drone2.out
CUM=$SB/logs/sonde_drone2_cumul.out
...
while [ "$fait" -lt "$CIBLE" ] && [ "$run" -lt 40 ]; do
  run=$((run + 1))
  /home/younes/arma3-marl/bancs/lancer_sonde_drone2.sh "$LOT" > /dev/null 2>&1
  ...
  for i in $(seq 1 60); do
    grep -aq "HMT|DR2|debut" "$LOG" && { demarre=1; break; }
    sleep 5
  done
  if [ "$demarre" -eq 0 ]; then
    echo "lot $run : LA SONDE N A PAS DEMARRE en 300 s -- $(tail -1 "$LOG" 2>/dev/null | cut -c1-90)"
    pkill -9 -f "staging/serverDR2\.cfg" 2>/dev/null; sleep 12
    continue
  fi

  # ── 2. on attend les 3 bras, la mort du serveur, ou le plafond ──
  for i in $(seq 1 60); do
    [ "$(grep -ac "HMT|DR2|rep|" "$LOG" 2>/dev/null)" -ge $((LOT * 2)) ] && break
    pgrep -f "staging/serverDR2\.cfg" > /dev/null || { sleep 5; break; }
    sleep 10
  done
  garde=0
  for r in $(seq 1 "$LOT"); do
    ...
    n=$(grep -ac "HMT|DR2|rep|$r|" "$LOG" 2>/dev/null)
    n=${n:-0}
    ...
    if grep -aq "HMT|DR2|rep|$r|bras|AUCUN" "$LOG" 2>/dev/null; then n=2; fi
    if [ "$n" -ge 2 ] && [ "$fait" -lt "$CIBLE" ]; then
      ...
      grep -a "HMT|DR2|debut" "$LOG" | tail -1 | sed "s/^/#rep$fait /" >> "$CUM"
      grep -a "HMT|DR2|rep|$r|" "$LOG" | sed "s/|rep|$r|/|rep|$fait|/" >> "$CUM"
    fi
  done
  if [ "$garde" -eq 0 ]; then
    echo "lot $run : DEMARRE mais repetition INCOMPLETE ($(grep -ac 'HMT|DR2|rep|' "$LOG" 2>/dev/null) lignes sur $((LOT * 2)))"
  else
