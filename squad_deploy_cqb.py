"""squad_deploy_cqb — FIRE & MOVEMENT integre sur le vrai complexe Arma, garnison DANS les batiments.
  - BASE DE FEU (AV, NFIRE) : politique apprise (obs 26-dim), pilotage setVelocity, supprime/approche.
  - ELEMENT D'ASSAUT (ASS, NASS) : pathfinder Arma (doMove -> buildingPos), entre et nettoie les retranches.
Sequence : suppression d'abord, assaut declenche apres ASSAULT_START cycles. Mesure garnison nettoyee + pertes.
...
S = 140.0; R = 60.0; ASSAULT_START = 6
...
# GARNISON : place les defenseurs (HMT_EN) DANS les batiments (buildingPos) -> intuables de l'exterieur
...
# BASE DE FEU (AV) : pilotage setVelocity (couper FSM/ANIM/AUTOCOMBAT, garder le tir), tableaux globaux
...
# ELEMENT D'ASSAUT (ASS) : pathfinder Arma ACTIF
...
time.sleep(6)
print("=== SQUAD DEPLOY CQB %s | srv%d | %d feu + %d assaut vs %d garnis dans batiments ===" % (CK, SRV, NFIRE, NASS, NDEF), flush=True)
...
for c in range(CYC):
    ...
    # ELEMENT D'ASSAUT : pathfinder Arma vers les defenseurs vivants (apres ASSAULT_START)
    if c >= ASSAULT_START:
        env.b.send('[] spawn { private _live = HMT_EN select {alive _x};'
                   ' { private _u=_x; if (count _live > 0) then { private _t=_live select (_forEachIndex % (count _live)); _u doMove (getPosATL _t); _u doTarget _t; }; } forEach (ASS select {alive _x}); };', wait=False)
    if c % 8 == 0:
        ls2 = env._query('diag_log format ["LIVES av=%1 ass=%2", ({alive _x} count AV), ({alive _x} count ASS)];', settle=0.15)
        av_a = ass_a = -1
        for l in ls2:
            mm = re.search(r"LIVES av=(\d+) ass=(\d+)", l)
            if mm:
                av_a = int(mm.group(1)); ass_a = int(mm.group(2))
        phase = "SUPPRESSION" if c < ASSAULT_START else "ASSAUT"
        print("  t=%4.0fs [%s] | dist_feu %3.0fm | garnison %d/%d | feu %d/%d assaut %d/%d" % (
            time.time() - t0, phase, md, n_alive, NDEF, av_a, NFIRE, ass_a, NASS), flush=True)

ls = env._query('diag_log format ["FINAL na=%1 av=%2 ass=%3", ({alive _x} count HMT_EN), ({alive _x} count AV), ({alive _x} count ASS)];', settle=0.3)
na = av_a = ass_a = -1
for l in ls:
    m = re.search(r"FINAL na=(\d+) av=(\d+) ass=(\d+)", l)
    if m:
        na = int(m.group(1)); av_a = int(m.group(2)); ass_a = int(m.group(3))
cc = Counter(act_hist); tot = max(len(act_hist), 1)
cleared = NDEF - na if na >= 0 else -1
print(">>> CQB %s srv%d | %.0fs | garnison NETTOYEE %d/%d (%.0f%%) | survivants feu %d/%d assaut %d/%d | issue=%s" % (
    CK, SRV, time.time() - t0, cleared, NDEF, 100 * cleared / NDEF if cleared >= 0 else -1, av_a, NFIRE, ass_a, NASS, outcome), flush=True)
...
print("SQUAD DEPLOY CQB FINI", flush=True)
