...
DEV = "cuda:0"
# JUGE DE PAIX : boucle EXACTE de train_hostage.py (qui a fait 80%), de zero, env actuel, pleine longueur.
# Sauvegarde a part (ne clobber PAS hostage_squad.pt).
...
def train(seed=0, iters=400, envs=4096, rollout=24, D=3):
    torch.manual_seed(seed)
    env = HostageEnv(num_envs=envs, A=9, D=D, device=DEV, seed=seed)
    ...
    for it in range(iters):
        ...
        for t in range(rollout):
            ...
            if dm.any():
                succ += info["success"][dm].float().sum().item(); wipe += info["squad_wipe"][dm].float().sum().item()
                pick += info["picked"][dm].float().sum().item(); nep += int(dm.sum())
    ...
    torch.save(net.state_dict(), "/home/younes/compose-embodiment/hostage_baseline.pt")
    print(">>> BASELINE finale : RESCOUSSE %.0f%%" % (100 * rr), flush=True)
    print("BASELINE FINI", flush=True)


print("=== JUGE DE PAIX : train_hostage exact, de zero, env actuel, 400 it ===", flush=True)
train()
