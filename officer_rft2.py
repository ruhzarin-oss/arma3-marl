...
DEV = "cuda:0"
OUT = "/home/younes/arma3-marl/officer_rft2_lora"
TABLE = "/home/younes/arma3-marl/officer_rft2_table.json"
POSTURES_OFF = ["prendre_colline", "defendre", "harceler"]   # le menu de l'officier

LEADS_TRAIN = [140., 110., 96., 75., 60., 38., 30., 20., 10., 0., -30., -45., -60., -96.]
...
def main():
    ...
    print("\n=== 2+3. CANDIDATS ENUMERES (correctif R1) ===", flush=True)
    print("  Le tirage a temperature 1 n'a JAMAIS produit 'harceler' (0 sur 256). Un RFT purement")
    print("  positif ne peut pas selectionner une action que la base n'emet pas : le goulot est")
    print("  l'EXPLORATION. L'espace d'actions ne compte que 3 postures -> on les ENUMERE toutes")
    print("  et on garde l'optimum MESURE. La recompense reste lue dans le monde, pas jugee par le LLM.")
    REPLI = 4
    garde = []
    for lead in LEADS_TRAIN:
        cible = oracle(tab, lead)
        garde += [(prompt_de(lead), cible)] * REPLI
        print("  lead %+6.0f -> %s" % (lead, cible), flush=True)
    print("  -> %d paires (%d situations x %d repliques)" % (len(garde), len(LEADS_TRAIN), REPLI))
