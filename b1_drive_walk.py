"""Comme b1_drive mais les soldats MARCHENT/COURENT (anime) : on pilote par doMove (Arma joue
l'animation + pathfinding) au lieu de setVelocity (qui faisait glisser). d6 commande, unites en place."""
...
WAKE = r'''
HMT_PICKED = 0;
{ _x enableAI "MOVE"; _x enableAI "PATH"; _x enableAI "TARGET"; _x enableAI "AUTOTARGET"; _x disableAI "AUTOCOMBAT"; _x disableAI "FSM"; _x setBehaviour "AWARE"; _x forceSpeed 4; _x setUnitPos "AUTO" } forEach HMT_FR;
{ _x enableAI "ALL"; _x setBehaviour "COMBAT"; _x setCombatMode "RED" } forEach HMT_GUARDS;
diag_log "HARMATTAN_WAKE walk-mode (doMove), gardes en combat";
'''
...
# WALK : doMove vers un point 25 m dans la direction du cap -> course animee + pathfinding
ACT_TPL = r'''
HMT_CX=20885; HMT_CY=16779; HMT_ACT = [__ACTS__];
...
def send_act(acts, timeout=15):
    ...
    n = b.send(sqf, wait=True, timeout=timeout); time.sleep(1.6)   # laisse le temps de marcher entre 2 ordres
