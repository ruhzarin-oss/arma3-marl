...
for etape in 50 100 150 200 250 300 350 400; do
  ...
  [ $iter0 -gt 0 ] && reprise="--reprise ckpt/v6b_g${GRAINE}_${iter0}.pt"
  $PY train_mission.py --iters 50 --envs 4096 --D 8 --seed $GRAINE --budget --reflexe \
      --Bmin 0.6 --Bmax 4.2 --iters_total $TOTAL --iter0 $iter0 \
      --out ckpt/v6b_g${GRAINE}_${etape}.pt $reprise 2>&1 | grep -vE "Warning|warn" | tail -1
  echo "--- BANC CERTIFIE, iteration $etape ---"
  bash eval_v6.sh ckpt/v6b_g${GRAINE}_${etape}.pt 1000
done
echo "V6_DONE"
