...
Criteres figes AVANT le run : leviathan/CRITERES_CHAMP2.md (empreinte CRITERES_CHAMP2).
...
REF_TEMOIN = {'prise': 0.873, 'expo': 0.0209, 'detour': 136.0}
...
ap.add_argument('--out', default='voir_champ2.json')
a = ap.parse_args()
DEV = a.device
...
def apprend(voyant, seed, etiquette):
    ...
    for r in range(1, a.rounds + 1):
        ...
        if r % 30 == 0 or r == a.rounds:
            ...
            torch.save(net.state_dict(), '%s/champ2_%s_g%d.pt' % (LEV, etiquette, seed))
    return net, piste


if __name__ == '__main__':
    if not os.path.exists(LEV + '/CRITERES_CHAMP2.md'):
        print('!! les criteres ne sont pas figes — ARRET.'); sys.exit(2)
    t0 = time.time()
    print('=== LE CHAMP : PLUS HABILE, OU SEULEMENT PLUS PAYEUR ? ===', flush=True)
    print('    2 bras x %d graines x %d rondes | criteres figes (CRITERES_CHAMP2)'
    ...
    for _n in ('temoin_arc', 'arc_plus_champ'):
    ...
    pap_t = pap('temoin_arc'); pap_s = pap('arc_plus_champ')
    gain = (pap_s / pap_t - 1.0) if pap_t and pap_t == pap_t and pap_t > 0 else float('nan')
    d_prise = pv - pa
    d_pertes = (moy('arc_plus_champ', 'pertes_par_prise') / moy('temoin_arc', 'pertes_par_prise') - 1.0) \
        if moy('temoin_arc', 'pertes_par_prise') else float('nan')
    ...
    if not ok_ref:
        ...
        print('  >>> LE CHAMP PAIE SUR LA PRISE-A-PERTES. Prise-a-pertes +%.0f %% (seuil +15 %%).' % (100 * gain), flush=True)
