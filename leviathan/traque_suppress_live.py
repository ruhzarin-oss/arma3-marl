#!/usr/bin/env python3
"""traque_suppress_live.py — A/B LIVE DE LA SUPPRESSION (derive de traque_reflex_live).
SUP=1 : la base de feu place un FEU NOURRI (doSuppressiveFire) sur le secteur ennemi -> cloue meme derriere couvert.
SUP=0 : baseline, tir vise seul (l ancien comportement). Mesure = survie FS + noeuds pris, contre ennemis LAMBS.
...
import sys, time, re, math, ast, os
...
SUP = os.environ.get("SUP", "1") == "1"   # A/B LIVE : SUP=1 la base de feu SUPPRIME (doSuppressiveFire) ; SUP=0 = tir vise seul (baseline aveugle)
...
def main():
    ...
    for step in range(STEPS):
        ...
        vx = [0.0] * N_FS; vy = [0.0] * N_FS; ncontact = 0; fire_cmds = []; supp_cmds = []
        ...
        for i in range(N_FS):
            ...
            if nthreat >= (99 if is_night else 4):                          # NUIT (pays aveugle, pas de QRF) : on n'evade plus, on ASSAILLE
                ...
                if SUP and threats:                                         # SUPPRESSION REELLE : feu nourri sur le centroide des menaces (clouer != tuer) -> LAMBS fait mordre
                    scx = sum(e[0] for e in threats) / len(threats); scy = sum(e[1] for e in threats) / len(threats)
                    supp_cmds.append((i, round(scx), round(scy)))
        ...
        for (fi, sx, sy) in supp_cmds:                                     # SUPPRESSION live : feu soutenu sur la position (doSuppressiveFire) -> cloue meme sans cible propre visible
            cmds.append('private _u=HMT_FS select %d; if (alive _u) then {_u doSuppressiveFire [%d,%d,0];};' % (fi, sx, sy))
        # sabotage
        sab = 0
        for i in range(N_FS):
        ...
        if nalive == 0: break
        time.sleep(1.0)
    print("=== FIN [SUP=%d] | survie %d/9 | coercion %.0f/88 | noeuds pris %d ===" % (SUP, sum(a[2] for a in sf), coercion, len(dead)), flush=True)
