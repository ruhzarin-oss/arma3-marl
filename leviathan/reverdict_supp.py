...
ap.add_argument('--out', default='reverdict_supp.json')
a = ap.parse_args()
...
if __name__ == '__main__':
    if not os.path.exists(LEV + '/CRITERES_REVERDICT.md'):
        print('!! les criteres ne sont pas figes — ARRET.'); sys.exit(2)
    print('=== RE-VERDICT : la courbe n1 suffit-elle a retourner le verdict ? ===', flush=True)
    ...
    if ok_nouveau and not ok_ancien:
        print('  >>> LE MONDE MESURE REND LE VERDICT, L ANCIEN NON.', flush=True)
        print('      La courbe n1 suffit a retourner le verdict n1. L approche est validee.', flush=True)
        print('      On avance SANS retoucher les courbes.', flush=True)
    elif ok_nouveau and ok_ancien:
        print('  >>> LES DEUX MONDES rendent le verdict.', flush=True)
        print('      L ancien monde le rendait DEJA : toute l analyse est a reprendre.', flush=True)
        print('      Il faut comprendre pourquoi le sandbox echouait ailleurs.', flush=True)
    elif verdicts['nouveau']['rapport_prise'] > 1.0:
        print('  >>> BONNE DIRECTION, MAUVAIS RAPPORT (prise x%.2f, cout x%.2f).'
        ...
        print('  >>> MAUVAISE DIRECTION : la courbe n1 SEULE ne suffit pas.', flush=True)
        print('      L ingredient manquant est tres probablement la SUPPRESSION :', flush=True)
        print('      l element de fixation ne sert a rien si le feu ne fixe pas.', flush=True)
        print('      C est un RESULTAT, pas un echec — il justifie et priorise la courbe n2.', flush=True)

    json.dump({'criteres': 'CRITERES_REVERDICT.md (dcb0a5d9e57d70a5)',
