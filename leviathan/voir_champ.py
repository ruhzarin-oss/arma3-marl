...
Criteres figes AVANT le run : leviathan/CRITERES_CHAMP.md (empreinte CRITERES_CHAMP).
...
SEUIL_EXPO = 0.0180
SEUIL_DETOUR = 155.0                          # moitie de l'ecart comblee
...
ap.add_argument('--out', default='voir_champ.json')
a = ap.parse_args()
DEV = a.device


def monde(champ, seed, ne):
    return AssaultTerrain(num_envs=ne, A=4, D=4, seed=seed, device=DEV, max_steps=60,
                          ...
                          degat_par_impact=DEG, arc_obs=True, champ_risque=champ)
...
def apprend(voyant, seed, etiquette):
    ...
    for r in range(1, a.rounds + 1):
        ...
        if r % 30 == 0 or r == a.rounds:
            ...
            torch.save(net.state_dict(), '%s/champ_%s_g%d.pt' % (LEV, etiquette, seed))
    return net, piste


if __name__ == '__main__':
    if not os.path.exists(LEV + '/CRITERES_CHAMP.md'):
        print('!! les criteres ne sont pas figes — ARRET.'); sys.exit(2)
    t0 = time.time()
    print('=== LE PRIX DU TERRAIN CHANGE-T-IL LE MOMENT DU DETOUR ? ===', flush=True)
    print('    2 bras x %d graines x %d rondes | criteres figes (CRITERES_CHAMP)'
          % (a.graines, a.rounds), flush=True)
    res = {'reperes': {}, 'bras': {'temoin_arc': [], 'arc_plus_champ': []}}
    ...
    for voyant, nom in ((False, 'temoin_arc'), (True, 'arc_plus_champ')):
    ...
    print('  %-16s %10s %12s %14s' % ('', 'prise', 'expo/m', 'detour a'), flush=True)
    for nom in ('temoin_arc', 'arc_plus_champ'):
    ...
    ea, ev = moy('temoin_arc', 'expo_par_metre'), moy('arc_plus_champ', 'expo_par_metre')
    pa, pv = moy('temoin_arc', 'prise'), moy('arc_plus_champ', 'prise')
    print('', flush=True)
    ...
    elif ev <= SEUIL_EXPO and pv >= pa - 0.05 and moy('arc_plus_champ', 'distance_du_detour') >= SEUIL_DETOUR:
        print('  >>> LE PRIX DU TERRAIN SUFFIT. Exposition %.4f (seuil %.4f), detour repousse' % (ev, SEUIL_EXPO), flush=True)
        print('      a %.0f m (seuil %.0f). L agent manoeuvre desormais AU TARIF DU LONG.'
              % (moy('arc_plus_champ', 'distance_du_detour'), SEUIL_DETOUR), flush=True)
        print('      Le temoin de mecanisme CONFIRME l explication, il ne fait pas que la tolerer.', flush=True)
    elif ev < ea - 0.004:
