...
for etape in 100 200 300 400; do
  ...
  [ $iter0 -gt 0 ] && reprise="--reprise ckpt/lag_g${G}_${iter0}.pt"
  $PY train_mission.py --iters 100 --envs 4096 --D 8 --seed $G --budget --lagrangien \
      --Bmin 0.6 --Bmax 4.2 --iters_total 400 --iter0 $iter0 \
      --out ckpt/lag_g${G}_${etape}.pt $reprise 2>&1 | grep -vE "Warning|warn" | grep -E "lambda|it +[0-9]+ \|" | tail -2
  for b in 0.9 2.2; do
    $PY banc_mission.py --pilote "reseau:ckpt/lag_g${G}_${etape}.pt" --episodes 2048 \
        --graine 1000 --pas 80 --budget $b \
        --journal "journaux/lag_g${G}_${etape}_b${b}.jsonl" >/dev/null 2>&1
  done
  $PY lire_lag.py "lag_g${G}_${etape}"
done
echo "LAG3B_DONE"
