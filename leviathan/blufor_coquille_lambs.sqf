...
HMT_WARM = {
    ...
    {
        _x disableAI "AUTOCOMBAT"; _x disableAI "FSM";       // <-- SEULEMENT ca (on garde visee/tir/path/anim)
        _x setBehaviour "COMBAT"; _x setCombatMode "RED"; _x setSpeedMode "FULL"; _x forceSpeed -1;
        (group _x) enableAttack false; (group _x) allowFleeing 0;
        _x setVariable ["HMT_WSHELL", true];
        _x addEventHandler ["Fired", { (_this select 0) forceSpeed 3; }];   // tire -> ralentis pour viser
        _x addEventHandler ["Suppressed", {                                  // supprime -> baisse la posture
            params ["_u", "", "_shooter"]; private _p = unitPos _u;
            if (_p isEqualTo "DOWN") exitWith {};
            if (_p isEqualTo "MIDDLE" && {_u distance2D _shooter > 30}) exitWith { _u setUnitPos "DOWN" };
            _u setUnitPos "MIDDLE";
        }];
    } forEach HMT_WPILOT;
    HMT_WAGENT_MODE = true;
    (format ["HARMATTAN_WARM pilots=%1 mode=LAMBS", count HMT_WPILOT]) call HMT_EMIT;
};
