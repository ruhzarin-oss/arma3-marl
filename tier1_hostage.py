...
DEV = "cuda:0"
CKPT = "/home/younes/compose-embodiment/hostage_squad_d6.pt"   # warm-start D=6 (69%)
...
def train(iters=350, envs=4096, rollout=24, D=6):
    ...
    for it in range(iters):
        ...
        rr = succ / max(nep, 1)
        if it % 25 == 0:
            print("  it %3d | RESCOUSSE %.0f%% | pickup %.0f%% | OTAGE TUE %.0f%% | escouade detruite %.0f%%" % (
                it, 100 * rr, 100 * pick / max(nep, 1), 100 * hdead / max(nep, 1), 100 * wipe / max(nep, 1)), flush=True)
    torch.save(net.state_dict(), OUT)
    print(">>> TIER 1 final : RESCOUSSE %.0f%% (otage vivant exige)" % (100 * rr), flush=True)
    print("TIER1 FINI", flush=True)


print("=== TIER 1 : l'otage peut MOURIR (tir fratricide + exposition exfil) — warm-start D=6 ===", flush=True)
train()
