...
def train(suffer, D=2, hit=0.16, iters=200, envs=8192, rollout=16, seed=0):
    ...
    env = AssaultTerrain(num_envs=envs, A=1, D=D, relief=40.0, hit=hit, max_steps=60, device=DEV, seed=seed, shell_obs=True, suffer=suffer)
...
print("=== AGENT SOUFFRANCE : suffer off vs on, 5 graines, D=2 h0.16 ===", flush=True)
for suffer in [False, True]:
    sc = [train(suffer, seed=s) for s in range(5)]
    succ = sum(1 for x in sc if x > 0.4)
    print(">>> suffer=%s : scores=%s | succes %d/5" % (suffer, [round(100*x) for x in sc], succ), flush=True)
