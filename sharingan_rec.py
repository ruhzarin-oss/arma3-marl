"""sharingan_rec v3 — ENREGISTREUR INCARNE. On donne a l'agent des YEUX, des OREILLES et un RESSENTI, puis on
capture les demos corps-entier de LAMBS. Obs = 14 features :
  base (10) : ox oy dgx dgy alive adx ady edx edy  + [9] SUPPRESSION (oreille : sous le feu, getSuppression)
  + [10] FATIGUE (ressenti : souffle, getFatigue)
  + [11] POSTURE (ressenti : stance STAND/CROUCH/PRONE -> 0/.5/1)
  + [12,13] ENNEMI CONNU dx,dy (oreille/memoire : findNearestEnemy + knowsAbout, meme hors LOS)
Action inferee : 0 tenir 1 avancer 2 suppresser(tir=delta munitions) 3 couvert(posture). Corpus -> npz."""
...
X, Y = [], []
print("=== Sharingan v3 INCARNE : %d episodes, maitre = LAMBS (yeux+oreilles+ressenti, obs=14) ===" % EPISODES, flush=True)
for ep in range(EPISODES):
    ...
    for st in range(STEPS):
        ...
        bo = base_obs(ax, ay, al, OBJ[0], OBJ[1], epx, epy, eal)
        bo[:, 9] = asup                                                            # [9] oreille : sous le feu
        kdx = np.where(ahk > 0, (akx - ax) / SCALE, 0.0); kdy = np.where(ahk > 0, (aky - ay) / SCALE, 0.0)
        stc = np.array([STANCE.get(p, 0.0) for p in apos])
        obs = np.concatenate([bo, afat[:, None], stc[:, None], kdx[:, None], kdy[:, None]], 1).astype(np.float32)  # 14
        ...
        for i in range(N_ATQ):
            ...
            if fired[i]:
                ...
                a = 0
            X.append(obs[i]); Y.append(a)
        prev_amm = np.where(amm >= 0, amm, prev_amm)
        time.sleep(0.4)
    print("ep %d/%d -> %d paires" % (ep + 1, EPISODES, len(X)), flush=True)
X = np.array(X, np.float32); Y = np.array(Y, np.int64)
np.savez("/home/younes/arma3-marl/staff/sharingan_demos14.npz", X=X, y=Y)
print("CORPUS INCARNE : %d paires | obs dim %d | distribution %s" % (len(X), X.shape[1], dict(collections.Counter(Y.tolist()))), flush=True)
print("REC FINI")
