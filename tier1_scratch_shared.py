...
DEV = "cuda:0"
# DIAGNOSTIC : recompense PARTAGEE (rw.mean), DE ZERO, env actuel, D=3, ff=exf=0.
# Compare a la v1 (80%) pour isoler : est-ce la recompense per-agent qui tuait la rescousse ?
...
def train(iters=150, envs=4096, rollout=24, D=3):
    torch.manual_seed(0)
    ...
    net = Net(O, NA, 512, 3).to(DEV)   # DE ZERO
    ...
    for it in range(iters):
        ...
        for t in range(rollout):
            ...
            nobs, rw, done, info = env.step(act); rwm = rw.mean(-1)   # PARTAGE
        ...
        rr = succ / max(nep, 1)
        if it % 15 == 0:
            print("  it %3d | RESCOUSSE %.0f%% | pickup %.0f%% | detruite %.0f%%" % (it, 100 * rr, 100 * pick / max(nep, 1), 100 * wipe / max(nep, 1)), flush=True)
    print(">>> PARTAGE de zero, final : RESCOUSSE %.0f%%" % (100 * rr), flush=True)
    print("SCRATCH SHARED FINI", flush=True)


print("=== DIAGNOSTIC : recompense PARTAGEE, de zero, env actuel, D=3 ===", flush=True)
train()
