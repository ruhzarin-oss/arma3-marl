"""train_role — COUCHE officier : l'OFFICIER assigne des ROLES (appui/assaut) + recompense la STRUCTURE
feu+mouvement (coordination COMMANDEE top-down). Test : equipe a roles assignes vs MELEE emergente.
role_obs=True (officier structure) vs False (emergent, le baseline). Memes grille+tache+reseau."""
...
def train(role, iters=300, envs=4096, rollout=16, lr=3e-4, gamma=0.99, gae=0.95, clip=0.2,
    ...
    env = AssaultTerrain(num_envs=envs, relief=relief, hit=hit, grid_obs=True, role_obs=role, device=dev, seed=seed)
    ...
    print("=== %s | obs_dim=%d ===" % ("OFFICIER (roles+structure)" if role else "EMERGENT", O), flush=True)
    ...
    torch.save(net.state_dict(), "/home/younes/arma3-marl/assault_%s.pt" % ("role" if role else "emerg"))
    return last, lastl
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    rr, lr_ = train(True, iters=a.iters, envs=a.envs, seed=a.seed)
    re, le = train(False, iters=a.iters, envs=a.envs, seed=a.seed)
    print("\n===== VERDICT OFFICIER (roles structures vs emergent) =====")
    print("OFFICIER (roles) : neutralises %.0f%% | pertes %.0f%%" % (100 * rr, 100 * lr_))
    print("EMERGENT         : neutralises %.0f%% | pertes %.0f%%" % (100 * re, 100 * le))
    print("ecart neutralises = %+.0f pts | delta pertes = %+.0f pts" % (100 * (rr - re), 100 * (lr_ - le)))
    print(">>> %s" % ("L'OFFICIER (structure) PAIE -> moins de pertes / plus de neutralise" if (rr - re) >= 0.08 or (le - lr_) >= 0.08 else
          "la structure imposee n'aide pas a roles FIXES -> il faut l'officier ADAPTATIF (config selon situation)"))
    print("ROLE FINI")
