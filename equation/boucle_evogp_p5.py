...
Un tour : apprendre sur les episodes d'exploration -> poser 3 jobs sur UN serveur ( 45 s, 180 s, formule ) -> attendre ->
...
def main():
    ...
    for tour in range(debut, 1 if o.fumee else o.tours):
        ...
        w = [MONDES[(2 * tour) % len(MONDES)], MONDES[(2 * tour + 1) % len(MONDES)]]
        menace = 3 if tour % 2 == 0 else 0
        base = {k: v for k, v in g.items() if not re.fullmatch(r"f\d+", k)}
        base.update(campagne=CAMPAGNE, instance=INSTANCE, graines=w, repetitions=2 if not o.fumee else 1, menace_p5=menace,
                    delai_mode=0, f_len=0)
        jobs = []
        codes = res["codes"]
        formule_job = dict(base, delai_mode=1, delai_porteur=45, f_len=len(codes), **{f"f{k}": c for k, c in enumerate(codes)},
                           version=f"BOUCLE-T{tour:02d}-FORMULE-g{w[0]}g{w[1]}",
                           note=f"Boucle EvoGP tour {tour} : formule {res['formule']} ( parcimonie {res['parcimonie']} ), menace_p5 {menace}.")
        if o.fumee:
            jobs = [(f"2026-09-17_BOUCLE_FUMEE_FORMULE.json", dict(formule_job, campagne="FUMEE-BOUCLE-EVOGP-17-09", version="FUMEE-BOUCLE-FORMULE"))]
        else:
            for delai in (45, 180):
                jobs.append((f"2026-09-17_BOUCLE_T{tour:02d}_E{delai}.json",
                             dict(base, delai_porteur=delai, version=f"BOUCLE-T{tour:02d}-E{delai}-g{w[0]}g{w[1]}",
                                  note=f"Boucle EvoGP tour {tour} : exploration, delai impose {delai} s, menace_p5 {menace}.")))
            jobs.append((f"2026-09-17_BOUCLE_T{tour:02d}_FORMULE.json", formule_job))
        for rang, (nom, job) in enumerate(jobs):
            poser(nom, job, rang)
        journal(dict(tour=tour, evenement="jobs poses", jobs=[n for n, _ in jobs], mondes=w, menace_p5=menace))
        t0 = time.time()
