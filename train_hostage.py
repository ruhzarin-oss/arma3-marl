    ...
    for it in range(iters):
        ...
        for t in range(rollout):
            ...
            if dm.any():
                ...
                lost += info["hdead"][dm].float().sum().item(); pick += info["picked"][dm].float().sum().item(); nep += int(dm.sum())
        ...
        rr = succ / max(nep, 1)
        if it % 30 == 0:
            print("  it %3d | RESCOUSSE %.0f%% | pickup %.0f%% | escouade aneantie %.0f%%"
                  % (it, 100 * rr, 100 * pick / max(nep, 1), 100 * wipe / max(nep, 1)), flush=True)
    torch.save(net.state_dict(), "/home/younes/compose-embodiment/hostage_squad.pt")
    return rr


print("=== ENTRAINEMENT RESCOUSSE OTAGE : escouade 9 vs 6 gardes, carte Arma ===", flush=True)
r = train()
print(">>> RESCOUSSE finale : %.0f%%" % (100 * r), flush=True)
print("HOSTAGE FINI", flush=True)
