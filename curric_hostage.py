...
DEV = "cuda:0"
CKPT = "/home/younes/compose-embodiment/hostage_squad.pt"   # base D=3
...
def stage(net, opt, D, iters=150, envs=4096, rollout=24):
    env = HostageEnv(num_envs=envs, A=9, D=D, device=DEV, seed=0)
    O, A = env.obs_dim, env.A; obs = env.reset(); rr = 0.0
    for it in range(iters):
        ...
        rr = succ / max(nep, 1)
        if it % 50 == 0:
            print("   D=%d it %3d | RESCOUSSE %.0f%% | pickup %.0f%% | aneantie %.0f%%" % (D, it, 100 * rr, 100 * pick / max(nep, 1), 100 * wipe / max(nep, 1)), flush=True)
    return rr


probe = HostageEnv(num_envs=1, A=9, D=3, device=DEV, seed=0)
net = Net(probe.obs_dim, probe.n_actions, 512, 3).to(DEV); net.load_state_dict(torch.load(CKPT, map_location=DEV))
opt = torch.optim.Adam(net.parameters(), lr=3e-4)
print("=== CURRICULUM PAR PALIERS : D=4 -> 5 -> 6 (base = D=3 a 80%) ===", flush=True)
for D in [4, 5, 6]:
    r = stage(net, opt, D)
    print(">>> PALIER D=%d termine : RESCOUSSE %.0f%%" % (D, 100 * r), flush=True)
    torch.save(net.state_dict(), "/home/younes/compose-embodiment/hostage_squad_d%d.pt" % D)
