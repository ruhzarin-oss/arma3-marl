...
NH = 128
# ⚠️ MESURE, PAS CHOIX ⟨sonde_phi, 26/08⟩. Mediane de trois trajectoires reelles (politique
# figee 0,355 · frontal 0,106 · flanc 0,352) : a ce poids le faconnage pese PAR PAS autant
# que le terme de progression du depot, ni plus ni moins. `frontal` s ecarte parce que c est
# une doctrine suicidaire a dix fois le danger — les deux survivantes s accordent.
W_PHI = 0.352
...
def phi(e):
    """Φ(s) = MOINS le danger moyen des hommes VIVANTS. -> (N,)

    Par environnement, comme le terme de progression du depot — la recompense de `boucle`
    est par environnement, pas par homme.
    """
    al = e._aalive().float()
...
def jouer_phi(e, choisir, garder=False, w_phi=0.0):
    """COPIE FIDELE de `boucle.jouer`, augmentee du seul terme de faconnage par potentiel.

    F = γ·Φ(s′) − Φ(s), la forme exacte que le depot utilise deja pour la progression, et
    dont le theoreme ⟨Ng, Harada & Russell 1999⟩ garantit qu elle NE DEPLACE PAS la politique
    optimale — meme si Φ est mal calibre. C est ce qui la rend LEGALE malgre le refus depose
    d un terme d exposition (la sandbox sur-tarife de +856 % contre +75 % sur Arma) : le
    sur-tarif deplacerait un terme de RECOMPENSE, il ne deplace pas un POTENTIEL.
    AUCUN CLAMP : c est le `clamp(min=0)` qui avait rendu la recompense pompable (AVEU du 11/08).
    """
    o = e.reset(); N = e.N
    ...
    phi_prec = phi(e)
    dg_cum = torch.zeros(N, device=e.dev)          # danger cumule : le temoin de mecanisme
    lps, vals, rs, masques = [], [], [], []
    for t in range(B.PAS):
        ...
        if w_phi:
            ...
            phi_prec = ph
        dg_cum = dg_cum + (-phi(e) if not w_phi else -phi_prec) * viv
        pris |= neuf
        if garder:
        ...
        if bool(fini.all()):
            break
    metres = (d0 - dmin).mean(); tenus = (d0 - dprec)
    stats = dict(prise=100.0 * float(pris.float().mean()),
                 metres=float(metres), metres_tenus=float(tenus.mean()),
                 # LE TEMOIN DE MECANISME : le danger paye par metre gagne. Un score qui
                 # monte SANS que ce nombre baisse = faconnage POMPE, et il faut un aveu.
                 danger_par_metre=float((dg_cum / tenus.clamp(min=1.0)).mean()))
...
BRAS = {"A":  (PolVecteur, False,  False, 0.0),   "B":  (PolCNN,     True,   False, 0.0),
        "C":  (PolCNN,     True,   True,  0.0),   "D":  (PolAplati,  True,   False, 0.0),
        "E":  (PolEntites, "ent",  False, 0.0),   "E2": (PolEntites, "ent",  True,  0.0),
...
def entrainer(bras, graine, iters=140, n=256, lr=3e-4, eval_tous=0, film=None):
    ...
    Cls, avec_r, brouille, wphi = BRAS[bras]
...
def doctrines(graines, n=256):
    ...
    for nom, doc in (("frontal", B.frontal), ("flanc", B.flanc)):
        ...
        for g in graines:
            e = B.monde(n, g)
            st, *_ = jouer_phi(e, lambda o, tt, _d=doc, _e=e: (_d(_e, tt), None, None))
...
if __name__ == "__main__":
    ...
    for b in a.bras:
        ...
        for g in a.graines:
            ...
            # ⚠️ LE BANC SAUVEGARDE SES POLITIQUES DESORMAIS. La faute est nommee : la greffe
            # a du tourner sur un artefact du depot faute d avoir garde les miennes.
            torch.save(pol.state_dict(), "/mnt/data/pol_%s_%d.pt" % (b, g))
            p, t, d = evaluer(pol, b, B.GRAINES_TEST, n=a.n)
