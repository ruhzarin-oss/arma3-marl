...
print("\n  CALIBRATION DE `feu_sur_connu` SUR LE VERDICT DES +75 %")
print("  " + "=" * 68)
print(f"    {'bouton':>8}{'surmortalite du VU':>22}{'n vus':>9}{'n non vus':>11}")
best, becart = None, 1e9
for v in (0.0, 0.05, 0.10, 0.15, 0.20, 0.30, 0.45, 0.60):
    s, nv, nn = contraste(v)
    if s is None:
...
if best is None:
    ...
    print(f"    Retenu : feu_sur_connu = {best:.2f}  -> surmortalite {s:.1f} % "
          f"(Arma : {CIBLE:.0f} %)")
    if becart > 25:
