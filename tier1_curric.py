...
DEV = "cuda:0"
CKPT = "/home/younes/compose-embodiment/hostage_squad_d6.pt"   # warm-start D=6
OUT = "/home/younes/compose-embodiment/hostage_squad_tier1.pt"
FF_MAX, EXF_MAX = 0.05, 0.03
WARM, RAMP = 60, 150   # it 0-60 invulnerable ; 60-210 montee ; 210+ plein
...
def frac(it):
    if it < WARM: return 0.0
    return min(1.0, (it - WARM) / RAMP)
...
def train(iters=350, envs=4096, rollout=24, D=6):
    ...
    net = Net(O, NA, 512, 3).to(DEV); net.load_state_dict(torch.load(CKPT, map_location=DEV))   # WARM-START D=6
    opt = torch.optim.Adam(net.parameters(), lr=3e-4)
    obs = env.reset(); rr = 0.0
    for it in range(iters):
        f = frac(it); env.ff_hit = FF_MAX * f; env.exfil_hit = EXF_MAX * f   # MONTEE de la fragilite
        ...
        rr = succ / max(nep, 1)
        if it % 25 == 0:
            print("  it %3d | frag %.0f%% | RESCOUSSE %.0f%% | pickup %.0f%% | OTAGE TUE %.0f%% | detruite %.0f%%" % (
                it, 100 * f, 100 * rr, 100 * pick / max(nep, 1), 100 * hdead / max(nep, 1), 100 * wipe / max(nep, 1)), flush=True)
    torch.save(net.state_dict(), OUT)
    print(">>> TIER 1 (curriculum) final : RESCOUSSE %.0f%% a fragilite pleine" % (100 * rr), flush=True)
    print("TIER1 FINI", flush=True)


print("=== TIER 1 CURRICULUM : fragilite otage 0 -> plein (warm-start D=6) ===", flush=True)
train()
