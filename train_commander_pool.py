"""ABLATION du commandant — ACTEUR par element identique a MAAC, mais critique MEAN-POOL (Deep Sets, SANS attention).
But : isoler ce qui fait le gain a K=4. Si pool ~ attention -> c'est la STRUCTURE par-element (pas l'attention).
Si attention > pool -> l'attention apporte vraiment. (Cf finding multi-graine de juin : attn ~ deep a nombre fixe.)
Tout le reste identique a train_commander_maac.py (acteur, PPO, curriculum K=4 D=4->8->12)."""
...
class Actor(nn.Module):                                        # IDENTIQUE a MAAC
...
class PoolCritic(nn.Module):                                   # MEAN-POOL (Deep Sets) : meme forme que AttnCritic, mean au lieu d'attention
    ...
    def forward(self, obs):                                    # obs (N,K,10) -> V (N,K)
        h = self.enc(obs)                                      # (N,K,d)
        ctx = h.mean(1, keepdim=True).expand(-1, K, -1)        # contexte = MOYENNE des elements (invariant nombre/ordre, PAS d'attention)
...
actor = Actor().to(DEV); critic = PoolCritic().to(DEV)
...
print("=== ABLATION POOL (Deep Sets, sans attention) : acteur par element + critique mean-pool, K=4, D=4->8->12 ===", flush=True)
N = 4096; rollout = 12
for D in [4, 8, 12]:
    ...
    torch.save({"actor": actor.state_dict(), "critic": critic.state_dict()}, "/home/younes/compose-embodiment/commander_pool_d%d.pt" % D)
torch.save({"actor": actor.state_dict(), "critic": critic.state_dict()}, "/home/younes/compose-embodiment/commander_pool.pt")
print("ABLATION POOL FINI -> commander_pool.pt", flush=True)
