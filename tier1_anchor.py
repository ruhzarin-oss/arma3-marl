...
DEV = "cuda:0"
CKPT = "/home/younes/compose-embodiment/hostage_squad_d6.pt"   # base maitrisee (rescousse) = REFERENCE
OUT = "/home/younes/compose-embodiment/hostage_squad_tier1.pt"
FF_MAX, EXF_MAX = 0.05, 0.03
BETA = 0.5   # force de l'ancre KL vers le d6 ("n'oublie pas comment liberer")
...
def sched(it):
    if it < 40:  return 0.0, 0.0, "BASE (ancre seule)"
...
def train(iters=350, envs=4096, rollout=24, D=6):
    ...
    ref = Net(O, NA, 512, 3).to(DEV); ref.load_state_dict(torch.load(CKPT, map_location=DEV))  # REFERENCE gelee
    ref.eval()
    ...
    for it in range(iters):
        ...
        with torch.no_grad():
            ref_logp = torch.log_softmax(ref.a_logits(ob), dim=-1)   # (M,A,NA) reference figee
        klsum = 0.0
        for ep in range(3):
            logits = net.a_logits(ob); dist = torch.distributions.Categorical(logits=logits); lp = dist.log_prob(ac); ratio = (lp - oldlp).exp()
            cur_logp = torch.log_softmax(logits, dim=-1)
            kl = (ref_logp.exp() * (ref_logp - cur_logp)).sum(-1).mean()   # KL(ref || cur) >= 0 : reste pres du d6
            pl = -torch.min(ratio * advf, torch.clamp(ratio, 0.8, 1.2) * advf).mean(); vl = ((net.value(ob) - retf) ** 2).mean(); ent = dist.entropy().mean()
            loss = pl + 0.5 * vl - 0.005 * ent + BETA * kl; opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(net.parameters(), 0.5); opt.step()
            klsum = kl.item()
        rr = succ / max(nep, 1)
        if it % 20 == 0:
            print("  it %3d | ff %.3f exf %.3f | RESCOUSSE %.0f%% | pickup %.0f%% | OTAGE TUE %.0f%% | detruite %.0f%% | KL %.3f" % (
                it, ff, exf, 100 * rr, 100 * pick / max(nep, 1), 100 * hdead / max(nep, 1), 100 * wipe / max(nep, 1), klsum), flush=True)
    torch.save(net.state_dict(), OUT)
    print(">>> TIER 1 ancre final : RESCOUSSE %.0f%% a fragilite pleine" % (100 * rr), flush=True)
    print("TIER1 FINI", flush=True)


print("=== TIER 1 ANCRE KL vers d6 : empile sans oublier la libration ===", flush=True)
train()
