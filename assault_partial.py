...
class AssaultTerrain:
    def __init__(self, num_envs=2048, A=4, D=4, R_spawn=170.0, terr_R=200.0, terr_G=64, relief=35.0,
                 ...
                 shell_obs=False, shellK=12, shell_R=60.0, suffer=False, D_min=2, partial=False, world_shell=False,
        ...
        self.suffer = suffer; self.D_min = D_min; self.partial = partial; self.world_shell = world_shell
    ...
    def _obs(self):
        ...
        if getattr(self, 'partial', False): nd = torch.where(los > 0.5, nd, torch.ones_like(nd))
    ...
    def _cover_shell(self):
        ...
        ed2 = torch.where(self._dalive().unsqueeze(1), ex * ex + ey * ey, BIG); km = ed2.argmin(2)
        bx = torch.gather(self.dpx, 1, km); by = torch.gather(self.dpy, 1, km)
        th0 = (torch.zeros_like(self.apx) if getattr(self, 'world_shell', False) else torch.atan2(by - self.apy, bx - self.apx))                       # cap vers la menace (N,A)
