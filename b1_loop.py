"""B1 etape 4+5 — BOUCLE COMPLETE : d6 pilote l'escouade dans le vrai Arma.
perception 29-obs (SQF) -> d6 -> action -> setVelocity. Pickup/portage/succes cote serveur.
Repere sandbox=Arma (S=140, centre [20885,16779]). d6 gele (hostage_v1_FINAL.pt)."""
...
private _alive=({alive _x} count HMT_FR);
private _succ=if (HMT_PICKED==1 && _dex<25) then {1} else {0};
diag_log format ["HARMATTAN_STATUS picked=%1 succ=%2 dh=%3 dex=%4 alive=%5", HMT_PICKED, _succ, round _cd, round _dex, _alive];
'''
...
print("d6 charge. spawn...", flush=True)
b.send(SPAWN, wait=True, timeout=20); time.sleep(3)
print("=== BOUCLE LIVE d6 dans Arma ===", flush=True)
for t in range(240):
    obs = read_aobs()
    if obs is None: print("obs vide a t=%d" % t, flush=True); break
    with torch.no_grad():
        act = torch.distributions.Categorical(logits=net.a_logits(obs)).sample()
    st = send_act(act.tolist())
    if t % 10 == 0 or (st and st["succ"]) or (st and st["picked"]):
        print("t=%2d | %s | acts=%s" % (t, st, act.tolist()), flush=True)
    if st and st["succ"]: print(">>> RESCOUSSE REUSSIE a t=%d <<<" % t, flush=True); break
    if st and st["alive"] == 0: print(">>> escouade aneantie a t=%d" % t, flush=True); break
print("FIN BOUCLE", flush=True)
