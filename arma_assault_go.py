"""arma_assault_go — lance l'ASSAUT (garnison + joueur deja en place via arma_place). Spawn l'escouade au
defile Sud-Est, NE supprime RIEN (garde garnison + joueur Zeus), tourne en temps reel. Tu regardes a la camera."""
import time, math
import numpy as np
import torch
...
OBJ = (12000, 21000); R_SPAWN = 170.0; SQ_N = 8; GARR_N = 5; DEFILE_K = 3


def spawn_squad(env, sx, sy):
    sqf = ('SQ_ASSAUT=[]; private _g0 = createGroup west;\n'
           ...
           'setAccTime 1; diag_log "HARMATTAN_SQUAD spawn";\n'
           % (SQ_N - 1, sx, SQ_N, sy, SQ_N))
...
if __name__ == "__main__":
    ...
    th = math.radians(DEFILE_K * 45.0); sx, sy = OBJ[0] + R_SPAWN * math.cos(th), OBJ[1] + R_SPAWN * math.sin(th)
    print(">> Escouade (8) spawn a 170 m au Sud-Est (defile). Elle monte a couvert puis assaut. Regarde !", flush=True)
    spawn_squad(env, sx, sy)
