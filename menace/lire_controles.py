"""Lecture des controles « menace visible » ( menace/CONTROLES_MENACE_VISIBLE.md ), une fois tous les jobs finis."""
...
def episodes(campagne, filtre=None):
    out = []
    for jf in glob.glob(f"{H}/runs/2026-09-1[7-9]_*/job.json"):
        j = json.load(open(jf))
...
for nom in sorted(par):
    ...
    for e in L:
        ...
        print(f"   {e['monde']:7s} fin fenetre : vues {f(o,'menaces_vues')} connues {f(o,'menaces_connues')} dist {f(o,'distance_menace')} err {o.get('erreur_position') if o else None} "
              f"mobile {f(o,'menace_mobile')} vue_depuis {f(o,'vue_depuis')} vehicule {f(o,'vehicule_connu')} | verite {f(o,'verite_menaces')} dist_vraie {f(o,'verite_distance_menace')} "
              f"| decision {d.get('point') if d else None} choix {d.get('choix') if d else None} connues_au_choix {f(d,'menaces_connues')}")
print("\n== CRITERES ECRITS AVANT")
def part(nom, test):
    L = [e for e in par.get(nom, []) if e["verdict"] == "ACCEPTE" and e["obs"]]
    return sum(test(e) for e in L), len(L)
k, n = part("MV-POSITIF", lambda e: f(e["obs"], "menaces_connues") > 0); print(f"   POSITIF  connues > 0 en fin de fenetre : {k}/{n}  ( attendu >= 80 % ) -> {'OUI' if n and k >= 0.8 * n else 'NON'}")
k, n = part("MV-NEGATIF", lambda e: f(e["obs"], "menaces_vues") == 0); print(f"   NEGATIF  vues = 0 en fin de fenetre : {k}/{n}  ( attendu >= 95 % ) -> {'OUI' if n and k >= 0.95 * n else 'NON'}")
k, n = part("MV-NUL", lambda e: f(e["obs"], "menaces_vues") == 0 and f(e["obs"], "menaces_connues") == 0 and f(e["obs"], "verite_menaces") == 0 and f(e["dec"], "menaces_connues") == 0)
...
print("\n== VARIANCE DE LA PERCEPTION A 90 S ( part des episodes ou la menace est connue au moment du choix ; attendu 30 a 70 % )")
for nom in ("MV-P2_TYPE1", "MV-P2_TYPE2", "MV-P1_TYPE1", "MV-P1_TYPE2", "MV-P4_TYPE1", "MV-P4_TYPE2"):
    ...
    k = sum((f(e["dec"], "menaces_connues") or 0) > 0 for e in L)
