...
EXT, PORT = 5841, 6073
LOG = SB + "/logs/serverUSINE.out"
NDEF, NATT, DIST = 6, 8, 70.0
T_RUN, TICK = 240.0, 2.0
BRAS = ["A3C", "SCRIPT"]
PAR_BLOC, BLOCS = 4, 5          # 20 manches de plus : 10 par bras -> 16 au total
RID0 = 18
SORTIE = os.environ.get("SORTIE", "/home/younes/arma3-marl/logs_train/corpus_a3c_lambs.jsonl")
...
def manche(b, bras, rid, fh):
    ...
    for L in list(b.lines)[marque:]:
        ...
        n += 1
    fh.flush()
    print("  manche %2d [%-6s] ordre_ok=%s ticks=%3d  fin: def=%s dedans=%s"
          % (rid, bras, ok, n, fin[0] if fin else "?", fin[1] if fin else "?"), flush=True)
    return n
...
if __name__ == "__main__":
    ...
    with open(SORTIE, os.environ.get("MODE", "a")) as fh:
        for bloc in range(BLOCS):
            ...
            b.sock.close()
            sh("pkill -9 -f serverUSINE.cfg; sleep 3")
