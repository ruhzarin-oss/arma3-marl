"""DURCIR d6 — warm-start, D=8, fine-tuning DOUX, hazard d'exfil monte en douceur (0->0.08).
Barre : rescousse MONTE au-dessus de 17% (d6-solo D=8) = on durcit ; s'effondre vers 0 = on arrete."""
import sys, torch
...
DEV = "cuda:0"
CKPT = "/home/younes/compose-embodiment/hostage_v1_FINAL.pt"
EXF_MAX = 0.08
...
def sched(it):
    if it < 40:  return 0.0, "BASE D=8 (exf=0)"
    if it < 160: return EXF_MAX * min(1.0, (it - 40) / 120.0), "ramp exfil"
    return EXF_MAX, "PLEIN exfil"


def train(iters=200, envs=4096, rollout=24, D=8, lr=1e-4):
    torch.manual_seed(0)
    env = HostageEnv(num_envs=envs, A=9, D=D, device=DEV, seed=0)
    O, NA, A = env.obs_dim, env.n_actions, env.A
    net = Net(O, NA, 512, 3).to(DEV); net.load_state_dict(torch.load(CKPT, map_location=DEV))
    opt = torch.optim.Adam(net.parameters(), lr=lr)
    obs = env.reset(); last = ""
    for it in range(iters):
        exf, lab = sched(it); env.ff_hit = 0.0; env.exfil_hit = exf
        ...
        for t in range(rollout):
            ...
            if dm.any():
                ...
                wipe += info["squad_wipe"][dm].float().sum().item(); hd += info["hdead"][dm].float().sum().item(); nep += int(dm.sum())
        with torch.no_grad(): lastv = net.value(obs)
        adv = gae(RW, VL, DN, lastv); ret = adv + VL; adv = (adv - adv.mean()) / adv.std().clamp(min=1.0); advA = adv.unsqueeze(-1).expand(rollout, envs, A)
        ...
        if it % 10 == 0:
            print("  it %3d | exf %.3f | RESCOUSSE %.0f%% | pickup %.0f%% | otage-tue %.0f%% | detruite %.0f%%" % (
                it, exf, 100 * succ / max(nep, 1), 100 * pick / max(nep, 1), 100 * hd / max(nep, 1), 100 * wipe / max(nep, 1)), flush=True)
    torch.save(net.state_dict(), "/home/younes/compose-embodiment/hostage_d8_hard.pt")
    print("HARDEN FINI -> hostage_d8_hard.pt", flush=True)
