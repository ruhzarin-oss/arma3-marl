"""V2 FORMATIONS : warm-start formation.pt + entraine les TRANSITIONS (colonne en approche -> ligne/coin/vee
a l'assaut, bascule a 45 % du chemin). La politique suit son slot ; au changement le slot saute -> elle
doit transiter sans casser la forme. Mesure : erreur de slot moyenne + % arrive EN FORMATION D'ASSAUT."""
import sys, torch
...
def evalrun(net, envs=2048, steps=170, seed=777):
    env = FormationEnv(num_envs=envs, A=8, device=DEV, seed=seed, transition=True); obs = env.reset()
...
env = FormationEnv(num_envs=4096, A=8, device=DEV, seed=0, transition=True); O = env.obs_dim; A = env.A; obs = env.reset()
net = Net(O, 9, 512, 3).to(DEV)
net.load_state_dict(torch.load("/home/younes/compose-embodiment/formation.pt", map_location=DEV))   # warm-start V1
opt = torch.optim.Adam(net.parameters(), lr=2e-4)
N = 4096; rollout = 24
print("=== V2 FORMATIONS : transitions (warm-start V1, obs=%d) ===" % O, flush=True)
se0, hd0 = evalrun(net); print("warm-start V1 sur transitions : erreur slot %.1f m | arrive en formation %.0f%%" % (se0, hd0), flush=True)
for it in range(260):
    ...
    if it % 30 == 0:
        se, hd = evalrun(net); print("it %3d | erreur slot moy %.1f m | arrive en formation d'assaut %.0f%%" % (it, se, hd), flush=True)
torch.save(net.state_dict(), "/home/younes/compose-embodiment/formation_v2.pt")
print("V2 FINI -> formation_v2.pt", flush=True)
