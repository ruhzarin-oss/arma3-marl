...
SORTIE = "/home/younes/arma3-marl/bc.json"
SOURCE_INIT = "/home/younes/arma3-marl/trois_voies.json"
...
# B contre C SEULEMENT : seule paire a une variable (l axe). A confondrait fixeurs et axe.
import os
obs = []
if os.path.exists(SORTIE):
    obs = json.load(open(SORTIE))
    print("  reprise : %d episodes deja acquis" % len(obs), flush=True)
elif os.path.exists(SOURCE_INIT):
    obs = [x for x in json.load(open(SOURCE_INIT)) if x["bras"] in (1, 2)]
    print("  amorce : %d episodes B/C repris du bras a trois voies" % len(obs), flush=True)
for ep in range(EPI):
    for bras in (1, 2):
        with b._lock:
            b.lines.clear()
...
if 1 in res and 2 in res:
    Bb, C = res[1], res[2]
    dis = (Bb[2] < C[1]) or (C[2] < Bb[1])
    print()
    if dis and C[0] > Bb[0]:
        print("  ⭐ C > B, IC DISJOINTS : LE DEBORDEMENT PAIE SUR ARMA.")
        print("     Le signe degele redevient un fait, conditionne au regime 8 contre 4 a 220 m.")
    elif dis and Bb[0] > C[0]:
        print("  ⭐ B > C, IC DISJOINTS : L AXE NE PAIE PAS, ET IL COUTE.")
        print("     Le carnet du projet est a reecrire.")
    else:
        n = min(Bb[3], C[3])
        print("  IC RECOUVRANTS a n=%d par bras." % n)
        if n >= 100:
            print("  -> ce n est PAS « pas de difference » : c est une BORNE.")
            print("     L effet, s il existe, est inferieur a ~17 points.")
        else:
            print("  -> n insuffisant (%d < 100) : sous-puissant, on ne conclut pas." % n)
json.dump(obs, open(SORTIE, "w"))
print("\n  ecrit : %s" % SORTIE)
b.sock.close()
