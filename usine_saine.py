...
BRAS = ["A3C", "SCRIPT"]
PAR_BLOC, BLOCS = 4, 3          # 20 manches de plus : 10 par bras -> 16 au total
RID0 = 8
SORTIE = os.environ.get("SORTIE", "/home/younes/arma3-marl/logs_train/corpus_sain.jsonl")
...
def manche(b, bras, rid, fh):
    ...
    fh.write(json.dumps({"rid": rid, "bras": bras, "manifeste": True, "erreurs_sqf": err,
                         "serveur_vivant": vivant, "ticks": n, "void": void}) + "\n")
    fh.flush()
    print("  manche %2d [%-6s] ordre_ok=%s ticks=%3d  fin: def=%s dedans=%s | erreurs SQF=%d vivant=%s%s"
          % (rid, bras, ok, n, fin[0] if fin else "?", fin[1] if fin else "?",
             err, vivant, "  <<< VOID" if void else ""), flush=True)
    return n
