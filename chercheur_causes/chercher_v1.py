"""
Chercheur de causes CHACAL v1.

Criteres : CRITERES_CHERCHEUR_CAUSES_V0.md ( commit 6bedbad ) amendes par CRITERES_CHERCHEUR_CAUSES_V1.md
( ecrit avant cette recherche ) : garde de variance intra-strate, p empirique a 20 000 permutations,
raisons separees de la decomposition de l effet, controle C+4 remplace par C+4p ( le mecanisme ).
...
SORTIE = "/mnt/data/hmt/chercheur_causes/resultats_v1"
...
P, BLOC, Q, MIN_N, N_BOOT, TAUTO, DECOMP = 20000, 2000, 0.05, 5, 300, 0.95, 0.8
...
def tester(strates):
    ...
    Y = X[rows]
    Yn = np.nan_to_num(Y)
    sd_intra = demean(Yn, sid).std(0)
    usable = ~np.isnan(Y).any(0) & (sd_intra > 1e-9 * (1 + np.abs(Yn.mean(0))))
    Yu = Y[:, usable]
    if Yu.shape[1] == 0:
    ...
    # sous permutation dans les strates, l esperance de T est exactement nulle : p bilateral empirique
    cnt = np.zeros(Yu.shape[1]); sd = None
    for _ in range(P // BLOC):
        Lp = np.empty((BLOC, len(rows)))
        for s in range(len(strates)):
            m = sid == s
            Lp[:, m] = rng.permuted(np.tile(lab[m], (BLOC, 1)), axis=1)
        Tp = (Lp * (ca + cb) - ca) @ Yu / W
        cnt += (np.abs(Tp) >= np.abs(T) - 1e-9 * (1 + np.abs(T))).sum(0)
        if sd is None: sd = Tp.std(0)
    p = (cnt + 1) / (P + 1)
    z = np.where(sd > 0, T / np.where(sd > 0, sd, 1), 0.0)
    sdw = sd_intra[usable]
    res = {}
...
_corr = {}
def corr_globale(ja, jb):
    k = (min(ja, jb), max(ja, jb))
    if k not in _corr:
        m = ~np.isnan(X[:, ja]) & ~np.isnan(X[:, jb])
        _, s_ = np.unique(SID[m], return_inverse=True)
        a, b = demean(X[m, ja], s_), demean(X[m, jb], s_)
        _corr[k] = float(np.corrcoef(a, b)[0, 1]) if a.std() > 1e-9 and b.std() > 1e-9 else 0.0
    return _corr[k]
...
def raisons(c, jy, exclure=(), top=5):
    ...
    for j, r in c["res"].items():
        ...
        cands.append(dict(variable=NOMS[j], j=j, proportion=float(1 - coef[0] / beta), t_raison=float(t),
                          ...
                          decomposition=abs(corr_globale(jy, j)) >= DECOMP))
    cands.sort(key=lambda d: -d["proportion"])
    cands = [d for d in cands if d["proportion"] > 0]
    garde = [d for d in cands if not d["decomposition"]][:top] + [d for d in cands if d["decomposition"]][:3]
    for d in garde:
        props = []
        ...
        d.pop("j")
    return dict(raisons=[d for d in garde if not d["decomposition"]], decomposition=[d for d in garde if d["decomposition"]])
...
for ci, c in enumerate(comps):
    for e_ in EFFETS:
        ...
        rs = raisons(c, J[e_])
        tete = (rs["raisons"] or rs["decomposition"])
        ch = raisons(c, J[tete[0]["variable"]], exclure={J[e_]}, top=3) if tete else dict(raisons=[], decomposition=[])
        chaine = ch["raisons"] + ch["decomposition"]
        decouvertes.append(dict(lecture=c["lecture"], levier=c["levier"], a=c["a"], b=c["b"], effet=e_, na=c["meta"]["na"], nb=c["meta"]["nb"],
                                ...
                                raisons=rs["raisons"], decomposition=rs["decomposition"], pourquoi_du_pourquoi=chaine))
...
for e_ in ("fini_issue=SUCCES", "fini|charges"):
    ...
    for j in MEDIATEURS:
        ...
        if xd.std() < 1e-9 or yd.std() < 1e-9: continue
        ...
        lignes_p.append(dict(variable=NOMS[j], r=r, p=math.erfc(abs(zz) / math.sqrt(2)), n=int(m.sum()), dof=dof, decomposition=abs(r) >= DECOMP))
...
controles["C+4_informatif"] = dict(testable=r is not None, passe=bool(r) and ok4, cause_trouvee=bool(r), raisons=noms4)
cs = [c for c in comps if c["lecture"] == "non_entrelacee" and c["levier"] == "job|delai_porteur" and (c["a"], c["b"]) == (45, 180)]
NOM4 = "n|e|charge_manquee|cause=PORTEUR_N_ARRIVE_PAS"
r4 = cs[0]["res"].get(J[NOM4]) if (cs and NOM4 in J) else None
controles["C+4p"] = dict(testable=bool(cs) and r4 is not None, passe=bool(r4 and r4.get("decouverte") and r4["ecart"] < 0),
                         detail=({k: r4[k] for k in ("moy_a", "moy_b", "z", "p", "q")} if r4 else None))
...
positifs_testables = sum(1 for k in ("C+1", "C+2", "C+3", "C+4p") if controles[k]["testable"])
cru = all(v["passe"] for k, v in controles.items() if v["testable"] and not k.endswith("informatif")) and positifs_testables >= 3
print("\n== CONTROLES")
...
json.dump(dict(criteres_commit="6bedbad+v1", episodes=n, variables=len(NOMS), leviers=LEVIERS, controles=controles, cru=cru,
...
for d in sorted(decouvertes, key=lambda d: -abs(d["z"])):
    ...
    for r in d["decomposition"][:2]:
        print(f"        decomposition : {r['variable']:38s} part {r['proportion']:.2f}  (l effet redit autrement)")
...
for e_, ls in pistes.items():
    print(f"   {e_}")
    for d in ls[:12]:
        print(f"      r={d['r']:+.2f} q={d['q']:.2g} n={d['n']:4d} {'D' if d['decomposition'] else ' '} {d['variable']:48s} leviers : {', '.join(d['leviers_qui_la_bougent'][:3]) or 'AUCUN - levier a construire'}")
print("\nsorties ->", SORTIE)
