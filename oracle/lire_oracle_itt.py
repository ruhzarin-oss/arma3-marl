...
portes = [
    ...
    ("Q3 une decision TRAVERSEE, choix et choix joue conformes" if not ITT else
     "Q3 [ INTENTION DE TRAITER ] decision conforme, OU compromission avant la decision", all((e.get("n_decisions") == 1 and e.get("choix") == e["option"] and e.get("choix_joue") == e["option"])
         or (ITT and e.get("n_decisions") == 0 and e.get("discrete") == 0) for e in A),
    ...
    ("Q6 le reglage joue est celui du job", all((e.get("avant_joue") == e["avant"] and e.get("balayage_joue") == e["balayage"])
        or (ITT and e.get("n_decisions") == 0) for e in A), f"{sum(1 for e in A if not (e.get('avant_joue') == e['avant'] and e.get('balayage_joue') == e['balayage']))} ecart(s)"),
    ("Q7 moteur_depuis_fenetre present", all(e.get("fenetre") in (0, 1) or (ITT and e.get("n_decisions") == 0) for e in A), f"{sum(1 for e in A if e.get('fenetre') not in (0, 1))} absent(s)"),
