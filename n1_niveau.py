...
SORTIE = "/home/younes/arma3-marl/courbe_toucher_hitpart_B2.json"

b = NativeBridge(port=PORT, timeout=20)
print("  pont %d ouvert (compteur %d)" % (PORT, b.counter), flush=True)

COND = [(d, p) for p in POST for d in DIST]
...
print("\n  " + "=" * 70)
i100 = COND.index((100, "UP"))
p, lo, hi = wilson(imps.get(i100, 0), tirs.get(i100, 0))
print("  BLOC 0 — CONTROLE POSITIF, 100 m debout : %.3f  IC95 [%.3f ; %.3f]  n=%d"
      % (p, lo, hi, tirs.get(i100, 0)))
print("  bande ecrite d avance [%.2f ; %.2f] : %s"
      % (BANDE[0], BANDE[1], "✔ l instrument est stable" if BANDE[0] <= p <= BANDE[1]
         else "⛔ INSTRUMENT INSTABLE — rien de la nuit n est lisible"))
print("\n  dist      debout            accroupi           couche")
for d in DIST:
    ligne = "  %4d m " % d
    for po in POST:
        j = COND.index((d, po)); n = tirs.get(j, 0)
        if n < 300:
            ligne += "   %5s(n=%4d)" % ("-", n)
        else:
            pp, l, h = wilson(imps.get(j, 0), n)
            ligne += "  %s%.3f(n=%4d)" % ("⛔" if pp > 1.0 else " ", pp, n)
    print(ligne)
print("\n  COMPARAISON AU 26/07 (debout) : 25m 0,75 · 50m 0,57 · 75m 0,41 · 100m 0,30 · 150m 0,24 · 200m 0,26")
rap = []
for d, ref in zip(DIST, [0.75, 0.57, 0.41, 0.30, 0.24, 0.26]):
    j = COND.index((d, "UP")); n = tirs.get(j, 0)
    if n >= 300:
        pp = imps.get(j, 0) / n; rap.append(pp / ref)
        print("    %4d m : HitPart %.3f contre %.2f -> x%.2f" % (d, pp, ref, pp / ref))
if rap:
    rap.sort()
    print("\n  ⭐ FACTEUR DE SOUS-COMPTE median : x%.2f" % rap[len(rap) // 2])
b.sock.close()
