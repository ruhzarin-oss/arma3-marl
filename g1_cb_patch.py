P = "/home/younes/compose-embodiment/g1_cb.py"
src = open(P).read().split("\n")
out = []
i = 0
SETUP = [
'    # === COMBAT BOUCLE FERMEE ===',
...
'                _nd = float(((AT.dpx[0] - AT.apx[0,0])**2 + (AT.dpy[0] - AT.apy[0,0])**2).min().sqrt())',
'                print("[CB] t=%3d cap=%s dmg=%.2f nd=%.0f" % (_ci, _LAB[_cap[0]], float(AT.admg[0,0]), _nd), flush=True)',
'            _c = _cap[0]',
'            if _c < 8:',
...
while i < len(src):
    ...
    out.append(l); i += 1
open(P, "w").write("\n".join(out))
print("patch ok, CB lines:", sum(1 for x in out if "[CB]" in x))
