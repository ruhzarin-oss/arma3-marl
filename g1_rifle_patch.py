P = "/home/younes/compose-embodiment/g1_rifle.py"
src = open(P).read().split("\n")
out = []
i = 0
SETUP = [
'    # === COMBAT + POSTURE DE TIR + FUSIL ===',
...
'    print("[BODIES]", ROB.body_names, flush=True)',
'    _arm_ids, _arm_nm = ROB.find_joints([".*_shoulder_pitch.*", ".*_shoulder_roll.*", ".*_elbow_pitch.*"])',
'    _arm_ids = _torch.tensor(_arm_ids, device=ROB.device, dtype=_torch.long)',
'    _defp = ROB.data.default_joint_pos[0, _arm_ids].clone()',
'    _aimp = _defp.clone()',
'    for _j in range(len(_arm_nm)):',
'        _n = _arm_nm[_j]',
'        if "elbow" in _n: _aimp[_j] = 1.4',
'        elif "shoulder_pitch" in _n: _aimp[_j] = -0.6',
'        elif "shoulder_roll" in _n: _aimp[_j] = (0.2 if "left" in _n else -0.2)',
'    _aim_act = (_aimp - _defp) / 0.5',
'    _hb, _hbn = ROB.find_bodies([".*right_palm.*"])',
'    _hid = _hb[0] if len(_hb) else 0; print("[RIFLE] body=", _hbn[:1] if _hbn else "none", flush=True)',
'    _rcfg = VisualizationMarkersCfg(prim_path="/World/Rifle", markers={"r": _simu.CuboidCfg(size=(0.6,0.05,0.05), visual_material=_simu.PreviewSurfaceCfg(diffuse_color=(0.05,0.05,0.05)))})',
'    RIFLE = VisualizationMarkers(_rcfg)',
'    _SPEED = 1.0; _K = 20; _MAXC = args_cli.video_length if args_cli.video else 220; _ci = 0; _g1s = [None, None]; _cap = [0]; _blend = [0.0]',
'    _LAB = ["N","NE","E","SE","S","SW","W","NW","HOLD","SUPP"]',
'    print("[CB] start=%s d0=%.0f arms=%s" % (_ATs, _d0, _arm_nm), flush=True)',
]
INJ = [
'            if _ci % _K == 0:',
...
'                _blend[0] = max(0.0, _blend[0] - 0.06)',
'            else:',
'                CMD.vel_command_b[:,0] = 0.0; CMD.vel_command_b[:,1] = 0.0; CMD.heading_target[:] = ROB.data.heading_w',
'                _blend[0] = min(1.0, _blend[0] + 0.04)',
'            actions[:, _arm_ids] = _blend[0] * _aim_act + (1.0 - _blend[0]) * actions[:, _arm_ids]',
'            RIFLE.visualize(translations=ROB.data.body_pos_w[:, _hid], orientations=ROB.data.body_quat_w[:, _hid])',
'            _ci += 1',
]
BREAK = [
'        if _ci >= _MAXC:',
...
while i < len(src):
    ...
    out.append(l); i += 1
open(P, "w").write("\n".join(out))
print("patch ok, CB:", sum(1 for x in out if "[CB]" in x), "RIFLE:", sum(1 for x in out if "RIFLE" in x))
