"""Furtivite sur Arma (server14) + LOG des positions reelles pas-a-pas -> JSON, pour rendu fidele.
Anneau de 6 gardes, infiltrateur pilote par stealth_curric.pt. On enregistre la trajectoire reelle
(offset depuis le centre, recupere de l'obs) + positions/caps des gardes (interroges sur Arma)."""
import os, json
...
DEV = "cpu"; ND = 6; S = 140.0; FOV = 25.0; DRANGE = 70.0; SECURE = 20.0
b = ArmaBridge()
...
diag_log format ["HARMATTAN_SETUP guards=%1", count HMT_STG_GUARDS];
'''.replace("__ND__", str(ND))

GPOS = r'''
HMT_CX=20885; HMT_CY=16779;
{ private _q=getPosATL _x; diag_log format ["HARMATTAN_GPOS %1 %2 %3", _forEachIndex, (_q#0)-HMT_CX, (_q#1)-HMT_CY]; } forEach HMT_STG_GUARDS;
'''
...
def read_gpos(timeout=15):
    n = b.send(GPOS, wait=True, timeout=timeout); time.sleep(0.4)
    lines = b._log_lines(); idx = max(i for i, ln in enumerate(lines) if ("HARMATTAN_RECV cmd %d" % n) in ln)
    g = {}
    for ln in lines[idx:]:
        m = re.search(r"HARMATTAN_GPOS (\d+) (-?[\d.]+) (-?[\d.]+)", ln)
...
print("=== FURTIVITE + LOG positions (server14, %d gardes) ===" % ND, flush=True)
best = None
for ep in range(5):
    b.send(SETUP, wait=True, timeout=15); time.sleep(2.0)
    guards = read_gpos()
    alert = 0; traj = []; res = "timeout"
    for t in range(80):
        obs = read_sobs()
        if obs is None: break
        d = obs[0, 15].item() * S; ax = -obs[0, 13].item() * d; ay = -obs[0, 14].item() * d; det = int(obs[0, 21].item() > 0.5)
        if det: alert += 1
        traj.append([round(ax, 1), round(ay, 1), det])
        if d < SECURE and alert <= 3: res = "INFILTRE INVU"; break
        if alert > 3: res = "ALARME"; break
        with torch.no_grad(): a = int(net.a_logits(obs).argmax(-1)[0])
        send_act(a)
    print("  ep%d : %s | pas=%d | vu cumul=%d" % (ep, res, len(traj), alert), flush=True)
    if res == "INFILTRE INVU":
        best = {"S": S, "fov_deg": FOV, "drange": DRANGE, "secure_r": SECURE, "guards": guards, "traj": traj, "result": res}
        break
    if best is None:
        best = {"S": S, "fov_deg": FOV, "drange": DRANGE, "secure_r": SECURE, "guards": guards, "traj": traj, "result": res}
json.dump(best, open("/home/younes/arma3-marl/stealth_traj.json", "w"))
print("LOG ECRIT -> stealth_traj.json (%s, %d pas, %d gardes)" % (best["result"], len(best["traj"]), len(best["guards"])), flush=True)
