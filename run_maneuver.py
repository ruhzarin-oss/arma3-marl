...
from enemy_profiles import apply_profile, metrics_dyn, PRO_SKILL_SQF, HUNT_SQF, REACTIVE_DEF_SQF, REACTIVE_DEPTH_SQF, ENEMY_PROFILES, DEFENSE_SQF
...
def run_one(brain, plan, srv, seed, max_steps, verbose, log_path, enemy="normal", geometry="standard", max_wall=1200.0, stall_wall=500.0):
    ...
    if prof["pro"]:   # skills montés toujours si pro ; boucle de CHASSE seulement si hunt (défaut True -> rétro-compat)
        env.b.send(PRO_SKILL_SQF + (HUNT_SQF if prof.get("hunt", True) else "")
                   ...
                   + (REACTIVE_DEPTH_SQF if prof.get("react_depth") else "")
                   + (DEFENSE_SQF[prof["def_sqf"]] if prof.get("def_sqf") else ""), wait=True)
    ...
    met = metrics_dyn(env, runner, garrison)
    met["abort"] = getattr(runner, "abort_reason", None)
    return met
...
if __name__ == "__main__":
    ...
    p.add_argument("--max_wall", type=float, default=1200.0); p.add_argument("--stall_wall", type=float, default=500.0)
    ...
    if a.smoke:
        ...
        m = run_one(brain, plan, srv=0, seed=1, max_steps=a.max_steps, verbose=True, log_path="op_journal.jsonl", enemy=a.enemy, geometry=a.geometry, max_wall=a.max_wall, stall_wall=a.stall_wall)
        ...
        def worker(srv):
            while True:
                ...
                t0 = time.time()
                try:
                    m = run_one(brain, plan, srv, seed, a.max_steps, False, "/dev/null", enemy=a.enemy, geometry=a.geometry, max_wall=a.max_wall, stall_wall=a.stall_wall)
